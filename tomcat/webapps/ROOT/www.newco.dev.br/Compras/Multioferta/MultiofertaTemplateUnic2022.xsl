...
<!--
	Multioferta (todos los estados): Templates. estilos 2022
	Ultima revision: ET 20jul23 10:43
-->
    ...
    <xsl:param name="empresa"/>
	 ...
	 <!--idioma-->                                              
      ...
      <!-- <xsl:value-of select="$numMOF"/> -->
        ...
        <!--elijo titulo especifico de cada MOF-->
        <xsl:choose>
      ...
      <!--idioma fin-->
         ...
         <tbody>
              <tr class="sinLinea"> 
               ...
               <td class="textLeft">  
				<strong>
				  	<a href="javascript:FichaCentro({CENTRO/CEN_ID});">
                   		<xsl:value-of select="CENTRO/CEN_NOMBRE"/>
					</a>
				</strong>&nbsp;
                    <xsl:choose>
	 ...
	 <!--<table class="borderRight" cellspacing="0" cellpadding="0" border="0">-->
         ...
         <tbody>
              <tr class="sinLinea"> 
               ...
               <td class="textLeft">
                  <strong>
				  <a href="javascript:FichaEmpresa({DATOSPROVEEDOR/EMP_ID});">
              ...
              <td class="labelRight" valign="top"><xsl:value-of select="document($doc)/translation/texts/item[@name='direccion']/node()"/>:</td>
               <td class="textLeft">
                <xsl:value-of select="DATOSPROVEEDOR/EMP_DIRECCION"/>
...
<xsl:template name="datosPedido">
	...
	<tr class="sinLinea">
		...
		<td class="textLeft"><xsl:value-of select="PED_FECHA"/>&nbsp;(<xsl:value-of select="COMPRADOR"/>)
		<!--	5mar20 Solo para COMPRADORES, si ha habido proceso de aprobación previo lo mostramos -->
		<xsl:if test="/Multioferta/MULTIOFERTA/USUARIO/ROL = 'COMPRADOR' and /Multioferta/MULTIOFERTA/APROBACIONES/APROBACION">
			<img name="Aprobacion" src="http://www.newco.dev.br/images/recibido.gif" width="16px" height="16px">
				<!--<xsl:attribute name="title">
					<xsl:for-each select="/Multioferta/MULTIOFERTA/APROBACIONES/APROBACION">
						<xsl:value-of select="MOA_FECHA"/>:&nbsp;<xsl:value-of select="USUARIO"/>
					</xsl:for-each>
				</xsl:attribute>-->
			</img>
		&nbsp;
    ...
    <tr class="sinLinea">
      	<td class="labelRight">
   		...
   		<td class="textLeft"><xsl:value-of select="PED_FECHAACEPTACION"/>&nbsp;(<xsl:value-of select="USUARIOACEPTACION"/>)</td>
    </tr>
    </xsl:if>
    <tr class="sinLinea">
      <xsl:choose>
        ...
        <xsl:choose>
			...
			<xsl:otherwise>
				<xsl:if test="/Multioferta/MULTIOFERTA/OCULTAR_FECHA_ENTREGA='N' or MO_URGENCIA='S'">
					...
					<xsl:if test="MO_BAJAPRIORIDAD='S' and not(/Multioferta/MULTIOFERTA/USUARIO/IDEMPRESA = /Multioferta/MULTIOFERTA/DATOSPROVEEDOR/EMP_ID)">&nbsp;
                		<span style="color:#008800; font-weight:bold; padding:3px 2px; background:#FFFF99;"><xsl:value-of select="document($doc)/translation/texts/item[@name='baja_prioridad']/node()"/></span>
					</xsl:if>
				</xsl:if>
            </xsl:otherwise>
		</xsl:choose>
    ...
    <xsl:choose> 
          ...
          <tr class="sinLinea">
            <td>&nbsp;</td>
            <td class="textLeft">
            	<xsl:if test="PED_IDPADRE!=''"> 
                    <a>
                    	<xsl:attribute name="href">javascript:FichaPedido('<xsl:value-of select="PED_IDPADRE"/>');</xsl:attribute>
                    	<xsl:value-of select="document($doc)/translation/texts/item[@name='ver_pedido']/node()"/>
                    </a><br/>
            	</xsl:if>
            	<xsl:if test="PED_IDHIJO!=''">
                    <a>
                    	<xsl:attribute name="href">javascript:FichaPedido('<xsl:value-of select="PED_IDHIJO"/>');</xsl:attribute>
      ...
      <xsl:choose>
        <xsl:when test="contains($numMOF,'13_RW') or contains($numMOF,'25_RW') or contains($numMOF,'35_RW')">
            ...
            <td class="textLeft">
                <input type="text" name="FECHA_ENTREGA_REAL" class="campopesquisa w100px" size="12" maxlength="10" onBlur="AvisarFechaRecepcion=0;">
		...
		<xsl:if test="/Multioferta/MULTIOFERTA/MO_IDLICITACION!=''">
        	 ...
        	 <td class="textLeft">    
				<a href="javascript:chLicV2({/Multioferta/MULTIOFERTA/MO_IDLICITACION});"><xsl:value-of select="/Multioferta/MULTIOFERTA/LICITACION"/></a>
        	 </td>
        	 </tr>
		</xsl:if>
    ...
    <tbody>
		...
		<tr class="sinLinea">
			...
			<td class="textLeft">
				...
				<xsl:otherwise>
					<span>
                		<xsl:if test="/Multioferta/MULTIOFERTA/MO_STATUS = 11">
					...
					<xsl:if test="MO_BAJAPRIORIDAD='S' and not(/Multioferta/MULTIOFERTA/USUARIO/IDEMPRESA = /Multioferta/MULTIOFERTA/DATOSPROVEEDOR/EMP_ID)">&nbsp;
                		<span style="color:#008800; font-weight:bold;padding:5px 5px; "><xsl:value-of select="document($doc)/translation/texts/item[@name='baja_prioridad']/node()"/></span>
					</xsl:if>
				<input type="hidden" name="FECHANO_ENTREGA" size="12" maxlength="10" value="{LP_FECHAENTREGA}" onFocus="this.blur();" class="inputOcultoPar"/>
			</td>
		</tr>
...
<xsl:template name="NEGOCIACION">    
        ...
        <tfoot> 
            ...
            <xsl:if test="/Multioferta/MULTIOFERTA/MO_OBSERVACIONES != '' or /Multioferta/MULTIOFERTA/MO_COMENTARIOS != ''">
                ...
                <td colspan="3" class="ochenta datosLeft"> 
                    ...
                    <p style="font-weight:bold; color:#000; background:#FF9; border:1px solid #666; padding:2px;" id="Comentarios"><br/><xsl:copy-of select="/Multioferta/MULTIOFERTA/MO_OBSERVACIONES" /><xsl:copy-of select="/Multioferta/MULTIOFERTA/MO_COMENTARIOS"/><br/>&nbsp;</p>
                </td>
                </tr>    
    ...
    <!--SI HAY COMENTARIOS
         ...
         <input type="hidden" name="NMU_COMENTARIOS" value=""/>
			...
			<!--	12abr21 Tipo de pedido	-->
			<xsl:if test="/Multioferta/MULTIOFERTA/INFO_TIPO_PEDIDO">
			<tr><td colspan="3">&nbsp;</td></tr>
			<tr height="10px">
				<td class="uno">&nbsp;</td>
				<td colspan="2" class="textLeft">
					<strong>
    					<xsl:value-of select="document($doc)/translation/texts/item[@name='tipo_pedido']/node()"/>:&nbsp;<xsl:value-of select="/Multioferta/MULTIOFERTA/INFO_TIPO_PEDIDO/TIPO_PEDIDO/DESCRIPCION"/>
					</strong>
				</td>
			</tr>
			<xsl:for-each select="/Multioferta/MULTIOFERTA/INFO_TIPO_PEDIDO/VALOR_TIPO_PEDIDO">
			<tr height="10px">
				<td class="uno">&nbsp;</td>
				<td colspan="2" class="textLeft">
					<strong>
    					<xsl:value-of select="TITULO"/>:&nbsp;<xsl:value-of select="VALOR"/>
					</strong>
				</td>
			</tr>
			</xsl:for-each>
			...
			<!--	22feb21 Clausulas legales  (opcional)	-->
			<xsl:if test="/Multioferta/MULTIOFERTA/CLAUSULAS_LEGALES/CLAUSULA">
			<tr><td colspan="3">&nbsp;</td></tr>
			<xsl:for-each select="/Multioferta/MULTIOFERTA/CLAUSULAS_LEGALES/CLAUSULA">
			<tr height="10px">
				<td class="uno">&nbsp;</td>
				<td colspan="2" class="textLeft">
					<strong>
    					<xsl:value-of select="MOC_CODIGO"/>.- <xsl:value-of select="MOC_COMENTARIO"/>
					</strong>
				</td>
			</tr>
			</xsl:for-each>
			...
			<!--	29set20	Bloque documentos		-->
			<p class="textLeft">
				<br/>
				&nbsp;&nbsp;<a class="btnNormal" href="javascript:VerTodosDocumentos({/Multioferta/MULTIOFERTA/MO_ID});"><xsl:value-of select="document($doc)/translation/texts/item[@name='documentos']/node()"/></a><br/><br/>
				<xsl:if test="/Multioferta/MULTIOFERTA/DOCUMENTOS/DOCUMENTOS_COMPRADOR/DOCUMENTO">
					<xsl:for-each select="/Multioferta/MULTIOFERTA/DOCUMENTOS/DOCUMENTOS_COMPRADOR/DOCUMENTO">
						&nbsp;<a>
				...
				<xsl:if test="/Multioferta/MULTIOFERTA/DOCUMENTOS/DOCUMENTOS_VENDEDOR/DOCUMENTO">
					<xsl:for-each select="/Multioferta/MULTIOFERTA/DOCUMENTOS/DOCUMENTOS_VENDEDOR/DOCUMENTO">
						&nbsp;<a>
							<xsl:attribute name="href">javascript:MostrarPagPersonalizada('http://www.newco.dev.br/Documentos/<xsl:value-of select="URL"/>','Documentos',100,80,0,0);</xsl:attribute>
							<xsl:value-of select="NOMBRETIPO"/>:&nbsp;<xsl:value-of select="NOMBRE"/>
						</a><br/>
 	...
 	<xsl:param name="numMOF" />  
      ...
      <!--idioma fin-->
        ...
        <table cellspacing="0" cellpadding="0">
			...
			<p class="textLeft">
				<br/>
				<strong><a class="btnNormal" href="javascript:DocumentosPedido('{/Multioferta/MULTIOFERTA/MO_ID}');"><xsl:value-of select="document($doc)/translation/texts/item[@name='documentos']/node()"/></a></strong><br/><br/>
				<xsl:if test="/Multioferta/MULTIOFERTA/DOCUMENTOS/DOCUMENTOS_COMPRADOR/DOCUMENTO">
					<xsl:for-each select="/Multioferta/MULTIOFERTA/DOCUMENTOS/DOCUMENTOS_COMPRADOR/DOCUMENTO">
						&nbsp;<a>
							<xsl:attribute name="href">javascript:Documento('<xsl:value-of select="URL"/>');</xsl:attribute>
							<xsl:value-of select="NOMBRETIPO"/>:&nbsp;<xsl:value-of select="NOMBRE"/>
						</a><br/>
					</xsl:for-each>
				</xsl:if>
...
<xsl:template name="tpProductos">
	...
	<table>
    	...
    	<xsl:attribute name="class">
        	<xsl:choose>
        	<xsl:when test="/Multioferta/MULTIOFERTA/PRODUCTOSMANUALES">w100px</xsl:when>
    	 ...
    	 <th class="w100px">  
    	  ...
    	  <!-- ref provee -->   
			<xsl:if test="not(/Multioferta/MULTIOFERTA/OCULTAR_REFERENCIA_PROVEEDOR)">
    		  <th> 
				<xsl:attribute name="class">
				<xsl:choose>
				<xsl:when test="LINEASMULTIOFERTA/LINEASMULTIOFERTA_ROW[1]/LMO_CATEGORIA = 'F' and /Multioferta/MULTIOFERTA/UTILIZAR_REFERENCIA_PROVEEDOR">textLeft w150px</xsl:when>
				<xsl:when test="LINEASMULTIOFERTA/LINEASMULTIOFERTA_ROW[1]/LMO_CATEGORIA = 'N'">textLeft w100px</xsl:when>
				<xsl:otherwise>textLeft w100px</xsl:otherwise>
				</xsl:choose>
				</xsl:attribute>
		  ...
		  <!--	Nuevo campo con info de pedidos anteriores	-->
			<xsl:if test="/Multioferta/MULTIOFERTA/DERECHOS='40_RW'">
		  	<td class="w1px">&nbsp;</td>  
			</xsl:if>
    	  <!--cantidad solicitada-->
    	  <th class="w100px textLeft"> 
		...
		<!--enseño productos solo si la cantidad es mayor que 0LMO_CANTIDAD_SINFORMATO>0-->
		 ...
		 <tr>
    		...
    		<td  class="color_status"><xsl:value-of select="LINEA"/></td><!--columna para texto añadir a pedido de productos manuales-->
  			<td>
                    	...
                    	<xsl:when test="LMO_CATEGORIA = 'F' and /Multioferta/MULTIOFERTA/UTILIZAR_REFERENCIA_PROVEEDOR">
                    	   <xsl:choose>
                        	   <xsl:when test="/Multioferta/MULTIOFERTA/MVM or /Multioferta/MULTIOFERTA/CDC">
                           			<a href="javascript:FichaAdjudicacion('{LMO_IDPRODUCTO}');">
    		  ...
    		  <td class="textLeft">   
        		  <xsl:choose>
            		  ...
            		  <xsl:otherwise>
                		  <!--usuarios normales-->
							<xsl:if test="/Multioferta/MULTIOFERTA/MO_ESPECIAL='REGULADO'">
                		  ...
                		  <xsl:choose>
                    		...
                    		<xsl:when test="NOMBRE_PRIVADO!=''">
                    		  <a href="javascript:FichaProducto('{LMO_IDPRODUCTO}&amp;IDEMPRESA_COMPRADORA={/Multioferta/MULTIOFERTA/MO_IDCLIENTE}');">
		   ...
		   <td class="textCenter">
			...
			<xsl:if test="/Multioferta/MULTIOFERTA/DERECHOS='40_RW'">
		  	<td>
				<img src="http://www.newco.dev.br/images/2017/info.png" class="static" title="{LPE_PEDIDOSANTERIORES}"/>
		  	</td>  
			</xsl:if>
		   <td>
   				<xsl:choose>
		  ...
		  <!-- <td>
		   ...
		   <xsl:when test="((/Multioferta/MULTIOFERTA/DATOSCLIENTE/EMP_OCULTARPRECIOREF !='S' or /Multioferta/MULTIOFERTA/DATOSCLIENTE/EMP_MOSTRARCOMISIONES_NM !='S') and /Multioferta/MULTIOFERTA/DATOSCLIENTE/EMP_PRECIOSCONIVA != 'S') or /Multioferta/MULTIOFERTA/IDPAIS = '55'">
			...
			<!--	4jun21 Info de mantenimiento (solo para compradores, en todos los estados)		-->
			<td align="center">
			<xsl:if test="LMO_INFOMANTENIMIENTO!=''">
				<img width="16px" height="16px" src="http://www.newco.dev.br/images/info.gif" title="{LMO_INFOMANTENIMIENTO}"/>
				<!--	<xsl:attribute name="title"><xsl:copy-of select="LMO_INFOMANTENIMIENTO"/></xsl:attribute></img>-->
			</xsl:if>
			</td>
		  ...
		  <tr class="fondoAmarillo">
  			  ...
  			  <td ><span class="rojo"><b>[*]</b></span><xsl:value-of select="document($doc)/translation/texts/item[@name='alternativa']/node()"/></td>
    		  <td class="textLeft"><xsl:value-of select="ALTERNATIVA/NOMBRE"/></td>
    		  <td><xsl:if test="ALTERNATIVA/REFERENCIA!=''"><xsl:value-of select="ALTERNATIVA/REFERENCIA"/></xsl:if></td>
    		  <td colspan="8">&nbsp;</td>
		  </tr>
		  </xsl:if>
		  <!--sin stock-->
			<!--11may21 Info de seguimiento de aprobacion del producto-->
			<xsl:if test="/Multioferta/MULTIOFERTA/SEGUIMIENTO_APROBACION and ($numMOF = 'MOF_40_RW' or $numMOF = 'MOF_40_RW_Abono')">
				<tr style="background:#FF9;">
					<td>&nbsp;</td>
					<td style="text-align:left;">&nbsp;<xsl:value-of select="LMO_FECHASEGUIMIENTO"/></td>
					<td style="text-align:left;">&nbsp;<xsl:value-of select="USUARIOSEGUIMIENTO"/></td>
					<td style="text-align:left" colspan="7">
						<input type="text" id="Seguimiento_{LMO_ID}" name="Seguimiento_{LMO_ID}" value="{LMO_SEGUIMIENTO}" style="width:1000px;" size="3000" onkeyup="javascript:ActivarBotonGuardar({LMO_ID});"/>
					</td>
					<td style="text-align:left;">
						&nbsp;<a class="btnDestacado" id="btnGuardarSeguimiento_{LMO_ID}" href="javascript:GuardarSeguimientoAprobacion({LMO_ID})" style="display:none;"><xsl:value-of select="document($doc)/translation/texts/item[@name='guardar']/node()"/></a>
						<img id="chkOK_{LMO_ID}" src="http://www.newco.dev.br/images/check.gif" style="display:none;"/><img id="chkError_{LMO_ID}" src="http://www.newco.dev.br/images/error.gif" style="display:none;"/>
					</td>
					<td colspan="1">&nbsp;</td>
				</tr>
			</xsl:if>
	...
	<tfoot class="rodape_tabela">
	 ...
	 <!-- pongo aqui la parte de comentarios y lo demas-->
    	...
    	<tr class="sinLinea">
			...
			<table class="w500px">
			<tr class="sinLinea">
				<td class="w300px label textRight">  
			...
			<tr class="sinLinea">
				<td class="label textRight">    
					...
					<xsl:otherwise>
    					<xsl:value-of select="document($doc)/translation/texts/item[@name='importe_total']/node()"/>
					</xsl:otherwise>
					</xsl:choose>
				</td>       
				<td class="textRight">
	...
	<!-- pongo aqui la parte de comentarios y lo demas-->
		...
		<table class="w500px">
		<tr>
    	  <td class="label w300px textRight">
    	   <xsl:choose>
				<xsl:when test="/Multioferta/MULTIOFERTA/DATOSCLIENTE/EMP_OCULTARPRECIOREF='S' and /Multioferta/MULTIOFERTA/DATOSCLIENTE/EMP_MOSTRARCOMISIONES_NM='S'">
    	  ...
    	  <td class="textRight">
        	...
        	<xsl:otherwise>
        		<xsl:apply-templates select="IMPORTE_TOTAL_FORMATO"/>
        	</xsl:otherwise>
        	</xsl:choose>
    	  </td>
	 	</tr>
    	<tr>
			...
			<tr class="sinLinea">
				...
				<td class="textRight">
					<!--el hidden se utiliza unicamente para evtar un extranno error en mvmnucleo-->
        			<input type="hidden" name="COSTELOGISTICA1" size="12" maxlength="12" onFocus="this.blur();" value="{MO_COSTELOGISTICA_FORMATO}" class="noinput" style="text-align:right"/>
        			<input type="text" name="COSTELOGISTICA" size="12" maxlength="12" onFocus="this.blur();" value="{MO_COSTELOGISTICA_FORMATO}" class="noinput" style="text-align:right"/>
				</td>
			</tr>
			</xsl:if>
			<xsl:if test="MO_COSTELOGISTICA_IVA">
			<tr class="sinLinea">
				<td class="label textRight">
					<xsl:value-of select="document($doc)/translation/texts/item[@name='IVA_coste_transporte']/node()"/>:
				</td>
				<td class="textRight">
					<!--el hidden se utiliza unicamente para evtar un extranno error en mvmnucleo-->
        			<input type="hidden" name="COSTELOGISTICA_IVA1" size="12" maxlength="12" onFocus="this.blur();" value="{MO_COSTELOGISTICA_IVA_FORMATO}" class="noinput" style="text-align:right"/>
        			<input type="text" name="COSTELOGISTICA_IVA" size="12" maxlength="12" onFocus="this.blur();" value="{MO_COSTELOGISTICA_IVA_FORMATO}" class="noinput" style="text-align:right"/>
				</td>
			</tr>
			</xsl:if>
	 ...
	 <!-- pongo aqui la parte de comentarios y lo demas-->
		...
		<tr>
    	  ...
    	  <td class="label textRight" style="width:250px;">
				<xsl:choose>
				<xsl:when test="/Multioferta/MULTIOFERTA/DATOSCLIENTE/EMP_OCULTARPRECIOREF='S' and /Multioferta/MULTIOFERTA/DATOSCLIENTE/EMP_MOSTRARCOMISIONES_NM='S'">
					<xsl:value-of select="document($doc)/translation/texts/item[@name='total_prov_s_iva']/node()"/>
				</xsl:when>
    	...
    	<td class="textRight">
         	<input type="text" name="MO_SUBTOTAL" size="16" maxlength="16" style="text-align:right;width:170px; " value="{IMPORTE_TOTAL_FORMATO}" class="noinput" onFocus="this.blur();"/>
		  	<!--solodebug<xsl:value-of select="IMPORTE_TOTAL_FORMATO"/>-->&nbsp;<xsl:value-of select="/Multioferta/MULTIOFERTA/DIVISA/PREFIJO"/><xsl:value-of select="/Multioferta/MULTIOFERTA/DIVISA/SUFIJO"/>
    	</td>
    	...
    	<tr class="sinLinea">
    	  <td class="label textRight"> 
			...
			<!--para multioferta-frame 28-->
				<input type="hidden" name="MO_DESCUENTOGENERAL" size="6" maxlength="6" style="text-align:right;font-weight:bold;" value="{MO_DESCUENTOGENERAL}" onFocus="this.blur();"/>
				<input type="text" name="MO_IMPORTEIVA" size="16" maxlength="16" onFocus="this.blur();" value="{MO_IMPORTEIVA_FORMATO}" class="noinput" style="text-align:right;width:170px;"/>&nbsp;<xsl:value-of select="/Multioferta/MULTIOFERTA/DIVISA/PREFIJO"/><xsl:value-of select="/Multioferta/MULTIOFERTA/DIVISA/SUFIJO"/>
			</td>
			</tr>
			...
			<tr class="sinLinea">
				...
				<td class="textRight">
        			<input type="text" name="COSTELOGISTICA" size="16" maxlength="16" onFocus="this.blur();" value="{MO_COSTELOGISTICA_FORMATO}" class="noinput" style="text-align:right;width:170px;"/>&nbsp;<xsl:value-of select="/Multioferta/MULTIOFERTA/DIVISA/PREFIJO"/><xsl:value-of select="/Multioferta/MULTIOFERTA/DIVISA/SUFIJO"/>
				</td>
			</tr>
			</xsl:if>
			<xsl:if test="MO_COSTELOGISTICA_IVA">
			<tr class="sinLinea">
				...
				<td class="textRight">
        			<input type="text" name="COSTELOGISTICA_IVA" size="16" maxlength="16" onFocus="this.blur();" value="{MO_COSTELOGISTICA_IVA_FORMATO}" class="noinput" style="text-align:right;width:170px;"/>&nbsp;<xsl:value-of select="/Multioferta/MULTIOFERTA/DIVISA/PREFIJO"/><xsl:value-of select="/Multioferta/MULTIOFERTA/DIVISA/SUFIJO"/>
				</td>
			</tr>
			</xsl:if>
			<tr class="sinLinea">
			...
			<td class="textRight">
				<input type="text" name="IMPORTE_FINAL_PEDIDO" size="16" maxlength="16" onFocus="this.blur();" value="{IMPORTE_FINAL_PEDIDO}" class="noinput" style="text-align:right;width:170px;"/>
				<!--solodebug<xsl:value-of select="IMPORTE_FINAL_PEDIDO"/>-->&nbsp;<xsl:value-of select="/Multioferta/MULTIOFERTA/DIVISA/PREFIJO"/><xsl:value-of select="/Multioferta/MULTIOFERTA/DIVISA/SUFIJO"/>
			</td>
			</tr>
		...
		<!--	El importe total lo incluimos en las tablas de cada caso
    	 <tr class="sinLinea">
...
<xsl:template name="CANTIDAD_ENTREGADA_MOF25">
    ...
    <xsl:choose>
      ...
      <xsl:when test="LPE_ENTREGADA='S'">
        <input type="text" class="campopesquisa w100px" name="CantidadEntregada_{LMO_ID}" size="6" style="text-align:right" maxlength="7"  value="{LMO_CANTIDAD_SINFORMATO}" OnBlur="UnidadesALotesRecibidasImagen(this.value,'{PRO_UNIDADESPORLOTE}','{LMO_CANTIDAD_SINFORMATO}',this);"  onFocus="this.select();AvisarCantidadesIntactas=0;"/>
      </xsl:when>
      <xsl:otherwise>
        <input type="text" class="campopesquisa w100px" name="CantidadEntregada_{LMO_ID}" size="6" style="text-align:right" maxlength="7"  value="{LPE_CANTIDADENTREGADA}" OnBlur="UnidadesALotesRecibidasImagen(this.value,'{PRO_UNIDADESPORLOTE}','{LMO_CANTIDAD_SINFORMATO}',this);" onFocus="this.select();AvisarCantidadesIntactas=0;" />
      </xsl:otherwise>
    </xsl:choose> 
    <xsl:text>&nbsp;</xsl:text>
	...
	<!--PENDIENTE:<a href="javascript:comentariosLinea({LMO_ID});"><img width="16px" height="16px" src="http://www.newco.dev.br/images/info.gif" title="{LMO_SEGUIMIENTO}"/></a>-->
	</xsl:if>
...
<xsl:template name="CANTIDAD_ENTREGADA_MOF13">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
		<input type="text" class="campopesquisa w100px" name="CantidadEntregada_{LMO_ID}" size="8" style="text-align:right" maxlength="7"  value="{LMO_CANTIDAD_SINFORMATO}" OnBlur="UnidadesALotesRecibidasImagen(this.value,'{PRO_UNIDADESPORLOTE}','{LMO_CANTIDAD_SINFORMATO}',this);" onFocus="this.select();"/>
      </xsl:otherwise>
    </xsl:choose> 
	&nbsp;
	<xsl:if test="LMO_ESPACK!='S'">
...
<!--
	...
	24ene22 Eliminado Template de muestras
...
<xsl:template name="botonesDosAbonoMultioferta"> 
      ...
      <!--idioma fin--> 
        ...
        <a class="btnDestacado" href="javascript:Actua(document.forms['form1'],'MultiofertaSave2022.xsql?BOTON=CONFIRMAR');">
             <xsl:value-of select="document($doc)/translation/texts/item[@name='aceptar_minu']/node()"/>
        </a>
		&nbsp;
        <a class="btnDestacado" href="javascript:Rechazar(document.forms['form1'],'MultiofertaSave2022.xsql?BOTON=RECHAZAR');">
...
<xsl:template name="botonesTresMultioferta">  
 <!--idioma-->                                             
	...
	&nbsp;
    <a class="btnDestacado" href=" javascript:Actua(document.forms['form1'],'MultiofertaSave2022.xsql?BOTON=COBRO_OK');">
...
<xsl:template name="botonesGuardarMultioferta">  
 <!--idioma-->                                             
	...
	&nbsp;
	<a class="btnDestacado" href="javascript:Actua(document.forms['form1'],'MultiofertaSave2022.xsql?BOTON=OK');">
...
<xsl:template name="botonesTresRecibidaMultioferta">  
 <!--idioma-->                                             
	...
	&nbsp;
	<a class="btnDestacado" href="javascript:Actua(document.forms['form1'],'MultiofertaSave2022.xsql?BOTON=RECEPCION_OK');">
...
<xsl:template name="botonesProvAceptarPedido">    
	...
	<xsl:otherwise>
		...
		&nbsp;
        <a class="btnDestacado" id="btnAceptar" href="javascript:Actua(document.forms['form1'],'MultiofertaSave2022.xsql?BOTON=CONFIRMAR');">
             <xsl:value-of select="document($doc)/translation/texts/item[@name='aceptar_minu']/node()"/>
        </a>
		&nbsp;
        <a class="btnDestacado" id="btnConsultar" href="javascript:Consultar(document.forms['form1'],'MultiofertaSave2022.xsql?BOTON=CONSULTAR');">
             <xsl:value-of select="document($doc)/translation/texts/item[@name='Consultar']/node()"/>
        </a>
		&nbsp;
        <a class="btnDestacado" id="btnRechazar" href="javascript:Rechazar(document.forms['form1'],'MultiofertaSave2022.xsql?BOTON=RECHAZAR');">
             <xsl:value-of select="document($doc)/translation/texts/item[@name='rechazar']/node()"/>
        </a>
		&nbsp;
...
<xsl:template name="botonesCuatroProgramaMultioferta">   
	...
	&nbsp;
	<a class="btnDestacado" href="javascript:Actua(document.forms['form1'],'MultiofertaSave2022.xsql?BOTON=RECHAZAR');">
...
<xsl:template name="botonesMultioferta13o25">    
 <!--idioma-->                                             
        <xsl:variable name="lang">
         	<xsl:choose>
                <xsl:when test="/Multioferta/LANG"><xsl:value-of select="/Multioferta/LANG" /></xsl:when>
      ...
      <!--idioma fin-->
		...
		<xsl:if test="not(/Multioferta/MULTIOFERTA/BLOQUEAR_RECEPCION)">
        <a class="btnDestacado" href="javascript:Actua(document.forms['form1'],'MultiofertaSave2022.xsql?BOTON=RECEPCION_OK');" id="botonRecibido" style="display:;">
             <xsl:value-of select="document($doc)/translation/texts/item[@name='pedido_recibido']/node()"/>
        </a>
		&nbsp;
        <a class="btnDestacado" href="javascript:Actua(document.forms['form1'],'MultiofertaSave2022.xsql?BOTON=RECEPCION_PARCIAL');" id="botonRecibidoParc" style="display:none;">
		...
		<xsl:if test="not(/Multioferta/MULTIOFERTA/OBLIGAR_PEDIDO_COMPLETO)">
			<xsl:if test="not(/Multioferta/MULTIOFERTA/BLOQUEAR_ABONO)">
        	<a class="btnDestacado" href="javascript:Actua(document.forms['form1'],'MultiofertaSave2022.xsql?BOTON=ABONO');" id="botonAbono" style="display:none;">
            	 <xsl:value-of select="document($doc)/translation/texts/item[@name='crear_abono']/node()"/>
        	</a>
			&nbsp;
			</xsl:if>
			<xsl:if test="not(/Multioferta/MULTIOFERTA/BLOQUEAR_BACKORDER)">
        	<a class="btnDestacado" href="javascript:Actua(document.forms['form1'],'MultiofertaSave2022.xsql?BOTON=BACKORDER');" id="botonBackorder" style="display:none;">
            	 <xsl:value-of select="document($doc)/translation/texts/item[@name='crear_backorder']/node()"/>
        	</a>
			&nbsp;
			</xsl:if>
			<xsl:if test="not(/Multioferta/MULTIOFERTA/BLOQUEAR_LEGALIZACION)">
        	<a class="btnDestacado" href="javascript:Actua(document.forms['form1'],'MultiofertaSave2022.xsql?BOTON=LEGALIZAR');" id="botonLegalizar" style="display:none;">
        ...
        <tr class="amarillo"> 
          <td class="veinte">&nbsp;</td>       
 	      <td>
			...
			<xsl:if test="not(/Multioferta/MULTIOFERTA/USUARIO_MULTIEMPRESA)">
            <a class="btnNormal">
...
<!--28abr23 Template de programacion de entregas	-->
<xsl:template name="tpProgramacion">
	<!--idioma-->                                             
    <xsl:variable name="lang">
        <xsl:choose>
            <xsl:when test="/Multioferta/LANG"><xsl:value-of select="/Multioferta/LANG" /></xsl:when>
    ...
    <xsl:variable name="doc">http://www.newco.dev.br/General/texts_<xsl:value-of select="$lang"/>.xml</xsl:variable>
    <!--idioma fin--> 
	<div class="divLeft fondoGrisClaro">
		<p class="tituloTabla textCenter"><xsl:value-of select="document($doc)/translation/texts/item[@name='Programacion_de_entregas']/node()"/></p>
		<div class="tabela tabela_redonda">
		<table cellspacing="6px" cellpadding="6px" class="w600px tableCenter">
			<thead class="cabecalho_tabela">
			<tr>
				<th class="w1x"></th>
				<th><xsl:value-of select="document($doc)/translation/texts/item[@name='referencia']/node()"/></th>
				<th class="textLeft"><xsl:value-of select="document($doc)/translation/texts/item[@name='nombre']/node()"/></th>
				<th class="textLeft"><xsl:value-of select="document($doc)/translation/texts/item[@name='Entregas']/node()"/></th>
			</tr>
			</thead>
			<!--	Cuerpo de la tabla	-->
			<tbody class="corpo_tabela">
			<xsl:for-each select="LINEASMULTIOFERTA/LINEASMULTIOFERTA_ROW">
				<xsl:if test="ENTREGAS/ENTREGA">
					<tr>
						<td class="color_status">&nbsp;</td>
            			<xsl:choose>
                			<xsl:when test="//USUARIO/IDEMPRESA = //DATOSPROVEEDOR/EMP_ID">
                				<td><xsl:value-of select="PRO_REFERENCIA" /></td>
                 				<td class="textLeft"><xsl:value-of select="PRO_NOMBRE" /></td>
                			</xsl:when>
                			<xsl:otherwise>
                				<td><xsl:value-of select="REFERENCIA_CLIENTE"/></td>
               					<td class="textLeft"><xsl:value-of select="NOMBRE_PRIVADO"/></td>
                			</xsl:otherwise>
            			</xsl:choose>
						<td class="textLeft">
						<xsl:for-each select="ENTREGAS/ENTREGA">
							<strong><xsl:value-of select="FECHA"/>:<xsl:value-of select="CANTIDAD"/></strong><br/>
						</xsl:for-each>
						</td>
					</tr>
				</xsl:if>
