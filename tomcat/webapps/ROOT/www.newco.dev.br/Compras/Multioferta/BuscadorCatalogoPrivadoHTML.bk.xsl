...
(formu.elements['LLP_PROVEEDOR'].value=='')) {
...
function Enviar(formDestino, Accion)
{
    ...
    else { form.elements['SOLO_OCULTOS'].value = 'N'; }
    
    //alert('FIDProveedor '+formDestino.elements['FIDProveedor'].value);
	...
	function CambiarUsuario(idUsuario){
		...
		if(form.elements['SOLO_OCULTOS_IN'].checked == true){
			...
			//Enviar(document.forms[0],'BUSCAR');
		}else{
    ...
    /*else{
      ...
      <body>  
       ...
       <!--idioma-->
		...
		<input type="hidden" name="LLP_NOMBRE"><!--	PRODUCTO	-->
			<xsl:attribute name="value">
				<xsl:value-of select="/ProductosEnPlantillas/LISTAPRODUCTOS/PRODUCTOBUSCADO"/>
			</xsl:attribute>
		</input>
		<input type="hidden" name="LLP_PROVEEDOR"><!--	PROVEEDOR	-->
			<xsl:attribute name="value">
				<xsl:value-of select="/ProductosEnPlantillas/LISTAPRODUCTOS/PROVEEDORBUSCADO"/>
			</xsl:attribute>
		</input>
		...
		<xsl:if test="/ProductosEnPlantillas/LISTAPRODUCTOS/LISTAEMPRESAS">
			...
			<table class="infoTable">
             <tr>
				<td class="labelRight quince"><xsl:value-of select="document($doc)/translation/texts/item[@name='empresas_cliente']/node()"/>:</td>
				<td class="textleft veintecinco">
				...
				<td class="veinte">
					<div class="boton">
						<a href="javascript:Enviar(document.forms[0],'BUSCAR');">
        ...
        <xsl:if test="/ProductosEnPlantillas/LISTAPRODUCTOS/IDUSUARIOCOMPROBAR != '18246'"><!--añadido el 6-11-13-->
        <xsl:choose>
                ...
                <!--elegir usuario para asisa-->
                     <tr height="30px;">
                        ...
                        <!--<xsl:value-of select="//IDUSUARIOCOMPROBAR"/>-->
                            ...
                            <xsl:call-template name="desplegable">
                                <xsl:with-param name="path" select="/ProductosEnPlantillas/LISTAPRODUCTOS/LISTAUSUARIOS/field"></xsl:with-param>
                        ...
                        <th class="dies">
                            ...
                            &nbsp;<xsl:value-of select="document($doc)/translation/texts/item[@name='solo_ocultos']/node()"/></p>          
                        </th>
                        <th>&nbsp;</th>
                     </tr>
                </table>
		...
		<table class="encuesta" border="0">
        	 <tr class="lejenda">	
                ...
                <!--emplantillar-->
                        ...
                        <xsl:call-template name="desplegable">
        				<xsl:with-param name="path" select="/ProductosEnPlantillas/LISTAPRODUCTOS/FIDProveedor/field"></xsl:with-param>
                        <xsl:with-param name="onChange">Enviar(document.forms[0],'BUSCAR');</xsl:with-param>
      					</xsl:call-template>
					</th>
					<th class="doce">
					...
					<th class="tres"><xsl:copy-of select="document($doc)/translation/texts/item[@name='ud_ba_lote_2line']/node()"/>
                   </th>
					<th class="seis">
                		...
                		<td>
                         ...
                         <td>   
				        		<xsl:if test="/ProductosEnPlantillas/LISTAPRODUCTOS/ADMIN_CDC">
                                    ...
                                    <xsl:otherwise>
	    								<a>
                                         <xsl:attribute name="href">javascript:MostrarPagPersonalizada('http://www.newco.dev.br/ProcesosCdC/MantenimientoReducido/MantenimientoReducido.xsql?PRO_ID=<xsl:value-of select="IDPRODUCTO"/>&amp;EMP_ID=<xsl:value-of select="/ProductosEnPlantillas/LISTAPRODUCTOS/IDCLIENTE"/>','catalogoprivado',100,80,75,0);
                                </xsl:attribute>
			...
			<xsl:for-each select="PLANTILLAS/PLANTILLA">
				...
				<xsl:otherwise>
					<xsl:choose>
					<xsl:when test="ID!='MASPLANTILLAS'">
						<a class="noDecor">
							<xsl:attribute name="href">javascript:EjecutarFuncionDelFrame('zonaPlantilla',<xsl:value-of select="ID"/>);</xsl:attribute>
							...
							<xsl:if test="/ProductosEnPlantillas/LISTAPRODUCTOS/MOSTRAR_BOTON_OCULTOS">
								<xsl:choose>
					...
					<td align="center">
                        ...
                        <xsl:value-of select="substring(MARCA,0,15)"/>
					</td> 
					<!--<td class="textLeft">
          				<xsl:for-each select="PLANTILLAS/PLANTILLA">
				        	<xsl:choose>
