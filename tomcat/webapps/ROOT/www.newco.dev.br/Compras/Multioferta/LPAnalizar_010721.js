...
//	Última revisión: ET 1jul21 15:00 LPAnalizar_010721.js
...
function aplicarFiltro(filtroTxt, IDTipoNeg){
	...
	//solodebug	debug('FiltroSeleccion:'+filtroTxt+' IDTipoNeg:'+IDTipoNeg);
	...
	jQuery.each(arrayLineasProductos, function(key, linea){
		...
		//solodebug ET 26may17	debug('FiltroSeleccion:'+FiltroSeleccion+' Sel:'+linea.ContieneSeleccion+' Producto:'+linea.Productos[0].filtroTxtBuscador.toLowerCase()+' Filt:'+linea.ContieneFiltroTXT);
...
function Actua(){
	...
	jQuery.each(arrayLineasProductos, function(key, linea){
		if(linea.Productos[0].Seleccionado == 'S'){
			...
			LineaEspecial=((linea.Productos[0].Regulado=='S')?'Regulado':(linea.Productos[0].Pack=='S')?'Pack':(linea.Productos[0].Convenio=='S')?'Convenio':(linea.Productos[0].Deposito=='S')?'Deposito':'Normal');

			//solodebug	debug('Pedido Especial:'+Especial+' Regulado:'+linea.Productos[0].Regulado+' Pack:'+linea.Productos[0].Pack+' LineaEspecial:'+LineaEspecial+' error:'+mensajeError);
			...
			else if ((Especial!=LineaEspecial)&&(Especial!='ERROR'))
			{
				...
				if ((Especial=='Convenio')||(LineaEspecial=='Convenio'))
					mensajeError += str_NoMezclarConvenioYNoConvenio;
				if ((Especial=='Deposito')||(LineaEspecial=='Deposito'))
