//	JS de MOF_25_RW
//	Ultima revisión ET 2oct18 11:32 MOF_25_RW_JS_070121.js
...
function Actua(formu,accion){
	...
	if(accion.match('RECEPCION_OK') || accion.match('RECEPCION_PARCIAL') || accion.match('ABONO') || accion.match('BACKORDER') || accion.match('LEGALIZAR')){
		...
		var fechaEntregatmp=new Date();
		if (formu.elements['FECHANO_ENTREGA'])		//	7ene21 Protegemos por si no existe el campo
		{										
			var fechaEntregatxt=obtenerSubCadena(formu.elements['FECHANO_ENTREGA'].value,2)+'/'+obtenerSubCadena(formu.elements['FECHANO_ENTREGA'].value,1)+'/'+obtenerSubCadena(formu.elements['FECHANO_ENTREGA'].value,3);
			fechaEntregatmp=new Date(fechaEntregatxt);
		}
...
/*	debug('inicializarImportes. MO_SUBTOTAL:'+form.elements['MO_SUBTOTAL'].value);
	debug('inicializarImportes. IMPORTE_FINAL_PEDIDO:'+form.elements['MO_SUBTOTAL'].value);

	debug('inicializarImportes. MO_SUBTOTAL(2):'+desformateaDivisa(form.elements['MO_SUBTOTAL'].value));
	debug('inicializarImportes. IMPORTE_FINAL_PEDIDO(2):'+desformateaDivisa(form.elements['IMPORTE_FINAL_PEDIDO'].value));

	debug('inicializarImportes. MO_SUBTOTAL(3):'+formateaDivisa(Round(desformateaDivisa(form.elements['MO_SUBTOTAL'].value),2)));
	debug('inicializarImportes. IMPORTE_FINAL_PEDIDO(3):'+formateaDivisa(Round(desformateaDivisa(form.elements['IMPORTE_FINAL_PEDIDO'].value),2)));

	debug('inicializarImportes. MO_SUBTOTAL(4):'+anyadirCerosDecimales(formateaDivisa(Round(desformateaDivisa(form.elements['MO_SUBTOTAL'].value),2)),2));
	debug('inicializarImportes. IMPORTE_FINAL_PEDIDO(4):'+anyadirCerosDecimales(formateaDivisa(Round(desformateaDivisa(form.elements['IMPORTE_FINAL_PEDIDO'].value),2)),2));


	debug('inicializarImportes. MO_SUBTOTAL B:'+SubtotalTxt);
	debug('inicializarImportes. IMPORTE_FINAL_PEDIDO B:'+TotalTxt);

	debug('inicializarImportes. MO_SUBTOTAL(B2):'+desformateaDivisa(SubtotalTxt));
	debug('inicializarImportes. IMPORTE_FINAL_PEDIDO(B2):'+desformateaDivisa(TotalTxt));

	debug('inicializarImportes. MO_SUBTOTAL(B3):'+formateaDivisa(Round(desformateaDivisa(SubtotalTxt),2)));
	debug('inicializarImportes. IMPORTE_FINAL_PEDIDO(B3):'+formateaDivisa(Round(desformateaDivisa(TotalTxt),2)));

	debug('inicializarImportes. MO_SUBTOTAL(B4):'+anyadirCerosDecimales(formateaDivisa(Round(desformateaDivisa(SubtotalTxt),2)),2));
	debug('inicializarImportes. IMPORTE_FINAL_PEDIDO(B4):'+anyadirCerosDecimales(formateaDivisa(Round(desformateaDivisa(TotalTxt),2)),2));
*/
//SubtotalTxt IVATxt CosteTransporteTxt
//CosteTransporteIVATxt TotalTxt


    form.elements['MO_SUBTOTAL'].value=anyadirCerosDecimales(formateaDivisa(Round(desformateaDivisa(SubtotalTxt),2)),2);
    form.elements['MO_IMPORTEIVA'].value=anyadirCerosDecimales(formateaDivisa(Round(desformateaDivisa(IVATxt),2)),2);
    form.elements['COSTELOGISTICA'].value=anyadirCerosDecimales(formateaDivisa(Round(desformateaDivisa(CosteTransporteTxt),2)),2);
    form.elements['COSTELOGISTICA_IVA'].value=anyadirCerosDecimales(formateaDivisa(Round(desformateaDivisa(CosteTransporteIVATxt),2)),2);
    form.elements['IMPORTE_FINAL_PEDIDO'].value=anyadirCerosDecimales(formateaDivisa(Round(desformateaDivisa(TotalTxt),2)),2);
