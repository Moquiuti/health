...
//	Ultima revision: ET 17may23 09:00 CVGenerar2022_170523.js
...
function inicializarDesplegableCentros(centroSeleccionado, idLugarSeleccionado, idCentroConsumoSeleccionado){
	...
	//solodebug debug('inicializarDesplegableCentros. centroSeleccionado:'+centroSeleccionado+' idLugarSeleccionado:'+idLugarSeleccionado+' idCentroConsumoSeleccionado:'+idCentroConsumoSeleccionado);

	for(var n=0; n<arrayCentros.length; n++){
		//solodebug	debug('inicializarDesplegableCentros. centroSeleccionado:'+centroSeleccionado+' revisando('+n+'):'+arrayCentros[n][0]);
	
		if(arrayCentros[n][0] == centroSeleccionado){
		
			//solodebug	debug('inicializarDesplegableCentros. centroSeleccionado:'+centroSeleccionado+' encontrado en posicion:'+n);
			...
			//coste de transporte
			oForm.elements['COSTE_LOGISTICA'].value	= anyadirCerosDecimales(FormatoNumero(Round(desformateaDivisa(arrayCentros[n][3]),2)),2);
	...
	for(var i=0; i<document.forms.length; i++){
		...
		for(var n=0; n<form.length; n++){
			...
			if(form.elements[n].name.substring(0,16)=='MO_IMPORTETOTAL_' && form.elements[n].value!='Por definir'){
				...
				//22ago22 desformateaDivisa ya incluye parseFloat. var total = parseFloat(desformateaDivisa(form.elements[n].value));
				var total = desformateaDivisa(form.elements[n].value);
				//var coste = parseFloat(desformateaDivisa(form.elements['COSTE_LOGISTICA_'+moidpedido].value));
				//22ago22 desformateaDivisa ya incluye parseFloat.  var iva = parseFloat(desformateaDivisa(form.elements['IVA_'+moidpedido].value));
				var iva = desformateaDivisa(form.elements['IVA_'+moidpedido].value);
				...
				//solodebug		console.log('inicializarImportes. total:'+total+' iva:'+iva+' costeTransporte:'+costeTransporte+' IvaCoste:'+IvaCoste+' totFinal:'+totFinal);

				// Aqui lo unico que hace es presentar el importe con 2 decimales en lugar de 4, pero no se cambia
				form.elements[n].value=anyadirCerosDecimales(FormatoNumero(Round(desformateaDivisa(form.elements[n].value),2)),2);
			}
...
function EnviarPedidos(formPrincipal, linka, formPedido){
	...
	if (arrCampos.length>0)
	{
		...
		{
			comentDeFormulario+=arrCampos[i].Etiqueta+jQuery("#Campo_"+i).val().replace(/#/g,'').replace(/|/g,'');
	...
	formPrincipal.action = "http://www.newco.dev.br/Compras/Multiofertas/CVGenerarSave2022.xsql";
	...
	for(var i=0; i<formPrincipal.length; i++){
		...
		if(nom.indexOf("COMENTARIO_") != -1){
			...
			if (ComentarioDeParametros!='')
				coments	+= ComentarioDeParametros + '\n\n';
				
			if (comentDeFormulario!='')
				coments	+= comentDeFormulario + '\n\n';
			
			
			coments	+=	sepComentario + formPrincipal.elements['COMENTARIO_'+mo_id].value.replace(/\'/g,'&apos;').replace(/#/g,'').replace(/|/g,'') 
							+ "#" + formPrincipal.elements['NUMERO_OFERT_PED_'+mo_id].value + "#";
...
function ultimosComentarios(nombreObjeto, nombreForm, tipoComentario){
	...
	MostrarPagPersonalizada(Dominio+'/Compras/NuevaMultioferta/UltimosComentarios2022.xsql?NOMBRE_OBJETO='+nombreObjeto+'&NOMBRE_FORM='+nombreForm+'&ACCION='+accion+'&TIPO='+tipoComentario+'&COMENTARIO='+document.forms[nombreForm].elements[nombreObjeto].value.replace(/\n/g,'\\\\n'),'comentarios',100,80,0,0);
}
...
{
	MostrarPagPersonalizada('http://www.newco.dev.br/Compras/Multioferta/MODocs2022.xsql?MO_ID='+IDMultioferta,'Documentos',100,80,0,0);
}
