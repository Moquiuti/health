...
//	Ultima revision: ET 28oct22 10:30 MantPedidosProgramados2022_281022.js
...
//	13set22 Extraemos tambien del XSL la funcion de inicio, que se montaba en base a condiciones XSL
function Inicio()
{

	var fecEntrega=jQuery('#FECHANO_ENTREGA').val();
	var fecFin=jQuery('#FECHANO_FIN').val();

	//solodebug
	debug("onLoad. FechaEntrega:"+fecEntrega+' FechaFin:'+fecFin);

	if (fecEntrega!='')
		jQuery('#dtFECHAENTREGA').val(FechaParaControlDate(fecEntrega));
	
	if (fecFin!='')
		jQuery('#dtFECHAFIN').val(FechaParaControlDate(fecFin));
	...
	if (esAdmin=='S')
	{
        ...
        if (read_only=='N')
		{
            ...
            CargarUsuarios('IDUSUARIO',IDUsuario,IDProveedor,idOfertaModelo);
		}
		else
		{
			//  nueva programacion
            CargarUsuarios('IDUSUARIO',IDUsuario,'-1','-1');
		}
	 }
	 else
	 {
		...
		{
            // existe la programacion
            CargarProveedores('IDPROVEEDOR',IDUsuario,'-1');   
		}
		else
		{
            // nueva programacion
	...
	if (ExisteProg=='S')
    	document.forms['form1'].elements['FECHA_INICIO'].value=incrementarFecha(inicioProg,0);
	else
		document.forms['form1'].elements['FECHA_INICIO'].value=incrementarFecha(fecActual,1);

    // inicializacion de la fecha de lanzamiento
	if (ExisteProg=='S')
	{
		if (estadoProg=='F')
          	document.forms['form1'].elements['FECHANO_LANZAMIENTO'].value=incrementarFecha(fecActual,1);
		else
			document.forms['form1'].elements['FECHANO_LANZAMIENTO'].value=incrementarFecha(fecLanzamiento,0);
	}
	else
	{
		document.forms['form1'].elements['FECHANO_LANZAMIENTO'].value=incrementarFecha(fecActual,1);
...
{
	var accion='http://www.newco.dev.br/Compras/PedidosProgramados/BorrarPedidoProgramado2022.xsql';
...
/*	14set22 No tienen sentido estas validaciones
...
function SolicitarFecha(nombreFecha,objCheck){
	...
	//solodebug
	debug('SolicitarFecha('+nombreFecha+')');
...
function GuardarCambios()
{
	...
	form.action='http://www.newco.dev.br/Compras/PedidosProgramados/MantPedidosProgramados2022.xsql';	//	13set22 antes Save
	form.elements['ACCION'].value='MODIFICAR';
	...
	if(((form.elements['PEDP_ID'].value<=0 || form.elements['PEDP_ID'].value=='') && form.elements['IDFRECUENCIA'].value=='Z')||(form.elements['IDFRECUENCIA'].value=='Z' && form.elements['IDFRECUENCIA_ANTERIOR'].value!=form.elements['IDFRECUENCIA'].value)){
		...
		//solodebug		debug('GuardarCambios fechaEntrega:'+ form.elements['FECHANO_ENTREGA'].value+' fechaLanzamiento:'+form.elements['FECHANO_LANZAMIENTO'].value+' fechaFin:'+form.elements['FECHANO_FIN'].value+' Recalcular:'+form.elements['RECALCULAR'].value);
		//solodebug	alert(' COntrol antes de guardar');
...
function habilitarDeshabilitarFechas(valor){
	...
	//solodebug
	debug('habilitarDeshabilitarFechas.valor:'+valor);
	...
	if(valor=='1')
	{
		...
		//14set22	calFechaEntrega.enabled=true;
		document.forms['form1'].elements['FECHANO_ENTREGA'].disabled=false;

		document.forms['form1'].elements['FECHANO_FIN'].value=document.forms['form1'].elements['FECHANO_ENTREGA'].value;
		//14set22	calFechaFin.enabled=false;
		document.forms['form1'].elements['FECHANO_FIN'].disabled=true;
	}
	else
	{
		...
		if(valor=='Z')
		{
			...
			document.forms['form1'].elements['FECHANO_ENTREGA'].value='';
			//14set22	calFechaEntrega.enabled=false;
		...
		else
		{
			...
			//14set22	calFechaFin.enabled=true;
...
//	13set22 Todas las funciones de calculo de fechas estaban montadas en una sola. Muy complicado de gestionar. Separamos.

//	13set22 Calcula fecha de fin de programa
function calculaFechaFin()
{

	form=document.forms['form1'];
	objFecha=document.forms['form1'].elements['FECHANO_FIN'];

	var FechaFinFormatoIngles=jQuery('#dtFECHAFIN').val();
	var FechaFin=FechaDeControlDate(FechaFinFormatoIngles);
	var fFechaFin=new Date(FechaFinFormatoIngles);
	
	console.log('calculaFechaFin:'+FechaFinFormatoIngles+' -> '+fFechaFin);
	...
	//	miramos que la fecha de FIN sea laborable, mas grande que la de INICIO y la de lanzamiento
    ...
    if(fechaAnterior)
	{
		...
		{
          jQuery('#FECHANO_FIN').val(FechaEntrega);
		  jQuery('#dtFECHAFIN').val(FechaParaControlDate(FechaEntrega));
        }
        else
		{
          jQuery('#FECHANO_FIN').val('');
		  jQuery('#dtFECHAFIN').val('');
        }
    }
	else if(!esLaborable)
	{
		...
		{
          jQuery('#FECHANO_FIN').val(FechaFinTmp);
		  jQuery('#dtFECHAFIN').val(FechaParaControlDate(FechaFinTmp));
        }
        else
		{
          jQuery('#FECHANO_FIN').val('');
	...
	{
		debug('calculaFechaFin. DATOS OK. FEchaFin:'+FechaFin);
		jQuery('#FECHANO_FIN').val(FechaFin);
	}

  //alert('LNZ: '+form.elements['FECHANO_LANZAMIENTO'].value+'\nENT: '+form.elements['FECHANO_ENTREGA'].value);
	//solodebug
	debug('calculaFechaFin. FEchaFin:'+form.elements['FECHANO_FIN'].value);

}

/*
//	13set22 Calcula fecha de lanzamiento de programa
function calculaFechaLanzamiento()
{
	form=document.forms['form1'];

	var FechaLanzamientoFormatoIngles=jQuery('#dtFECHAENTREGA').val();
	var FechaLanzamiento=FechaDeControlDate(FechaLanzamientoFormatoIngles);
	var fFechaLanzamiento=new Date(FechaLanzamientoFormatoIngles);
	
	console.log('calculaFechaLanzamiento:'+FechaLanzamientoFormatoIngles+' -> '+fFechaLanzamiento);
	...
	//solodebug
	debug('calculaFechaLanzamiento. Entrega:'+form.elements['FECHANO_ENTREGA'].value+' Lanzamiento:'+form.elements['FECHANO_LANZAMIENTO'].value+' Fin:'+form.elements['FECHANO_FIN'].value);

}
*/


//	7oct22 Calcula fecha de INICIO de programa
function calculaFechaEntrega()
{
	//	Si se ha cambiado la fecha de entrega, recalcularemos el programa
	recalcularPrograma=1;
	
	
	form=document.forms['form1'];

	var FechaEntregaFormatoIngles=jQuery('#dtFECHAENTREGA').val();
	var FechaEntrega=FechaDeControlDate(FechaEntregaFormatoIngles);
	var fFechaEntrega=new Date(FechaEntregaFormatoIngles);  
	
	console.log('calculaFechaEntrega:'+FechaEntregaFormatoIngles+' -> '+fFechaEntrega);

	//7oct22
    form.elements['FECHANO_ENTREGA'].value=FechaEntrega;
	...
	//solodebug
	debug('calculaFechaEntrega. fechaAnterior: '+fechaAnterior+' fechaPosterior: '+fechaPosterior+' fechaLaborable '+fechaLaborable+' fechaConCambio: '+fechaConCambio);
      ...
      if(fechaAnterior){
		...
		//solodebug
		debug('calculaFechaEntrega.'+FechaEntregaMinima+'->'+FechaDeControlDate(FechaEntregaMinima));

          jQuery('#dtFECHAENTREGA').val(FechaParaControlDate(FechaEntregaMinima));
      ...
      if(!esLaborable){
        if(confirm('La fecha de entrega actual no corresponde a un día laborable.\n\n¿Desea asignar el '+FechaEntregaTmp+' como la fecha de entrega?')){
          jQuery('#dtFECHAENTREGA').val(formatoFecha(FechaEntregaTmp,'E','I'));
          form.elements['FECHANO_ENTREGA'].value=FechaEntregaTmp;
          form.elements['FECHANO_LANZAMIENTO'].value=convertirFechaATexto(calcularDiasHabiles(fFechaEntregaTmp,-plazoEntregaUsado));
        }
        else{
	...
	//solodebug
	debug('calculaFechaEntrega fechaEntrega:'+ form.elements['FECHANO_ENTREGA'].value+' fechaLanzamiento:'+form.elements['FECHANO_LANZAMIENTO'].value+' fechaFin:'+form.elements['FECHANO_FIN'].value);
	}
}
...
function calculaFecha(form,objFecha, mas){  // nom: nombre del objFecha que queremos calcular
  ...
  //
  console.log("calculaFecha:"+objFecha.name);
...
//	16set22 Mostramos el calendario para programacion manual
function Calendario()
{
	//javascript:MostrarPag('http://www.newco.dev.br/Compras/PedidosProgramados/LanzamientosPedidosProgramados2022.xsql?IDUSUARIO='+IDUsuario+'&PEDP_ID='+pedp_id+'&FECHAACTIVA='+document.forms['form1'].elements['FECHANO_ENTREGA'].value+'&VENTANA=NUEVA'+'&ACCION=LanzamientosPedidosProgramadosSave2022.xsql&ACTUALIZARPAGINA='+progManual+'&TITULO=Calendario%20programa&READ_ONLY='+read_only,'lanzamientosProgramas');
	javascript:MostrarPag('http://www.newco.dev.br/Compras/PedidosProgramados/LanzamientosPedidosProgramados2022.xsql?PEDP_ID='+pedp_id+'&FECHAACTIVA='+document.forms['form1'].elements['FECHANO_ENTREGA'].value,'CalendarioPrograma');
}
