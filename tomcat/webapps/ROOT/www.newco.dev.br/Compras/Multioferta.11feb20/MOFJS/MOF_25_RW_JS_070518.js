//	JS de MOF_25_RW
//	Ultima revisión ET 8may18 10:13
...
/*
	nacho 26/2/2002
	aÃ±adimos el boton cancelar pedido para el estado 10 que llama a la funcion cancelarPedido()
*/
  ...
  for(var n=0;n<form.length;n++){
    if(form.elements[n].name=='COMBO_ENTREGA_REAL' || form.elements[n].name=='FECHA_ENTREGA_REAL' || form.elements[n].name=='NMU_COMENTARIOS' || form.elements[n].name.substring(0,18)=='CantidadEntregada_'){
      if(form.elements[n].name=='NMU_COMENTARIOS'){
        ...
        CadenaDatos+=form.elements[n].name+'|'+cadenaTemp+'Â·';
      }
      else{
        CadenaDatos+=form.elements[n].name+'|'+form.elements[n].value+'Â·';
      }
    }
  }
  for(var n=0;n<document.forms['form1'].length;n++){
    if(document.forms['form1'].elements[n].name.substring(0,12)=='CHKRECIBIDO_'){
      CadenaDatos+=document.forms['form1'].elements[n].name+'|'+document.forms['form1'].elements[n].value+'Â·';
    }
  }   
  return CadenaDatos;
}
...
function cargarDatosActuales(form, CadenaDatos){
  ...
  var arrParNombreValor=CadenaDatos.split('Â·');
