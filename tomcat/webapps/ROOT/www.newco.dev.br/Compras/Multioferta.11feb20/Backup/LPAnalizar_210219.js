/*
	...
	Últimos cambios: 21feb19 14:58
*/


var v_SolicitudMuestras	= 'NO';			//	10abr07	ET	control de solicitud de muestras
var v_OrdenarProductos	= 'NO';			//	11nov15	DC	control para ordenar productos alfabéticamente (Solución para Brasil)
...
var v_OcultarNoSeleccionados= 'NO';		//	26may17	ET	Para plantillas largas, ocultaremos las líneas no seleccionadas
var FiltroSeleccion = false;			//	26may17	ET	Variable global para informar si estamos filtrando por texto o no
var cLimitePlantillaVisible=150;		//	26may17	ET	Limite de plantilla máximo para el funcionamiento tradicional
...
//console.log(limiteComprasMensual);
//console.log(fechaCuota);
//console.log(comprasPrevias);
	...
	jQuery("#strFiltro").on('input propertychange', function(){
		...
		//26mar18	no funciona bien para OHSJD, mostramos todos
		//if (filtroTXT.length>=3)		//	26may17 empezamos a mostrar cadenas a partir de 3 caracteres, para evitar demasiados resultados
	...
	// DC - 11nov2015 - Brasil pide ordenar los productos por orden alfabético (sin bloques subfamilias)
	// Aqui preparamos el array de ordenación para preparar los datos para dibujar la tabla
	prepararTablaProductos();

	// Aqui dibujamos la tabla con los productos con la info que viene del PL/SQL
	//dibujaLineasPedido();

	// Validamos por si hay cantidades que no se corresponden con las UdsXLote (no son multiplo)
	validarCantidades();

	// Calculamos la linea de totales (es necesario??)
// 28may15 - DC - Se inicializa a '0'	sumar_pedidoDirecto();
...
function inicializarDesplegableCentros(centroSeleccionado){
	...
	else
	{
		for(var n=0; n<arrayCentros.length; n++)
		{
			...
			if (arrayCentros[n][0]==centroSeleccionado) CentroExiste='S';
		}
	...
	if (centroSeleccionado==-1)	mostrarDesplegableLugarDeEntrega(IDPrimerCentro, IDLugarEntrega);
	else mostrarDesplegableLugarDeEntrega(centroSeleccionado, '');
}
...
function mostrarDesplegableLugarDeEntrega(centroSeleccionado, IDLugarEntregaActual){

	//solodebug console.log('mostrarDesplegableLugarDeEntrega centroSeleccionado:'+centroSeleccionado+' IDLugarEntregaActual:'+IDLugarEntregaActual);
	...
	if (IDLugarEntregaActual!='')
		document.forms['formBot'].elements['IDLUGARENTREGA_'+centroSeleccionado].value = IDLugarEntregaActual;
...
function aplicarFiltro(string){
	var arrStr	= string.split(" ");
	var check;

	//solodebug ET 26may17 	console.log('FiltroSeleccion:'+string);
	...
	jQuery.each(arrayLineasProductos, function(key, linea){
		...
		if( (!FiltroSeleccion ||linea.ContieneSeleccion == 'S') && linea.ContieneFiltroTXT == 'S'){
			linea.Mostrar = 'S';
		}else{
			linea.Mostrar = 'N';
		}
		
		//solodebug ET 26may17	console.log('FiltroSeleccion:'+FiltroSeleccion+' Sel:'+linea.ContieneSeleccion+' Producto:'+linea.Productos[0].stringBuscador.toLowerCase()+' Filt:'+linea.ContieneFiltroTXT);
...
var macroInputText2	= '<input type="text" size="#SIZE#" name="#NAME#" class="#CLASS#" value="#VALUE#" style="#STYLE#" maxlength="#MAXLENGTH#" onBlur="#ONBLUR#" onInput="#ONINPUT#"/>';
...
//	jQuery.each(arrayLineasProductos, function(key, linea){
	jQuery.each(ColumnaOrdenadaProds, function(key, value){
		...
		jQuery.each(arrayLineasProductos[value].Productos, function(key2, producto){
			...
			thisMacro = thisMacro.replace('#STYLE#', 'text-align:right;');
			thisMacro = thisMacro.replace('#MAXLENGTH#', '6');
...
//			functionJS= 'if(UnidadesALotes(this)) Cantidad2Importe(' + key + ', ' + hayPrecio + ');';
			...
			disabled='';
			if(producto.Tarifa=='' || (producto.TipoSinStock!='' && producto.TipoSinStock=='D')){
...
function sumar_pedidoDirecto(){
	...
	if(!porDefinir){
		...
		document.forms['sumaTotal'].elements['sumaTotal'].value=anyadirCerosDecimales(document.forms['formBot'].elements['sum'].value,2) + sufDivisa;
		//suma brasil
		document.forms['formBot'].elements['sumBrasil'].value=anyadirCerosDecimales(formateaDivisa(Round(sumaBrasil,2)),2);
		document.forms['sumaTotal'].elements['sumaTotalBrasil'].value=anyadirCerosDecimales(document.forms['formBot'].elements['sumBrasil'].value,2) + sufDivisa;
	}else{
...
function controlPedido(){
	var pedMinimo		= document.getElementById('CEN_PEDIDOMINIMO').value;
...
/* 29may15 - DC - La forma y plazo de pago ya no se pasan al frontal
			copiarFormaPago();
*/
...
*/
			...
			if(document.forms['sumaTotal'].elements['sumaTotal'].value=='Sol.Muestra' || document.forms['sumaTotal'].elements['sumaTotal'].value=='Sol.Amostra'){
				...
				if(mensajeError==""){
					...
					alert(mensajeError);
					//16jul09	ET	volvemos a activar el botï¿½n
					jQuery('#divContinuar').show();
				}
				//fin if-else envio
