...
function globalEvents(){
	...
	jQuery("#Submit").click(function(){
		// Escondemos el boton de 'Entrar' para evitar el doble-click
		jQuery("#Submit").hide();
		AbrirVentana(document.forms['Login']);
		// Despues de 3 segundos volvemos a mostrar el boton de 'Enviar' para que no tengan que recargar la pagina para hacer un nuevo login
		setTimeout(function(){jQuery('#Submit').show()},3000);
		return false;
	});
	
	//como trabajamos
	jQuery("#catalogacion").click(function(){
					alert('mirta');
				jQuery("#catalogacionBox").css('display','block');
				//jQuery(this).css('background','#ffff66');
			});
	
	//multioferta frame			  
	jQuery("#verComentOculto").click (function() { /*alert('mi');*/ jQuery("#comentOcultos .comentOculto").show();});
	
	jQuery("#olvidoContra").mouseover (function() { this.style.cursor="pointer"; this.style.color="#8d5fc1"; });
	jQuery("#olvidoContra").mouseout (function() { this.style.cursor="default"; this.style.color="#000"; });
...
//inicio de eliminar coockie 13-10-11
...
function EnviaContacto(formu){
	...
	//alert ('mi '+pais);
	//alert(formu.elements['CONTACTO_TIPO_EMP'].value);
 ...
 /**   Copia de General.js - Funciones generales - (c) Medical Virtual Market, 2001   */
 ...
 //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	/*
		  ...
		  if (!(er_mes31dias.test(Fecha.value) || 
			...
			errores='Introduzca una fecha v?lida en el formato dd/mm/aaaa en el campo '+Fecha.name+'\n';
			return(errores);		
		  }
		  else return (errores);
	      ...
	      if (ch >= "0" && ch <= "9"){
	        ...
	        if(ch == "."){
	          ...
	          if(ch == ","){
	            ...
	            alert('Error en el dato '+checkString+', ha introducido un valor de num?rico incorrecto');
	            objeto.focus();	              
	            return false;	            
	          }
	        }
...
// Nombre: Decimales
// Descripci?n: efectuar el redondeo o el corte de un n?mero a los decimales precisados
// Autor: PGP 20000918
// Entradas:
//  num: numero a tratar
...
// Salida:
//  Devuelve el n?mero con los decimales pedidos
//
// Idea: multiplicar el n?mero por 10 elevado a la cantidad de decimales pedidos,
...
//
// Notas:
//  - Los n?meros comprendidos en valor absoluto >=0 y <1 s?lo se recortan, no se redondean.
...
{  
	...
	else
	{
        ...
        /* fin mostra alerta personalizada */
        ...
        function MostrarAlerta(titulo,codigo){  
          ...
          if (is_nav){
            ...
            // mostramos la ventana
            ventanaAlerta=window.open(pag,titulo,'toolbar=no,menubar=no,status=no,scrollbars=yes,resizable=yes,width='+ample+',height='+alcada+',screenX='+esquerra+',screenY='+alt);
            ventanaAlerta.focus();
...
//
// Funci?n para controlar la salida de debug de los procesos javascript en desarrollo
//
function debug (str) {
 ...
 */
 ...
 function anyadirCerosDecimales(valor,posicionesDecimales){
   ...
   if(tieneComa==0 && posicionesDecimales>0){
        ...
        */
        ...
        function obtenerFrame(framePadre,frameBuscado){
              ...
              if(framePadre.name==frameBuscado){
				...
				//alert('else mi '+ framePadre.name +' '+framePadre);
	...
	/*para viamed		
	function obtenerFr(framePadre,frameBuscado){
              var objeto=new Object();
			   var frameP= new Object();
              if('newMain'==frameBuscado){
                return framePadre;
              }
              else{
                if(framePadre.length>0){
                  for(var n=0;n<framePadre.length;n++){
                    frameP=obtenerFrame('newMain');
					objeto = (frameP.frames[n],frameBuscado);
                    if(objeto!=null){
                      return objeto;
                    }
                  }
...
//	4jul11	En el EIS y buscadores no permitimos cadenas de menos de long		
...
function sendPwd(email){
	...
	jQuery.ajax({
		cache:	false,
		url:	'http://www.newco.dev.br/MenusYDerechos/EnviaPwd.xsql',
		type:	"GET",
		data:	"EMAIL="+email+"&_="+d.getTime(),
		contentType: "application/xhtml+xml",
		success: function(objeto){
			var data = eval("(" + objeto + ")");

			if(data.Password.estado == 'OK'){
				jQuery('#emailError').html(okEmail).show();
                        }else{
				jQuery('#emailError').html(errEmailSQL).show();
			}
		},
