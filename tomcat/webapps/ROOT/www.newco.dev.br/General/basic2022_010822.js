/*
	...
	Ultima revision: 2jun23 12:35 basic2022_010822.js basic2022_010822.js

	Lanzar script si se cambia el nombre del fichero PRODUCCION:
	/home/mvm/scripts/changefilename.sh basic2022_010822.js basic2022_010822.js

	En desarrollo:
	./changefilename.sh basic2022_010822.js basic2022_010822.js
...
function MostrarPagPersonalizada(url,titulo,p_ancho,p_alto,p_desfaseLeft,p_desfaseTop){  
  ...
  if(titulo==null) var titulo='MedicalVM';
    ...
    win=window.open(url,titulo,'toolbar=no,menubar=no,status=no,scrollbars=yes,resizable=yes,width='+ample+',height='+alcada+',screenX='+esquerra+',screenY='+alt);
    win.focus();
}
...
//	Reemplaza el "." decimal por ",". Solo debe haber una ocurrencia
...
/*	3nov22	Simplificamos el codigo -> 4nov22 estas lineas dan error return cadNumero.replace('.',','); is not a function.
	return cadNumero.replace('.',',');*/
}


/*
    funcion que recibe un obj tipo texto si el numero (con o sin decimales) no tiene formato se lo da, si lo tiene no hace nada
...
function FormatoNumero(valor)
{
	...
	//3nov22	var nuevoValor;
	//3nov22	nuevaParteEntera;
	//3nov22	var coma=0;
	//3nov22	var punto=0;
	var vectorValores;
	var parteEntera;
	...
	{
		console.log('ERROR en FormatoNumero:'+valor+', devolviendo:'+valor+',00');
...
/*
	formateaDivisa, desformateaDivisa se mantienen por compatibilidad en MOFJS y Licitaciones, sustituir en cuanto sea posible
*/
//   recibe:   100000.10
//             100000,12
...
//	13set22 COnvierte una fecha del formato espannol al formato utilizado en input tipo date
function FechaParaControlDate(FechaEsp)
{
	var FechaTxt=FechaEsp.split('/');
	var Anno=FechaTxt[2];
	var Mes=FechaTxt[1];
	var Dia=FechaTxt[0];
	...
	FechaDt=Anno+'-'+Mes+'-'+Dia;
	return FechaDt;
}

//	13set22 COnvierte una fecha del formato espannol al formato utilizado en input tipo date
function FechaDeControlDate(FechaD)
{
	var FechaTxt=FechaD.split('-');
	var Anno=FechaTxt[0];
	var Mes=FechaTxt[1];
	var Dia=FechaTxt[2];
...
//	14set22 Convierte de tipo Date a cadena en formato dd/mm/yyyy
...
var sepCSV			=';';		//	22dic20
var sepTextoCSV		='';		//	22dic20
var saltoLineaCSV	='\r\n';	//	22dic20

//	quitamos ";" de las cadenas
function StringtoCSV(Cadena)
{
	//solodebug	debug('StringtoCSV. ['+Cadena+']');

	var CadCSV=Cadena.toString();
	if ((CadCSV)&&(CadCSV!=''))
	{
		CadCSV=CadCSV.replace('&amp;','&').replace(';','');
...
function Campos(formu){
  ...
  for (j=0;j<formu.elements.length;j++){
    if(formu.elements[j].type!='checkbox')
      Msg=Msg+'Campo '+j+':'+formu.elements[j].name+':'+formu.elements[j].id  
...
// 9mar23 Cambia a la ficha de centro
function chFichaCentro(IDCentro)
{
	document.location="http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/CENDetalle2022.xsql?ID="+IDCentro;
}

// Abre la ficha de producto
function MantenProducto(IDProducto, Busqueda, IDCliente)
{
	MostrarPagPersonalizada("http://www.newco.dev.br/Administracion/Mantenimiento/Productos/PROManten2022.xsql?PRO_ID="+IDProducto+"&TIPO=M&PRO_BUSQUEDA="+Busqueda+"&IDCLIENTE="+IDCliente,'Mantenimiento Producto',100,80,0,0);
}
...
function FichaAdjudicacion(IDProducto,IDEmpresa)	
{
	MostrarPagPersonalizada("http://www.newco.dev.br/ProcesosCdC/MantenimientoReducido/MantenimientoReducido2022.xsql?PRO_ID="+IDProducto+"&EMP_ID="+IDEmpresa,'FichaAdjudicacion',100,80,0,0);
}

// 10mar23 Cambia a la ficha de mantenimiento del producto estandar
function chMantenProdEstandar(IDProdEstandar)	
{
	document.location="http://www.newco.dev.br/ProcesosCdC/CatalogoPrivado/MantProductosEstandar2022.xsql?CATPRIV_IDPRODUCTOESTANDAR="+IDProdEstandar;
}

// Abre la ficha del pedido
function FichaPedido(IDMultioferta)
{
	MostrarPagPersonalizada("http://www.newco.dev.br/Compras/Multioferta/MultiofertaFrame2022.xsql?MO_ID="+IDMultioferta,'Pedido '+IDMultioferta,100,80,0,0);
}

// 16dic22 Cambia a la ficha del pedido
function chFichaPedido(IDMultioferta)
{
	document.location="http://www.newco.dev.br/Compras/Multioferta/MultiofertaFrame2022.xsql?MO_ID="+IDMultioferta;
}
...
{
	debug("http://www.newco.dev.br/Gestion/Comercial/FichaProductoLicitacion2022.xsql?LIC_PROD_ID="+IDProductoLic+"&LIC_ID="+IDLicitacion);
	MostrarPagPersonalizada("http://www.newco.dev.br/Gestion/Comercial/FichaProductoLicitacion2022.xsql?LIC_PROD_ID="+IDProductoLic+"&LIC_ID="+IDLicitacion,'FichaProductoLic_'+IDProductoLic,100,80,0,0);
}

// cambia a la ficha de producto de la licitacion
function chFichaProductoLicitacion(IDLicitacion, IDProductoLic, Origen)
{
	document.location="http://www.newco.dev.br/Gestion/Comercial/FichaProductoLicitacion2022.xsql?LIC_PROD_ID="+IDProductoLic+"&LIC_ID="+IDLicitacion+"&ORIGEN="+Origen;
}
...
function OfertaProveedorLicitacion(IDLicitacion, IDProveedorLic)
{
	...
	MostrarPagPersonalizada("http://www.newco.dev.br/Gestion/Comercial/ImprimirOferta2022.xsql?LIC_ID="+IDLicitacion+"&LIC_PROV_ID="+IDProveedorLic,'Oferta',100,80,0,0);
}
...
function chMantenCentro(IDCentro)
{
...
//	2ene23 Cambia al mantenimiento de usuario
function chMantenUsuario(IDUsuario)
{
	document.location="http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/USManten2022.xsql?IDUSUARIO="+IDUsuario;
}

//	Cambia al buscador de licitaciones
function chVencedores(IDLicitacion, IDProductoLic)
{
	document.location="http://www.newco.dev.br/Gestion/Comercial/licOfertasSeleccionadas2022.xsql?LIC_ID="+IDLicitacion+"&LIC_PROD_ID="+IDProductoLic;
}
...
// Abre la pagina principal de la licitacion (version antigua)
...
// 20feb23 Abre la licitacion para proveedor
function LicitacionProveedor(IDLicitacion)
{
	MostrarPagPersonalizada("http://www.newco.dev.br/Gestion/Comercial/LicProveedor2022.xsql?LIC_ID="+IDLicitacion,'Licitacion '+IDLicitacion,100,80,0,-10);
}

// Abre la licitacion para proveedor en la misma pagina
function chLicitacionProveedor(IDLicitacion)
{
	document.location="http://www.newco.dev.br/Gestion/Comercial/LicProveedor2022.xsql?LIC_ID="+IDLicitacion;
}

// Nueva version de la licitacion.
function chLicPorProducto(IDLicitacion, ProdActivo)
{
	document.location="http://www.newco.dev.br/Gestion/Comercial/LicitacionV2_2022.xsql?LIC_ID="+IDLicitacion+'&LIC_PROD_ID='+ProdActivo;
}

// Nueva version de la licitacion (mantenemos la anterior provisionalmente)
function chLicV2(IDLicitacion, ProdActivo)
{
	document.location="http://www.newco.dev.br/Gestion/Comercial/LicitacionV2_2022.xsql?LIC_ID="+IDLicitacion+'&LIC_PROD_ID='+ProdActivo;
}

// Nueva version de la licitacion en pop-up
function FichaLicV2(IDLicitacion, ProdActivo)
{
	MostrarPagPersonalizada("http://www.newco.dev.br/Gestion/Comercial/LicitacionV2_2022.xsql?LIC_ID="+IDLicitacion+'&LIC_PROD_ID='+ProdActivo,'Licitacion '+IDLicitacion,100,80,0,-10);
}
