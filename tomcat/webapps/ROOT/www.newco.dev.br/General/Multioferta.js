...
///////////////
       ...
       function CambioDivisa(){      
         for(var i=0;i<document.forms.length;i++){
	   var elForm=document.forms[i]; 
	   if(elForm.name!='comentarios' && elForm.name!='form_iddivisa'){
...
///////////////
	...
	if (estado==10 && accion.match('ENVIAR')) { 
	     ...
	     if (ComprobarFecha(document.forms['form1'])) {
	  	comentariosToForm1(document.forms['comentarios'],document.forms['anadir'],'NMU_COMENTARIOS');
	  	comentariosToForm1(document.forms['form1'],document.forms['anadir'],'FECHA_ENTREGA');
	  	comentariosToForm1(document.forms['form1'],document.forms['anadir'],'MO_FORMAPAGO');
	...
	function Actua_Check_Number(formu,accion){
	  ...
	  if (porDefinir==1){
		...
		if ((document.forms['comentarios'].elements['NMU_COMENTARIOS'].value=="") && (accion.match("PENDIENTE")==null)) {
		    MensajeAyudaComentarioProveedor();
		    return;
		}
	...
	function SaltaTarifas(codprod,cliente,mo_id,rol,tipo){
	 ...
	 // prompt(newlocation,newlocation);
        ...
        function Focaliza(){
          document.forms['anadir'].elements['FECHANO_PAGO'].focus();      
        }
          ...
          if  (test(formu)) {
        	 ...
        	 if (comparaFechas(formu.elements['FECHA_ENTREGA'].value,fechaActual)=='<')
        	    {  
                 ...
                 if ( (comparaFechas(document.forms['anadir'].elements['FECHANO_PAGO'].value,fechaActual)=='<') &&  !(document.forms['anadir'].elements['LPP_IMPORTE'].value=='') && (document.forms['anadir'].elements['LPP_IMPORTE'].value>0) )
        	    {  
