...
<head>
	...
	<script type="text/javascript" src="http://www.newco.dev.br/General/basic_180608.js"></script>
 	...
 	<script type="text/javascript" src="http://www.newco.dev.br/Personal/BandejaTrabajo/WFStatus_220118.js"></script>
...
-->
		...
		<!-- Se recarga cada minuto la zona de las pestañas por si hay cambios... tambien la pagina actual 
		<xsl:if test="INICIO/CUADRO_AVANZADO">
		jQuery(document).ready(function(){
			setInterval(function(){recargarPestanas();}, 300000);
		});
		</xsl:if> Desactivado 14nov16	-->

        //solodebug  console.log("JS WFSTATUS FINAL");
	</script>
...
<body>
	...
	<xsl:otherwise>
		...
		<td width="15%" style="border:10px solid white;">
			<span style="font-size: 15px;font-weight: bold;">Pedidos(12h)</span><br/>
			<span style="font-size: 30px;color:green;"><xsl:value-of select="INICIO/RESUMEN_PROCEDIMIENTOS_CDC_1DIA/PEDIDOS"/></span><br/>
			<span style="font-size: 20px;color:green;"><xsl:value-of select="INICIO/RESUMEN_PROCEDIMIENTOS_CDC_1DIA/PEDIDOS_IMPORTE"/><xsl:value-of select="INICIO/PEDIDOSPROBLEMATICOS/COMPRADOR/DIVISA/SUFIJO"/></span>
		</td>
		<td width="15%" style="border:10px solid white;">
			<span style="font-size: 15px;font-weight: bold;">Pedidos 30d</span><br/>
			<span style="font-size: 30px;color:green;"><xsl:value-of select="INICIO/RESUMEN_PROCEDIMIENTOS_CDC/PEDIDOS"/></span><br/>
			<span style="font-size: 20px;color:green;"><xsl:value-of select="INICIO/RESUMEN_PROCEDIMIENTOS_CDC/PEDIDOS_IMPORTE"/><xsl:value-of select="INICIO/PEDIDOSPROBLEMATICOS/COMPRADOR/DIVISA/SUFIJO"/></span>
		</td>
		<!--
		<td width="15%" style="border:10px solid white;">
			<span style="font-size: 20px;">Ped.Retr.(hoy)</span><br/>
			<span style="font-size: 40px;color:red;"><xsl:value-of select="INICIO/RESUMEN_PROCEDIMIENTOS_CDC_1DIA/PEDIDOS"/></span>
			<span style="font-size: 40px;color:red;">15%</span>
		</td>
		-->
		<td width="15%" style="border:10px solid white;">
			<span style="font-size: 15px;font-weight: bold;">Ped.Retr.30d</span><br/>
			<span style="font-size: 30px;color:red;"><xsl:value-of select="INICIO/RESUMEN_PROCEDIMIENTOS_CDC/PEDIDOS_RETRASADOS"/></span><br/>
			<span style="font-size: 20px;color:red;"><xsl:value-of select="INICIO/RESUMEN_PROCEDIMIENTOS_CDC/PEDIDOS_RETRASADOS"/>%</span>
		</td>
		<td width="15%" style="border:10px solid white;">
			<span style="font-size: 15px;font-weight: bold;">Gestiones usuario(12h)</span><br/>
			<span style="font-size: 30px;color:green;"><xsl:value-of select="INICIO/RESUMEN_PROCEDIMIENTOS_CDC_1DIA/CONTROLPEDIDOS_USUARIO"/></span><br/>
			<span style="font-size: 20px;color:green;"><xsl:value-of select="INICIO/RESUMEN_PROCEDIMIENTOS_CDC_1DIA/CONTROLPEDIDOS_USUARIO_PORC"/>% (del equipo)</span><!-- porcentaje sobre pedidos con problemas--><!-- el color debe venir de la base de datos-->
		</td>
		<td width="15%" style="border:10px solid white;">
			<span style="font-size: 15px;font-weight: bold;">Gestiones equipo(12h)</span><br/>
			<span style="font-size: 30px;color:green;"><xsl:value-of select="INICIO/RESUMEN_PROCEDIMIENTOS_CDC_1DIA/CONTROLPEDIDOS_EQUIPO"/></span><br/>
			<span style="font-size: 20px;color:green;"><xsl:value-of select="INICIO/RESUMEN_PROCEDIMIENTOS_CDC_1DIA/CONTROLPEDIDOS_EQUIPO_PORC"/>% (ped.probl.)</span><!-- porcentaje sobre pedidos con problemas--><!-- el color debe venir de la base de datos-->
		</td>
		</tr>
		</table>
		</div>
	...
	<table class="buscador" border="0">
      ...
      <tr>
        ...
        <td style="text-align:right;" colspan="2">
          ...
          <!--<xsl:value-of select="document($doc)/translation/texts/item[@name='eur']/node()"/>-->
		  <xsl:value-of select="INICIO/PEDIDOSPROBLEMATICOS/COMPRADOR/DIVISA/SUFIJO"/>
        </td>
        <!--pedidos retrasados-->
		...
		<tfoot>
			...
			<tr class="lineBorderBottom5">
				...
				<td class="quince datosLeft">
					<p>
            		<!--26may16	<xsl:if test="INICIO/PEDIDOSPROBLEMATICOS/COMPRADOR/ADMIN = 'MVM' or INICIO/PEDIDOSPROBLEMATICOS/COMPRADOR/ADMIN = 'MVMB'">-->
            ...
            <!--26may16	<xsl:if test="not(INICIO/PEDIDOSPROBLEMATICOS/COMPRADOR/ADMIN = 'MVM') and not(INICIO/PEDIDOSPROBLEMATICOS/COMPRADOR/ADMIN = 'MVMB')">-->
...
<xsl:template name="CLIENTE">
	...
	<div class="divleft">
		<!--PEDIDOS PROBLEMATICOS-->
		<xsl:if test="INICIO/PEDIDOSPROBLEMATICOS/VENDEDOR/BLOQUEARBANDEJA">
			...
			<xsl:for-each select="LISTATAREAS/TAREA">
				...
				<xsl:when test="../TIPO_TAREA='PENDIENTES_APROBACION'">
					<td align="right">
						...
						<strong><xsl:value-of select="SUBTOTAL"/><xsl:value-of select="INICIO/PEDIDOSPROBLEMATICOS/DIVISA/SUFIJO"/></strong>
					</td>
				</xsl:when>
				<xsl:when test="../../LISTATAREAS/ROL='E' or ../../LISTATAREAS/ROL='R'">
					<td align="right">
						...
						<strong><xsl:value-of select="TOTAL"/><xsl:value-of select="INICIO/PEDIDOSPROBLEMATICOS/DIVISA/SUFIJO"/></strong>
					</td>
				</xsl:when>
				<xsl:otherwise>
...
<xsl:template name="buscadorInicioAdmin">
	...
	<table class="buscador" border="0">
	  ...
	  -->
		...
		<xsl:otherwise>
			<input type="hidden" name="IDEMPRESA" value="{INICIO/PEDIDOSPROBLEMATICOS/COMPRADOR/IDEMPRESA}" />
		</xsl:otherwise>
		</xsl:choose>
    	...
    	<th width="120px" style="text-align:left;">
			...
			-->
			<a href="javascript:FiltrarBusqueda();" title="Buscar" class="btnDestacadoPeq">
		...
		<xsl:when test="$num &lt; number(5)">
        	<div class="docLine" id="docLine_{$type}" style="float:left;">
				...
				<div class="docLongEspec" id="docLongEspec_{$type}" style="float:left;clear:both;">
                    ...
                    <!--<div class="fakefile">
                        <xsl:choose>
                            <xsl:when test="//WorkFlowPendientes/LANG = 'spanish'">
