// JS Página de Inicio
// Ultima revisión ET: 21jun23 10:37 WFStatus2022_210623.js. CUidado: aplicar los cambios tambien a ResumenPedidosActivosHTML2022.xsl
...
function AlertNoticias()
{
	...
	for (var i=0;i<Noticias.length;++i)
	{
		if (Noticias[i].Destacada)
		{
			...
			if (Noticias[i].Url!='')
			{
				MostrarPagPersonalizada('http://www.newco.dev.br/Documentos/'+Noticias[i].Url,'Documento noticia',100,100,0,0);
			}
		}
	}
	alert (txtNoticias);
...
function EnviarInfoProveedor(IDOferta){
        //id oferta es mo_id
	var form=document.forms[0];
        form.action = "WFStatusSave2022.xsql";
...
function IndicadorLicitaciones(Tipo)
{
	...
	document.location.href="http://www.newco.dev.br/Gestion/Comercial/Licitaciones2022.xsql?FIDEMPRESA="+IDEmpresa+"&FIDCENTROPEDIDO="+IDCentro+"&BUSQUEDASESPECIALES="+Tipo;
...
function ResumenPedidos(){
	var form=document.forms[0];
	form.action='ResumenPedidosActivos2022.xsql';
...
function Licitacion(ID){
	if (EsVendedor=='S')	//17feb23
		MostrarPagPersonalizada('http://www.newco.dev.br/Gestion/Comercial/LicProveedor2022.xsql?LIC_ID='+ID,'Licitacion',100,100,0,0);
	else
		MostrarPagPersonalizada('http://www.newco.dev.br/Gestion/Comercial/MantLicitacion2022.xsql?LIC_ID='+ID,'Licitacion',100,100,0,0);
}
...
function DescargarExcel(){
  var form=document.forms[0];
	var IDEmpresa='', IDCentro='', IDProveedor='', IDFiltroMotivo='', IDFiltroResponsable='', IDFiltroEstado='', IDFiltroSemaforo='', IDFiltroVendedor='';
	...
	var IDAreaGeo='',IDLugarEntrega='', fTexto='',plazoConsulta='';
  ...
  if(form.elements.IDFILTROVENDEDOR && form.elements.IDFILTROVENDEDOR.value !== '') IDFiltroVendedor = form.elements.IDFILTROVENDEDOR.value;		//21ene22
  ...
  if(form.elements.IDLUGARENTREGA) IDLugarEntrega = form.elements.IDLUGARENTREGA.value;			//21jun23
	...
	jQuery.ajax({
		...
		data: "IDEMPRESA="+IDEmpresa+"&IDCENTRO="+IDCentro+"&IDPROVEEDOR="+IDProveedor+"&IDFILTROMOTIVO="+IDFiltroMotivo+"&IDFILTRORESPONSABLE="+IDFiltroResponsable
					...
					+"&FIDAREAGEOGRAFICA="+IDAreaGeo+"&IDLUGARENTREGA="+IDLugarEntrega+"&FTEXTO="+fTexto+"&PLAZOCONSULTA="+plazoConsulta+"&IDFILTROVENDEDOR="+IDFiltroVendedor
					+"&_="+d.getTime(),
    type: "GET",
    contentType: "application/xhtml+xml",
    error: function(objeto, quepaso, otroobj){
...
function DocumentosPendientes(Tipo)
{
	if (Tipo=='MANTEN')
	{
		document.location.href="http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/EMPDocs2022.xsql";
	}
	else if (Tipo=='EMAIL')
	{
...
//	24ene22 Abrimos la multioferta desde un enlace directo
function AbrirMultioferta(IDMultioferta)
{
	javascript:MostrarPagPersonalizada('http://www.newco.dev.br/Compras/Multioferta/MultiofertaFrame2022.xsql?MO_ID='+IDMultioferta,'Multioferta',100,100,0,0);
}

//	24ene22 Abrimos la multioferta desde un enlace directo
function AbrirControl(IDPedido)
{
	javascript:MostrarPagPersonalizada('http://www.newco.dev.br//Personal/BandejaTrabajo/ControlPedidos2022.xsql?IDPEDIDO='+IDPedido,'ControlPedido',100,100,0,0);
}

//	Pasamos a la pagina de control de pedido
function ControlPedidos(IDPedido){
	CambiarAPagina('ControlPedidos2022.xsql?IDPEDIDO='+IDPedido);
}
