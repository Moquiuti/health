...
//	15dic16	Nuevo dise駉, cambio radical en el funcionamiento de las pesta馻s
//	Ultima revision: ET 15dic16 09:43
...
function DesactivarPestanna(idPestanna)
{
	...
	ColoresPestanna(idPestanna,'#555555','#F0F0F0');
	idPestannaActiva='';
}
...
/*
//	15dic16	esconde todos los bloques de pestannas
function EsconderTablasPestannas(){
	jQuery("#pedidosBox").hide();
	jQuery("#todosPreciosBox").hide();
	jQuery("#roturasBox").hide();
	jQuery("#historicoCentrosBox").hide();
	jQuery("#precioConsumoClinicasBox").hide();
	jQuery("#todosPreciosBox").hide();
	jQuery("#infoProveedorBox").hide();
...
//	Comprobar si hay cambios que requieran guardarlos datos antes de ejecutar una acci贸n
...
function EjecutarFuncionDelFrame(nombreFrame,nombreFuncion){
	...
	/*	24oct12	Ya no trabajamos sobre el precio te锟絩ico sino sobre el margen
	//	Copia el precio te贸rico como precio de referencia a todos los centros
	...
	{
		var Msg='';
		//3jun08	Alf pide no comprobar if (confirm('Desea sustituir el precio de referencia en todos los centros por '+Precio+' y poner el campo "Precio sin hist贸ricos" a: '+Teorico))
		//{
	...
	*/
	...
	//	Copia el precio te贸rico como precio de refrencia a todos los centros
	...
	*/
	...
	//	3mar09	ET	Cambio estatico: una sola tabla visible, se selecciona por pesta锟絘s
	...
	function cambiosAPartirDePrecioFinal(form,IDEmpresa)
	{
			...
			&& form.elements['PRECIOFINAL_'+IDEmpresa].value!='' && ValidarNumeroSinMensaje(form.elements['PRECIOFINAL_'+IDEmpresa],4))
		{
			...
			if (PrecioFinal!=0)
			{
                            //a锟絘dido 25-6-15 si nuevo modelo de negocio pero comision a 0 =>modelo escalado opcion medica
                            if (ComAh == 0){
...
//funcion que ense锟絘 en el eis los datos del centro o de la empresa segun un producto, agrupar por producto siempre
