//	JS para el envio de mantenimiento de Convocatorias Especiales via ajax
//	Ultima revisión: ET 1jul22 15:15 ConvocatoriasEspeciales2022_230522.js
...
function Inicializar()
{
	var titulo=ncRefProducto
				+sepCampos+ncProducto+sepCampos+ncMedida
				+sepCampos+ncUdBasica+sepCampos+ncUdesLote+sepCampos+ncMarca+sepCampos+ncFabricante
...
function selConvocatoria()
{
	...
	var IDEstado =IDEstadoConvocatoria();
		
	if (IDConvocatoria!='' && IDConvocatoria!='-1' && (IDEstado=='CURS'||IDEstado=='REOFERTA'))	
	{
		oForm.elements['inputFile'].disabled=false;
		jQuery("#IDESTADO").html('');
	}
	else
	{
		oForm.elements['inputFile'].disabled=true;
		
		if (IDConvocatoria!='' && IDConvocatoria!='-1')
			jQuery("#IDESTADO").html('<strong>'+strFueraDePlazo+'</strong>');
		else
			jQuery("#IDESTADO").html('');
	}
	

	if (ConvocatoriaInformada()=='S')
	{
		document.forms[0].elements['COMPLETAR'].disabled=false;
		document.forms[0].elements['SUSTITUIR'].disabled=false;
		document.forms[0].elements['COMPLETAR'].checked=true;
		onChangeChecks('COMPLETAR');
	}
	else
	{
		document.forms[0].elements['COMPLETAR'].checked=false;
		document.forms[0].elements['SUSTITUIR'].checked=false;
		document.forms[0].elements['COMPLETAR'].disabled=true;
		document.forms[0].elements['SUSTITUIR'].disabled=true;
	}
}
...
function EnviarFichero(files)
{
	...
	reader.onload = function (e) {
		...
		if ((ConvocatoriaInformada()=='S')&&(document.forms[0].elements['COMPLETAR'].value=='N')&&(Stat!='')) Cont=confirm(Stat);
...
//	Comprueba si la convocatoria actual tiene productos informados
function ConvocatoriaInformada()
{
	var IDConvocatoria = document.forms['frmProductos'].elements['FIDCONVOCATORIA'].value;
	var Informada='N';
	
	for (i=0;(i<Convocatorias.length)&&(Informada=='N');++i)
	{
		if ((Convocatorias[i].ID==IDConvocatoria)&&(Convocatorias[i].Fichero!='')) Informada='S';
	}

	//solodebug	console.log('ConvocatoriaInformada:'+Informada);
	
	return Informada;
}

//	Comprueba si la convocatoria actual tiene productos informados
function IDEstadoConvocatoria()
{
	var IDConvocatoria = document.forms['frmProductos'].elements['FIDCONVOCATORIA'].value;
	var IDEstado='N';
	
	for (i=0;(i<Convocatorias.length)&&(IDEstado=='N');++i)
	{
		if (Convocatorias[i].ID==IDConvocatoria) IDEstado=Convocatorias[i].IDEstado;
	}

	//solodebug	
	console.log('IDEstadoConvocatoria:'+IDEstado);
...
function prepararEnvioCatProv(nombreFichero)
{
	...
	jQuery.ajax({
		...
		type:	"GET",
		data:	"NOMBRE="+nombreFichero+"&IDCONVOCATORIA="+document.forms['frmProductos'].elements['FIDCONVOCATORIA'].value+"&COMPLETAR="+document.forms[0].elements['COMPLETAR'].value+"&_="+d.getTime(),
		contentType: "application/xhtml+xml",
		error: function(objeto, quepaso, otroobj){
			ficError='ERROR_CONECTANDO';
		},
...
function EnviarTodosProductosCatProv(licNumProductosEnviados, ForzarNombre)
{
	...
	if (licNumProductosEnviados>=ficProductos.length)
	{
		...
		if (ficControlErrores==0)
		{
			...
			MostrarPagPersonalizada('http://www.newco.dev.br/ProcesosCdC/ConvocatoriasEspeciales/BuscadorProductos2022.xsql','convesp_buscadores',100,80,0,-10);
...
function EnviarFicheroCatProv(ficProductos)
{
	...
	if (ficControlErrores=='')
	{
		...
		if (jQuery('#chkForzar').is(':checked')) ForzarNombre='S';	// 1jul22 .attr('checked')
...
//	26oct18 Comprueba cambios en los checkboxes
function onChangeChecks(CheckChanged)
{
	if (CheckChanged=='COMPLETAR')
		if (document.forms[0].elements['COMPLETAR'].checked) Completar='S';
		else Completar='N';
	else	
		if (document.forms[0].elements['SUSTITUIR'].checked) Completar='N';
		else Completar='S';
		
	document.forms[0].elements['COMPLETAR'].value=Completar;
	if (Completar=='S')
	{
		document.forms[0].elements['COMPLETAR'].checked=true;
		document.forms[0].elements['SUSTITUIR'].checked=false;
	...
	{
		document.forms[0].elements['COMPLETAR'].checked=false;
		document.forms[0].elements['SUSTITUIR'].checked=true;
	}
	document.forms[0].elements['COMPLETAR'].disabled=false;
	document.forms[0].elements['SUSTITUIR'].disabled=false;
	
	console.log('onChangeChecks CheckChanged:'+CheckChanged+' Completar:'+Completar);
}
