//	JS asociado al mantenimiento convocatoria
...
function Inicializa()
{
	...
	FechaDec=jQuery("#FECHADECISION").val();
	if ((FechaDec)&&(FechaDec!=''))
	{
		jQuery("#dtFECHADECISION").val(FechaParaControlDate(FechaDec));
	}
...
{	
	...
	jQuery("a.MenuLic").css('background','#E6E6E6');
...
function Guardar()
{
	//Pendiente activar Parsley var parsleyInstance = jQuery('form').parsley();
	//Mientras validacion manual
	if (jQuery("#LIC_CONV_NOMBRE").val()=='')
	{
		alert(alrt_NombreObligatorio.replace('[[NOMBRECAMPO]]','NOMBRE'))
		return;
	}
	
	dtFechaDec=jQuery("#dtFECHADECISION").val();
	if ((dtFechaDec)&&(dtFechaDec!=''))
	{
		jQuery("#FECHADECISION").val(FechaDeControlDate(dtFechaDec));
	}
...
{
//	var IDConvocatoria=document.forms["frmConv"].elements['LIC_CONV_ID'].value;
	document.location='http://www.newco.dev.br/ProcesosCdC/Convocatorias/ProveedorYOfertas2022.xsql?FIDCONVOCATORIA='+IDConvocatoria;
}
...
function listadoExcel(SoloAdjudicadas){
	var d = new Date();

//	var IDConvocatoria=document.forms["frmConv"].elements['LIC_CONV_ID'].value;
	jQuery.ajax({
		url:	"http://www.newco.dev.br/Gestion/Comercial/LicitacionesExcel.xsql",
		data:	"IDCONVOCATORIA="+IDConvocatoria+"&SOLOADJUDICADAS="+SoloAdjudicadas+"&_="+d.getTime(),
...
//	Cambia a la ficha de proveedor de la convocatoria
function chProveedorConvocatoria(IDConvocatoria, IDProveedor)
{
	document.location="http://www.newco.dev.br/ProcesosCdC/Convocatorias/ProveedorConvocatoria2022.xsql?LIC_CONV_ID="+IDConvocatoria+"&amp;IDPROVEEDOR="+IDProveedor;
}
