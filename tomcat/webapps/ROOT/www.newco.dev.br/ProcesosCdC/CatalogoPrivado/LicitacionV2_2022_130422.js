...
//	Ultima revisión ET 11jul22 09:00 LicitacionV2_2022_130422.js
		...
		success: function(objeto){
			...
			if(data.OfertasProductoLic.posicion && data.OfertasProductoLic.posicion!= ''){
				...
				for (var i=0; i<data.OfertasProductoLic.ofertas.length; ++i)
				{
					...
					//2may22 var Precio=parseFloat(Oferta["PRECIO"].replace('.','').replace(',','.'));
...
function AbrirProducto(Pos)
{
	...
	var headTablaOfertas='<div class="tabela tabela_redonda"><div class="divLeft h50px">&nbsp;</div><table cellspacing="6px" cellpadding="6px"><thead class="cabecalho_tabela"><tr>'
		...
		/*	20abr22 Se duplicaba el titulo de referencia prov
		if (IDPais != '55')
		{
			alert('refprov')
			headTablaOfertas+='<th class="w50px">'+strRefProv+'</th>';
		}*/
	...
	var linTablaOfertas='<tr id="OFE_[LIC_OFE_ID]" class="filaOferta conhover">'
					...
					//	20abr22 QUITAMOS Icono anyadir campos avanzados '<td>[CAMPOS_AVANZADOS]</td>'
	...
	else 
	{
		...
		for (var i=0; i<arrProductos[Pos].Ofertas.length;++i)
		{
				...
				/*					
						...
						<xsl:if test="FICHA_TECNICA/ID">
							<a href="http://www.newco.dev.br/Documentos/[FICHA_TECNICA/URL]" target="_blank" style="text-decoration:none;">
								<xsl:attribute name="alt">[document($doc)/translation/texts/item[@name='ficha_tecnica']/node()]</xsl:attribute>
								<xsl:attribute name="title">[document($doc)/translation/texts/item[@name='ficha_tecnica']/node()]</xsl:attribute>
								FT
							</a>
						&nbsp;
						</xsl:if>
						<xsl:if test="REGISTRO_SANITARIO/ID">
							<a href="http://www.newco.dev.br/Documentos/[REGISTRO_SANITARIO/URL]" target="_blank" style="text-decoration:none;">
								<xsl:attribute name="alt">[document($doc)/translation/texts/item[@name='registro_sanitario']/node()]</xsl:attribute>
								<xsl:attribute name="title">[document($doc)/translation/texts/item[@name='registro_sanitario']/node()]</xsl:attribute>
								RS
							</a>
						&nbsp;
						</xsl:if>
						<xsl:if test="CERTIFICADO_EXPERIENCIA/ID">
							<a href="http://www.newco.dev.br/Documentos/[CERTIFICADO_EXPERIENCIA/URL]" target="_blank" style="text-decoration:none;">
								<xsl:attribute name="alt">[document($doc)/translation/texts/item[@name='cert_experiencia']/node()]</xsl:attribute>
								<xsl:attribute name="title">[document($doc)/translation/texts/item[@name='cert_experiencia']/node()]</xsl:attribute>
								CE
							</a>
						</xsl:if>
				*/
				
				Linea=Linea.replace('[COLUMNA_DOCUMENTOS]',colDocs);
			}
			else
				Linea=Linea.replace('[COLUMNA_DOCUMENTOS]','');
			...
			/*	21abr22 Estas acciones se reservan para la ficha completa
			if (esCdC=='S')
			{
				var chkMarca;
				
				if (arrProductos[Pos].Ofertas[i].MARCA_ACEPTABLE=='S')
					chkMarca='<td><input type="checkbox" class="muypeq chkmarca" name="chkMarca_[LIC_OFE_ID]" id="chkMarca_[LIC_OFE_ID]" disabled="disabled"/></td>';
				else
					chkMarca='<td><input type="checkbox" class="muypeq chkmarca" name="chkMarca_[LIC_OFE_ID]" id="chkMarca_[LIC_OFE_ID]"/></td>';

				Linea=Linea.replace('[CHECK_MARCA]',chkMarca);
			}
			else
				Linea=Linea.replace('[CHECK_MARCA]','');
			*/
			...
			if (IDPais==55)
				...
				Linea=Linea.replace('[TIPOIVA]','<td>'+arrProductos[Pos].Ofertas[i].LIC_OFE_TIPOIVA+'%</td>');
...
function IniciarLicitacion(IDEstado){
	...
	if ((arrProveedores.length>0) && (arrProductos.length>0) && (isLicAgregada == 'N' || arrCentros.length>0))
	{		
