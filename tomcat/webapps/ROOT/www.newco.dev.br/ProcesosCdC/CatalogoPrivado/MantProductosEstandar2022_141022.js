...
//	Ultima revisión: ET 14oct22 10:20 MantProductosEstandar2022_141022.js


jQuery(document).ready(onloadEvents);

function onloadEvents(){
	//solodebug	debug("onloadEvents. Inicio.");
...
function ValidarFormulario(form){
	...
	if((!errores && (form.elements['CATPRIV_NOMBRE']) && esNulo(form.elements['CATPRIV_NOMBRE'].value))){
		errores++;
	...
	var ReferenciaOriginal=form.elements['REFERENCIAORIGINAL'].value.toUpperCase().trim();
	var nuevaReferencia;
	
	if	(form.elements['CATPRIV_REFERENCIA'])
		nuevaReferencia=form.elements['CATPRIV_REFERENCIA'].value.toUpperCase().trim();
	else
		nuevaReferencia=ReferenciaOriginal;

	var NombreOriginal=form.elements['NOMBREORIGINAL'].value.toUpperCase().trim();
	var nuevoNombre;

	if	(form.elements['CATPRIV_NOMBRE'])
		nuevoNombre=form.elements['CATPRIV_NOMBRE'].value.toUpperCase().trim();
	else
	...
	if	(form.elements['CATPRIV_REFERENCIA'])
    	inputText = [document.forms[0].elements['CATPRIV_REFERENCIA'].value,document.forms[0].elements['CATPRIV_NOMBRE'].value,document.forms[0].elements['CATPRIV_NOMBRE_PRIVADO'].value,document.forms[0].elements['CATPRIV_REFCLIENTE'].value];
	else
		inputText = [document.forms[0].elements['CATPRIV_NOMBRE_PRIVADO'].value,document.forms[0].elements['CATPRIV_REFCLIENTE'].value];
	...
	//	COmprobar que el contenido de PrecioReferencia es numerico
	if (isNaN(desformateaDivisa(form.elements['CATPRIV_PRECIOREFERENCIA'].value)))
	{
		alert(alrt_PrecioIncorrecto+'. '+str_Precio+' Ref:'+form.elements['CATPRIV_PRECIOREFERENCIA'].value);
		errores++;
	}
	else
	{
	...
	//	COmprobar que el contenido de PrecioReferencia es numerico
	if (isNaN(desformateaDivisa(form.elements['CATPRIV_PRECIOMAXIMO'].value)))
	{
		alert(alrt_PrecioIncorrecto+'. '+str_Precio+'  Max:'+form.elements['CATPRIV_PRECIOMAXIMO'].value);
		errores++;
	}
	...
	for (i=0;i<form.elements.length;++i)
	{
		...
		if (form.elements[i].name.substring(0,10)=='CHK_CENTRO')
		{
			...
			cadenaCentros=cadenaCentros+ID+'#'+Autorizado+'#'+form.elements["REFCENTRO_"+ID].value+'#'+form.elements["ORDEN_"+ID].value+'#'+form.elements["PRECIOREF_"+ID].value
													+'#'+form.elements["MARCAS_"+ID].value+'#'+form.elements["UDBASICA_"+ID].value+'|';
...
function esReferenciaEstricta(form){
	...
	//	1set22 SI no existe el campo, devolvemos OK
	if(!form.elements['CATPRIV_REFERENCIA'])
		return 1;
...
function GuardarProducto(form, accion){
	...
	//8set21 Desactivar boton
	jQuery('#btnGuardar').attr('href', "javascript: void(0);");
	jQuery('#btnGuardar').addClass('gris');
...
function UltimaRefProductoPorGrupo(){
	...
	//solodebug	debug('UltimaRefProductoPorGrupo IDEmpresa:'+IDEmpresa+' IDGrupo:'+IDGrupo);
	...
	jQuery.ajax({
		...
		success: function(objeto){
			...
			//solodebug	debug('UltimaRefProductoPorGrupo IDEmpresa:'+IDEmpresa+' IDGrupo:'+IDGrupo+' Res:'+objeto);

			jQuery('span#ley_ref_prodestandar').html('PRUEBA');
...
//	17abr19 Ejecuta el traspaso
function Desactivar()
{
	GuardarProducto(document.forms[0], 'TRASPASO');
}

//	23abr19 Reactiva un producto traspasado
function Reactivar()
{
	var oForm=document.forms[0];
	oForm.elements['ACCION'].value='REACTIVAR';
	EnviarCambios(oForm);
}

//	5jun20 Selecciones/Deselecciona todos los centros
function TodosCentros()
{
	...
	for (i=0;i<form.elements.length;++i)
	{
		...
		if (form.elements[i].name.substring(0,10)=='CHK_CENTRO')
		{
			if (Valor=='INICIO')
			{
				if (form.elements[i].checked==true) Valor=false;
				else Valor=true;
			}
