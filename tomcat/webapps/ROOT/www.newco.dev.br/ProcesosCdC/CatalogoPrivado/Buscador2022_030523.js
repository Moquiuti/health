...
//	Ultima revision: ET 3may23 16:00 Buscador2022_030523.js
...
//	if(formu.elements['IDFAMILIA'].value=='-1' && formu.elements['PRODUCTO'].value=='' && formu.elements['IDPROVEEDOR'].value=='-1'){
	...
	//solodebug console.log('Buscar. TRASPASADOS:'+formu.elements['TRASPASADOS'].value);
...
function handleRequestFamiliaDespl(req){
	...
	console.log("handleRequestFamiliaDespl:"+req.responseText);
...
function SeleccionaCategoria(IDEmpresa){
	...
	jQuery.ajax({
		...
		success: function(objeto){
			var data = JSON.parse(objeto);			//3may23 eval("(" + objeto + ")");
			var Resultados = new String('');
...
//	Inicializa los cuadros de busqueda
