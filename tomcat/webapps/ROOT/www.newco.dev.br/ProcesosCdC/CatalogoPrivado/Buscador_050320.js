...
//	Ultima revision: ET 25nov19 12:45 Buscador_050320.js
...
function onLoadEvents(){
	...
	jQuery('img#toogleBuscador').click(function(){
		if(jQuery('tr#auxToogle').is(":hidden")){
			...
			//25nov19	jQuery('img#toogleBuscadorLic').hide();
		}else{
			jQuery('tr#auxToogle').hide();
			//25nov19	jQuery('img#toogleBuscadorLic').show();
		}
	});
...
function Busqueda(formu,origen,type){
	...
	// PS 20170125 var objFrame=new Object();
...
//	var idcentro = jQuery('#IDCENTRO').val();
	...
	var Regulados;
	if(jQuery('#REGULADOS').length && jQuery('#REGULADOS').attr("checked") == true){
		Regulados = 'S';
	}else{
		Regulados = '';
	}
	...
	var Orden1;
	if(jQuery('#ORDEN1').length && jQuery('#ORDEN1').attr("checked") == true){
		Orden1 = 'S';
	}else{
		Orden1 = '';
	}
	...
	var SinStock;
	if(jQuery('#SIN_STOCK').length && jQuery('#SIN_STOCK').attr("checked") == true){
		SinStock = 'S';
	}else{
		SinStock = '';
	}
	...
	var ProvBloqueado;
	if(jQuery('#PROV_BLOQUEADO').length && jQuery('#PROV_BLOQUEADO').attr("checked") == true){
		ProvBloqueado = 'S';
	}else{
		ProvBloqueado = '';
	}
...
//	var idcentro = jQuery('#IDCENTROCLIENTE').val();
	...
	var Traspasados;
	if(jQuery('#TRASPASADOS').length && jQuery('#TRASPASADOS').attr("checked") == true){
		Traspasados = 'S';
	}else{
		Traspasados = '';
	}
	...
	;
	if(jQuery('#CLASIF_PROVISIONAL').length && jQuery('#CLASIF_PROVISIONAL').attr("checked") == true){
		ClasifProv = 'S';
	}else{
		ClasifProv = '';
	}
	...
	jQuery.ajax({
		...
		data:	"IDCLIENTE="+idcliente+"&IDINFORME="+idinforme+"&IDFAMILIA="+idfamilia+"&IDCATEGORIA="+idcategoria+"&IDSUBFAMILIA="+idsubfamilia+"&IDGRUPO="+idgrupo
					...
					+"&MES="+mes+"&ANNO="+anno+"&PRIMEROSPEDIDOS="+primerosPedidos+"&CON_CONSUMO="+conConsumo+"&SIN_CONSUMO="+sinConsumo+"&INFORMAR_X_CENTRO="+informarXCentro
					...
					+"&CONSUMO_MINIMO="+consumoMinimo+"&IDLICITACION="+idlicitacion,
		type:	"GET",
		contentType: "application/xhtml+xml",
		beforeSend: function(){
			null;
