...
<xsl:template match="/HistoricosPorCentroAgregado">
<html>
<head>
	<!--idioma-->
	...
	<script type="text/javascript">
		...
		var AgruparPor	= '<xsl:value-of select="AGRUPAR_POR"/>';
		...
		var str_Paginacion	= '<xsl:value-of select="document($doc)/translation/texts/item[@name='leyenda_paginacion2']/node()"/>';
...
<body>
	...
	<div class="divLeft">
		<h1 class="titlePage">
			...
			[<xsl:value-of select="document($doc)/translation/texts/item[@name='agregando_por']/node()"/>&nbsp;
			<span id="idAgregacion">
				<xsl:choose>
				<xsl:when test="AGRUPAR_POR = 'FAMILIA'">
					<xsl:value-of select="HISTORICOPORCENTRO/NOMBRESNIVELES/NIVEL2"/>
				</xsl:when>
				<xsl:when test="AGRUPAR_POR = 'SUBFAMILIA'">
					<xsl:value-of select="HISTORICOPORCENTRO/NOMBRESNIVELES/NIVEL3"/>
				</xsl:when>
				<xsl:when test="AGRUPAR_POR = 'GRUPO'">
					<xsl:value-of select="HISTORICOPORCENTRO/NOMBRESNIVELES/NIVEL4"/>
				</xsl:when>
				<xsl:when test="AGRUPAR_POR = 'REFESTANDAR'">
