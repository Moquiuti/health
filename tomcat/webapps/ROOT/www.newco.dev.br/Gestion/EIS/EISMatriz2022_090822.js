// JavaScript Document
//	Ultima revision ET 9ago22 16:04 EISMatriz2022_090822.js
...
function globalEvents(){
	...
	ActivarPestanna("pes_Acumulado");

	// Click en tabla datos
	jQuery("#pes_Acumulado").click(function()
	{
		ActivarPestanna("pes_Acumulado");
		generarTabla('A');
	});
	...
	// Click en grafico
	jQuery("#pes_PorcH").click(function()
	{
		ActivarPestanna("pes_PorcH");
		generarTabla('V');
	});

	// click en SQL
	jQuery("#pes_PorcV").click(function()
	{
		ActivarPestanna("pes_PorcV");
		generarTabla('H');;
	});
...
//	16mar22 Activa la pestanna correspondiente
...
function EnviarComentarioPrecalc(){
	...
	jQuery.ajax({
		...
		data: "US_ID="+usid+"&IDPREDEFINIDA="+idpredefinida+"&IDHOR="+idHor+"&IDVER="+idVer+"&LINEA="+linea+"&COLUMNA="+columna+"&IDCOMENTARIO="+commentColor+"&_="+d.getTime(),
		type: "GET",
...
function EnviarComentario(){
	...
	jQuery.ajax({
		...
		data: "IDEMPRESA="+IDEmpresa+"&IDINDICADOR="+IDIndicador+"&IDAGRUPARPOR_HOR="+IDAgruparPor_Hor+"&IDAGRUPARPOR_VER="+IDAgruparPor_Ver+"&IDHOR="+idHor+"&IDVER="+idVer+"&FILTROS="+Filtros+"&COMENTARIO="+commentColor+"&_="+d.getTime(),
		type: "GET",
		contentType: "application/xhtml+xml",
		beforeSend:function(){
			jQuery('#confirmBox').hide();
...
function ordenamientoBurbuja(col, tipo){
	...
	ColumnaOrdenada=new Array(1000);
...
var cellTitleStart = '<th align="center">';
var cellTitleStartClass = '<th align="center" class="selected">';
var cellStartMod = '', cellStartIndicMod = '';
var cellTitleStartIndic = '<th align="left">';
var cellStartIndic = '<td align="left">';
var cellStartIndicClass = '<td align="left" class="selected">';
var cellTitleEnd = '</th>';
var rowIndStart = '<tr>';
...
var cellEnd = '</td>';
//16mar22	var macroEnlaceGrupo='<a class="grupo" href="javascript:OrdenarPorFila(\'#NUM_FILA#\');">';
var macroEnlaceGrupo='<a href="javascript:OrdenarPorFila(\'#NUM_FILA#\');">';
...
function LineaTotalesHTML(IDIndicador, Totales, linea, HayOcultas){
	...
	for(k=1;k<=NumCabeceraH;k++){
		...
		total = formateaDivisa(Round(Totales[k],0));

		// Si es matrix porcentual => anyado % al final
		if(tipoMatriz != 'A')	total = total + '%';

		cadenaHtml+=cellStart+Enlace+total+'&nbsp;</a>'+cellEnd;
	}
	cadenaHtml+=rowEnd
...
function LineaTotalesHTML_2(IDIndicador){
	...
	cadenaHtml=Replace(rowStart,'#CLASEPAROIMPAR#','')+cellStartThis+cellEnd+cellStartThis+Replace(macroEnlaceGrupo,'#NUM_FILA#','99999Total')+'Total</a>'+cellEnd;
	...
	for(var k=0;k<NumCabeceraH;k++){
		if(ColumnaActiva[FilaOrdenada[k]] == 'S'){
			...
			//	solodebug	console.log('LineaTotalesHTML2: cellStartAux:'+cellStartAux);
			...
			if(tipoMatriz == 'A'){
				//9ago22 total = FormatNumber(FilaTotales[FilaOrdenada[k]],4,',','.');
				total = formateaDivisa(Round(FilaTotales[FilaOrdenada[k]],2));
            }else if(tipoMatriz == 'V')
				//9ago22 total = FormatNumber(FilaTotalesPercent_V[FilaOrdenada[k]],4,',','.') + '%';
				total = formateaDivisa(Round(FilaTotalesPercent_V[FilaOrdenada[k]],2)) + '%';
			else if(tipoMatriz == 'H')
				//9ago22 total = FormatNumber(FilaTotalesPercent_H[FilaOrdenada[k]],4,',','.') + '%';
				total = formateaDivisa(Round(FilaTotalesPercent_H[FilaOrdenada[k]],2)) + '%';

			//16mar22 Enlace=Replace(Enlace,'#VALOR#',total);
			...
			if(k == (NumCabeceraH-1) && tipoMatriz != 'A')
				...
				//9ago22 total = FormatNumber(ColumnaTotales[NumLineas],4,',','.');
				total = formateaDivisa(Round(ColumnaTotales[NumLineas],2));


			// Por el momento no se pueden poner comentarios en la fila de totales
			//cadenaHtml+=cellStartAux+Enlace+total+'&nbsp;'+ linkToEIS +'</a>'+cellEnd;
			//cadenaHtml+=cellStart+'<span style="font-weight:bold;color:#3B5998;font-family: Geneva,Arial,Helvetica,sans-serif;">'+total+'</span>'+'&nbsp;'+ linkToEIS +cellEnd;
			//16mar22 cadenaHtml+=cellStart+'<a class="numeric celdanormal">'+total+'</a>'+'&nbsp;'+ linkToEIS +cellEnd;		//	23ene18
			cadenaHtml+=cellStart+total+cellEnd;
		}
	}
	cadenaHtml+=rowEnd

	//	solodebug	console.log('LineaTotalesHTML2: cadenaHtml:'+cadenaHtml);
...
function dibujarCabeceraH(cadenaHtml){
	...
	cadenaHtml+=cellTitleStartIndic+cellTitleEnd;
	cadenaHtml=cadenaHtml + cellTitleStartIndic + 
            '<a href="javascript:OrdenarPorColumna(-1);">' + txtConcepto + '</a>' + mostrarCol + cellTitleEnd;
	...
	for(var i=0;i<NumCabeceraH;i++){
		...
		if(ColumnaActiva[FilaOrdenada[i]] == 'S'){
			...
			cadenaHtml+= cellTitleStartMod +'<a href="javascript:OrdenarPorColumna('+FilaOrdenada[i]+');">'+ NombresCabeceraH[FilaOrdenada[i]]+'</a>'+cadenaBorrarCol+cellTitleEnd;
        }
	}
...
function generarTabla(tipo){
	...
	//	16mar22 Solo una cabecera
	var cadenaHtml='<thead class="cabecalho_tabela">'+dibujarCabeceraH(cadenaHtml)+'</thead>'
		...
		for(var i=0;i<NumLineas;i++){
			...
			if(LineaActiva[ColumnaOrdenada[i]]=='S'){
				...
				if(IDIndicadorActual!=IDIndicadores[ColumnaOrdenada[i]]){
					...
					if(IDIndicadorActual!=''){
						...
						if(ratio == 'RATIO'){
							...
							cadenaHtml+=LineaTotalesHTML(IDIndicadorActual, Totales, linea, HayOcultas);
						}
					...
					cadenaHtml+=rowIndStart+cellIndStart+NombresIndicadores[ColumnaOrdenada[i]]+cellEnd+rowEnd;
				...
				if((FilaOrdenacion==(ColumnaOrdenada[i]))&&(FOrden!=''))
					cellStartIndicMod = cellStartIndicClass;
				else
					cellStartIndicMod = cellStartIndic;

				//cadenaHtml+=Replace(rowStart,'#CLASEPAROIMPAR#',((linea%2)==0?'par':'impar'))+'<td align="left">'+pos+cellStartThis+Replace(macroEnlaceGrupo,'#IDGRUPO#',IDGrupos[ColumnaOrdenada[i]])+NombresGrupos[ColumnaOrdenada[i]]+'</a>'+cellEnd;
				cadenaHtml+=Replace(rowStart,'#CLASEPAROIMPAR#',((linea%2)==0?'par':'impar'))+'<td align="left">'+f_pos+cellEnd;
				cadenaHtml+=cellStartIndicMod+Replace(macroEnlaceGrupo,'#NUM_FILA#',ColumnaOrdenada[i])+NombresGrupos[ColumnaOrdenada[i]]+'</a>'+cellEnd;
				...
				for(var k=1;k<=NumCabeceraH;k++){
					...
					//formatNumbre
					//cadenaHtml+=cellStart+Enlace+FormatNumber( parseFloat(Valores[ColumnaOrdenada[i]*13+k-1]),0,',','.')+'</a>'+cellEnd;
					//cadenaHtml+=cellStart+Enlace+FormatNumber(Valores[ColumnaOrdenada[i]*13+k-1],0,',','.')+'</a>&nbsp;'+cellEnd;
					//cadenaHtml+=cellStart+Enlace+Valores[ColumnaOrdenada[i]*13+k-1]+'</a>&nbsp;'+cellEnd;
					...
					if(matrizPrecalculada=='N' && k==NumCabeceraH){
						if(tipoMatriz == 'A'){
							//9ago22 Valor = FormatNumber(ColumnaTotales[ColumnaOrdenada[i]],4,',','.');
							Valor = formateaDivisa(Round(ColumnaTotales[ColumnaOrdenada[i]],0));
						}else if(tipoMatriz == 'H'){
							//9ago22 Valor = FormatNumber(ColumnaTotalesPercent_H[ColumnaOrdenada[i]],4,',','.');
							Valor = formateaDivisa(Round(ColumnaTotalesPercent_H[ColumnaOrdenada[i]],2));
						}else if(tipoMatriz == 'V'){
							//9ago22 Valor = FormatNumber(ColumnaTotalesPercent_V[ColumnaOrdenada[i]],4,',','.');
							Valor = formateaDivisa(Round(ColumnaTotalesPercent_V[ColumnaOrdenada[i]],2));
						}
						...
						//9ago22 Valor		= (Val[ColumnaOrdenada[i]*NumCabeceraH+FilaOrdenada[k-1]] != 0) ? FormatNumber(Val[ColumnaOrdenada[i]*NumCabeceraH+FilaOrdenada[k-1]],4,',','.') : '.';
						Valor		= (Val[ColumnaOrdenada[i]*NumCabeceraH+FilaOrdenada[k-1]] != 0) ? formateaDivisa(Round(Val[ColumnaOrdenada[i]*NumCabeceraH+FilaOrdenada[k-1]],2)) : '.';
					...
					cadenaHtml+=cellStartThis+Enlace+Valor+Comentario+'</a>&nbsp;'+linkToEIS+cellEnd;
/*
					if(Val[ColumnaOrdenada[i]*NumCabeceraH+FilaOrdenada[k-1]] < 1 && Val[ColumnaOrdenada[i]*NumCabeceraH+FilaOrdenada[k-1]] > 0){
						//cadenaHtml+=cellStartThis+Enlace+FormatNumber(Valores[ColumnaOrdenada[i]*NumCabeceraH+FilaOrdenada[k-1]],4,',','.')+'</a>&nbsp;'+cellEnd;
						//cadenaHtml+=cellStartThis+Imagen+Enlace+Valor+Comentario+'</a>&nbsp;'+cellEnd;
						cadenaHtml+=cellStartThis+Enlace+Valor+Comentario+'</a>&nbsp;'+cellEnd;
					}else{
						//cadenaHtml+=cellStartThis+Enlace+FormatNumber(Valores[ColumnaOrdenada[i]*NumCabeceraH+FilaOrdenada[k-1]],4,',','.')+'</a>&nbsp;'+cellEnd;
						//cadenaHtml+=cellStartThis+Imagen+Enlace+Valor+Comentario+'</a>&nbsp;'+cellEnd;
						cadenaHtml+=cellStartThis+Enlace+Valor+Comentario+'</a>&nbsp;'+cellEnd;
...
*/
		...
		if(ratio == 'RATIO'){
			...
			//cadenaHtml+=LineaTotalesHTML(IDIndicadorActual, Totales, linea);
			cadenaHtml+='</tbody><tfoot class="rodape_tabela">'+LineaTotalesHTML_2(IDIndicadorActual, Totales, linea)+'</tfoot>';
		}
	}else{
		cadenaHtml+='<tr>'+cellIndCStart+txtSinResultados+cellEnd+rowEnd+'</tbody><tfoot class="rodape_tabela">'+cellIndCStart+'&nbsp;'+cellEnd+rowEnd+'</tfoot>';
	}
	
	//cadenaHtml+='<tr class="subTituloTabla"><td colspan="14" align="left">'+BotonArribaHTML(txtTodos, 'Seleccionar', 'javascript:Seleccionar();', 50)+BotonArribaHTML(txtResultados, 'Dejar todos los seleccionados', 'javascript:Activar(true);', 50)+BotonArribaHTML(txtVolver, 'Ver todas las lineas de datos', 'javascript:TodasLineasActivas();', 50)+'</td></tr>';
	//cadenaHtml+=rowIndStart+cellIndStart+cellEnd+rowEnd;
...
//			mostrarFilaTotal	= data.DatosEISMatriz[1].Indicador[0].FilaTotales;
			...
			// Redefinimos la variable (filtro) para guardar los comentarios de la matriz
			filtrosComentarios	= data.DatosEISMatriz[3].filtrosComentario;
...
function MostrarListado(Fila, Columna){
	var Enlace;
	var form=document.forms[0];

	Enlace='http://www.newco.dev.br/Gestion/EIS/EISListado.xsql?'
		+'IDINDICADOR='+form.elements['OR_IDINDICADOR'].value
		+'&'+'ANNO='+form.elements['OR_ANNOINICIO'].value
		+'&'+'MES='+form.elements['OR_MESINICIO'].value
		...
		+'&'+'AGRUPARPOR='+form.elements['OR_AGRUPARPOR_VER'].value
		+'&'+'IDGRUPO='+Fila;

	alert ('MostrarListado:'+Fila+','+Columna+' URL:'+Enlace);
	//alert('IDINDICADOR='+Indicador+' ANNO='+Anno+' MES='+Mes);

	MostrarPag(Enlace, 'Listado');
}

function MostrarEISMatriz(){
	MostrarPag('http://www.newco.dev.br/Gestion/EIS/EISMatriz.xsql', 'Consulta');
}
...
function MostrarXML(){
	...
	Enlace='http://www.newco.dev.br/Gestion/EIS/EISMatrizXML.xsql?'
...
function MostrarFichaAgrupacionVer(ID, PERIODO, TIPO){
	var Enlace;
	var Presentar=true;
	var form=document.forms[0];
	//alert(form.elements['OR_AGRUPARPOR_VER'].value);

	//si estamos en resumen proveedor enseño resumen listado
	if (TIPO == 'RESUMEN_PROV' || TIPO == 'RESUMEN'){
		var Enlace = 'http://www.newco.dev.br/Gestion/EIS/EISMatrizPrecalculada.xsql?IDPRECALCULADA='+ID+'&PERIODO='+PERIODO;
		window.open(Enlace,'_self');
	}
	//si no estamos en resumen prov enseño ficha empresa
	else{
		switch (form.elements['OR_AGRUPARPOR_VER'].value){
			...
			case 'PRO': Enlace='http://www.newco.dev.br/Administracion/Mantenimiento/Productos/PRODetalle.xsql?PRO_ID=';
				break;
			case 'IND': Enlace='http://www.newco.dev.br/Gestion/EIS/EISMatrizPrecalculada.xsql?IDPRECALCULADA=';
				break;
			...
			case 'FAM': Enlace='Familia';
				Presentar=false;
				break;
			default:	Enlace=form.elements['AGRUPARPOR_VER'].value;
				Presentar=false;
				break;
		}
		//alert(Enlace+':'+form.elements['AGRUPARPOR_VER'].value+':'+ID);
		var periodo = '&PERIODO='+PERIODO;
		if (Presentar==false)
			alert('No hay una ficha definida agrupando por '+Enlace)
		else
			MostrarPag(Enlace+ID+periodo, '_blank');
	}//fin de else si no hay periodo
}

//	Abre una pagina con la ficha del grupo seleccionado
function MostrarFichaAgrupacionHor(ID){
	var Enlace;
	var Presentar=true;
	var form=document.forms[0];

	//alert(form.elements['AGRUPARPOR_HOR'].value);

	switch (form.elements['AGRUPARPOR_HOR'].value){
		case 'EMP': Enlace='http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/EMPDetalleFrame.xsql?VENTANA=NUEVA&EMP_ID=';
			break;
		case 'EMP2': Enlace='http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/EMPDetalleFrame.xsql?VENTANA=NUEVA&EMP_ID=';
			break;
		case 'CEN': Enlace='http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/CENDetalle.xsql?ID=';
			break;
		default:	Enlace=form.elements['AGRUPARPOR_HOR'].value;
			Presentar=false;
			break;
	}

	//alert(Enlace+':'+form.elements['AGRUPARPOR_HOR'].value+':'+ID);
...
//	27feb20	quitamos ";" de las cadenas
...
function NumbertoCSV(Number) {return (sepTextoCSV+Number.toString().replaceAll('.',',')+sepTextoCSV+sepCSV);}


//	27feb20	Exportación a excel 
function listadoExcel()
{
	...
	cadenaCSV=StringtoCSV(Indicador)+saltoLineaCSV+saltoLineaCSV;

	// etiquetas de meses
	cadenaCSV+=StringtoCSV('');
	for(i=0;i<NumCabeceraH-1;i++)
	{
		cadenaCSV+=StringtoCSV(NombresCabeceraH[i]);
	}
	cadenaCSV+='TOTAL'+saltoLineaCSV;
	
	//	Nombres de grupos y filas de datos
	for(var l=0;l<=NumLineas-1;l++)
	{
		if(LineaActiva[l]=='S')
		{
			cadenaCSV+=StringtoCSV(NombresGrupos[l]);
			...
			for(i=0;i<NumCabeceraH-1;i++)
			{
				cadenaCSV+=NumbertoCSV(Valores[l*NumCabeceraH+i]);
			}
			cadenaCSV+=ColumnaTotales[l]+saltoLineaCSV;
		}
		
	}
	cadenaCSV+=StringtoCSV('TOTAL');
	for(i=0;i<NumCabeceraH;i++)
	{
		cadenaCSV+=NumbertoCSV(FilaTotales[i]);
	}
	cadenaCSV+=saltoLineaCSV;
	
	DescargaMIME(StringToISO(cadenaCSV), 'DatosMatrizEIS.csv', 'text/csv');
