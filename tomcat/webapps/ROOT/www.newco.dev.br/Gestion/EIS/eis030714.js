...
function globalEvents(){
	...
	// Si se cambia la seleccion del desplegable de Anno => Recargamos la página haciendo peticion a EISDatos2.xsql (para recargar desplegables de Empresa, Proveedor, etc)
	jQuery("#ANNO").change(function(){
		document.forms['formEIS'].action = 'EISDatos2.xsql';
	...
	if(jQuery('#ANNO').val() < 2012){
		...
		if(jQuery('#AGRUPARPOR').val() == 'FAM' || jQuery('#AGRUPARPOR').val() == 'SF' || jQuery('#AGRUPARPOR').val() == 'CAT' || jQuery('#AGRUPARPOR').val() == 'GRU')
			alert('No es posible agrupar por \'Familia\' o \'Subfamilia\' para años anteriores a 2012.\nPor favor, cambia uno de esos valores!');
	}
...
function MostrarFichaAgrupacion(ID){
	...
	switch (AgruparPor)
	{
		...
		case 'REF': Enlace='Referencia';
					Presentar=false;
					break;
		case 'REFPROV': Enlace='Referencia Proveedor';
					Presentar=false;
					break;
		case 'CAT': Enlace='Categoria';
					Presentar=false;
					break;
		case 'FAM': Enlace='Familia';
					Presentar=false;
					break;
		case 'SF': Enlace='Subfamilia';
					Presentar=false;
					break;
		case 'GRU': Enlace='Grupo';
					Presentar=false;
					break;
		case 'EST': Enlace='Estado';
					Presentar=false;
					break;
		case 'DES': Enlace='Descripcion';
					Presentar=false;
					break;
		default:
			return;
...
//	Genera de forma dinï¿½mica la tabla de datos del EIS
