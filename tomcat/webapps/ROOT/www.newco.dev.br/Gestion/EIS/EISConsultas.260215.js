	...
	jQuery.ajax({
		...
		success:function(data){
			...
			for(var i=0; i<doc.Empresas.length; i++){
				...
				lineasTabla += "<a href=\"javascript:tablaResumenCliente(" + doc.Empresas[i].datosEmpresa.empID + ");\"><img src=\"http://www.newco.dev.br/images/tabla.gif\"/></a>&nbsp;";
...
function tablaResumenCliente(IDEmpresa){
	var tHeadHTML = '', tBodyHTML = '';
	var d = new Date();

	jQuery.ajax({
		cache:	false,
		url:	'EISTablaResumenClienteAJAX.xsql',
		type:	"GET",
		...
		success: function(objeto){
			...
			if(data.Indicadores.length > 0){
				tHeadHTML = '<tr>';
					tHeadHTML += '<td>&nbsp;</td>';
					jQuery.each(data.Indicadores[0].Columnas, function(key, columna){
...
function TablaResumenCliente(flag){
	if(flag){
