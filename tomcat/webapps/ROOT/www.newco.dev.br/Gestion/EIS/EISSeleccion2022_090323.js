...
//	Ultima revisión: ET 9mar23 12:00 EISSeleccion2022_090323.js
...
function MostrarTipoSel()
{
    ...
    if (tipo == 'EMP'){
		...
		jQuery("#CHK_AUTORIZADOS").prop("disabled", false);
		jQuery("#CHK_EXCLUIDOS").prop("disabled", false);
    }
    else if (tipo == 'TDOC'){
        jQuery("#EMPRESAS").hide();
        jQuery("#CENTROS").hide();
	...
	if (tipo != 'EMP2')					//9mar23 
	{
		jQuery("#CHK_AUTORIZADOS").prop('checked',false);
		jQuery("#CHK_AUTORIZADOS").prop('disabled','disabled');
		jQuery("#CHK_EXCLUIDOS").prop('checked',false);
		jQuery("#CHK_EXCLUIDOS").prop('disabled','disabled');
	}
}
...
function Salir()
{
	window.location="http://www.newco.dev.br/Gestion/EIS/EISSelecciones2022.xsql?IDCLASIFICACION="+jQuery('#IDCLASIFICACION');	
}
...
{
	//9mar23 Si cambia la clasificacion, desmarcamos las opciones anteriores
	jQuery("#CHK_AUTORIZADOS").prop('checked',false);					//1jul22 attr
	jQuery("#CHK_EXCLUIDOS").prop('checked',false);

	if ((jQuery("#IDCLASIFICACION").val()=='AREAGEO')||(jQuery("#IDCLASIFICACION").val()=='MATERIAL'))
	{
		//9mar23 jQuery("#CHK_AUTORIZADOS").prop('checked',true);			//1jul22 attr
		jQuery("#CHK_AUTORIZADOS").prop('disabled','disabled');
		jQuery("#CHK_EXCLUIDOS").prop('disabled','disabled');
	}
	else
	{
		jQuery("#CHK_AUTORIZADOS").prop("disabled", false);
		jQuery("#CHK_EXCLUIDOS").prop("disabled", false);					//1jul22 removeAttr
	}
}
...
function AnadirEmpresaPorCod()
{
	...
	*/
		...
		if (Nif!='')		//	Descarta líneas en blanco
		{
			...
			for (var i=0; ((i<obj.options.length)&&(Enc=='N'));i++){
        		...
        		if (( NifObj=== Nif+')')||(NifObj === NifSinGuion+')')||(NifObjSinGuion === Nif+')'))			//	Comprobamos que el siguiente caracter sea ")" para evitar confusion con códigos cortos
				{
					...
					//solodebug	
					console.log('AnadirEmpresaPorCod. Comprobando ('+i+'):'+obj.options[i].text.substr(1,Nif.length)+' Encontrado.');
		
					Enc='S';
            		var id = obj.options[i].value;

            		//27nov21	if (registroSel.includes(id+'|'))			//CUIDADO, includes solo es para arrays, para string utilizar indexOf
					//27nov21	{ 
	...
	//	3may22 Este trozo estaba desactivado desde nov21, pero es necesario para insertar
	if (NumIns>0)
	{
...
//	9set22 Descargar excel de contenido de la seleccion
function DescargarExcel()
{
	var destino=document.forms['Seleccion'].elements['REGISTRO_SEL'];
	var IDSeleccion=jQuery("#IDSELECCION").val();
	
	cadenaCSV=strID+sepCSV+strNombre+saltoLineaCSV;
	
	var sel = document.getElementById("REGISTRO_SEL"); 
	for (var i = 0; i < sel.length; i++) {
      	 cadenaCSV+=sel[i].value+sepCSV+sel[i].text+saltoLineaCSV;
	}
	
	//solodebug debug(cadenaCSV);
	
	DescargaMIME(StringToISO(cadenaCSV), 'Seleccion_'+IDSeleccion+'.csv', 'text/csv');
