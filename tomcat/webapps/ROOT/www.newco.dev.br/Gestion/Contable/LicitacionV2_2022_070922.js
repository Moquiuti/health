...
//	Ultima revisión ET 7set22 12:00 LicitacionV2_2022_070922.js
...
function AbrirProducto(Pos)
{
	...
	else 
	{
		...
		for (var i=0; i<arrProductos[Pos].Ofertas.length;++i)
		{
			...
			Linea=Linea.replaceAll('[SPANCONSUMO]','<span class="'+Color+'">'+formateaDivisa(arrProductos[Pos].Ofertas[i].CONSUMO)+'</span>');
...
function ActualizarProducto(Pos){
	...
	listaProductos = IDProdLic+'|'+jQuery('#UdBasica').val()+'|'+nuevaCant+'|'+jQuery('#PrecioRef').val()+'|'+jQuery('#PrecioObj').val();	
	...
	if (arrProductos[Pos].Cantidad!=nuevaCant)
	{
		...
		if (IDOfeAdj!='')
		{
			...
			arrProductos[Pos].Ofertas[PosOfeAdj].CANTIDADADJUDICADA=(NumAdj==1)?nuevaCant:(ValorMax+nuevaCant-CantidadTotal);
			
			var Consumo=ValorMax*desformateaDivisa(arrProductos[Pos].Ofertas[PosOfeAdj].PRECIO);
			
			//solodebug	
			console.log('ActualizarProducto. Asignando cantidad a oferta IDOfeAdj:'+IDOfeAdj+' ValorMax:'+ValorMax+' Precio:'+(arrProductos[Pos].Ofertas[PosOfeAdj].PRECIO+' Consumo:'+Consumo);
			
			arrProductos[Pos].Ofertas[PosOfeAdj].CONSUMO=Consumo;

			var Color=CompruebaColorOferta(Pos, PosOfeAdj);
			jQuery('#Consumo_'+IDOfeAdj).html('<span class="'+Color+'">'+formateaDivisa(Consumo.toFixed(2))+'</span>');
