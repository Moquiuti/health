...
//	ultima revision ET 23feb17 15:07
...
function prepararEnvio(nombreFichero, licRequisicao, licTituloPdc, licDataVencimento, licHoraVencimento, licMoeda, licObservacao, licUrgencia)
{
	...
	jQuery.ajax({
		...
		type:	"GET",
		data:	"NOMBRE="+nombreFichero+"&DATOSFICHERO="+datosFichero+"&_="+d.getTime(),
...
function crearLicitacion(IDFichero, nombreFichero, licRequisicao, licTituloPdc, licDataVencimento, licHoraVencimento, licMoeda, licObservacao, licUrgencia)
{
	var d= new Date();
	...
	jQuery.ajax({
		...
		type:	"GET",
		data:	"NOMBRE="+nombreFichero+"&INTF_ID="+IDFichero+"&DATOSFICHERO="+datosFichero+"&_="+d.getTime(),
		contentType: "application/xhtml+xml",
		error: function(objeto, quepaso, otroobj){
			licError='ERROR_CONECTANDO';
		},
...
function EnviarLicitaciones(fileName, licIDLicitacion, licRequisicao, licTituloPdc, licDataVencimento, licHoraVencimento, licMoeda, licObservacao, licUrgencia, licProductos)
{
	...
	if (licIDFichero==-1)
	{
		licControlErrores+='Error desconocido al crear el fichero';
	}

	//	Control de errores: -2 fichero ya existe
	if (licIDFichero==-2)
	{
		licControlErrores+='Este fichero ya ha sido enviado';
	}

	//	Si no han habido errores con el fichero, creamos la licitacion
	if (licControlErrores=='')
	{
		crearLicitacion(licIDFichero, fileName, licRequisicao, licTituloPdc, licDataVencimento, licHoraVencimento, licMoeda, licObservacao, licUrgencia);

		//	Control de errores: -1 No se ha podido crear
		if (licIDLicitacion==-1)
		{
			licControlErrores+='Error desconocido al crear la licitacion';
		}
	}
	...
	if (licControlErrores=='')
	{
		for  (i=0;i<licProductos.length;++i)
		{
			...
			{	
				licControlErrores+=licControlErrores+' No se ha podido incluir el producto: ('+licProductos[i].Codigo+') '+licProductos[i].Descripcion+'<br/>';
			}
	...
	if (licControlErrores=='')
	{
		...
		jQuery('#infoProgreso').html('Licitacion preparada');
		jQuery('#infoProgreso').show();
