...
//	Ultima revision ET 19jun23 10:46 LicProveedor2022_250723.js
...
*/
		...
		{
			//	19jun23 Construimos la lista, poniendo la parte entera de las cantidades
			var v_ListaEntregas='';
			for (var i=0;i<=PieceCount(arrProductos[ColumnaOrdenadaProds[numProd]].Entregas,'#');++i)		//	Cuidado, devuelve 0 aunque haya una programacion, por que no acaba con #
			{
				var Entrega=Piece(arrProductos[ColumnaOrdenadaProds[numProd]].Entregas,'#',i);
				
				if (Entrega!='')
					v_ListaEntregas+=Piece(Entrega,'|',0)+':'+Piece(Piece(Entrega,'|',1),'.',0);	//	6jun23 Cortamos punto decimal
										
				//solodebug	console.log('PreparaLineaProductoBloqueada ('+numProd+','+ contLinea+','+Indice+') Entrega:'+Entrega+'. v_ListaEntregas:'+v_ListaEntregas);
			}			
			
			thisCell += '<a class="floatLeft" href="javascript:EntregasProducto('+ColumnaOrdenadaProds[numProd]+');"><img src="http://www.newco.dev.br/images/2022/icones/calendar-clock.png" class="valignM" title="'+str_ProgEntregas+': '+v_ListaEntregas+'"/></a>&nbsp;';
		}		
			...
			success: function(objeto){
				...
				if((tipo=='BORRAR')||(data.OfertaActualizada.IDOferta > 0)){	//al borrar devuelve IDOferta en blanco
					...
					if (tipo=='ALTER')
					{
						...
						items['Entregas']	=arrProductos[Indice].Entregas;			//25jul23 Recupera tambien el campo Entregas
...
function EntregasProducto(Pos)
{
	...
	//solodebug	
	debug("EntregasProducto:"+ Entregas);
	for (var i=0;i<=PieceCount(Entregas,'#');++i)
	{
		var Entrega=Piece(Entregas,'#',i);
		
		//solodebug debug("EntregasProducto("+i+"). Entrega:"+ Entrega);		
		if (Entrega!='')
		{
			//solodebug debug("EntregasProducto("+i+"). Incluir entrega:"+ Entrega);		
		...
		{
			//solodebug debug("EntregasProducto("+i+"). Entrega vacia:"+ Entrega);		
	...
	//solodebug debug("EntregasProducto("+i+"). Fin.");		

	//	19jun23 Vaciamos el final de la tabla
	for (;i<=9;++i)		
	{
		//solodebug debug("EntregasProducto("+i+"). Vaciando.");		
