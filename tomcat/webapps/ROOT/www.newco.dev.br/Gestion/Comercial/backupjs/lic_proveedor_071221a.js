...
//	Ultima revisión ET 7dic21 11:00 lic_proveedor_071221a.js
...
//	13feb20 Separamos el envío del cambio de estado
...
//	Añade una lista de ofertas a la licitación
/*	5nov21 Parece que solo se utiliza en la ficha de producto
function AnadirOfertas(oForm){
	var IDLicProv	= oForm.elements['LIC_PROV_ID'].value;		// LIC_PROV_ID
	var thisRowID, thisPosArr, IDLicProd, IDFicha, IDRegSanitario, listaOfertas= '';
	var refProv;
	var d = new Date();
	...
	jQuery('table#lProductos_PROVE tbody tr.infoProds').each(function(index, element){
		...
		if(jQuery("#IDREGSANITARIO_" + thisPosArr).val() > 0){
			IDRegSanitario = jQuery("#IDREGSANITARIO_" + thisPosArr).val();
		}else{
			IDRegSanitario = '';
		}
...
//	3dic20 Prepara el HTML correspondiente a una línea de producto bloqueada (producto adjudicado)
...
*/
		...
		thisMacro = thisMacro.replace('#CLASS#', 'IDFicha select400');
		thisCell += thisMacro;
		...
		thisCell += dibujaSubirDocOferta('FT', (Indice));
		thisCell += cellEnd;
		thisRow += thisCell;
...
function PreparaLineaProducto(numProd, contLinea)
{
	...
	try
	{
		...
		thisMacro = thisMacro.replace('#CLASS#', 'btnDestacado');
	...
	//			thisCell += '<strong>' + arrProductos[ColumnaOrdenadaProds[numProd]].Nombre + '</strong>&nbsp;';
		...
		if ((arrProductos[ColumnaOrdenadaProds[numProd]].Marcas!='')||(arrProductos[ColumnaOrdenadaProds[numProd]].PrincActivo!=''))
			thisCell += '<img src="http://www.newco.dev.br/images/2017/info.png" title="Marcas: '+arrProductos[ColumnaOrdenadaProds[numProd]].Marcas+'&#13;'+strPrincActivo+': '+arrProductos[ColumnaOrdenadaProds[numProd]].PrincActivo+'" class="static"/>'
	...
	//				thisCell += '<img src="" class="static"/>';
		...
		if (incluirRS=='S')
		{
			...
			thisMacro = thisMacro.replace('#CLASS#', 'Regsan medio');
			thisMacro = thisMacro.replace('#SIZE#', '10');
			thisMacro = thisMacro.replace('#MAXLENGTH#', '15');
			thisMacro = thisMacro.replace('#ONKEYUP#', 'javascript:ActivarBotonGuardar('+(Indice)+');');
		...
		thisMacro = thisMacro.replace('#ID#', 'Desc_' + (Indice));
		thisMacro = thisMacro.replace('#STYLE#', 'width:100%;');
		thisMacro = thisMacro.replace('#CLASS#', 'descripcion');
		thisMacro = thisMacro.replace('#SIZE#', '30');
		...
		thisMacro = thisMacro.replace('#ID#', 'IDFICHA_' + (Indice));
		thisMacro = thisMacro.replace('#CLASS#', 'IDFicha grande');
		//7dic21	thisMacro = thisMacro.replace('#ONCHANGE#', 'javascript:ActivarBotonGuardar('+(Indice)+');');
		...
		thisCell += '<a id="verDocFT_'+Indice+'" href="javascript:VerDocumentoOferta(\'FT\', '+Indice+')" style="display:none">'+str_VerDoc+'</a>&nbsp;';
		
		thisCell += divEnd;
		// Aqui falta el template carga documentos
		//15nov21	thisCell += dibujaSubirFT(Indice);
		thisCell += dibujaSubirDocOferta('FT', Indice);
		thisCell += cellEnd;
		thisRow += thisCell;
		thisRow += rowEnd;
		...
		if (incluirRS=='S')
		{
			...
			thisMacro = thisMacro.replace('#CLASS#', 'IDRegSan grande');
			//7dic21 thisMacro = thisMacro.replace('#ONCHANGE#', 'javascript:ActivarBotonGuardar('+(Indice)+');');
			...
			thisCell += macroSelectEnd + '&nbsp;';
			thisCell += '<a id="verDocRS_'+Indice+'" href="javascript:VerDocumentoOferta(\'RS\', '+Indice+')" style="display:none">'+str_VerDoc+'</a>&nbsp;';
			thisCell += divEnd;
			// Aqui falta el template carga documentos
			//15nov21thisCell += dibujaSubirRS(Indice);
			thisCell += dibujaSubirDocOferta('RS', Indice);
			thisCell += cellEnd;
			thisRow += thisCell;
			thisRow += rowEnd;
		}
		...
		if (incluirCE=='S')
		{
			...
			thisMacro = thisMacro.replace('#CLASS#', 'IDCertExp grande');
			//7dic21 thisMacro = thisMacro.replace('#ONCHANGE#', 'javascript:ActivarBotonGuardar('+(Indice)+');');
			...
			thisCell += '<a id="verDocCE_'+Indice+'" href="javascript:VerDocumentoOferta(\'CE\', '+Indice+')" style="display:none">'+str_VerDoc+'</a>&nbsp;';

			thisCell += divEnd;
			// Aqui falta el template carga documentos
			//15nov21thisCell += dibujaSubirCE(Indice);
			thisCell += dibujaSubirDocOferta('CE', Indice);
			thisCell += cellEnd;
			thisRow += thisCell;
			thisRow += rowEnd;
		}
...
function guardarOfertaFila(thisPosArr, tipo, recal)
{
	...
	//solodebug
	debug("guardarOfertaFila. thisPosArr:"+thisPosArr+" IDProdLic:"+IDProdLic+'???'+jQuery('#IDPRODLIC_' + thisPosArr).val()+" RefCliente:"+RefCliente
		+ " marca:"+oForm.elements['Marca_' + thisPosArr].value
	...
	debug("guardarOfertaFila. thisPosArr:"+thisPosArr+' FTObligatoria:'+FTObligatoria+ ' IDFicha:'+jQuery("#IDFICHA_" + thisPosArr).val()
			+' RSObligatorio:'+RSObligatorio+ ' RegSan:'+jQuery("#REGSAN_" + thisPosArr).val() + ' IDRegSan:'+jQuery("#IDREGSAN_" + thisPosArr).val() + ' IDCertExper:'+jQuery("#IDCERTEXPER_" + thisPosArr).val()
		);
	...
	else if(!errores && precioFormat != 0)
	{
		...
		if ((precioObj!='')&&(precioFormat > precioObjFormat))
		{
			...
			{
				errores++;
				alert(val_precioObjObligatorio.replace("[[REF]]",RefCliente).replace("[[PRECIO]]",precio));
			}
...
*/
	...
	if(enviarOferta){
		...
		var IDFicha = '',IDRegSanitario='', IDCertExperiencia='';					//	5nov21 Nuevo campo para registro sanitario, texto reg. sanitario, certif.experiencia.
		if(jQuery("#IDFICHA_" + thisPosArr).val() > 0){
			IDFicha = jQuery("#IDFICHA_" + thisPosArr).val();
		}
		...
		console.log("AnadirUnaOfertaAJAX. thisPosArr:"+thisPosArr+" LIC_ID:"+IDLicitacion+"&LIC_PROD_ID:"+IDLicProd+"&LIC_PROV_ID:"+IDLicProv+"&REFPROV:"+encodeURIComponent(ScapeHTMLString(RefProv))+"&DESC:"+encodeURIComponent(ScapeHTMLString(Descripcion))+"&MARCA:"
					...
					+"&IDCERTEXPERIENCIA="+IDCertExperiencia+"&ALTERNATIVA:"+Alternativa);
...
function ActualizarOfertas(Referencias)
{
	...
	{
		var Ref,RefProv,RefProv2,Nombre,Marca,UdsXLote,Precio,RegSan;
...
function dibujaSubirDocOferta(Tipo, ID){
	var innerHTML, thisRow, thisCell, txtSubir;
	
	//solodebug debug('dibujaSubirDocOferta. Tipo:'+Tipo+' ID:'+ID);
	
	if (Tipo=='FT')
		txtSubir=str_SubirFicha;
	else
		txtSubir=str_SubirRegSan;
		
	var strJS="javascript:mostrarTablaDocOferta(\'"+Tipo+"\',"+ID+");";
	innerHTML = '<div style="width:200px;float:left;height:30px;text-align:center"><a id="a_'+Tipo+'_'+ID+'" href="'+strJS+'">'+str_SubirFicha+'</a></div>';

	innerHTML += divStartClassID.replace('#ID#', 'cargaDoc'+Tipo);
	innerHTML = innerHTML.replace('#CLASS#', 'divLeft50 cargaDoc'+Tipo);

	innerHTML += '<table id="tbl_'+Tipo+'_'+ID+'" class="infoTable" border="0" style="width:1200px;display:none;">';
	thisRow = rowStart;
	...
	//15nov21 thisMacro = thisMacro.replace('#ONCHANGE#', 'addDocFile(document.forms[\'ProductosProveedor\'],' + ID + ');');
	...
	//solodebug debug('dibujaSubirDocOferta. Tipo:'+Tipo+' ID:'+ID+' innerHTML:'+innerHTML);	
...
function VerDocumentoOferta(tipo, Pos)
{
	...
	if (IDDoc!='-1')
	{
		...
		{
			//solodebug 
			debug('VerDocumentoOferta. Tipo:'+tipo+' Pos:'+Pos+' IDDoc:'+IDDoc+' AbrirDoc('+Pos+'):'+arrDocs[Pos].URL);	
...
function DocumentoSeleccionado(tipo, Pos)
{
	...
	//solodebug	
	debug("DocumentoSeleccionado. tipo:"+tipo+" Pos:"+Pos+ "IDDoc:"+IDDoc);
...
function IDDocumentoSeleccionado(tipo, Pos)
{
	...
	//solodebug	
	debug("IDDocumentoSeleccionado. tipo:"+tipo+" Pos:"+Pos+ "IDDoc:"+IDDoc);
...
function ArrayDocumentos(tipo)
{
	...
	//solodebug	
	debug("ArrayDocumentos. tipo:"+tipo+" arrDocs:"+arrDocs.name+'('+arrDocs.length+')'+arrDocs[0].URL);
...
function BuscaIDEnArray(arrDocs, IDDoc)
{
	...
	//solodebug 
	debug('BuscaIDEnArray. IDDoc:'+IDDoc);	
	
	for(var k=0; (k<arrDocs.length) && (Pos==-1); k++)
	{
		//solodebug 
		debug('VerDocumento. IDDoc:'+IDDoc+' Revisando '+k+':['+arrDocs[k].ID+']:'+arrDocs[k].URL);	

		if(IDDoc == arrDocs[k].ID)
		{

			//solodebug 
			debug('VerDocumento. IDDoc:'+IDDoc+' Pos:'+Pos+' AbrirDoc('+k+'):'+arrDocs[k].URL);	
