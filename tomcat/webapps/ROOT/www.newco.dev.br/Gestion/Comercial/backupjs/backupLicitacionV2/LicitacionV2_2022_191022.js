...
//	Ultima revisión ET 20oct22 19:15 LicitacionV2_2022_201022.js
...
{
	var Pos=0;
	for (var i=0; i</*23set22 arrConsumoProvs.*/arrProveedores.length;++i)
	{
		//23set22	if (arrConsumoProvs[i].TieneOfertas=='S')
		if (arrProveedores[i].TieneOfertas=='S')
		{
			//23set22	arrConsumoProvs[i].ColMatriz=Pos;
...
function IncluirProveedores()
{
	...
	for (var i=0;i</*23set22 arrConsumoProvs.*/arrProveedores.length;++i)
		//23set22	jQuery('#IDPROVEEDOR').append("<option value='"+arrConsumoProvs[i].ID+"' >"+arrConsumoProvs[i].NombreCorto+"</option>")
		jQuery('#IDPROVEEDOR').append("<option value='"+arrProveedores[i].ID+"' >"+arrProveedores[i].NombreCorto+"</option>")
}
		...
		success: function(objeto){
			...
			if(data.OfertasProductoLic.posicion && data.OfertasProductoLic.posicion!= ''){
				...
				for (var i=0; i<data.OfertasProductoLic.ofertas.length; ++i)
				{
					...
					Oferta["VENDEDOR"]=data.OfertasProductoLic.ofertas[i].VENDEDOR;								//11ago22
					...
					//23set22	if(Oferta["ColProveedor"]>=0) Oferta["ColMatriz"]=arrConsumoProvs[Oferta["ColProveedor"]].ColMatriz;
...
{
	var Res=-1;
	for (var i=0; ((Res==-1)&&(i</*23set22 arrConsumoProvs.*/arrProveedores.length));++i)
	{
		//23set22	if (arrConsumoProvs[i].IDProvLic==IDProveedorLic) Res=i;
...
function AbrirProducto(Pos, Foco)
{
	...
	else 
	{
		...
		if ((esAutor=='S')&&(editarCant=='S')&&((EstadoLic=='EST')||(EstadoLic=='COMP')||(EstadoLic=='CURS')||(EstadoLic=='INF')))									//7set22 Comprobamos estado lic.
		{
			...
			linDatosProd=linDatosProd.replace('[UD_BASICA]','<input class="campopesquisa w80px" type="text" name="UdBasica" id="UdBasica" value="'+arrProductos[Pos].UdBasica+'"/>');		
		}
		else
		{
		...
		{
			linDatosProd=linDatosProd.replace('[CANT_PRODUCTO]','<input class="campopesquisa w80px" type="text" size="5" name="Cantidad" id="Cantidad" value="'+arrProductos[Pos].Cantidad+'"/>');		
		}
		else
		{
		...
		for (var i=0; i<arrProductos[Pos].Ofertas.length;++i)
		{
			...
			//23set22	Linea=Linea.replaceAll('[PROV_CONSUMOADJUDICADO]',arrConsumoProvs[PosProveedor].ConsumoAdj);				
			...
			//solodebug	console.log('AbrirProducto. Presentar consumo: '+arrProductos[Pos].Ofertas[i].CONSUMO+'->'+formateaDivisa(arrProductos[Pos].Ofertas[i].CONSUMO))
			...
			Linea=Linea.replaceAll('[SPANCONSUMO]','<span class="'+Color+'">'+formateaDivisa(Consumo)+'</span>');		//19set22 CUidado, antes formateaDivisa(arrProductos[Pos].Ofertas[i].CONSUMO
...
function MostrarConsumoOferta(PosProd, PosOfe)
{
	...
	jQuery('#Consumo_'+IDOferta).html('<span class="'+Color+'">'+formateaDivisa(Consumo)+'</span>');
...
{		
	...
	for (var i=0;i<arrProductos[Pos].OfertasBack.length;++i)
	{
		if (arrProductos[Pos].OfertasBack[i].OfertaAdjud=='S')
		{
			...
			//23set22	arrConsumoProvs[PosProveedor].ConsumoAdj_Float-=arrProductos[Pos].OfertasBack[i].PRECIO_FLOAT*parseInt(arrProductos[Pos].OfertasBack[i].CANTIDADADJUDICADA);
			//23set22	arrConsumoProvs[PosProveedor].ConsumoAdj=formateaDivisa(arrConsumoProvs[PosProveedor].ConsumoAdj_Float);
			...
			arrProveedores[PosProveedor].ConsumoAdj=formateaDivisa(arrProveedores[PosProveedor].ConsumoAdj_Float);

			//23set22	--arrConsumoProvs[PosProveedor].OfertasAdj;
	...
	for (var i=0;i<arrProductos[Pos].Ofertas.length;++i)
	{
		if (arrProductos[Pos].Ofertas[i].OfertaAdjud=='S')
		{
			...
			//23set22	arrConsumoProvs[PosProveedor].ConsumoAdj_Float+=arrProductos[Pos].Ofertas[i].PRECIO_FLOAT*parseInt(arrProductos[Pos].Ofertas[i].CANTIDADADJUDICADA);
			//23set22	arrConsumoProvs[PosProveedor].ConsumoAdj=formateaDivisa(arrConsumoProvs[PosProveedor].ConsumoAdj_Float);
...
function BuscaProveedor(ID)
{
	var Pos=-1;
	//	Repasa las ofertas, sumando la seleccion actual
	for (var i=0;(Pos==-1)&&(i</*23set22 arrConsumoProvs.*/arrProveedores.length);++i)
	{
		//23set22	if (arrConsumoProvs[i].ID==ID) Pos=i;
				...
				if(data.OfertaActualizada.IDOferta > 0)
				{
					...
					if(data.OfertaActualizada.TodosInformados == 'Si')
					{
						...
						*/	
						...
						debug('Nuevo consumo:'+formateaDivisa(Consumo));

						jQuery('#Consumo_'+IDOferta).html('<span class="'+Color+'">'+formateaDivisa(Consumo.toFixed(2))+'</span>');
...
function DatosProducto(Pos){
	...
	if(!errores){
		jQuery('#Cantidad').val( formateaDivisa(valAux) );
	}
...
//	8set22 Actualiza datos del producto: cantidad, precio de referencia y unidad basica. Adaptado desde FichaProductoiLicitacion.js
function ActualizarProducto(Pos){
	...
	var PrecioObj=(jQuery('#PrecioObj').length)?jQuery('#PrecioObj').val():'';
	
	listaProductos = IDProdLic+'|'+jQuery('#UdBasica').val()+'|'+nuevaCant+'|'+jQuery('#PrecioRef').val()+'|'+PrecioObj;	
	...
	if (arrProductos[Pos].Cantidad!=nuevaCant)
	{
		...
		for (var i=0;i<arrProductos[Pos].Ofertas.length;++i)
		{
			...
			var Color=CompruebaColorOferta(Pos, i);
			jQuery('#Consumo_'+ID).html('<span class="'+Color+'">'+formateaDivisa(Consumo)+'</span>');
		}
	...
	jQuery.ajax({
		...
		success: function(objeto){
			...
			if(data.ProductosActualizados.estado == 'OK'){
				...
				arrProductos[Pos].Cantidad=jQuery('#Cantidad').val();
				arrProductos[Pos].UdBasica=jQuery('#UdBasica').val();
				arrProductos[Pos].PrecioHist=jQuery('#PrecioRef').val();
...
function CompruebaColorOferta(PosProd, PosOfe)
{
	...
	if (arrProductos[PosProd].PrecioHist!='')
	{
		...
		debug('CompruebaColorOferta('+PosProd+','+ PosOfe+') PrecioHistNum:'+PrecioHistNum+' PrecioNum:'+PrecioNum);
		...
		debug('CompruebaColorOferta('+PosProd+','+ PosOfe+') PrecioHistNum:'+PrecioHistNum+' PrecioNum:'+PrecioNum+' Color:'+Color);
	}
	else
		debug('CompruebaColorOferta('+PosProd+','+ PosOfe+') No hay precio historico. Color:'+Color);
		...
		jQuery.ajax({
			...
			success: function(objeto){
				...
				if(data.PedidoDesdeOferta.estado == 'OK'){
					...
					//solodebug	debug("Pedido. arrProveedores.length:"+arrProveedores.length+' /*23set22 arrConsumoProvs.*/arrProveedores.length:'+/*23set22 arrConsumoProvs.*/arrProveedores.length);
...
//	Exportación completa a Excel de la licitación
