...
//	Ultima revisión ET 15ago22 09:00 LicitacionV2_2022_150822.js
...
function AbrirProducto(Pos)
{
	...
	headTablaOfertas+='<th class="w80px textLeft">'+strMarca+'</th>';				//	permitimos que esta columna se ensanche si hay espacio
	...
	//debug('AbrirProducto.Pos:'+Pos+' esCdC:'+esCdC+' headTablaOfertas:'+headTablaOfertas);
	...
	var linTablaOfertas='<tr id="OFE_[LIC_OFE_ID]" class="filaOferta conhover">'
					...
					+'[PRODUCTO]'
					//	Marca
					+'<td class="textLeft">[MARCA]</td>[CHECKMARCA]'						//	12ago22
	...
	else 
	{
		...
		for (var i=0; i<arrProductos[Pos].Ofertas.length;++i)
		{
			...
			//	PRECIO
			//	Color a aplicar al precio Y AL AHORRO (11ago22)			
			if (arrProductos[Pos].Ofertas[i].SUPERIOR=='S')
				Linea=Linea.replaceAll('[COLOR]','rojo');
			else if (arrProductos[Pos].Ofertas[i].IGUAL=='S')
				Linea=Linea.replaceAll('[COLOR]','naranja');
			else				
				Linea=Linea.replaceAll('[COLOR]','verde');
...
function GuardarOferta(Pos, IDOferta)
{
	...
	/*	15ago22 Por ahora solo permitimos guadar el preico
	...
	if(enviarOferta)
	{
		...
		var Marca = encodeURIComponent(arrProductos[Pos].Ofertas[PosOferta].MARCA);
			...
			success: function(objeto){
				...
				if(data.OfertaActualizada.IDOferta > 0)
				{
					...
					if(data.OfertaActualizada.TodosInformados == 'Si')
					{
						...
						//	PENDIENTE Ahorro=....   arrProductos[Pos].Ahorro=Ahorro;
						//	PENDIENTE: si se ha revisado mejor precio, revisar color ahorro

						alert('PENDIENTE RECALCULAR TOTALES');
						
						//	REDIBUJAR PRODUCTO o al menos ahorro
