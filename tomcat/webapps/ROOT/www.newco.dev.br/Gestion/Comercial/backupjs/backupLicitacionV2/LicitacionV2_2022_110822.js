...
//	Ultima revisión ET 12ago22 11:09 LicitacionV2_2022_110822.js
...
function AbrirProducto(Pos)
{
	...
	var linTablaOfertas='<tr id="OFE_[LIC_OFE_ID]" class="filaOferta conhover">'
					...
					+'<td><span class="fuentePeq [COLOR]">[AHORRO]</span></td>'				//	11ago22 Nueva columna AHORRO
		...
		jQuery.ajax({
			...
			success: function(objeto){
				...
				if(data.Resultado.Estado == 'OK'){
					...
					//	REvisamos otras ofertas con la misma marca
					for (var i=0;i<arrProductos[Pos].Ofertas.length;++i)
					{
						if ((arrProductos[Pos].Ofertas[i].MARCA_ACEPTABLE=='N')&&(arrProductos[Pos].Ofertas[i].MARCA==marca))
						{
							jQuery("#chkMarca_"+arrProductos[Pos].Ofertas[i].ID).attr('disabled', true);
							jQuery("#chkMarca_"+arrProductos[Pos].Ofertas[i].ID).prop('checked', true);
