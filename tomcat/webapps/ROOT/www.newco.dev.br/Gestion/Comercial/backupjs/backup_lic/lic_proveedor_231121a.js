...
//	Ultima revisión ET 22nov21 10:00 lic_proveedor_231121a.js
...
function PreparaLineaProducto(numProd, contLinea)
{
	...
	//solodebug	
	console.log('PreparaLineaProducto ('+numProd+','+ contLinea+','+Indice+')');
	...
	//				thisCell += '<img src="" class="static"/>';
		...
		// Celda/Columna Descripcion
		console.log('PreparaLineaProducto. Celda/Columna Descripcion');
		thisCell = cellStart;
...
function dibujaSubirDocOferta(Tipo, ID){
	...
	innerHTML = '<a id="a_'+Tipo+'_'+ID+'" href="'+strJS+'">'+str_SubirFicha+'</a>';

	innerHTML += divStartClassID.replace('#ID#', 'cargaDoc'+Tipo);
	innerHTML = innerHTML.replace('#CLASS#', 'divLeft80 cargaDoc'+Tipo);
...
/*
//	5nov21 Muestra los campos para ver/subir/actualizar el registro sanitario
function dibujaSubirRS(ID){

	console.log('dibujaSubirRS. ID:'+ID);

	var innerHTML, thisRow, thisCell;

	innerHTML = divStartClassID.replace('#ID#', 'cargaDocRS');
	innerHTML = innerHTML.replace('#CLASS#', 'divLeft80');

	innerHTML += '<table class="infoTable" border="0" style="width:1200px;">';
	thisRow = rowStart;

	thisCell = cellStartClass.replace('#CLASS#', 'labelRight trenta');
	thisCell += spanStartClass.replace('#CLASS#', 'textRS_' + ID);
	thisCell += str_SubirFicha;
	thisCell += spanEnd;
	thisCell += cellEnd;
	thisRow += thisCell;
	...
	thisCell += divStartClassID.replace('#CLASS#', 'docLongEspec').replace('#ID#', 'docLongEspec_' + ID);
	thisMacro = macroInputFile.replace('#NAME#', 'inputFileDoc');
	thisMacro = thisMacro.replace('#ID#', 'inputFileDoc_RS_' + ID);
	...
	thisCell += macroEnlace.replace('#HREF#', 'javascript:cargaDoc(document.forms[\'ProductosProveedor\'],\'RS\',' + ID + ');');
	thisCell += spanStartClass.replace('#CLASS#', 'textFT') + str_SubirRegSan + spanEnd;
	thisCell += macroEnlaceEnd;
	thisCell += divEnd;
	thisCell += cellEnd;
	thisRow += thisCell;
	...
	thisCell = cellStart;
	thisCell += divStartIDAlign.replace('#ID#', 'waitBoxDoc_RS_' + ID).replace('#ALIGN#', 'center');
	thisCell += '&nbsp;';
	thisCell += divEnd;
	thisCell += divStartIDStyAlg.replace('#ID#', 'confirmBox_RS_' + ID).replace('#STYLE#', 'display:none;').replace('#ALIGN#', 'center');
	thisCell += spanStartClassStyle.replace('#CLASS#', 'cargado').replace('#STYLE#', 'font-size:10px;');
	thisCell += '¡' + str_DocCargado + '!';
	thisCell += spanEnd;
	thisCell += divEnd;
