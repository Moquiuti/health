...
//	Ultima revisión ET 5nov21 12:00 lic_proveedor_051121.js
...
function guardarOfertaFila(thisPosArr, tipo, recal)
{
	...
	debug("guardarOfertaFila. thisPosArr:"+thisPosArr+' FTObligatoria:'+FTObligatoria+ ' IDFicha:'+jQuery("#IDFICHA_" + thisPosArr).val()
		+' RSObligatorio:'+RSObligatorio+ ' IDRegSan:'+jQuery("#IDREGSAN_" + thisPosArr).val());
	...
	else if(!errores && precioFormat != 0)
	{
		...
		if ((precioObjEstricto=='S')&&(precioObj!='')&&(precioFormat > precioObjFormat))
		{
			errores++;
...
*/
	...
	if(enviarOferta){
		...
		var IDFicha = '',IDRegSanitario='';									//	5nov21 Nuevo campo para registro sanitario
		if(jQuery("#IDFICHA_" + thisPosArr).val() > 0){
			IDFicha = jQuery("#IDFICHA_" + thisPosArr).val();
		}
		...
		console.log("AnadirUnaOfertaAJAX. thisPosArr:"+thisPosArr+" LIC_ID:"+IDLicitacion+"&LIC_PROD_ID:"+IDLicProd+"&LIC_PROV_ID:"+IDLicProv+"&REFPROV:"+encodeURIComponent(ScapeHTMLString(RefProv))+"&DESC:"+encodeURIComponent(ScapeHTMLString(Descripcion))+"&MARCA:"
					+encodeURIComponent(ScapeHTMLString(Marca))+"&UDSXLOTE:"+UdsXLote+"&CANTIDAD:"+Cantidad+"&PRECIO:"+Precio+"&TIPOIVA:"+TipoIVA+"&IDFICHA:"+IDFicha+"&IDREGSANITARIO="+IDRegSanitario
					+"&ALTERNATIVA:"+Alternativa);
...
function dibujaSubirDocOferta(Tipo, ID){
	...
	debug('dibujaSubirDocOferta. Tipo:'+Tipo+' ID:'+ID);
	...
	thisCell += spanStartClass.replace('#CLASS#', 'textFT') + txtSubir + spanEnd;
	thisCell += macroEnlaceEnd;
	thisCell += divEnd;
	thisCell += cellEnd;
	thisRow += thisCell;
	*/
	...
	debug('dibujaSubirDocOferta. Tipo:'+Tipo+' ID:'+ID+' innerHTML:'+innerHTML);	
