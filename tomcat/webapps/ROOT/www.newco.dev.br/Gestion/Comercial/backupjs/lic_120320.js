...
//	Ultima revisión ET 12mar20 15:00 lic_120320.js
...
function AnadirProductos(oForm){
	...
	//solodebug debug('AnadirProductos:'+Referencias+' sendProductos:'+sendProductos+' loops:'+loops);
...
function AnadirProductosAjax(oForm, Referencias, loop, loops, error)
{
	...
	//solodebug	debug('AnadirProductosAjax: '+loop+'/'+loops+ ' estLic:'+estadoLicitacion);
	...
	if (loop==loops)
	{
		if (!error)
		{
			...
			else
			{
				...
				//solodebug	debug('AnadirProductosAjax: '+loop+'/'+loops+ ': prepararTablaProductos');
		...
		success: function(objeto){
			...
			if(data.ListaProductos.length > 0){
				...
				jQuery.each(data.ListaProductos, function(key, producto){
					...
					//solodebug debug('recuperaDatosProductos ('+arrProductos.length+'):'+producto.ProdRefCliente+' '+producto.ProdNombre);
