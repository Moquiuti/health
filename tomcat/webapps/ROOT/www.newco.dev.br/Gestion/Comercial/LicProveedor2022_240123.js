...
//	Ultima revision ET 27may22 13:00 LicProveedor2022_240123.js
...
function PreparaLineaProducto(numProd, contLinea)
{
	...
	//27may22		thisCell += '<img src="http://www.newco.dev.br/images/2017/info.png" title="'+strPrincActivo+': '+arrProductos[ColumnaOrdenadaProds[numProd]].PrincActivo+'" class="static"/>'
		...
		thisCell = cellStart;
			//solodebug	
			console.log("PreparaLineaProducto("+numProd+"): TipoIVA");
		if(IDPais != '55'){
			//solodebug	
			console.log("PreparaLineaProducto("+numProd+"): TipoIVA. Desplegable");
			//27ene23 thisCell += arrProductos[ColumnaOrdenadaProds[numProd]].TipoIVA + '%';
			thisCell += DesplegableTipoIva(Indice, arrProductos[ColumnaOrdenadaProds[numProd]].TipoIVA);
		}else{
			thisCell += '&nbsp;';
		}
		thisCell += cellEnd;
...
{
	//solodebug	
	console.log('DesplegableTipoIva. NumeroLinea:'+NumeroLinea+' TipoIva:'+TipoIva);
...
function guardarOfertaFila(thisPosArr, tipo, recal)
{
	...
	if(enviarOferta){
		...
		//solodebug
		console.log("Preparando AnadirUnaOfertaAJAX. thisPosArr:"+thisPosArr+" LIC_ID:"+IDLicitacion+"&LIC_PROD_ID:"+IDLicProd+"&LIC_PROV_ID:"+IDLicProv+"&REFPROV:"+encodeURIComponent(ScapeHTMLString(RefProv))+"&DESC:"+encodeURIComponent(ScapeHTMLString(Descripcion))+"&MARCA:"
					...
					+"&IDCERTEXPERIENCIA="+IDCertExperiencia+"&ALTERNATIVA:"+Alternativa+"&IDOFERTALIC="+IDOfeLic);
...
function dibujaSubirDocOferta(Tipo, ID){
	...
	innerHTML = '<div class="w200px floatLeft"><a id="a_'+Tipo+'_'+ID+'" href="'+strJS+'" class="btnDestacadoPeq">'+str_SubirFicha+'</a></div>';
