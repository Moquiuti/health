...
<!-- 
	...
	ultima revision: ET 24oct22 11:45
-->
...
<xsl:template name="Tabla_Productos_Ofertas_Cliente">
	...
	<div id="topPesProds h30px">
		<!-- Link a pagina anterior -->
		<div id="pagAnterior" class="divLeftw floatLeft w120px marginLeft20 lineHeight30">&nbsp;</div>
		<!-- Desplegable para numProductos en pagina, leyenda paginacion y desplegable tipo vista -->
		<div id="pagTamanyo" class="divLeftw floatLeft w800px textLeft marginLeft20 lineHeight30 fuentePeq">
		...
		<div id="pagSiguiente" class="divLeftw  w120px marginLeft20 lineHeight30">&nbsp;</div>
	</div>
	...
	<xsl:if test="/Mantenimiento/LICITACION/AUTOR and /Mantenimiento/LICITACION/LIC_PORPRODUCTO = 'S' and (/Mantenimiento/LICITACION/LIC_IDESTADO = 'INF' or /Mantenimiento/LICITACION/LIC_IDESTADO = 'CURS')">
		...
		<xsl:if test="/Mantenimiento/LICITACION/LIC_IDESTADO != 'CONT'">
			<a class="btnDestacadoPeq" id="botonActualizar2" href="javascript:ActualizarProductos();">
				<xsl:value-of select="document($doc)/translation/texts/item[@name='actualizar_datos']/node()"/>
			</a>&nbsp;
		</xsl:if>
		<!--10jul22 No hace falta este boton de mejores precios, ya hay uno a nivel de la pagina
	...
	<xsl:if test="/Mantenimiento/LICITACION/AUTOR"><!-- 1set16	No para licitaciones agregadas	-->
		<div style="margin-top: 20px;"><!--Boton actualizar bajo la tabla	-->
			<a class="btnDestacadoPeq" id="botonActualizar" href="javascript:ActualizarProductos();">
				<xsl:attribute name="style">
					<xsl:choose>
					<xsl:when test="/Mantenimiento/LICITACION/LIC_IDESTADO = 'CONT'">display:none;</xsl:when>
...
<xsl:if test="/Mantenimiento/LICITACION/LIC_PORPRODUCTO = 'S'">
	<div class="FBox" id="floatingBox" style="display:none;clear:both;margin-top:5px;left:700px;">
...
<xsl:if test="/Mantenimiento/LICITACION/AUTOR">
	...
	<!-- DIV Flotante -->
	<div class="FBox" id="floatingBox" style="display:none;clear:both;left:700px;">
	...
	<xsl:when test="/Mantenimiento/LICITACION/PRODUCTOSLICITACION/PRODUCTO and /Mantenimiento/LICITACION/PERMITIR_EDICION">
		...
		<thead>
			<tr class="subTituloTabla">
				...
				<td class="w1px">&nbsp;</td>
				<td class="w100px">&nbsp;</td><!-- columna iconos campos avanzados producto informados -->
				<td class="textLeft">
					<!-- Permitimos ordenacion -->
					<a href="javascript:OrdenarProdsPorColumna('NombreNorm');" style="text-decoration:none;" id="anchorNombre">
						<xsl:value-of select="document($doc)/translation/texts/item[@name='nombre']/node()"/>
				...
				<td class="w100px"><xsl:value-of select="document($doc)/translation/texts/item[@name='unidad_basica']/node()"/></td>
				<td class="w50px"><xsl:value-of select="document($doc)/translation/texts/item[@name='Cant_']/node()"/></td>
				<xsl:if test="/Mantenimiento/LICITACION/MOSTRAR_PRECIO_OBJETIVO">
					<td class="w100px"><xsl:copy-of select="document($doc)/translation/texts/item[@name='precio_objetivo_sIVA_2line']/node()"/></td>
					<td class="w50px">
						<!-- Permitimos ordenacion -->
				...
				<xsl:choose>
				<xsl:when test="/Mantenimiento/LICITACION/IDPAIS != '55'">
					<td class="w50px"><xsl:value-of select="document($doc)/translation/texts/item[@name='tipo_iva']/node()"/></td>
				</xsl:when>
				<xsl:otherwise>
					<td class="w1px">&nbsp;</td>
				</xsl:otherwise>
				...
				<td class="w120px">&nbsp;</td><!-- Botones Guardar / + / borrar -->
...
<div id="lProductos" style="display:none;">
	<xsl:choose>
	<xsl:when test="/Mantenimiento/LICITACION/PRODUCTOSLICITACION/PRODUCTO and /Mantenimiento/LICITACION/PERMITIR_EDICION">
		...
		<thead>
			<tr>
				...
				<td>&nbsp;</td><!-- columna anyadir campos avanzados + iconos campos avanzados informados -->
				<td class="w300px"><xsl:value-of select="document($doc)/translation/texts/item[@name='descripcion']/node()"/></td>
				<td class="w100px"><xsl:value-of select="document($doc)/translation/texts/item[@name='unidad_basica']/node()"/></td>
				<td class="w100px"><xsl:value-of select="document($doc)/translation/texts/item[@name='cantidad']/node()"/></td>
				<td>&nbsp;</td><!-- Celda para el boton para guardar los datos de la oferta fila a fila -->
	...
	<xsl:when test="/Mantenimiento/LICITACION/PRODUCTOSLICITACION/PRODUCTO">
		<form name="PublicarCompras" id="PublicarCompras" method="post">
			...
			<xsl:when test="/Mantenimiento/LICITACION/AUTOR or /Mantenimiento/LICITACION/MULTICENTRO">
				<span class="labelLeft marginLeft200"><xsl:value-of select="document($doc)/translation/texts/item[@name='centro']/node()"/>:&nbsp;</span>
				<xsl:call-template name="desplegable">
			...
			<table class="tableCenter w1200px" id="lProductos_CENTRO" cellspacing="6px" cellpadding="6px">
			<thead class="cabecalho_tabela">
				<tr>
					<th class="w1px">&nbsp;</th>
			...
			<!-- Aqui dentro se informara el cuerpo de la tabla via javascript -->
			<tbody class="corpo_tabela"></tbody>
		...
		<tr id="TblRefProductosCant"><td colspan="4">
			<form id="frmRefProductosCant" name="frmRefProductosCant" method="post">
			<table class="infoTable divLeft incidencias" cellspacing="6px" cellpadding="6px">
				<tr>
					<td class="w200px">&nbsp;</td>
					<td colspan="3" class="datosRight fuentePeq"><xsl:value-of select="document($doc)/translation/texts/item[@name='leyenda_nuevo_poducto_X_ref_licitaciones']/node()"/>:
				...
				<tr>
					<td>&nbsp;</td>
					<td class="labelRight w200px grisMed"><xsl:value-of select="document($doc)/translation/texts/item[@name='referencias']/node()"/>:</td>
					<td class="w300px datosLeft">
						<textarea name="LIC_LISTA_REFPRODUCTO_CANT" id="LIC_LISTA_REFPRODUCTO_CANT" class="w300px" rows="15"/>
					</td>
					<td>
						<a id="EnviarProductosCantPorRef" class="btnDestacado" href="javascript:ActualizarCantidadesEnLicitacionAgregada();">
							<xsl:value-of select="document($doc)/translation/texts/item[@name='enviar_referencia']/node()"/>
						</a>
						<div id="idEstadoEnvio" style="display:none;"/>
					</td>
