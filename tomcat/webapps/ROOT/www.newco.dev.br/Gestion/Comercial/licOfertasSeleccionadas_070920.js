...
//	Ultima revision ET 4set20 10:50 licOfertasSeleccionadas_040920.js
			...
			if ((arrProveedores[i].IDProvLic==IDProveedorLic)&&(arrProveedores[i].IDCentro==IDCentro))
			{
				...
				for (var j=0;(j<arrProveedores[i].Productos.length);++j)
				{
					...
					if (cant%udesLote!=0)
					{
						...
						mensaje+=strAvisoCambioUnidades.replace('[[REFPRODUCTO]]',arrProveedores[i].Productos[j].RefCliente).replace('[[PRODUCTO]]',arrProveedores[i].Productos[j].Nombre).replace('[[UNIDADES_OLD]]',arrProveedores[i].Productos[j].Cantidad).replace('[[UNIDADESPORLOTE]]',arrProveedores[i].Productos[j].UdsXLote).replace('[[UNIDADES_NEW]]',nuevaCant)+'\n\n';
					}
				}
...
function LanzarTodosPedidos(Actual, NumErrores)
{
	...
	jQuery.ajax({
		...
		type:	"GET",
		data:	"LIC_ID="+IDLicitacion+"&IDPROVEEDORLIC="+arrProveedores[Actual].IDProvLic+"&IDCENTRO="+arrProveedores[Actual].IDCentro+"&_="+d.getTime(),
		contentType: "application/xhtml+xml",
		beforeSend: function(){
		},
		error: function(objeto, quepaso, otroobj){
