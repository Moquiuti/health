...
<!--
	...
	Ultima revision: ET 21abr23 11:05 Licitaciones2022_150722.js
-->
...
<head>
	...
	<script type="text/javascript" src="http://www.newco.dev.br/Gestion/Comercial/Licitaciones2022_150722.js"></script>
...
<form name="Buscador" method="post" action="Licitaciones2022.xsql">
	...
	<tr>
		...
		<xsl:choose>
		<xsl:when test="/Licitaciones/LICITACIONES/INCLUIR_AUTOR and not (/Licitaciones/LICITACIONES/MVM)">
			<xsl:choose>
		...
		<th class="w200px textLeft">
			...
			<input type="text" name="FTEXTO" class="campopesquisa w200px" >
...
<thead class="cabecalho_tabela">
	<tr>
        ...
        <xsl:if test="/Licitaciones/LICITACIONES/INCLUIR_CODIGO_SOLICITUD">
        	<th class="w40px textLeft"><a href="javascript:Orden('CODSOLICITUD');"><xsl:value-of select="document($doc)/translation/texts/item[@name='cod_solicitud']/node()"/></a></th>
        </xsl:if>
		<th class="w40px textLeft">
...
<!--        <xsl:if test="/Licitaciones/LICITACIONES/MVM or /Licitaciones/LICITACIONES/MVMB">
		...
		<xsl:if test="/Licitaciones/LICITACIONES/MULTICENTROS">
			<th class="w50px textLeft"><a href="javascript:Orden('NUMCEN');"><xsl:value-of select="document($doc)/translation/texts/item[@name='num_centros']/node()"/></a></th>
        </xsl:if>
		<th class="w80px textCenter"><a href="javascript:Orden('NUMPROD');" title="{document($doc)/translation/texts/item[@name='Productos_expli']/node()}"><xsl:value-of select="document($doc)/translation/texts/item[@name='num_productos']/node()"/></a></th>
		<xsl:if test="not(/Licitaciones/LICITACIONES/REDUCIR_NUM_COLUMNAS)">
		...
		<th class="w60px textCenter"><a href="javascript:Orden('PENDPUBLICAR');" title="{document($doc)/translation/texts/item[@name='prov_pend_pub_expli']/node()}"><xsl:value-of select="document($doc)/translation/texts/item[@name='prov_pend_pub']/node()"/></a></th>
		<xsl:if test="/Licitaciones/LICITACIONES/HAY_LICITACIONES_SPOT">
			<th class="w60px textCenter"><a href="javascript:Orden('PEDPENDIENTES');" title="{document($doc)/translation/texts/item[@name='Ped_pend_expli']/node()}"><xsl:value-of select="document($doc)/translation/texts/item[@name='Ped_pend']/node()"/></a></th>
			<th class="w60px textCenter"><a href="javascript:Orden('PEDENVIADOS');" title="{document($doc)/translation/texts/item[@name='Ped_env_expli']/node()}"><xsl:value-of select="document($doc)/translation/texts/item[@name='Ped_env']/node()"/></a></th>
        </xsl:if>
		<th class="w60px textCenter"><a href="javascript:Orden('FECHALIC');"><xsl:value-of select="document($doc)/translation/texts/item[@name='fecha_alta_abbr']/node()"/></a></th>
		<xsl:if test="/Licitaciones/LICITACIONES/MOSTRAR_FECHA_MODIFICACION">
			<th class="w60px textCenter"><a href="javascript:Orden('FECHAMOD');"><xsl:value-of select="document($doc)/translation/texts/item[@name='fecha_modificacion']/node()"/></a></th>
        </xsl:if>
		<th class="w60px textCenter"><a href="javascript:Orden('FECHADEC');"><xsl:value-of select="document($doc)/translation/texts/item[@name='fecha_decision_abbr']/node()"/></a></th>
		...
		<xsl:if test="not(/Licitaciones/LICITACIONES/REDUCIR_NUM_COLUMNAS)">
			<th class="w60px textLeft"><a href="javascript:Orden('PORCNEG');"><xsl:value-of select="document($doc)/translation/texts/item[@name='porcentage_negociado']/node()"/></a></th>
		</xsl:if>
		<th class="w60px textLeft"><a href="javascript:Orden('PORCAHO');"><xsl:value-of select="document($doc)/translation/texts/item[@name='porcentage_ahorro']/node()"/></a></th>
		<th class="w1px">&nbsp;</th>
		<th class="w1px">&nbsp;</th>
	</tr>
</thead>
...
<xsl:when test="/Licitaciones/LICITACIONES/LICITACION">
	...
	<xsl:for-each select="/Licitaciones/LICITACIONES/LICITACION">
		...
		<tr>
			<!--9jun22 el hover queda mal en regsitros de 2 lineas
			15jul22 Solo destacamos la celda del numero de licitacion
			<xsl:choose>
			...
			<td>
				<!--15jul22 Solo destacamos la celda de linea de licitacion-->
				<xsl:choose>
				<xsl:when test="IDESTADO='CONT' and LIC_PEDIDOSPENDIENTES>0">
					<xsl:attribute name="class">textLeft fondoRojo</xsl:attribute>
				</xsl:when>
				<xsl:when test="MODIFICADO_1HORA">
					<xsl:attribute name="class">textLeft fondoNaranja</xsl:attribute>
				</xsl:when>
				<xsl:when test="MODIFICADO_24HORAS">
					<xsl:attribute name="class">textLeft fondoAmarillo</xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
					<xsl:attribute name="class">textLeft color_status</xsl:attribute>
				</xsl:otherwise>
				</xsl:choose>					
			...
			<xsl:if test="/Licitaciones/LICITACIONES/INCLUIR_CENTRO_PEDIDO">
				<td colspan="5" class="textLeft">
					<xsl:if test="/Licitaciones/LICITACIONES/OCULTAR_TITULO">
						<img src="http://www.newco.dev.br/images/2017/info.png" class="valignM"><xsl:attribute name="title"><xsl:value-of select="LIC_TITULO"/></xsl:attribute></img>
        			</xsl:if>
					<strong><a href="javascript:FichaCentro({CENTROPEDIDO/ID});" class="noDecor"><xsl:value-of select="CENTROPEDIDO/NOMBRE"/></a></strong>
				</td>
				<xsl:if test="/Licitaciones/LICITACIONES/FIDAREAGEOGRAFICA/field">
           			<td><span class="fuentePeq"><xsl:value-of select="CENTROPEDIDO/AREAGEOGRAFICA"/></span></td>
				</xsl:if>
        	</xsl:if>
        	<xsl:choose>
			<xsl:when test="/Licitaciones/LICITACIONES/INCLUIR_AUTOR or /Licitaciones/LICITACIONES/MOSTRAR_USUARIO_GESTOR">
				<xsl:choose>
				<xsl:when test="/Licitaciones/LICITACIONES/MOSTRAR_USUARIO_GESTOR">
					<td class="textLeft" colspan="7">
    					<xsl:choose>
						<xsl:when test="USUARIOCREADOR">
							<span class="fuentePeqOsc"><xsl:value-of select="document($doc)/translation/texts/item[@name='creador']/node()"/>:&nbsp;<xsl:value-of select="USUARIOCREADOR"/></span>
						</xsl:when>
						<xsl:otherwise>
							<xsl:if test="RESPONSABLE!=''">
								<span class="fuentePeqOsc"><xsl:value-of select="document($doc)/translation/texts/item[@name='autor']/node()"/>:&nbsp;<xsl:value-of select="RESPONSABLE"/></span>
							</xsl:if>
						</xsl:otherwise>
						</xsl:choose>
						<xsl:if test="USUARIOGESTOR!=''">
							<br/><span class="fuentePeqOsc"><xsl:value-of select="document($doc)/translation/texts/item[@name='gestor']/node()"/>:&nbsp;<xsl:value-of select="USUARIOGESTOR"/></span>
						</xsl:if>
					</td>
					<xsl:if test="not(/Licitaciones/LICITACIONES/REDUCIR_NUM_COLUMNAS)">
						<td colspan="2">&nbsp;</td>
				...
				<xsl:otherwise>
					<td class="textLeft" colspan="3"><span class="fuentePeq"><xsl:value-of select="document($doc)/translation/texts/item[@name='autor']/node()"/>:&nbsp;<xsl:value-of select="RESPONSABLE"/></span></td>
					<td>
						<xsl:attribute name="colspan">
							<xsl:choose>
							<xsl:when test="not(/Licitaciones/LICITACIONES/REDUCIR_NUM_COLUMNAS)">8</xsl:when>
			...
			<xsl:otherwise>
					<td>
						<xsl:attribute name="colspan">
							<xsl:choose>
							<xsl:when test="not(/Licitaciones/LICITACIONES/REDUCIR_NUM_COLUMNAS)">9</xsl:when>
		...
		<tr>
        	...
        	<xsl:if test="/Licitaciones/LICITACIONES/INCLUIR_CODIGO_SOLICITUD">
				<td class="textLeft">	
					<strong>
						<!-- nueva version de la licitacion	-->
						<a href="javascript:chLicV2({$LicID});"><xsl:value-of select="LIC_CODIGOSOLICITUD"/></a>
					</strong>
				</td>
        	</xsl:if>
			<td class="textLeft">
				<strong><a href="javascript:chLicV2({$LicID});" title="{LIC_TITULO}"><xsl:value-of select="LIC_CODIGO"/></a></strong>
			</td>
        	<xsl:if test="not (/Licitaciones/LICITACIONES/OCULTAR_TITULO)">
				<td class="textLeft">
					...
					<a title="{LIC_TITULO}">
					<xsl:attribute name="href">javascript:chLicV2(<xsl:value-of select="ID"/>);</xsl:attribute>
    				<xsl:choose>
			...
			<td class="textLeft">
        		...
        		<xsl:if test="(IDESTADO='SUS' or IDESTADO='B') and MOTIVOSUSPENDER!=''">
					<img src="http://www.newco.dev.br/images/2017/info.png" class="valignM"><xsl:attribute name="title"><xsl:value-of select="MOTIVOSUSPENDER"/></xsl:attribute></img>
        		</xsl:if>
				&nbsp;
					<!-- nueva version de la licitacion	-->
					<a href="javascript:chLicV2({$LicID});"><xsl:value-of select="ESTADO"/></a>
			</td>
			<xsl:if test="/Licitaciones/LICITACIONES/MULTICENTROS">
				<td class="textLeft">
    				<xsl:choose>
			...
			<td class="textCenter">
    			<xsl:choose><xsl:when test="PRODUCTOSADJUDICADOS>0"><a href="javascript:chVencedores({$LicID});"><xsl:value-of select="PRODUCTOSADJUDICADOS"/></a></xsl:when><xsl:otherwise>0</xsl:otherwise></xsl:choose>/<a href="javascript:chLicV2({$LicID});"><xsl:value-of select="PRODUCTOSCONOFERTA"/></a>/<xsl:choose><xsl:when test="LIC_PROD_ID!=''"><a href="javascript:chFichaProductoLicitacion({$LicID},{LIC_PROD_ID},'LICITACIONES');"><xsl:value-of select="LIC_NUMEROLINEAS"/></a></xsl:when><xsl:otherwise>0</xsl:otherwise></xsl:choose>
			</td>
			<xsl:if test="not(/Licitaciones/LICITACIONES/REDUCIR_NUM_COLUMNAS)">
				<td class="textCenter">
					<a href="javascript:chVencedores({$LicID});"><xsl:value-of select="LIC_PROVEEDORESADJUDICADOS"/></a>/<xsl:value-of select="LIC_NUMEROPROVEEDORESINF"/>/<xsl:value-of select="LIC_NUMEROPROVEEDORES"/>
				</td>
        	</xsl:if>
			<td class="textCenter"><xsl:value-of select="LIC_PROVEEDORESPENDPUBLICAR"/></td>
			<xsl:if test="/Licitaciones/LICITACIONES/HAY_LICITACIONES_SPOT">
				<td class="textCenter"><xsl:if test="LIC_PEDIDOSPENDIENTES>0"><xsl:attribute name="style">font-weight:bold;</xsl:attribute></xsl:if><xsl:value-of select="LIC_PEDIDOSPENDIENTES"/></td>
				<td class="textCenter">
					<xsl:choose>
			...
			<td class="textCenter"><span class="fuentePeqOsc"><xsl:value-of select="LIC_FECHAALTA"/></span></td>
			<xsl:if test="/Licitaciones/LICITACIONES/MOSTRAR_FECHA_MODIFICACION"><td class="textCenter"><span class="fuentePeq"><xsl:value-of select="LIC_FECHAMODIFICACION"/></span></td></xsl:if>
			<td class="textCenter"><span class="fuentePeqOsc"><xsl:value-of select="LIC_FECHADECISIONPREVISTA"/></span></td>
			<xsl:if test="/Licitaciones/LICITACIONES/IDPAIS = 34"><!--	para Brasil no mostramos la columna de consumo 	-->
			<td>
				<xsl:choose>
				<xsl:when test="not(/Licitaciones/LICITACIONES/MOSTRAR_PRECIO_IVA)">
...
<thead class="cabecalho_tabela">
	<tr>
		...
		<th class="w150px textLeft"><a href="javascript:Orden('ESTADO');"><xsl:value-of select="document($doc)/translation/texts/item[@name='estado']/node()"/></a></th>
		<th class="textLeft">
			<xsl:choose>
	...
	<xsl:when test="/Licitaciones/LICITACIONES/LICITACION">
		<xsl:for-each select="/Licitaciones/LICITACIONES/LICITACION">
			...
			<tr class="con_hover">
				...
				<td class="textLeft">&nbsp;<strong><a href="javascript:chLicitacionProveedor({$LicID});"><xsl:value-of select="LIC_CODIGO"/></a></strong></td>
				<td class="textLeft">
        		...
        		<xsl:if test="not (/Licitaciones/LICITACIONES/OCULTAR_TITULO)">
					<td class="textLeft">&nbsp;<strong><a href="javascript:chLicitacionProveedor({$LicID});"><xsl:value-of select="substring(LIC_TITULO,1,50)"/></a></strong></td>
				</xsl:if>
				<td class="textLeft">&nbsp;<strong><a href="javascript:chLicitacionProveedor({$LicID});"><xsl:value-of select="ESTADO"/></a></strong>
				...
				<td class="textRight"><xsl:value-of select="LINEASOFERTADAS"/>&nbsp;/&nbsp;<xsl:value-of select="LIC_NUMEROLINEAS"/></td>
            	...
            	<td class="textLeft fuentePeq"><xsl:value-of select="VENDEDOR"/></td>
				<td>&nbsp;</td>
			</tr>
		</xsl:for-each>
...
<br/>
<br/>
<br/><!--18nov22 incluimos algunos saltos de linea mas para facilitar el scroll	-->
<br/>
<br/>
<br/>
<br/>
