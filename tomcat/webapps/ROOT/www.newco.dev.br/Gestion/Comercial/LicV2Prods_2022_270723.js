...
//	Ultima revisión ET 23mar23 12:15 LicV2Prods_2022_270723.js
...
function dibujaTablaProductosOFE()
{
	...
	if(totalProductos > 0){
		...
		for(var i=firstProduct; i<lastProduct; i++){
			...
			thisCell += '&nbsp;' + macroEnlaceRef.replace('#LIC_ID#',IDLicitacion).replace('#LIC_PROD_ID#', arrProductos[ColumnaOrdenadaProds[i]].IDProdLic);
	...
	*/
							...
							// Precio con IVA
							thisCell += macroEnlaceRef2.replace('#LIC_ID#',IDLicitacion).replace('#LIC_PROD_ID#', arrProductos[ColumnaOrdenadaProds[i]].IDProdLic);
...
function dibujaTablaProductosEST(){
	...
	if(totalProductos > 0){
		...
		for(var i=firstProduct; i<lastProduct; i++){
			...
			// Celda/Columna Ref.Cliente
			thisCell = cellStart + macroEnlaceRef.replace('#LIC_ID#',IDLicitacion).replace('#LIC_PROD_ID#', arrProductos[ColumnaOrdenadaProds[i]].IDProdLic);
			if(arrProductos[ColumnaOrdenadaProds[i]].RefCentro != ''){
				thisCell += arrProductos[ColumnaOrdenadaProds[i]].RefCentro;	// Mostramos Ref.Centro si existe
			}
			else if(arrProductos[ColumnaOrdenadaProds[i]].RefCliente != '')
...
//	23mar23 Adjudica producto a producto, múltiples ofertas seleccionadas (recuperado desde lic2022_260122.js)
function AdjudicarOfertasMultiples()
{
	var errores='',resAdjudicacion='',prodAdju=0,prodOpcion1=0,cadAdjudicados;
	...
	else
	{
		...
		//	debug('AdjudicarOfertasMultiples. cadAdjudicados:'+cadAdjudicados);
...
//	Enviamos los productos uno a uno
var errorAdjud;
function AdjudicarOfertasMultiplesAjax(fila)
{
	...
	else if ((fila==arrProductos.length)||(errorAdjud=='S'))
	{
	...
	jQuery.ajax({
		...
		success: function(objeto){
			...
			if(data.Resultado.Estado == 'OK'){
				jQuery('#spCargaProductos').html((fila+1)+'/'+arrProductos.length);
				++fila;
...
function DescartarOferta(PosProd, IDOfertaLic)
{
	...
	{
		jQuery('#lMotivoBor_'+IDOfertaLic).show();
		jQuery('#btnDescartarBor_'+IDOfertaLic).hide();
	}
}

function CancDescartarOferta(IDOfertaLic)
{
	jQuery('#lMotivoBor_'+IDOfertaLic).hide();
	jQuery('#btnDescartarBor_'+IDOfertaLic).show();
}
...
function EjecDescartarOferta(PosProd, IDOfertaLic)
{
	...
	var IDMotivo=jQuery("#IDMOTIVOSELECCIONBOR_"+IDOfertaLic).val();
	var Motivo=jQuery("#MOTIVOSELECCIONBOR_"+IDOfertaLic).val();
	var PosOferta=BuscaEnArray(arrProductos[PosProd].Ofertas, "ID",IDOfertaLic);
