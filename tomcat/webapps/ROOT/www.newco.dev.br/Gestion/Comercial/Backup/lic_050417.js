...
function cargaDatosOfertasPorProveedores()
{
	...
	for(var j=0; j<arrProductos.length; j++){
		...
		for(var i=0; i < arrProveedores.length; i++){
			...
			//solodebug	if ((j==2)&&(i==2)) alert('Oferta:'+oferta['IDProvLic']);
			
			Ofertas.push(oferta);

			//solodebug	if ((j==2)&&(i==2)) alert('Array ofertas:'+Ofertas[0].IDProvLic+','+Ofertas[1].IDProvLic+','+Ofertas[2].IDProvLic);
		}
		
		arrProductos[j].Ofertas=Ofertas;		//	IMPORTANTE: el push da un error
		
		//solodebug	if (j==2) alert('Array productos:'+arrProductos[j].Ofertas[0].IDProvLic+','+Ofertas[1].IDProvLic+','+Ofertas[2].IDProvLic);
	}	
...
function cargaDatosOfertasProveedoresAjax(PosProvActual)
{
	...
	{	
		...
		if (arrProveedores[PosProvActual].TieneOfertas!='S')
		{
			++PosProvActual;
			//solodebug 
			console.log('cargaDatosOfertasProveedoresAjax: PosProvActual:'+PosProvActual+ ' total:'+arrProveedores.length+ ' => llamada recursiva SIN carga de datos');
		...
		console.log('cargaDatosOfertasProveedoresAjax: TIENE OFERTA. Proveedor('+PosProvActual+ '): ['+arrProveedores[PosProvActual].IDProvLic+'?'+LicProvID+'] '+arrProveedores[PosProvActual].NombreCorto);
					...
					// Si el proveedor ha informado de algunas ofertas, pero esta esta vacia
						...
						if((estadoLicitacion=='CURS' || estadoLicitacion=='INF') && isLicPorProducto=='S' && isAutor=='S'){
							...
							if(arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].OfertaAdjud == 'S'){
								items['checked']	= true;
								items['numOferta']	= (arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].columna - 1);
							...
							//solodebug	alert('Prov:' + arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].ID+' columna:'+(arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].columna - 1));
							...
							thisMacro = thisMacro.replace('#VALUE#', (arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].columna - 1));
		...
		if(jQuery("tr#posArr_" + key).length){
			...
			jQuery(".RADIO_" + key).each(function(key2, thisRadio){
				if(jQuery(thisRadio).is(':checked')){ // Si esta checked, entonces sumamos el consumo en la posicion del array que toca
					numOfertas++;
					var thisPos = jQuery("#Prov_" + thisRadio.value).val();
					
					//solodebug	alert('thisPos:'+thisPos);
