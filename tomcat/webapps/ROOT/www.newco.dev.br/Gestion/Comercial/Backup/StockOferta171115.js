/*
	StockOferta171115.js
	Librer韆 de funciones para el mantenimiento de anuncios de Stock
	David/Mirta/ET
	Ultima modificacion: 18/4/16 09:04
*/
...
function ValidarFormulario(oForm, IDEstado, proyecto){
    //alert('proy '+proyecto);
        var regex_numero	= new RegExp("^[0-9]+$","g"); // Expresion regular para controlar el campo ud lote que solo puede incluir n鷐eros, puntos o comas
	...
	if(IDEstado == 'OFERTA' &&  proyecto === 'MVM'){
        ...
        if(oForm.elements.STOCK_PRECIOPUBLICOLOTE && oForm.elements.STOCK_PRECIOPUBLICOLOTE.value === ''){
			...
			if(!esNulo(precio) && isNaN(precio)){
				...
				alert("Precio retail:"+oForm.elements.STOCK_PRECIOPUBLICOLOTE.value+" Coste env韔:"+precio);
...
//Ense馻r el input file para subir documentos
		...
		if(resp.documentos && resp.documentos.length > 0){
			for(var i=0; i<resp.documentos.length; i++){
				if(resp.documentos[i].error && resp.documentos[i].error != ''){
					...
					/*en lugar del elemento nombre del form cojo el nombre del fichero diretamente, si hay espacios el sistema pone ghion bajo, entonces cuento cuantos ghiones son, divido al penultimo y a锟絘do la ultima palabra, si la ultima palabra esta vac锟絘 implica que hay 2 ghiones bajos, entonces divido a los 2 ghiones bajos y cojo la primera parte.*/
...
function periodicUpdate(){
	...
	periodicTimer++;
        //a馻dido un alert pk si no no coje el texto del fichero subido, imagen, solo para mvm.
        /*if (document.forms['frmStockOfertaID']){
