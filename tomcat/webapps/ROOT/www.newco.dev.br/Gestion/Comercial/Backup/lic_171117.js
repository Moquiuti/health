			...
			success: function(objeto){
				...
				if(data.ProductosLicitacion.ListaProductosOfertas.length > 0){
					jQuery.each(data.ProductosLicitacion.ListaProductosOfertas, function(key, producto){
						...
						//solodebug		
						/*if (fila<3)
							console.log ('Proveedor (LicProvID:'+LicProvID+') '+arrProveedores[PosProvActual].NombreCorto+' en PosProvActual '
										...
										+' num.ofertas:'+arrProductos[fila].NumOfertas);*/
						//solodebug				+ 'adjudicada:'+producto.Adjudicada
						//solodebug				+','+arrProductos[fila].Ofertas[PosProvActual/*columna*/].OfertaAdjud);
...
function ValidarFormulario(oForm, nombre, flag){
	...
	//console.log(comparaFechas(oForm.elements['LIC_FECHADECISION'].value, oForm.elements['LIC_FECHAADJUDICACION'].value));
		...
		if ((jQuery('#CHK_LIC_CONTINUA').length) && (oForm.elements['CHK_LIC_CONTINUA'].checked)){	//14nov17	Primero comprobamos que existe el checkbox
			...
			if (jQuery('#CHK_LIC_CONTINUA').length){			//	Si existe el checkbox, asignamos su valor, sino mantenemos el que hubiera
		...
		if((!errores) && (esNulo(listRefs))){
			...
			for (i=0;i<arrLista.length;++i)
			{
				if (arrLista[i]!='')
				{
					...
					//var PrecioRef=Piece(arrLista[i],':',2);
					var Restocadena=Piece(arrLista[i],':',2);
...
function AnadirProductos(oForm){
	...
	//solodebug	console.log('Referencias:'+Referencias);		
	...
	//solodebug console.log('AnadirProductos:'+Referencias+' sendProductos:'+sendProductos+' loops:'+loops);
...
function AnadirProductosAjax(oForm, Referencias, loop, loops, error)
{
	...
	jQuery.ajax({
		...
		success: function(objeto){
			...
			//solodebug	6oct17	console.log('AnadirProductosAjax: '+loop+'/'+loops+':'+cadEnvio);
					...
					// Si el proveedor ha informado de algunas ofertas, pero esta esta vacia
						...
						if((estadoLicitacion=='CURS' || estadoLicitacion=='INF') && isLicPorProducto=='S' && isAutor=='S'){
							...
							if(arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].OfertaAdjud == 'S'){
								...
								//solodebug 8may17	console.log('dibujaTablaProductosOFE. ID:'+items['OfertaID']+' fila:'+ColumnaOrdenadaProds[i]+'col:'+j+' ->OfertaAdjud');
...
function ordenamientoBurbuja(col, tipo){
	...
	//solodebug	console.log('ordenamientoBurbuja. col:'+col+' tipo:'+tipo);
	...
	//solodebug	console.log('ordenamientoBurbuja. column:'+column+' posArr:'+posArr);
...
function dibujaTablaProveedoresEST(){
	...
	if(totalProvs > 0){
		for(var i=0; i<totalProvs; i++){
			...
			//solodebug	27set17	console.log(arrProveedores[ColumnaOrdenadaProvs[i]].IDProveedor+':'+arrProveedores[ColumnaOrdenadaProvs[i]].NombreCorto+':'+arrProveedores[ColumnaOrdenadaProvs[i]].NivelDocumentacion);
...
function compruebaUdsXLote(){
	...
	jQuery.each(arrProductos, function(key, producto){
		...
		//	solodebug	if(UdsXLoteFormat) console.log('compruebaUdsXLote.UdsXLoteFormat:'+UdsXLoteFormat+' CantidadFormat:'+CantidadFormat+' esEntero:'+esEntero(Math.round((CantidadFormat/UdsXLoteFormat)*1000)/1000));
		...
		if(UdsXLoteFormat && !esEntero(Math.round((CantidadFormat/UdsXLoteFormat)*1000)/1000)){
			...
			//	solodebug	console.log('compruebaUdsXLote.UdsXLoteFormat:'+UdsXLoteFormat+' CantidadFormat:'+CantidadFormat+' esEntero:'+esEntero(Math.round((CantidadFormat/UdsXLoteFormat)*1000)/1000)+ ' return FALSE');

			return false;
		}
	});

	//	solodebug	console.log('compruebaUdsXLote.UdsXLoteFormat:'+UdsXLoteFormat+' CantidadFormat:'+CantidadFormat+' esEntero:'+esEntero(Math.round((CantidadFormat/UdsXLoteFormat)*1000)/1000)+ ' return '+toReturn);
