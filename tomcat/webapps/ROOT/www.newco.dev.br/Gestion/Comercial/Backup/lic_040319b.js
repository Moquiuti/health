...
//	Ultima revision: ET 05mar19 08:10
...
function cargaDatosOfertasProveedoresAjax(PosProvActual)
{
	...
	{	
		...
		//solodebug 13abr17	
		console.log('cargaDatosOfertasProveedoresAjax: TIENE OFERTA. Proveedor('+PosProvActual+ '): ['+arrProveedores[PosProvActual].IDProvLic+'/'+arrProveedores[PosProvActual].IDProveedor+'] '+arrProveedores[PosProvActual].NombreCorto);
...
function MuestraMatrizOfertas(){
	...
	else
	{
		for (j=0; j<arrProductos.length; ++j)
		{
			...
			texto+=	'  |  '+j+':'+jQuery("#IDPRODLIC_"+ColumnaOrdenadaProds[j]).val()+':'+jQuery("#RefProv_"+ColumnaOrdenadaProds[j]).val()+':'+jQuery("#Marca_"+ColumnaOrdenadaProds[j]).val();
...
function addDocFile(uploadForm, ID){
	...
	console.log('addDocFile. form:'+uploadForm.name+' ID:'+ID);
	...
	if(uploadElem.value != ''){
		...
		console.log('addDocFile. Doc:'+uploadElem.value);
		
		
	}else{
		uploadFilesDoc.splice(ID, 1);
		
		console.log('addDocFile. Doc:'+uploadFilesDoc.value);
...
function cargaDoc(uploadForm, tipo, valueID){
	...
	console.log('cargaDoc. form:'+uploadForm.name+ ' tipo:'+tipo+ ' ID:'+valueID);

	if(uploadForm.uploadFrameDoc && uploadForm.uploadFrameDoc.document && uploadForm.uploadFrameDoc.document.getElementsByTagName("body")[0]){
		uploadForm.uploadFrameDoc.document.getElementsByTagName("body")[0].innerHTML = "";
	}
	...
	if(msg != ''){
		...
		if(hasFilesDoc(uploadForm)){
			console.log('cargaDoc. form:'+uploadForm.name+ ' tipo:'+tipo+ ' ID:'+valueID+ ' hasfiles ok!');
			
			//	6mar19 Recuperamos los valores originales del formulario
			targetOrig=uploadForm.target;
			enctypeOrig=uploadForm.enctype;
			actionOrig=uploadForm.action;
...
function periodicUpdateDoc(uploadForm, valueID, tipo){
	...
	console.log('periodicUpdateDoc. tipo:'+tipo+ ' ID:'+valueID+' uploadForm:'+uploadForm.name);
	...
	console.log('periodicUpdateDoc. tipo:'+tipo+ ' ID:'+valueID+' form:'+uploadForm.name+'::'+logFormElements(uploadForm));
	//+ ' uploadFrameDoc:'+uploadForm.uploadFrameDoc.name);
//		+uploadForm.elements[20].name+uploadForm.elements[21].name+uploadForm.elements[22].name+uploadForm.elements[23].name);
...
/*5mar19	
		...
		{
			tipoDoc = document.forms['ProductosProveedor'].elements['TIPO_DOC'].value; 
		}
...
*/
/*	
		if(uploadForm.getElementById('ProductosProveedor'))
		{
			tipoDoc = uploadForm.forms['ProductosProveedor'].elements['TIPO_DOC'].value; 
		}
		else
		*/
...
function handleFileRequestDoc(form, resp,valueID, tipo){
	...
	console.log('handleFileRequestDoc. resp:'+resp+' tipo:'+tipo+ ' ID:'+valueID);
...
// DC - 06oct14 - Se hace esta funcion dinamica para que se puedan subir fichas tecnicas y contratos en dos forms diferentes...
					...
					{
						console.log("handleFileRequestDoc. Actualizado OK. nombreDoc:"+nombreDoc);
...
function MostrarFloatingBox(){
	...
	jQuery.each(arrConsumoProvs, function(key, consumoProv){
		//solodebug console.log('MostrarFloatingBox.Control proveedores seleccionados. Prov:('+consumoProv.IDProvLic+')'+consumoProv.NombreCorto+'. ofertasAdj:'+consumoProv.OfertasAdj+'->0');	//21jul18
		...
		if(producto.TieneSeleccion == 'S'){
			jQuery.each(producto.Ofertas, function(key2, oferta){
				...
				if(oferta.OfertaAdjud == 'S'){
					...
					ConsumoPrev		+= CantidadFormato * PrecioFormato;		//21jul18
	...
	jQuery.each(arrConsumoProvs, function(key, consumoProv){
		if(parseInt(consumoProv.OfertasAdj) > 0){
			...
			console.log('MostrarFloatingBox.Control proveedores seleccionados. Prov:('+consumoProv.IDProvLic+')'+'. ofertasAdj:'+consumoProv.OfertasAdj);	//21jul18
			numProveedoresAdj++;
		}
	});
...
//	if(numOfertas == numProductos){
	...
	//solodebug	
	console.log('MostrarFloatingBox ConsumoPot:'+ConsumoPot+' ConsumoPrev:'+ConsumoPrev);
...
function recalcularFloatingBox(obj, flagPedMin, ofertaGuardada){
	...
	//solodebug	13abr17	
	console.log('RecalcularFloatingBox: firstProduct'+firstProduct+ ' oldOfertaID:'+oldOfertaID+' thisOfertaID:'+thisOfertaID+ ' oldConsumoPot:'+oldConsumoPot);
	...
	for(var i=0; i<ColumnaOrdenadaProds.length; i++){
		if(ColumnaOrdenadaProds[i] == thisPosArr){
			...
			if(wasChecked){			
				...
				//solodebug	13abr17	
				console.log('RecalcularFloatingBox: firstProduct'+firstProduct+ 'i:'+i+ ' oldOfertaID:'+oldOfertaID+' arrRadios[(i-firstProduct)].OfertaID:'+arrRadios[(i-firstProduct)].OfertaID);
	...
	for(var j=0; j<arrProductos[thisPosArr].Ofertas.length; j++)
	{
		...
		{	
			...
			//solodebug	
			console.log('RecalcularFloatingBox: ID:'+ thisOfertaID+ ' fila:'+thisPosArr+' Col:'+j+'. Estaba adjudicada:'+arrProductos[thisPosArr].Ofertas[j].OfertaAdjud+' Ahora:'+(isChecked?'S':'N'));
	...
	if(wasChecked !== true && isChecked !== false){
		if(PrecioHist != 0){
			...
			newConsumoPrev	= oldConsumoPrev + (newPrecioOferta * Cantidad);		//	21jul18
		}	
		...
		if(PrecioHist != 0){
			...
			newConsumoPot	= oldConsumoPot;
			newConsumoPrev	= oldConsumoPrev + (newPrecioOferta * Cantidad) - (oldPrecioOferta * Cantidad);		//	21jul18
		}
	...
	//solodebug	
	console.log('RecalcularFloatingBox: wasChecked:'+wasChecked+' isChecked:'+isChecked+' oldNumOfertas:'+oldNumOfertas+' newNumOfertas:'+newNumOfertas+' newConsumoPot:'+newConsumoPot+' newConsumoPrev:'+newConsumoPrev);
...
*/
		...
		//solodebug	
		console.log('RecalcularFloatingBox: Mostrar. newNumOfertas:'+newNumOfertas);
	}
	else
	{
		//solodebug	
		console.log('RecalcularFloatingBox: NO Mostrar. newNumOfertas:'+newNumOfertas);
	}
...
function actualizarRadio(IDProdLic, IDOfertaLic, recFloatingBox){
	...
	//solodebug
	var f=new Date();
	console.log(f.getHours()+":"+f.getMinutes()+":"+f.getSeconds()+ " actualizarRadio IDProdLic:"+IDProdLic+" IDOfertaLic:"+IDOfertaLic + " INICIO");
	...
	//solodebug
	var f=new Date();
	console.log(f.getHours()+":"+f.getMinutes()+":"+f.getSeconds()+ " actualizarRadio IDProdLic:"+IDProdLic+" IDOfertaLic:"+IDOfertaLic + " recalcularFloatingBox INICIO");
	...
	//solodebug
	var f=new Date();
	console.log(f.getHours()+":"+f.getMinutes()+":"+f.getSeconds()+ " actualizarRadio IDProdLic:"+IDProdLic+" IDOfertaLic:"+IDOfertaLic + " recalcularFloatingBox FINAL");
...
function borrarDoc(tipo){
	...
	console.log("borrarDoc tipo:"+tipo);
...
function AdjudicarOfertasMultiples()
{
	...
	//solodebug	console.log('AdjudicarOfertasMultiples 13jul18 10:52');
	...
	else
	{
		...
		//console.log('AdjudicarOfertasMultiples:'+debug);
	...
	//solodebug	
	console.log('AdjudicarOfertasMultiples: Saliendo. errores: ['+errores+'] resAdjudicacion: ['+resAdjudicacion+']');
...
function AdjudicarOfertasMultiplesAjax(fila)
{
	...
	//solodebug	
	console.log('AdjudicarOfertasMultiplesAjax:'+fila+ ' errorAdjud:'+errorAdjud);
	...
	//solodebug	
	console.log('AdjudicarOfertasMultiplesAjax. fila:'+fila+' IDProdLic:'+arrProductos[fila].IDProdLic);
	...
	jQuery.ajax({
		...
		success: function(objeto){
			...
			if(data.Resultado.Estado == 'OK'){
				...
				//solodebug		console.log('AdjudicarOfertasMultiplesAjax. SIGUIENTE. fila:'+fila);
