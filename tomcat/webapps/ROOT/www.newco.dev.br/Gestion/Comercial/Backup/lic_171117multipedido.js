//	Funciones JS de la licitación
//	ultima revision: 17nov17 10:15
...
function onloadEvents(){
	...
	//	14set16	Cargamos los datos de ofertas de los proveedores informados, mostrando en el status cual carga en cada momento
	...
	jQuery('table#lProductos_PROVE tbody').on('change', 'input[type=text].precio', function(){
		validarPreciosProv(this);
	});
	...
	jQuery("#pes_lInfoPedido").click(function(){
		...
		if(chk){
			...
			jQuery("a.MenuLic").css('background','#F0F0F0');
			jQuery("a.MenuLic").css('color','#555555');
			jQuery("#pes_lInfoPedido").css('background','#3b569b');
			jQuery("#pes_lInfoPedido").css('color','#D6D6D6');
	...
	jQuery("#pes_lProductos").click(function(){
		...
		jQuery("a.MenuLic").css('background','#F0F0F0');
		jQuery("a.MenuLic").css('color','#555555');
		jQuery("#pes_lProductos").css('background','#3b569b');
		jQuery("#pes_lProductos").css('color','#D6D6D6');
		...
		ColocaFloatingBox();	//5dic16
	
	});
	
	// Codigo javascript para mostrar/ocultar la tabla correspondiente cuando se clica en la pestanya 'Anyadir Productos por Catalogo'
	jQuery("#subpes_CatPrivProductos").click(function(){
		if(lang == 'spanish'){
			jQuery("#subpes_CatPrivProductos > img").attr('src',"/images/botonAnadirProdCatalogo1.gif");
			jQuery("#subpes_RefProductos > img").attr('src',"/images/botonAnadirProdRef.gif");
		}else{
			jQuery("#subpes_CatPrivProductos > img").attr('src',"/images/botonAnadirProdCatalogo1-BR.gif");
			jQuery("#subpes_RefProductos > img").attr('src',"/images/botonAnadirProdRef-BR.gif");
		}

		jQuery("#TblCatPrivProductos").show();
		jQuery("#TblRefProductos").hide();
	});

	// Codigo javascript para mostrar/ocultar la tabla correspondiente cuando se clica en la pestanya 'Anyadir Productos por Referencia'
	jQuery("#subpes_RefProductos").click(function(){
		if(lang == 'spanish'){
			jQuery("#subpes_CatPrivProductos > img").attr('src',"/images/botonAnadirProdCatalogo.gif");
			jQuery("#subpes_RefProductos > img").attr('src',"/images/botonAnadirProdRef1.gif");
		}else{
			jQuery("#subpes_CatPrivProductos > img").attr('src',"/images/botonAnadirProdCatalogo-BR.gif");
			jQuery("#subpes_RefProductos > img").attr('src',"/images/botonAnadirProdRef1-BR.gif");
		}

		jQuery("#TblCatPrivProductos").hide();
		jQuery("#TblRefProductos").show();
	});
	...
	jQuery("#pes_lProductosCompacta").click(function(){
		...
		if(chk){
			...
			jQuery("a.MenuLic").css('background','#F0F0F0');
			jQuery("a.MenuLic").css('color','#555555');
			jQuery("#pes_lProductosCompacta").css('background','#3b569b');
			jQuery("#pes_lProductosCompacta").css('color','#D6D6D6');
	...
	jQuery("#pes_lProveedores").click(function(){
		...
		if(chk){
			...
			jQuery("a.MenuLic").css('background','#F0F0F0');
			jQuery("a.MenuLic").css('color','#555555');
			jQuery("#pes_lProveedores").css('background','#3b569b');
			jQuery("#pes_lProveedores").css('color','#D6D6D6');
	...
	jQuery("#pes_lUsuarios").click(function(){
		...
		if(chk){
			...
			jQuery("a.MenuLic").css('background','#F0F0F0');
			jQuery("a.MenuLic").css('color','#555555');
			jQuery("#pes_lUsuarios").css('background','#3b569b');
			jQuery("#pes_lUsuarios").css('color','#D6D6D6');
	...
	jQuery("#pes_lCentros").click(function(){
		...
		if(chk){
			...
			jQuery("a.MenuLic").css('background','#F0F0F0');
			jQuery("a.MenuLic").css('color','#555555');
			jQuery("#pes_lCentros").css('background','#3b569b');
			jQuery("#pes_lCentros").css('color','#D6D6D6');
	...
	// Codigo javascript para mostrar/ocultar tablas cuando se clica en la pestanya 'Informe'
	jQuery("#pes_lInforme").click(function(){
		var chk = true;
		...
		if(chk){
			...
			jQuery("a.MenuLic").css('background','#F0F0F0');
			jQuery("a.MenuLic").css('color','#555555');
			jQuery("#pes_lInforme").css('background','#3b569b');
			jQuery("#pes_lInforme").css('color','#D6D6D6');
	...
	if(jQuery(".FBox").length){
		...
		//var posicion = jQuery(".FBox").offset();
		//var margenSuperior = 10, margenDerecho = 10;
		...
		jQuery(window).scroll(function(){
			...
			jQuery(".FBox").offset({ top: jQuery(window).scrollTop()+80, left: jQuery(window).width()-500 });

		/*	Este bloque de código para mover el FB de forma dinámica da muchos problemas, lo eliminaremos
		
			if(jQuery(window).scrollTop() > posicion.top && jQuery(window).scrollLeft() > posicion.left){
				jQuery(".FBox").stop().animate({
					marginTop: margenSuperior + jQuery(window).scrollTop() - posicion.top,
                    marginRight: margenDerecho - jQuery(window).scrollLeft() + posicion.left
				});
            }else if(jQuery(window).scrollTop() > posicion.top){
				jQuery(".FBox").stop().animate({
					marginTop: margenSuperior + jQuery(window).scrollTop() - posicion.top,
                    marginRight: margenDerecho
				});
            }else if(jQuery(window).scrollLeft() > posicion.left){
				jQuery(".FBox").stop().animate({
	...
	jQuery('select#LIC_MESES').change(function(){
		...
		if((this.value == '0')&&(isLicAgregada=='N')){ // Pedido puntual, y no es licitación agregada		
...
function ColocaFloatingBox()
{
	...
	//solodebug	console.log('ColocaFloatingBox.top: '+(jQuery(window).scrollTop()+80)+' left:'+(jQuery(window).width()-500));
}




//	14set16	Carga los datos de las ofertas de los proveedores informados, vía ajax
//	FALTA ADAPTAR ESTA FUNCION PARA PODER ACTUALIZAR ONLINE TODA LA LICITACION
//var PosProvActual=0;
function cargaDatosOfertasPorProveedores()
{
	//var	lProveedoresConOfertas='';
	//var	lProveedoresSinOfertas='';
	var texto='';
	...
	for(var j=0; j<arrProductos.length; j++){
		...
		for(var i=0; i < arrProveedores.length; i++){
			...
			//solodebug	13abr17	if (j==0) console.log('cargaDatosOfertasPorProveedores. Inicializar ofertas. columna:'+i+' IDProvLic:'+oferta['IDProvLic']+' IDProv:'+oferta['IDProv']);
...
function cargaDatosOfertasProveedoresAjax(PosProvActual)
{
	...
	//solodebug 		console.log('cargaDatosOfertasProveedoresAjax: Proveedor('+k+ '): ['+arrProveedores[k].IDProvLic+'] '+arrProveedores[k].NombreCorto);
	//solodebug 		
	//solodebug }
	//solodebug
	...
	{
		console.log('cargaDatosOfertasProveedoresAjax: PosProvActual:'+PosProvActual+ ' total:'+arrProveedores.length+ ' => TODOS CARGADOS');
		jQuery('#idEstadoLic').html(mensajeEstadoActual);
		ctrlCargaProv=false;
		return 'OK';
	}
	...
	{
		//solodebug console.log('cargaDatosOfertasProveedoresAjax: PosProvActual:'+PosProvActual+ ' total:'+arrProveedores.length+ ' => procesar este');
	}
	...
	{	
		...
		//solodebug console.log('cargaDatosOfertasProveedoresAjax: PosProvActual:'+PosProvActual+ ' total:'+arrProveedores.length+ ' => comprobando si tiene ofertas');
		...
		{
			++PosProvActual;
			//solodebug console.log('cargaDatosOfertasProveedoresAjax: PosProvActual:'+PosProvActual+ ' total:'+arrProveedores.length+ ' => llamada recursiva SIN carga de datos');
		...
		//solodebug 13abr17	console.log('cargaDatosOfertasProveedoresAjax: TIENE OFERTA. Proveedor('+PosProvActual+ '): ['+arrProveedores[PosProvActual].IDProvLic+'/'+arrProveedores[PosProvActual].IDProveedor+'] '+arrProveedores[PosProvActual].NombreCorto);
		...
		jQuery.ajax({
			...
			success: function(objeto){
				...
				jQuery('#idEstadoLic').html(arrProveedores[PosProvActual/*columna*/].NombreCorto);
				...
				if(data.ProductosLicitacion.ListaProductosOfertas.length > 0){
					jQuery.each(data.ProductosLicitacion.ListaProductosOfertas, function(key, producto){
						...
						for (j=0; j<arrProductos.length; ++j){
							if (arrProductos[j].Ofertas[PosProvActual/*columna*/].IDProdLic==producto.LIC_PROD_ID) fila=j;
						}
						...
						// var items		= [];
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].linea = producto.Linea;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].PosProvActual/*columna*/ = PosProvActual/*columna*/+1;					//	21set16, numerando a partir de la PosProvActual/*columna*/ 1
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].IDProdLic	= producto.LIC_PROD_ID;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].IDProd		= producto.IDProdEstandar;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].RefEstandar	= producto.ProdRefEstandar;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].RefCliente	= producto.ProdRefCliente;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].Nombre		= producto.ProdNombre;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].NombreNorm	= producto.ProdNombreNorm;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].UdBasica	= producto.ProdUdBasica;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].FechaAlta	= producto.FechaAlta;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].FechaMod	= producto.FechaModificacion;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].Consumo	= producto.Consumo;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].ConsumoIVA	= producto.ConsumoIVA;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].Cantidad	= producto.Cantidad;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].PrecioHist	= producto.PrecioReferencia;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].PrecioObj	= producto.PrecioObjetivo;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].TipoIVA	= producto.TipoIVA;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].Ordenacion	= '0';
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].PrecioColor = producto.Color;				//	16ene17

						// Campos Avanzados producto
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].InfoAmpliada	= producto.InfoAmpliada;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].Documento	= [];
						if(producto.Documento.ID != ''){
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].Documento.ID		= producto.Documento.ID;
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].Documento.Nombre		= producto.Documento.Nombre;
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].Documento.Descripcion	= producto.Documento.Descripcion;
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].Documento.Url		= producto.Documento.Url;
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].Documento.Fecha		= producto.Documento.Fecha;
						}
						// FIN Campos Avanzados producto
						//arrProductos[fila].Ofertas[PosProvActual/*columna*/]	= [];
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].ID		= producto.IDOferta;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].IDProvLic	= producto.LIC_PROV_ID;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].IDProducto	= producto.IDProductoOfe;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].RefProv	= producto.RefOfe;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].Nombre	= producto.NombreOfe;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].Marca	= producto.MarcaOfe;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].FechaAlta	= producto.FechaAltaOfe;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].FechaMod	= producto.FechaModificacionOfe;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].UdsXLote	= producto.UdsXLoteOfe;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].Cantidad	= producto.CantidadOfe;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].Precio	= producto.PrecioOfe;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].TipoIVA	= producto.TipoIVAOfe;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].Consumo	= producto.ConsumoOfe;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].ConsumoIVA	= producto.ConsumoIVAOfe;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].IDEstadoEval	= producto.IDEstadoEvaluacionOfe;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].EstadoEval	= producto.EstadoEvaluacionOfe;

						// Campos Avanzados oferta
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].InfoAmpliada	= producto.InfoAmpliadaOferta;
						arrProductos[fila].Ofertas[PosProvActual/*columna*/].Documento	= [];
						if(producto.DocumentoOferta.ID != ''){
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].Documento.ID		= producto.DocumentoOferta.ID;
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].Documento.Nombre		= producto.DocumentoOferta.Nombre;
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].Documento.Descripcion	= producto.DocumentoOferta.Descripcion;
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].Documento.Url		= producto.DocumentoOferta.Url;
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].Documento.Fecha		= producto.DocumentoOferta.Fecha;
						}
						// FIN Campos Avanzados oferta

						arrProductos[fila].Ofertas[PosProvActual/*columna*/].FichaTecnica	= [];
						if(producto.FichaTecnica.ID != ''){
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].FichaTecnica.ID		= producto.FichaTecnica.ID;
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].FichaTecnica.Nombre	= producto.FichaTecnica.Nombre;
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].FichaTecnica.Descripcion	= producto.FichaTecnica.Descripcion;
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].FichaTecnica.Url		= producto.FichaTecnica.Fichero;
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].FichaTecnica.Fecha	= producto.FichaTecnica.Fecha;
                    	}

						arrProductos[fila].Ofertas[PosProvActual/*columna*/].OfertaAdjud	= producto.Adjudicada;	//	21set16 Informar oferta adjudicada

						if (arrProductos[fila].Ofertas[PosProvActual/*columna*/].OfertaAdjud=='S'){	//	21set16 Informar producto con oferta adjudicada
							arrProductos[fila].TieneSeleccion='S';
							arrProductos[fila].ColSeleccionada=PosProvActual/*columna*/;
							numProdsSeleccion++;			//	26set16

							//solodebug	alert('fila:'+fila+' PosProvActual/*columna*/:'+PosProvActual/*columna*/+arrProductos[fila].TieneSeleccion+ 'numProdsSeleccion:'+numProdsSeleccion);
						...
						if (producto.Informada=='S')
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].NoInformada	= 'N';
						else
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].NoInformada	= 'S';

						//	9oct17	desde la base de datos se devuelve '' si no está informado
						if (((arrProductos[fila].Ofertas[PosProvActual/*columna*/].UdsXLote=='0')||(arrProductos[fila].Ofertas[PosProvActual/*columna*/].UdsXLote==''))
								&&((producto.PrecioOfe=='0,0000')||(producto.PrecioOfe=='')))
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].NoOfertada='S';
						else{
							arrProductos[fila].Ofertas[PosProvActual/*columna*/].NoOfertada='N';
							arrProductos[fila].NumOfertas++;						//	30set16 Actualizamos el contador de ofertas
						}


						//solodebug		console.log ('Proveedor (LicProvID:'+LicProvID+') '+arrProveedores[PosProvActual].NombreCorto+' en PosProvActual/*columna*/ '+PosProvActual/*columna*/+' producto '+arrProductos[fila].Nombre
						//solodebug				+' oferta:'+arrProductos[fila].Ofertas[PosProvActual/*columna*/].Precio+ 'adjudicada:'+producto.Adjudicada+','+arrProductos[fila].Ofertas[PosProvActual/*columna*/].OfertaAdjud);
					...
					//solodebug console.log('cargaDatosOfertasProveedoresAjax: PosProvActual:'+PosProvActual+ ' total:'+arrProveedores.length+ ' => llamada recursiva tras carga de datos');
					++PosProvActual;
					cargaDatosOfertasProveedoresAjax(PosProvActual);
					return;	//27mar17
	...
	{
		console.log('cargaDatosOfertasProveedoresAjax: PosProvActual:'+PosProvActual+ ' total:'+arrProveedores.length+ ' => ERROR, no ha salido correctamente de la función');
	}
...
function MuestraMatrizOfertas(){
	...
	for (j=0; j<arrProductos.length; ++j){
		...
		for (i=0; i<arrProveedores.length; ++i){
			texto+=arrProductos[j].Ofertas[i].Precio+'|';
		}
		texto+='\n\r';
	}
...
function ValidarFormulario(oForm, nombre, flag){
	...
	if(nombre == 'datosGenerales'){
		if((!errores) && (esNulo(oForm.elements['LIC_TITULO'].value))){
			errores++;
			alert(val_faltaTitulo);
			oForm.elements['LIC_TITULO'].focus();
		}

		if((!errores) && (esNulo(oForm.elements['LIC_FECHADECISION'].value))){
			errores++;
		...
		if((!errores) && (esNulo(oForm.elements['LIC_DESCRIPCION'].value))){
			errores++;
			alert(val_faltaDescripcion);
			oForm.elements['LIC_DESCRIPCION'].focus();
		}

		if((!errores) && (esNulo(oForm.elements['LIC_CONDENTREGA'].value))){
			errores++;
			alert(val_faltaCondEntr);
			oForm.elements['LIC_CONDENTREGA'].focus();
		}

		if((!errores) && (esNulo(oForm.elements['LIC_CONDPAGO'].value))){
			errores++;
			alert(val_faltaCondPago);
			oForm.elements['LIC_CONDPAGO'].focus();
		}

		if((!errores) && jQuery("#LIC_FECHAADJUDICACION").length){
	...
	//console.log(comparaFechas(oForm.elements['LIC_FECHADECISION'].value, oForm.elements['LIC_FECHAADJUDICACION'].value));
					if(comparaFechas(oForm.elements['LIC_FECHADECISION'].value, oForm.elements['LIC_FECHAADJUDICACION'].value) == '>'){
						errores++;
		...
		if ((jQuery('#CHK_LIC_CONTINUA').length) && (oForm.elements['CHK_LIC_CONTINUA'].checked)){	//14nov17	Primero comprobamos que existe el checkbox
			oForm.elements['LIC_CONTINUA'].value='S';
		}
		else{
			if (jQuery('#CHK_LIC_CONTINUA').length){			//	Si existe el checkbox, asignamos su valor, sino manetnemos el que hubiera
		...
		if((!errores) && (esNulo(listRefs))){
			...
			for (i=0;i<arrLista.length;++i)
			{
				if (arrLista[i]!='')
				{
					...
					var Fecha=Piece(arrLista[i],':',2);	//	24abr17
					
					if ((Cant!='')&&(!jQuery.isNumeric(Cant)))	//	18abr17	COmprobar que la cantidad no sea null antes de chequear error
					{
						errores++;
						alert(val_malCantidad.replace("[[REF]]",Ref));
					}
					
					//	17may17	COmprobaciones para licitaciones multipedidos
					if ((isLicMultipedido=='S') && (Fecha==''))
					{
						errores++;
						alert(alrt_malFechaObligatoria.replace("[[REF]]",Ref));
					} 
					else
					{
						//	17may17	COmprobaciones para licitaciones multipedidos
						if ((isLicMultipedido=='N') && (Fecha!='') && (arrProductos.length>0))
						{
							errores++;
							alert(alrt_malFechaProhibida.replace("[[REF]]",Ref));
						}
						else
						{
							...
							if (Fecha!='')
							{
								if (CheckDate(Fecha)!='')	//	24abr17	COmprobar que la cantidad no sea null antes de chequear error
								{
									errores++;
									alert(val_malFechaPedido.replace("[[REF]]",Ref));
								}
								//	24abr17 Faltaría comprobar fecha de pedido posterior a fecha de decisión
								else
								{
									if (comparaFechas(Fecha,fechaDecisionLic)!='>')	//	24abr17	COmprobar que la cantidad no sea null antes de chequear error
									{
										errores++;
										alert(val_malFechaPedidoAntDecision.replace("[[REF]]",Ref));
									}
								}
							}
							else
	...
	if(nombre == 'provPedidoMinimo'){
		...
		if((!errores) && (esNulo(oForm.elements['LIC_PROV_PEDIDOMINIMO'].value))){
			...
			oForm.elements['LIC_PROV_PEDIDOMINIMO'].focus();
		}else if((!errores) && (IDPais==55) && (esNulo(oForm.elements['LIC_PROV_FRETE'].value))){	//solo para Brasil controlaremos el campo FRETE
			errores++;
	...
	if(nombre == 'productosProveedor'){
		...
		jQuery("#lProductos_PROVE > tbody > tr.infoProds").each(function(){
			...
			if(!errores && esNulo(precioFormat)){
				...
				if(IDPais != '55' && precioFormat > (precioObjFormat*2)){
					controlPrecio += val_elPrecioProd +" "+ precio +" "+ val_precioProd.replace("[[REF]]",RefCliente)+" "+ val_doblePrecio +" "+ precioObj +" "+ "\n";
				}
			}
			...
			if(IDPais != 55){
				...
				if(!errores && esNulo(valAux) && (precioFormat != 0 || UdsXLoteFormat != 0)){
					...
					jQuery('#RefProv_' + thisPosArr).val(str_SinOfertar.toUpperCase());
					jQuery('#Desc_' + thisPosArr).val(str_SinOfertar.toUpperCase());
					jQuery('#Marca_' + thisPosArr).val(str_SinOfertar.toUpperCase());
				}
			}
    });
...
// 18set16 En el mismo formulario, opcionalmente, permitimos incluir tambien las cantidades, el separador puede ser TAB,:,| o #
// A la base de datos se envia en formato REFCLIENTE1[#CANTIDAD1]|REFCLIENTE2[#CANTIDAD2]|... o REFMVM1[#CANTIDAD1]|REFMVM2[#CANTIDAD2]|...
//	23ene17	Enviamos los productos de 20 en 20 o 30 en 30 para mantener informado al cliente
//	18abr17 Si estamos en estado "en curso" habrá que recargar la licitación
function AnadirProductos(oForm){
	...
	//solodebug	alert('Referencias:'+Referencias);		
	...
	sendProductos = PieceCount(Referencias,'|');
	...
	jQuery('#idEstadoEnvio').html('0/'+sendProductos);
	jQuery('#idEstadoEnvio').show();
	
	//solodebug alert('AnadirProductos:'+Referencias+' sendProductos:'+sendProductos+' loops:'+loops);
	...
	//6oct17	prepararTablaProductos();

	AnadirProductosAjax(oForm, Referencias, 0, loops, '');
}

//	22mar17	COnvertimos en una función recursiva para pdoer actualizar la info en Chrome
function AnadirProductosAjax(oForm, Referencias, loop, loops, error)
{
	...
	if (loop==loops)
	{
		if (!error)
		{
			...
			else
			{
				...
				ColumnaOrdenacionProds = 'linea';	//	22mar17	Con otras ordenaciones se pueden producir errores por campos no inicializados
				prepararTablaProductos();
			}
		}
	...
	//solodebug	console.log('AnadirProductosAjax: '+Referencias+ 'limit:'+limit+' sendProductos:'+sendProductos);

	//6oct17	for (j=0;(j<limit)&&((loop*limit+j)<sendProductos);++j)
	for (j=0;(j<limit);++j)
	{
		cadEnvio+=Piece(Referencias,'|',loop*limit+j)+'|';
		//solodebug	console.log('AnadirProductosAjax: '+cadEnvio+' j:'+j);
	}

	//solodebug	console.log('AnadirProductosAjax: '+cadEnvio+' final');

	jQuery.ajax({
		cache:	false,
		url:	'http://www.newco.dev.br/Gestion/Comercial/NuevosProductos.xsql',
		type:	"GET",
		data:	"LIC_ID="+IDLicitacion+"&LISTA_REFERENCIAS="+cadEnvio+"&TIPOIVA="+TipoIVA+"&_="+d.getTime(),
		contentType: "application/xhtml+xml",
		//async: false,
		success: function(objeto){
			var data = eval("(" + objeto + ")");
			...
			jQuery('#idEstadoEnvio').html((limit*loop)+'/'+sendProductos);
			++loop;
			
			//solodebug	3may17	console.log('AnadirProductosAjax: '+loop+'/'+loops+':'+cadEnvio);
			
			//	18abr17	En el caso de licitación en curso, al añadir productos recargamos la pagina
			//if (estadoLicitacion=='CURS')
			//	location.reload();
			//else
				AnadirProductosAjax(oForm, Referencias, loop, loops, error);
...
/*
function AnadirProductos_original(oForm){
	var cadEnvio; limit = 20;
	...
	{
		cadEnvio='';
		for (j=0;(j<limit)&&((loop*limit+j)<sendProductos);++j)
			cadEnvio+=Piece(Referencias,'|',loop*limit+j)+'|';
			
		jQuery('#idEstadoEnvio').html((limit*loop)+'/'+sendProductos);
		
		//solodebug	alert(cadEnvio);
		...
		if (Ref!='')
		{
			...
			for (j=0;((j<arrProductos.length) && (!Encont));++j)
			{
				if ((arrProductos[ColumnaOrdenadaProds[j]].RefCliente==Ref)||(arrProductos[ColumnaOrdenadaProds[j]].RefEstandar==Ref))
				{
					...
					jQuery('#BOTONGUARDAR_' + (arrProductos[ColumnaOrdenadaProds[j]].linea - 1)).show();
					Encont=true;
					Cambios=true;
				}
			}
...
//	1jun17	Actualiza las ofertas desde Areatext. Formato: Ref Nombre Marca Udes/lote Precio (separadores válidos: espacio, tabulador, etc)
function ActualizarOfertasDesdeAreaText(oForm)
{
	var Cambios=false, Modificados=0, NoEncontrados=0, RefNoEncontradas='';
	...
	for (i=0;i<=numRefs;++i)
	{
		var Ref,RefProv,Nombre,Marca,Udesxlote,Precio;
		var Producto	= Piece(Referencias, '|',i);
		if (IDPais==55)
		{
		...
		else
		{
			...
			Udesxlote	= Piece(Producto, ':', 4);
			Precio		= Piece(Producto, ':', 5);
		}
		
		
		//solodebug		console.log('Oferta ('+i+'):'+Ref+':'+Nombre+':'+Marca+':'+Udesxlote+':'+Precio);
		...
		if (Ref!='')
		{
			...
			for (j=0;((j<arrProductos.length) && (!Encont));++j)
			{
				if ((arrProductos[ColumnaOrdenadaProds[j]].RefCliente==Ref)||(arrProductos[ColumnaOrdenadaProds[j]].RefEstandar==Ref))
				{
					...
					jQuery('#UdsLote_' + (arrProductos[ColumnaOrdenadaProds[j]].linea-1)).val(Udesxlote);
...
// 9jun17	Funcion recursiva para guardar los datos de todas las ofertas modificadas
function guardarTodasOfertasAjax(Pos){
	...
	if (!jQuery('#BOTONGUARDAR_'+Pos).is(':visible'))
	{
...
function ActualizarProductos(eliminarOfertas){
	...
	for(var loop = 1; loop <= loops; loop++){
		...
		jQuery(IDTablaProds + " tbody tr").each(function(index, element){
			...
			//solodebug	
			//solodebug	alert ('ActualizarProductos thisRow:'+jQuery(element).html()+' thisRowID:'+thisRowID+' thisPosArr:'+thisPosArr
			//solodebug		+' orden:'+ColumnaOrdenadaProds[thisPosArr]+'ID sin reordenar:'+arrProductos[thisPosArr].IDProdLic
			//solodebug		+'ID ordenado:'+arrProductos[ColumnaOrdenadaProds[thisPosArr]].IDProdLic);
			...
			if(index >= lenAuxIni && index < lenAuxFin){
				...
				if (isLicAgregada=='N')	items['Cantidad']=jQuery('#CANTIDAD_' + thisPosArr).val();
	...
	//solodebug	console.log('dibujaTablaProductos'); 
...
function recuperaDatosProductosDesdeLista(arrayCambiosProductos){
	...
	for (i=0;i<arrayCambiosProductos.length;++i){
		...
		if (isLicAgregada=='N')	jQuery('#CANTIDAD_' + arrayCambiosProductos[i].fila).val(arrayCambiosProductos[i].Cantidad);
		...
		for (j=0;((j<arrProductos.length) && (!found));++j)
		{
			if (arrProductos[j].IDProdLic==arrayCambiosProductos[i].IDProdLic)
			{
				...
				if (isLicAgregada=='N')	arrProductos[j].Cantidad=arrayCambiosProductos[i].Cantidad;
	...
	//solodebug		alert
	//console.log('recuperaDatosProductosDesdeLista: num.filas:' + arrayCambiosProductos.length+'\n\r'+msg);
...
function recuperaDatosProductos(flag){
		...
		success: function(objeto){
			...
			if(data.ProductosLicitacion.ListaProductosOfertas.length > 0){
				jQuery.each(data.ProductosLicitacion.ListaProductosOfertas, function(key, producto){
					...
					//solodebug
					console.log('recuperaDatosOfertaProductos IDProv:'+LicProvID+' IDOfe:'+producto.IDOferta+' Oferta'+producto.PrecioOfe+'Color:'+producto.Color);
...
function recuperaDatosProductosConOfertas(){
	...
	//solodebug alert('numProveedoresActivos:'+numProveedoresActivos);
	
	//	21set16	Probamos de recuperar los datos por proveedor
	cargaDatosOfertasPorProveedores();
	prepararTablaProductos();
    validarBotonAdjudicar();
	
/*	
	if (numProveedoresActivos>50)	//20set16	Solo para el caso de muchos proveedores informados, recargamos
	{
		//	14set16	Este proceso puede ser muy lento con grandes licitaciones, mejor recargar todo
		location.reload();
	}
	else
	{

		var ConsumoHist=0, ConsumoHistIVA=0, Consumo=0, ConsumoIVA=0;
		var CheckCamposAdjudicar = true;
		var d = new Date();

		jQuery.ajax({
			cache:	false,
			url:	'RecuperaProductosConOfertas.xsql',
			type:	"GET",
			data:	"LIC_ID="+IDLicitacion+"&IDIDIOMA="+IDIdioma+"&_="+d.getTime(),
			contentType: "application/xhtml+xml",
			error: function(objeto, quepaso, otroobj){
				alert('error'+quepaso+' '+otroobj+' '+objeto);
			},
			success: function(objeto){
				var data = eval("(" + objeto + ")");
				arrProductos	= new Array();
				arrConsumoProvs	= new Array();

				if(data.ListaProductos.length > 0){
					numProdsSeleccion = data.TotalSeleccionados;

					NumProvsOfertas	= data.ListaProveedores.length;
					jQuery.each(data.ListaProveedores, function(key, proveedor){
						var items		= [];
						items['columna']	= proveedor.Columna;
						items['IDProvLic']	= proveedor.IDPROVEEDOR_LIC;
						items['IDProveedor']	= proveedor.IDProveedor;
						items['PedidoMin']	= proveedor.PedidoMinimo;
						items['NombreCorto']	= proveedor.NombreCorto;

						items['Consumo']	= proveedor.Consumo;
						items['ConsumoIVA']	= proveedor.ConsumoIVA;
						...
						items['OfertasAdj']	= proveedor.OfertasAdj;
						// DC - 26feb16 - Variable auxiliar que nos permite modificar su valor al vuelo cuando se hacen cambios en el DOM sin perder el valor guardado en la BBDD
						items['OfertasAdjAux']	= proveedor.OfertasAdjAux;

						arrConsumoProvs.push(items);

						// Aqui informamos los nuevos consumos en el arrProveedores (pueden haber cambiado si se han modificado las cantidades)
						// Tiene que ser en la posicion que toque. No tiene pq ser en el mismo indice (debido a los estados 'SUS')
						jQuery.each(arrProveedores, function(key2, objProv){
							if(objProv.IDProvLic == proveedor.IDPROVEEDOR_LIC){
								// Actualizamos los valores de consumos
								arrProveedores[key2].ConsumoProv	= proveedor.Consumo;
								arrProveedores[key2].ConsumoProvIVA	= proveedor.ConsumoIVA;
								arrProveedores[key2].ConsumoPot		= proveedor.ConsumoPotencial;
								arrProveedores[key2].ConsumoPotIVA	= proveedor.ConsumoPotencialIVA;
								arrProveedores[key2].ConsumoAdj		= proveedor.ConsumoAdjudicado;
								arrProveedores[key2].ConsumoAdjIVA	= proveedor.ConsumoAdjudicadoIVA;
								arrProveedores[key2].Ahorro		= proveedor.Ahorro;
								arrProveedores[key2].AhorroIVA		= proveedor.AhorroIVA;
								arrProveedores[key2].OfertasAdj		= proveedor.OfertasAdj;
							}
						});
					});

					jQuery.each(data.ListaProductos, function(key, producto){
						// Calculamos nuevos valores de Consumo y ConsumoHist
						if(producto.Cantidad != '' && producto.PrecioReferencia != '')
							ConsumoHist	+= parseFloat(producto.Cantidad.replace(',', '.')) * parseFloat(producto.PrecioReferencia.replace(',', '.'));
						if(producto.Cantidad != '' && producto.PrecioReferenciaIVA != '')
							ConsumoHistIVA	+= parseFloat(producto.Cantidad.replace(',', '.')) * parseFloat(producto.PrecioReferenciaIVA.replace(',', '.'));
						if(producto.Cantidad != '' && producto.PrecioObjetivo != '')
							Consumo		+= parseFloat(producto.Cantidad.replace(',', '.')) * parseFloat(producto.PrecioObjetivo.replace(',', '.'));
						if(producto.Cantidad != '' && producto.PrecioObjetivo != '')
							ConsumoIVA	+= parseFloat(producto.Cantidad.replace(',', '.')) * parseFloat(producto.PrecioObjetivoIVA.replace(',', '.'));

						// Comprobamos todos los productos estan informados para poder adjudicar la licitacion
						if(producto.TieneSeleccion == 'N' && producto.NoAdjudicado == 'N'){
							CheckCamposAdjudicar = false;
						}
						...
						items['TieneSeleccion']	= producto.TieneSeleccion;
						items['Validado']	= producto.Validado;
						items['NoAdjudicado']	= producto.NoAdjudicado;
						items['Ordenacion']	= '0';

						items['Sospechoso']	= producto.Sospechoso;

						items['AhorroMax']	= producto.AhorroMaximo;
						items['SinAhorro']	= producto.SinAhorro;

						items['Ofertas']	= new Array();
						jQuery.each(producto.Ofertas, function(key2, oferta){
							var objOferta			= [];
							objOferta['columna']		= oferta.Columna;
							objOferta['IDOferta']		= oferta.LIC_OFE_ID;
							objOferta['IDProvLic']		= oferta.LIC_OFE_IDPROVEEDORLIC;
							objOferta['IDProdLic']		= oferta.LIC_OFE_IDPRODUCTO;
							objOferta['Referencia']		= oferta.Referencia;
							objOferta['Nombre']		= oferta.Nombre;
							objOferta['Marca']		= oferta.Marca;
							objOferta['FechaAlta']		= oferta.FechaAlta;
							objOferta['FechaMod']		= oferta.FechaModificacion;
							objOferta['UdsXLote']		= oferta.UnidadesLote;
							objOferta['Cantidad']		= oferta.Cantidad;
							objOferta['Precio']		= oferta.Precio;
							objOferta['PrecioIVA']		= oferta.PrecioIVA;
							objOferta['TipoIVA']		= oferta.TipoIVA;
							objOferta['Consumo']		= oferta.Consumo;
							objOferta['ConsumoIVA']		= oferta.ConsumoIVA;
							objOferta['IDEstadoEval']	= oferta.IDEstadoEvaluacion;
							objOferta['Ahorro']		= oferta.Ahorro;
							objOferta['EstadoEval']		= oferta.EstadoEvaluacion;

							objOferta['PrecioColor']	= oferta.ColorPrecio;
							objOferta['OfertaAdjud']	= oferta.OfertaAdjudicada;
	//						objOferta['OfertaAdjScrn']	= oferta.OfertaAdjudicada;

							objOferta['Sospechoso']		= oferta.Sospechoso;

							// Campos Avanzados oferta
							objOferta['InfoAmpliada']	= oferta.InfoAmpliadaOferta;
							objOferta['Documento']	= [];
							if(oferta.DocumentoOferta.ID != ''){
								objOferta['Documento']['ID']		= oferta.DocumentoOferta.ID;
								objOferta['Documento']['Nombre']	= oferta.DocumentoOferta.Nombre;
								objOferta['Documento']['Descripcion']	= oferta.DocumentoOferta.Descripcion;
								objOferta['Documento']['Url']		= oferta.DocumentoOferta.Url;
								objOferta['Documento']['Fecha']		= oferta.DocumentoOferta.Fecha;
							}
							// FIN Campos Avanzados oferta

							objOferta['FichaTecnica']	= [];
							if(oferta.FichaTecnica.ID != ''){
								objOferta['FichaTecnica']['ID']		= oferta.FichaTecnica.ID;
								objOferta['FichaTecnica']['Nombre']	= oferta.FichaTecnica.Nombre;
								objOferta['FichaTecnica']['Descripcion']= oferta.FichaTecnica.Descripcion;
								objOferta['FichaTecnica']['Url']	= oferta.FichaTecnica.Fichero;
								objOferta['FichaTecnica']['Fecha']	= oferta.FichaTecnica.Fecha;
							}

							objOferta['NoInformada']	= oferta.NoInformada;
							objOferta['NoOfertada']		= (oferta.PrecioIVA == '0,0000' && oferta.UnidadesLote == '0,00') ? 'S' : 'N';

							items['Ofertas'].push(objOferta);
						});

						arrProductos.push(items);
					});

					// Informamos de los nuevos valores para:
					objLicitacion['ConsHist']	= formatCurrency(ConsumoHist);
					objLicitacion['ConsHistIVA']	= formatCurrency(ConsumoHistIVA);
					objLicitacion['Consumo']	= formatCurrency(Consumo);
					objLicitacion['ConsumoIVA']	= formatCurrency(ConsumoIVA);
					objLicitacion['ConsumoAdj']	= data.TotalAdjudicado;
					objLicitacion['ConsumoAdjIVA']	= data.TotalAdjudicadoIVA;
					objLicitacion['Ahorro']		= data.Ahorro;

					if(CheckCamposAdjudicar !== false){
...
function AnadirProveedor(oForm){
	...
	var IDUsuario	= oForm.elements['LIC_IDUSUARIOPROVEEDOR'].value;
		...
		success: function(objeto){
			...
			if(data.ListaProveedores.length > 0){
				...
				jQuery.each(data.ListaProveedores, function(key, proveedor){
					...
					items['Estrellas']	= proveedor.Estrellas;	//	27set17
...
function AnadirUsuario(oForm){
	...
	jQuery.ajax({
		...
		type:	"GET",
		data:	"LIC_ID="+IDLicitacion+"&US_FIRMA_ID="+IDUsuarioFirm+"&LIC_USU_FIRMA="+Firma+"&_="+d.getTime(),
		contentType: "application/xhtml+xml",
		error: function(objeto, quepaso, otroobj){
			alert('error'+quepaso+' '+otroobj+' '+objeto);
		},
			...
			if(data.ListaCentros.length > 0){
				...
				jQuery.each(data.ListaCentros, function(key, centro){
					...
					if(estadoLicitacion == 'EST'){
						tbodyCentro += "<a class=\"accBorrar\" href=\"javascript:borrarCentro(" + centro.Licc_ID + ");\">" +
                                                    "<img src=\"http://www.newco.dev.br/images/2017/trash.png\">" +
                                                    "</a>";
					}else{
						tbodyCentro += '&nbsp';
...
//eliminar un usuario de la licitacion
function borrarCentro(idCentroEliminar){
	var d = new Date();
	...
	jQuery.ajax({
		...
		type:	"GET",
		data:	"ID_CENTRO_ELIMINAR="+idCentroEliminar+"&_="+d.getTime(),
		contentType: "application/xhtml+xml",
		error: function(objeto, quepaso, otroobj){
			alert('error'+quepaso+' '+otroobj+' '+objeto);
		},
...
function publicarPedidoMinimo(oForm){
	...
	jQuery.ajax({
		...
		type:	"GET",
		data:	"LIC_PROV_ID="+IDProvLic+"&LIC_PROV_PEDIDOMINIMO="+PedidoMinimoProv+"&LIC_PROV_COMENTARIOSPROV="+ComentariosProv+"&LIC_PROV_FRETE="+Frete+"&LIC_PROV_PLAZOENTREGA="+PlazoEntrega+"&_="+d.getTime(),
		contentType: "application/xhtml+xml",
		beforeSend: function(){
			jQuery("#MensPedidoMinimo").hide();
		},
...
function IniciarLicitacion(IDEstado){
	...
	{
		//	solodebug	console.log('IniciarLicitacion. Comprobando producto:'+j+' Ud.Basica:'+arrProductos[j].UdBasica+' Cant:'+arrProductos[j].Cantidad);
...
//cargar documentos
function cargaDoc(form, tipo, valueID){
	// valueID puede ser:
	...
	if(msg != ''){
		...
		if(hasFilesDoc(form)){
			...
			form.action = action;
			waitDoc(valueID, "Please wait...");
			form_tmp = form;
			man_tmp = true;
			periodicTimerDoc = 0;
			periodicUpdateDoc(valueID, tipo);
			form.submit();
		}
	}//fin else
}
 ...
 */
function periodicUpdateDoc(valueID, tipo){
	// valueID puede ser:
	//	si tipo in (FT) => valueID = LicProdID
	//	si tipo in (OA, OMVM, OMVMB, OFNCP, etc...) => valueID = LicID
	...
	if(uploadFrameDoc && uploadFrameDoc.document && uploadFrameDoc.document.getElementsByTagName("p")[0]) {
		if(document.getElementById('ProductosProveedor')){ tipoDoc = document.forms['ProductosProveedor'].elements['TIPO_DOC'].value; }
		tipoDoc = tipo;
		...
		if(uFrame.innerHTML.substr(0, 1) != '{' && uFrame.innerHTML.substr(0, 1) != '['){
			...
			handleFileRequestDoc(response,valueID, tipo);
			return true;
		}
	}else{
		setTimeout(function(){periodicUpdateDoc(valueID, tipo)}, 1000);
		return false;
	}
	return true;
}
 ...
 * @return Boolean
 */
function handleFileRequestDoc(resp,valueID, tipo){
	// valueID puede ser:
	//	si tipo in (FT) => valueID = LicProdID
	//	si tipo in (OA, OMVM, OMVMB, OFNCP, etc...) => valueID = LicID
...
/*
	if(document.getElementById('SubirContrato')){
		var borrados = document.forms['SubirContrato'].elements['DOCUMENTOS_BORRADOS'].value;
		var borr_ante = document.forms['SubirContrato'].elements['BORRAR_ANTERIORES'].value;
		var tipoDoc = document.forms['SubirContrato'].elements['TIPO_DOC'].value;
		var prove = document.forms['SubirContrato'].elements['LIC_PROV_IDPROVEEDOR'].value;
	}
*/
	tipoDoc = tipo;
...
// DC - 06oct14 - Se hace esta funcion dinamica para que se puedan subir fichas tecnicas y contratos en dos forms diferentes...
					if(tipoDoc == 'FT'){
						//recargamos las ofertas y las fichas
						SeleccionaFichas(prove,'IDFICHA_' + valueID,currentDocID);
					}
/* DC - 06mar15 - Edu decide quitar esta funcionalidad para replantearla en un futuro
                		        else{
...
function prepararTablaProductos(dibujaTabla = true){		//	17nov17
...
{
	//solodebug	alert('ActivarCambio:'+NumeroLinea);
	
	jQuery('#AVISOACCION_'+NumeroLinea).html('');
	jQuery('#BOTONGUARDAR_'+NumeroLinea).show();
}
...
function dibujaTablaProductosCENTRO(){
	...
	if(totalProductos > 0){
		for(var i=0; i<totalProductos; i++){
			...
			thisMacro = thisMacro.replace('#ONKEYUP#', 'javascript:ActivarBotonGuardar('+(arrProductos[ColumnaOrdenadaProds[i]].linea - 1)+')');
			...
			thisMacro	= thisMacro.replace('#ID#', 'BOTONGUARDAR_'+ (arrProductos[ColumnaOrdenadaProds[i]].linea - 1));	//	17set16	ID
...
function dibujaTablaProductosPROVE(){
	...
	if(totalProductos > 0){
		...
		for(var i=firstProduct; i<lastProduct; i++){
			...
			// Celda/Columna Ficha Tecnica
			thisCell = cellStart;
			thisMacro = macroEnlace2.replace('#HREF#', 'javascript:verFichas(' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1) + ');');
			thisMacro = thisMacro.replace('#STYLE#', 'text-decoration:none;');
			thisCell += thisMacro;
...
//				thisCell += '<img src="" class="static"/>';
			...
			thisCell = thisCell.replace('#STYLE#','padding:2px 0px 2px 4px;');
//			thisCell += '<strong>' + arrProductos[ColumnaOrdenadaProds[i]].Nombre + '</strong>&nbsp;';
			thisCell += '<strong>' + NombreProductoConEstilo(i) + '</strong>&nbsp;';
			thisCell += cellEnd;
			thisRow += thisCell;
...
//				thisCell += '<img src="" class="static"/>';
			...
			thisCell = cellStart;
			if(IDPais != '55'){
				thisMacro = macroInputTextonKeyUp.replace('#NAME#', 'RefProv_' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1));
				thisMacro = thisMacro.replace('#ID#', 'RefProv_' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1));
				thisMacro = thisMacro.replace('#CLASS#', 'refProv');
				thisMacro = thisMacro.replace('#SIZE#', '10');
				thisMacro = thisMacro.replace('#MAXLENGTH#', '15');
				thisMacro = thisMacro.replace('#ONKEYUP#', 'javascript:ActivarBotonGuardar('+(arrProductos[ColumnaOrdenadaProds[i]].linea - 1)+');');
				if(isLicPorProducto == 'S'){
					if(arrProductos[ColumnaOrdenadaProds[i]].Oferta.RefProv != ''){
						thisMacro = thisMacro.replace('#VALUE#', arrProductos[ColumnaOrdenadaProds[i]].Oferta.RefProv);
					}else{
						thisMacro = thisMacro.replace('#VALUE#', str_SinOfertar);
					}
				}else{
					thisMacro = thisMacro.replace('#VALUE#', arrProductos[ColumnaOrdenadaProds[i]].Oferta.RefProv);
				}
			...
			// Celda/Columna Descripcion
			thisCell = cellStart;
			thisMacro = macroInputTextonKeyUp.replace('#NAME#', 'Desc_' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1));
			thisMacro = thisMacro.replace('#ID#', 'Desc_' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1));
			...
			thisMacro = thisMacro.replace('#ONKEYUP#', 'javascript:ActivarBotonGuardar('+(arrProductos[ColumnaOrdenadaProds[i]].linea - 1)+');');
			if(isLicPorProducto == 'S'){
				if(arrProductos[ColumnaOrdenadaProds[i]].Oferta.Nombre != '' || (arrProductos[ColumnaOrdenadaProds[i]].Oferta.Precio != '0,0000' && arrProductos[ColumnaOrdenadaProds[i]].Oferta.Precio != '')){
					thisMacro = thisMacro.replace('#VALUE#', arrProductos[ColumnaOrdenadaProds[i]].Oferta.Nombre);
				}else{
					thisMacro = thisMacro.replace('#VALUE#', str_SinOfertar);
				}
			}else{
			...
			// Celda/Columna Marca
			thisCell = cellStart;
			thisMacro = macroInputTextonKeyUp.replace('#NAME#', 'Marca_' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1));
			thisMacro = thisMacro.replace('#ID#', 'Marca_' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1));
			thisMacro = thisMacro.replace('#CLASS#', 'marca');
			thisMacro = thisMacro.replace('#SIZE#', '10');
			thisMacro = thisMacro.replace('#MAXLENGTH#', '100');
			thisMacro = thisMacro.replace('#ONKEYUP#', 'javascript:ActivarBotonGuardar('+(arrProductos[ColumnaOrdenadaProds[i]].linea - 1)+');');
			if(isLicPorProducto == 'S'){
				if(arrProductos[ColumnaOrdenadaProds[i]].Oferta.Marca != '' || (arrProductos[ColumnaOrdenadaProds[i]].Oferta.Precio != '0,0000' && arrProductos[ColumnaOrdenadaProds[i]].Oferta.Precio != '')){
					thisMacro = thisMacro.replace('#VALUE#', arrProductos[ColumnaOrdenadaProds[i]].Oferta.Marca);
				}else{
					thisMacro = thisMacro.replace('#VALUE#', str_SinOfertar);
				}
			}else{
			...
			// Celda/Columna Consumo
			thisCell = cellStart;
			thisCell += arrProductos[ColumnaOrdenadaProds[i]].Consumo;
			thisCell += cellEnd;
			thisRow += thisCell;

			// Celda/Columna UdsXLote
			thisCell = cellStart;
			thisMacro = macroInputTextonKeyUp.replace('#NAME#', 'UdsLote_' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1));
			thisMacro = thisMacro.replace('#ID#', 'UdsLote_' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1));
			thisMacro = thisMacro.replace('#CLASS#', 'unidades peq');
			thisMacro = thisMacro.replace('#SIZE#', '6');
			thisMacro = thisMacro.replace('#MAXLENGTH#', '15');
			thisMacro = thisMacro.replace('#ONKEYUP#', 'javascript:ActivarBotonGuardar('+(arrProductos[ColumnaOrdenadaProds[i]].linea - 1)+');');
			if(isLicPorProducto == 'S'){
				if(arrProductos[ColumnaOrdenadaProds[i]].Oferta.UdsXLote != ''){
					thisMacro = thisMacro.replace('#VALUE#', arrProductos[ColumnaOrdenadaProds[i]].Oferta.UdsXLote);
				}else{
					thisMacro = thisMacro.replace('#VALUE#', '0');
				}
			}else{
			...
			thisCell = cellStart;
			thisMacro = macroInputTextonKeyUp.replace('#NAME#', 'Precio_' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1));
			thisMacro = thisMacro.replace('#ID#', 'Precio_' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1));
			thisMacro = thisMacro.replace('#CLASS#', 'precio peq');
			thisMacro = thisMacro.replace('#SIZE#', '8');
			thisMacro = thisMacro.replace('#MAXLENGTH#', '8');
			thisMacro = thisMacro.replace('#ONKEYUP#', 'javascript:ActivarBotonGuardar('+(arrProductos[ColumnaOrdenadaProds[i]].linea - 1)+');');
			if(isLicPorProducto == 'S'){
				if(arrProductos[ColumnaOrdenadaProds[i]].Oferta.Precio != ''){
					thisMacro = thisMacro.replace('#VALUE#', arrProductos[ColumnaOrdenadaProds[i]].Oferta.Precio);
				}else{
					thisMacro = thisMacro.replace('#VALUE#', '0');
				}
			}else{
			...
			// Celda/Columna para guardar los datos de la oferta fila a fila
			thisCell = cellStartClass.replace('#CLASS#', 'acciones');
			functionJS	= 'javascript:guardarOfertaFila(\'' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1) + '\')';
			thisMacro	= macroEnlace3.replace('#CLASS#', 'guardarOferta');
			thisMacro	= thisMacro.replace('#HREF#', functionJS);
			thisMacro	= thisMacro.replace('#STYLE#', 'text-decoration:none;');
			thisCell	+= thisMacro;
			...
			thisMacro	= macroImagen5.replace('#SRC#', 'http://www.newco.dev.br/images/guardar.gif');
			thisMacro	= thisMacro.replace('#ID#', 'BOTONGUARDAR_'+ (arrProductos[ColumnaOrdenadaProds[i]].linea - 1));	//	17set16	Identificamos el botón para poderlo mostrar/ocultar
			thisMacro	= thisMacro.replace('#CLASS#', '');
			thisMacro	= thisMacro.replace('#TITLE#', str_Guardar);
			thisMacro	= thisMacro.replace('#ALT#', str_Guardar);
			thisMacro	= thisMacro.replace('#STYLE#', 'display:none;');	//	17set16	botones ocultos
			...
			thisCell	+= macroEnlaceEnd;
			//thisCell += '<a href="javascript:guardarOfertaFila(' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1) + ');" class="guardarOferta">Guardar</a>';
			thisCell += cellEnd;
			thisRow += thisCell;
			...
			thisRow = rowStartIDClassStyl.replace('#ID#', 'FT_' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1));
			...
			thisMacro = macroSelect.replace('#NAME#', 'IDFICHA_' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1));
			thisMacro = thisMacro.replace('#ID#', 'IDFICHA_' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1));
			...
			for(var k=0; k<arrFichasTecnicas.length; k++){
				...
				if(arrProductos[ColumnaOrdenadaProds[i]].Oferta.FichaTecnica.ID == arrFichasTecnicas[k].ID){
			...
			thisCell += dibujaSubirFT((arrProductos[ColumnaOrdenadaProds[i]].linea - 1));
			thisCell += cellEnd;
			thisRow += thisCell;
...
function dibujaTablaProductosOFE(){
	...
	//solodebug	MuestraMatrizOfertas();
	//solodebug	alert("dibujaTablaProductosOFE totalProductos:"+totalProductos);
...
//				thisCell += '<img src="" class="static"/>';
			...
			for(var j=0; j<arrProductos[ColumnaOrdenadaProds[i]].Ofertas.length; j++){
				...
				if (arrConsumoProvs[j].TieneOfertas=='S'){	//	COmprueba proveedor con ofertas /Mantenimiento/LICITACION/PRODUCTOSLICITACION/PROVEEDORESLICITACION/PROVEEDOR/TIENE_OFERTAS
					...
					// Si el proveedor ha informado de algunas ofertas, pero esta esta vacia
						thisCell = cellStartColspanCls.replace('#COLSPAN#', '4');
						thisCell = thisCell.replace('#CLASS#', 'colSinOferta center borderLeft');
						thisCell += str_OfertaNoOfertada;
						thisCell += cellEnd;
						thisRow += thisCell;
					}else{	// Si esta oferta esta informada

						// Celda para licitaciones por producto (radio button o imagen checked)
						thisCell = cellStartClassStyle.replace('#CLASS#', 'borderLeft');
						...
						if((estadoLicitacion=='CURS' || estadoLicitacion=='INF') && isLicPorProducto=='S' && isAutor=='S'){
							...
							//solodebug	13abr17	console.log('Prov:' + arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].ID+' columna:'+(arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].columna));
							...
							thisCell += thisMacro;
	/*
						}else if((estadoLicitacion=='CURS' || estadoLicitacion=='INF') && isLicPorProducto=='S'){
							if(arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].OfertaAdjud == 'S'){
								thisMacro = macroImagen.replace('#SRC#', 'http://www.newco.dev.br/images/check.gif');
								thisMacro = thisMacro.replace('#TITLE#', str_OfertaAdjudicada);
								thisMacro = thisMacro.replace('#ALT#', str_OfertaAdjudicada);
						...
						}else if((estadoLicitacion=='ADJ' || estadoLicitacion=='FIRM' || estadoLicitacion=='CONT') && isLicPorProducto=='S'){
	*/
						...
						thisCell = thisCell.replace('#CLASS#', 'PrecioOferta_' + (arrProductos[ColumnaOrdenadaProds[i]].linea - 1) + ' datosRight colPrecio');
						thisCell = thisCell.replace('#STYLE#', 'display:none;');
						if(mostrarPrecioIVA == 'S'){
	...
	*/
	...
	//solodebug 31mar17 ET problemas floatingbox	console.log('dibujaTablaProductosOFE. isLicPorProducto:'+isLicPorProducto+' estadoLicitacion:'+estadoLicitacion);
...
//			console.log(producto.Ofertas[posArr].IDOferta + ': yes, key: ' + key);
...
//			valAux = (arrProductos[ColumnaOrdenadaProds[i]].Ofertas[posArr].Precio.replace(/\./g,'').replace(',','.') != '') ? arrProductos[ColumnaOrdenadaProds[i]].Ofertas[posArr].Precio.replace(/\./g,'').replace(',','.') : '0';
			...
			//solodebug	console.log('ordenamientoBurbuja. i:'+i+' arrProductos[ColumnaOrdenadaProds[i]][col]: ('+arrProductos[ColumnaOrdenadaProds[i]][col]+')');
			
			if (Number.isInteger(arrProductos[ColumnaOrdenadaProds[i]][col]))	//	24oct17	Fallaba la ordenación con numeros enteros, daba error en replace
...
function dibujarFilaConsumoProdEST(){
	...
	if(isAutor == 'S'){
		...
		htmlTFOOT += cellEnd;
		htmlTFOOT += cellStartColspan.replace('#COLSPAN#', '6');
		htmlTFOOT += cellEnd;
		htmlTFOOT += rowEnd;
    }
...
function prepararTablaProveedores(){
...
function dibujaTablaProveedoresADJ(){
	...
	if(totalProvs > 0){
		for(var i=0; i<totalProvs; i++){
			...
			// Celda/Columna Nombre
			thisCell = cellStartClass.replace('#CLASS#', 'datosLeft');
			thisCell += '&nbsp;' + macroEnlace.replace('#HREF#', 'javascript:FichaEmpresa(' + arrProveedores[ColumnaOrdenadaProvs[i]].IDProveedor + ');');
...
//					thisMacro = macroEnlace2.replace('#HREF#', 'javascript:conversacionProveedor(' + arrProveedores[ColumnaOrdenadaProvs[i]].IDProveedor + ', ' + IDUsuario + ', ' + arrProveedores[ColumnaOrdenadaProvs[i]].IDUsuarioProv + ', \'' + arrProveedores[ColumnaOrdenadaProvs[i]].Nombre + '\');');
					...
					if(arrProveedores[ColumnaOrdenadaProvs[i]].HayConversa == 'S'){
						thisMacro = macroImagen.replace('#SRC#', 'http://www.newco.dev.br/images/bocadillo.gif');
						thisMacro = thisMacro.replace('#TITLE#', str_leerConversacion);
						thisMacro = thisMacro.replace('#ALT#', str_leerConversacion);
					}else{
						thisMacro = macroImagen.replace('#SRC#', 'http://www.newco.dev.br/images/bocadilloPlus.gif');
...
//					thisMacro = macroEnlace2.replace('#HREF#', 'javascript:conversacionProveedor(' + arrProveedores[ColumnaOrdenadaProvs[i]].IDProveedor + ', ' + IDUsuario + ', ' + arrProveedores[ColumnaOrdenadaProvs[i]].IDUsuarioProv + ', \'' + arrProveedores[ColumnaOrdenadaProvs[i]].Nombre + '\');');
			...
			/*	Quitamos estas columnas para ganar espacio
			// Celda/Columna Comentarios CdC
			thisCell = cellStart;
			if(arrProveedores[ColumnaOrdenadaProvs[i]].CommsCdC.length > 50){
				thisMacro = macroEnlaceAcc.replace('#HREF#', '#');
				thisMacro = thisMacro.replace('#CLASS#', 'tooltip');
				thisCell += thisMacro;
				thisCell += arrProveedores[ColumnaOrdenadaProvs[i]].CommsCdC.substr(0, 50) + '...';
				thisCell += spanStartClass.replace('#CLASS#', 'classic');
			...
			thisCell = cellStart;
			if(arrProveedores[ColumnaOrdenadaProvs[i]].CommsProv.length > 50){
				thisMacro = macroEnlaceAcc.replace('#HREF#', '#');
				thisMacro = thisMacro.replace('#CLASS#', 'tooltip');
				thisCell += thisMacro;
				thisCell += arrProveedores[ColumnaOrdenadaProvs[i]].CommsProv.substr(0, 50) + '...';
				thisCell += spanStartClass.replace('#CLASS#', 'classic');
			...
			*/
			...
			if(arrProveedores[ColumnaOrdenadaProvs[i]].IDEstadoProv != 'CURS' && arrProveedores[ColumnaOrdenadaProvs[i]].IDEstadoProv != 'SUS'){
				...
				thisMacro = thisMacro.replace('#STYLE#', 'text-decoration:none;');
				thisCell += thisMacro;
				thisMacro = macroImagen.replace('#SRC#', 'http://www.newco.dev.br/images/imprimir.gif');
				...
				thisMacro = thisMacro.replace('#STYLE#', 'text-decoration:none;');
				thisCell += thisMacro;
				thisMacro = macroImagen.replace('#SRC#', 'http://www.newco.dev.br/images/iconoExcel.gif');
...
function dibujaTablaProveedoresEST(){
	...
	if(totalProvs > 0){
		for(var i=0; i<totalProvs; i++){
			...
			// Celda/Columna Nombre
			thisCell = cellStartClass.replace('#CLASS#', 'datosLeft');
			thisCell += '&nbsp;<img src="http://www.newco.dev.br/images/StarSmall'+arrProveedores[ColumnaOrdenadaProvs[i]].Estrellas+'.png" class="static"/>&nbsp;' + macroEnlace.replace('#HREF#', 'javascript:FichaEmpresa(' + arrProveedores[ColumnaOrdenadaProvs[i]].IDProveedor + ');');
			thisCell += arrProveedores[ColumnaOrdenadaProvs[i]].NombreCorto;
			thisCell += macroEnlaceEnd;
			...
			if(isAutor == 'S'){
				if((estadoLicitacion == 'EST')||(estadoLicitacion == 'COMP')){
					if(arrProveedores[ColumnaOrdenadaProvs[i]].IDEstadoProv == 'SUS'){
						...
						thisMacro = thisMacro.replace('#STYLE#', 'text-decoration:none;');
						thisCell += thisMacro;
						thisMacro = macroImagen.replace('#SRC#', 'http://www.newco.dev.br/images/rollbackVerde.gif');
					...
					if(arrProveedores[ColumnaOrdenadaProvs[i]].IDEstadoProv == 'SUS'){
						...
						thisMacro = thisMacro.replace('#STYLE#', 'text-decoration:none;');
						thisCell += thisMacro;
						thisMacro = macroImagen.replace('#SRC#', 'http://www.newco.dev.br/images/2017/reload.png');
...
function dibujarFilaConsumoProvADJ(){
	...
	htmlTFOOT += rowStartStyle.replace('#STYLE#', 'border-top:1px solid #999;height:30px;');
	htmlTFOOT += cellStartColspan.replace('#COLSPAN#', '7');
	htmlTFOOT += cellEnd;
	...
	htmlTFOOT += '</strong>&nbsp;' + cellEnd;
	htmlTFOOT += cellStartClass.replace('#CLASS#', 'datosRight') + '<strong>';
	htmlTFOOT += objLicitacion['Ahorro'] + '%';
	htmlTFOOT += '</strong>&nbsp;' + cellEnd;
	htmlTFOOT += cellStart + '<strong>' + str_TotalProductos + ':</strong>' + cellEnd;
...
function guardarConversProv(){
	...
	var IDUsuClie	= oForm.elements['IDUSUARIOCLIENTE'].value;
...
function MostrarFloatingBox(){
	...
	var Cantidad, CantidadFormato, PrecioRef, PrecioRefFormato, Precio, PrecioFormato, ConsumoPot = 0, ConsumoPrev = 0, Ahorro = 0;
	var chckPedMinimo = false, valAux;

	//solodebug	console.log('MostrarFloatingBox');
			...
			jQuery.each(producto.Ofertas, function(key2, oferta){
				...
				if(oferta.OfertaAdjud == 'S'){
					...
					if(PrecioRefFormato != 0){
						ConsumoPot		+= CantidadFormato * PrecioRefFormato;
						ConsumoPrev		+= CantidadFormato * PrecioFormato;
					}
	...
	jQuery('#FBNumOfertas').html(numOfertas+'/'+numProductos);
	...
	jQuery('#FBProvs').html(numProveedoresAdj);
...
//	if(numOfertas == numProductos){
	...
	//solodebug	alert('MostrarFloatingBox ConsumoPrev:'+ConsumoPrev);

	//solodebug	console.log('MostrarFloatingBox ConsumoPrev:'+ConsumoPrev);
	

	//if(ConsumoPrev)	-> 31mar17 mostramos el FloatingBox en todos los casos, si no, no aparece ein precio histórico informado
...
function recalcularFloatingBox(obj, flagPedMin, ofertaGuardada){
	...
	var oldConsumoPot = 0, newConsumoPot, oldConsumoPrev = 0, newConsumoPrev, oldAhorro = 0, 
		newAhorro, oldNumOfertas = 0, newNumOfertas, oldNumProvsAdj = 0, newNumProvsAdj = 0;
	var chckPedMinimo = false, valAux;
	

	if(jQuery('#floatingBox').is(":visible")){
		// Buscamos los valores antiguos en el div flotante y los formateamos para hacer calculos
		oldConsumoPot	= parseFloat(jQuery('#FBConsPot').html().replace(/\./g,'').replace(',', '.'));
		oldConsumoPrev	= parseFloat(jQuery('#FBConsPrev').html().replace(/\./g,'').replace(',', '.'));
		oldNumOfertas	= parseFloat(jQuery('#FBNumOfertas').html().replace(/\/[0-9]+/, ''));
		oldNumProvsAdj	= parseFloat(jQuery('#FBProvs').html().replace(/\/[0-9]+/, ''));
		jQuery('#floatingBox').hide();
	}

	//solodebug	13abr17	console.log('RecalcularFloatingBox: firstProduct'+firstProduct+ ' oldOfertaID:'+oldOfertaID+' thisOfertaID:'+thisOfertaID+ ' oldConsumoPot:'+oldConsumoPot);
	...
	for(var i=0; i<ColumnaOrdenadaProds.length; i++){
		if(ColumnaOrdenadaProds[i] == thisPosArr){
			...
			if(wasChecked){			
				...
				//solodebug	13abr17	console.log('RecalcularFloatingBox: firstProduct'+firstProduct+ 'i:'+i+ ' oldOfertaID:'+oldOfertaID+' arrRadios[(i-firstProduct)].OfertaID:'+arrRadios[(i-firstProduct)].OfertaID);
	...
	//solodebug	console.log('RecalcularFloatingBox: oldOfertaID:'+oldOfertaID+' thisOfertaID:'+thisOfertaID);


	for(var j=0; j<arrProductos[thisPosArr].Ofertas.length; j++){
		if(arrProductos[thisPosArr].Ofertas[j].ID == thisOfertaID){	//	21set16	IDOferta
		
			//	5may17	Marcamos la oferta seleccionada/deseleccionada en el array de ofertas
			
			//solodebug	console.log('RecalcularFloatingBox: ID:'+ thisOfertaID+ ' fila:'+thisPosArr+' Col:'+j+'. Estaba adjudicada:'+arrProductos[thisPosArr].Ofertas[j].OfertaAdjud+' Ahora:'+(isChecked?'S':'N'));
			arrProductos[thisPosArr].Ofertas[j].OfertaAdjud = (isChecked?'S':'N');
	
			(mostrarPrecioIVA == 'S') ? newPrecioOferta = parseFloat(arrProductos[thisPosArr].Ofertas[j].PrecioIVA.replace(',', '.')) : newPrecioOferta = parseFloat(arrProductos[thisPosArr].Ofertas[j].Precio.replace(',', '.'));
			break;
		}
	}

	// DC - 26feb16 - Parte para mostrar en el floating box el número de proveedores con ofertas seleccionadas
	...
	if(wasChecked){
		...
		if(thisPosArrProv !== oldPosArrProv){
			...
			arrConsumoProvs[thisPosArrProv].OfertasAdjAux++;
			arrConsumoProvs[oldPosArrProv].OfertasAdjAux--;
	...
	if(wasChecked !== true && isChecked !== false){
		if(PrecioHist != 0){
			newConsumoPot	= oldConsumoPot + (PrecioHist * Cantidad);
			newConsumoPrev	= oldConsumoPrev + (newPrecioOferta * Cantidad);
		}else{
			newConsumoPot	= oldConsumoPot;
			newConsumoPrev	= oldConsumoPrev;
		}

		if (newConsumoPot==0)
			newAhorro	= 0;			//	31mar17 Sin esto, muestra NaN
		else
			newAhorro	= (newConsumoPot - newConsumoPrev) * 100 / newConsumoPot;

		newNumOfertas	= oldNumOfertas + 1;
	}else if(wasChecked !== false && isChecked !== false){
		if(PrecioHist != 0){
			newConsumoPot	= oldConsumoPot;
			newConsumoPrev	= oldConsumoPrev + (newPrecioOferta * Cantidad) - (oldPrecioOferta * Cantidad);
		}else{
			newConsumoPot	= oldConsumoPot;
			newConsumoPrev	= oldConsumoPrev;
		}
	...
	//solodebug	console.log('RecalcularFloatingBox: wasChecked:'+wasChecked+' isChecked:'+isChecked+' oldNumOfertas:'+oldNumOfertas+' newNumOfertas:'+newNumOfertas);


	if(newNumOfertas > 0){
		jQuery('#FBConsPot').html(FormateaNumeroNacho(newConsumoPot.toFixed(2)));
		jQuery('#FBConsPrev').html(FormateaNumeroNacho(newConsumoPrev.toFixed(2)));
		jQuery('#FBAhorro').html(FormateaNumeroNacho(newAhorro.toFixed(2)) + '%');
		jQuery('#FBNumOfertas').html(newNumOfertas + '/' + ColumnaOrdenadaProds.length);
		jQuery('#FBProvs').html(newNumProvsAdj);
...
*/
		...
		//solodebug	console.log('RecalcularFloatingBox: Mostrar. newNumOfertas:'+newNumOfertas);
	}
	else
	{
		//solodebug	console.log('RecalcularFloatingBox: NO Mostrar. newNumOfertas:'+newNumOfertas);
	}
...
function GuardarSeleccion(){
	...
	if(!errores){
		...
		jQuery.ajax({
			cache:	false,
			url:	'http://www.newco.dev.br/Gestion/Comercial/AdjudicarProductos2AJAX.xsql',
			type:	"GET",
...
function AdjudicarOfertas(){
	...
	//solodebug	
	console.log('AdjudicarOfertas numProdsSeleccion:'+numProdsSeleccion+' productosOlvidados:'+productosOlvidados);
		

	if(productosOlvidados == 'N'){

		//	solodebug	ET 22/2/16	
		console.log('Probando, paso1, pedidos minimos');//ET 22/2/16
		...
		if(mesesSelected == 0){
			if	(isLicAgregada == 'S')
			{
				...
				if (!checkAdj) alert(alrt_pedidoMinimoGlobalKO);
			}
			else
		...
		//	solodebug	ET 22/2/16	
		console.log('Probando, paso2, pedidos minimos:'+checkAdj);//ET 22/2/16
		...
		if(checkAdj){
			...
			//	solodebug	ET 22/2/16	
			console.log('Probando, paso3, comprueba unidades lote:'+checkUdsXLote);//ET 22/2/16
			...
			if(numProdsSeleccion < totalProductos){
				...
				//	solodebug	
				console.log('Probando, paso5, pedir confirmacion');//ET 22/2/16
			...
			//	solodebug	
			console.log('Probando, paso, confirmar adjudicación:'+confirmChk);//ET 22/2/16
...
function compruebaUdsXLote(){
	...
	jQuery.each(arrProductos, function(key, producto){
		...
		//	solodebug	
		console.log('compruebaUdsXLote.UdsXLoteFormat:'+UdsXLoteFormat+' CantidadFormat:'+CantidadFormat+' esEntero:'+esEntero(Math.round((CantidadFormat/UdsXLoteFormat)*1000)/1000));
		...
		if(UdsXLoteFormat && !esEntero(Math.round((CantidadFormat/UdsXLoteFormat)*1000)/1000)){
			...
			//	solodebug	
			console.log('compruebaUdsXLote.UdsXLoteFormat:'+UdsXLoteFormat+' CantidadFormat:'+CantidadFormat+' esEntero:'+esEntero(Math.round((CantidadFormat/UdsXLoteFormat)*1000)/1000)+ ' return FALSE');
	...
	//	solodebug	
	console.log('compruebaUdsXLote.UdsXLoteFormat:'+UdsXLoteFormat+' CantidadFormat:'+CantidadFormat+' esEntero:'+esEntero(Math.round((CantidadFormat/UdsXLoteFormat)*1000)/1000)+ ' return '+toReturn);
...
function compruebaUdsXLoteAgr(){
	...
	jQuery.each(arrProductosPorCentro, function(key, productoPorCentro)
	{
		...
		for (j=0;j<productoPorCentro.Centros.length;++j)
		{
			...
			//solodebug	console.log('compruebaUdsXLoteAgr: Producto, ref:'+productoPorCentro.RefCliente+' cant:'+Cantidad+' IDCentro:'+IDCentro);
			...
			jQuery.each(productoConOfertas.Ofertas, function(key2, oferta){
				...
				if(oferta.OfertaAdjud == 'S'){
					...
					//solodebug	console.log('compruebaUdsXLoteAgr: Producto, IDProductoLic:'+IDProductoLic+' ref:'+productoPorCentro.RefCliente+' IDCentro:'+IDCentro+' cant:'+Cantidad+' IDCentro:'+IDCentro+ 'Oferta, udes lote:'+oferta.UdsXLote);
			...
			if(UdsXLoteFormat && !esEntero(Math.round((CantidadFormat/UdsXLoteFormat)*1000)/1000)){
				...
				//solodebug	console.log('compruebaUdsXLoteAgr, NO ENCAJAN: Producto, IDProductoLic:'+IDProductoLic+' IDCentro:'+IDCentro+' cant:'+Cantidad+'Oferta, udes lote:'+UdsXLoteFormat);
...
function calcularCantidadesAuto(){
	...
	jQuery.each(arrProductos, function(key, producto){
		...
		if((UdsXLoteFormat>0) && !esEntero(Math.round((CantidadFormat/UdsXLoteFormat)*1000)/1000)){
			...
			//console.log(cont+' '+producto.Nombre);
			//console.log(CantidadFormat+' vs '+UdsXLoteFormat);
			//console.log((CantidadFormat/UdsXLoteFormat));
			//console.log(Math.ceil(CantidadFormat/UdsXLoteFormat));
	...
	//solodebug 
	console.log('calcularCantidadesAuto:'+ListaProductos);
			...
			*/
			...
			jQuery.each(productoConOfertas.Ofertas, function(key2, oferta){
				...
				if(oferta.OfertaAdjud == 'S'){
					...
					//solodebug
					console.log('calcularCantidadesAutoAgr: Producto, IDProductoLic:'+IDProductoLic+' ref:'+productoPorCentro.RefCliente+' IDCentro:'+IDCentro+' cant:'+Cantidad+' IDCentro:'+IDCentro+ 'Oferta, udes lote:'+oferta.UdsXLote);
			...
			if(UdsXLoteFormat && !esEntero(Math.round((CantidadFormat/UdsXLoteFormat)*1000)/1000)){
				...
				//solodebug	console.log('calcularCantidadesAutoAgr:'+cont+' '+NombreProducto+' Cant:'+CantidadFormat+' Udes/Lote:'+UdsXLoteFormat+ ' ratio:'+(CantidadFormat/UdsXLoteFormat)+' ceil:'+(Math.ceil(CantidadFormat/UdsXLoteFormat))+' cant. corregida:'+newCantidad);
				...
				for (l=0;l<productoPorCentro.Centros.length;++l)
				{
					...
					console.log('calcularCantidadesAutoAgr: ['+l+ '] IDCentro:'+productoPorCentro.Centros[l].IDCentro+ ' Cant.:'+productoPorCentro.Centros[l].Cantidad+ ' Cant.Total Act:'+CantidadTotal);
				}

				//solodebug	console.log('calcularCantidadesAutoAgr:'+cont+' '+NombreProducto+' Cant:'+CantidadFormat+' Udes/Lote:'+UdsXLoteFormat+ ' Cant.Total	Ant:'+arrProductos[PosProducto].Cantidad+' Cant.Total Act:'+CantidadTotal);
...
function ActualizarCantidades(ListaProductos){
	var d = new Date();

	//solodebug 
	console.log('ActualizarCantidades:'+ListaProductos);
	...
	jQuery.ajax({
		...
		type:	"POST",
		async:	false,
		data:	"LIC_ID="+IDLicitacion+"&LISTA_PRODUCTOS="+encodeURIComponent(ListaProductos)+"&_="+d.getTime(),
		error: function(objeto, quepaso, otroobj){
			alert('error'+quepaso+' '+otroobj+' '+objeto);
		},
		success: function(objeto){
...
function ActualizarCantidadesAgr(ListaProductos){
	var d = new Date();
...
//				(mostrarPrecioIVA == 'S') ? PrecioOferta = oferta.PrecioIVA : PrecioOferta = oferta.Precio;
	...
	var error = 0, msg = '', importePedido, pedidoMinimo;
...
function dibujaSubirFT(ID){
	...
	thisMacro = thisMacro.replace('#ONCHANGE#', 'addDocFile(' + ID + ');');
	thisCell += thisMacro;
	thisCell += divEnd;
	thisCell += divEnd;
	thisCell += spanEnd;
...
function ComprobarCatalogoCliente(){
	var d = new Date();
...
function recuperaLugaresEntrega(IDUsuario){
	var d = new Date();

	jQuery.ajax({
		url:	"http://www.newco.dev.br/Gestion/Comercial/LugaresEntregaPedidoAJAX.xsql",
		data:	"IDUSUARIO="+IDUsuario+"&_="+d.getTime(),
		type:	"GET",
		contentType: "application/xhtml+xml",
		beforeSend: function(){
			null;
...
// 13jul16	Funcion (AJAX) que descarga la OC
function DescargarOC(){
	
	MostrarPagPersonalizada('http://www.newco.dev.br/Gestion/AdminTecnica/ExportacionOC.xsql?LIC_ID='+IDLicitacion,'',100,100,0,0);

/*	jQuery.ajax({
			url: 'http://www.newco.dev.br/Gestion/AdminTecnica/ExportacionOC.xsql',
			data: "LIC_ID="+IDLicitacion+"&_="+d.getTime(),
    	type: "GET",
...
function guardarDatosInforme(){
	...
	jQuery.ajax({
		...
		type:	"GET",
		data:	"LIC_ID="+IDLicitacion+"&LIC_INF_PRESENTACION="+Presentacion+"&LIC_INF_ANALISIS="+Analisis+"&LIC_INF_CONCLUSIONES="+Conclusiones+"&_="+d.getTime(),
		contentType: "application/xhtml+xml",
		error: function(objeto, quepaso, otroobj){
			alert('error'+quepaso+' '+otroobj+' '+objeto);
		},
...
function MostrarFloatingBox_EST(){
...
function actualizarRadio(IDProdLic, IDOfertaLic){
	var thisPosArr, 
...
function abrirCamposAvanzadosOfe(posArr){
	...
	if(IDDoc != ''){
		...
		jQuery("#camposAvanzados span#NombreDoc").html(nombreDoc);
...
function borrarDoc(tipo){
	...
	if(tipo == 'LIC_PRODUCTO_FT'){
		...
		valueID	= arrProductos[posArr].Oferta.ID;
	}
		...
		jQuery.ajax({
			...
			success: function(objeto){
				...
				if(data.CamposAvanzadosOfe.estado == 'OK'){
					recuperaDatosOfertaProductos(IDLicProveedor);
					alert(alrt_CamposAvanzadosOK);
					// Escondemos el formulario de campos avanzados
					showTabla(false);
        	                }else{
...
function guardarOfertaFila(thisPosArr){
	// Validaciones
	...
	if(!errores && esNulo(precioFormat)){
		...
		else if (!errores && (esNulo(oForm.elements['Marca_' + thisPosArr].value) || (oForm.elements['Marca_' + thisPosArr].value=='SIN OFERTAR'))){
			errores++;
...
/*
console.log(esNulo(UdsXLoteFormat));
console.log(isNaN(UdsXLoteFormat));
console.log(UdsXLoteFormat == 0 && precioFormat != 0);
console.log(UdsXLoteFormat);
...
*/
	...
	if(enviarOferta){
		...
		jQuery.ajax({
			...
			type:	"GET",
			data:	"LIC_ID="+IDLicitacion+"&LIC_PROD_ID="+IDLicProd+"&LIC_PROV_ID="+IDLicProv+"&REFPROV="+RefProv+"&DESC="+Descripcion+"&MARCA="+Marca+"&UDSXLOTE="+UdsXLote+"&CANTIDAD="+Cantidad+"&PRECIO="+Precio+"&TIPOIVA="+TipoIVA+"&IDFICHA="+IDFicha+"&_="+d.getTime(),
			beforeSend: function(){
				//17set16	jQuery("#BtnActualizarOfertas").hide();
				//17set16	jQuery(".guardarOferta").hide();
				jQuery('#BOTONGUARDAR_'+thisPosArr).hide();
			},
			error: function(objeto, quepaso, otroobj){
			...
			success: function(objeto){
				...
				if(data.OfertaActualizada.IDOferta > 0){
					...
					if(data.OfertaActualizada.TodosInformados == 'Si'){
						...
						//solodebug		alert('guardado ok');
					
						//error, "undefined"	arrProductos[thisPosArr].oferta.Precio=Precio;			//	ET 3set16 Faltaba actualizar la matriz
						//error, "undefined"	arrProductos[thisPosArr].oferta.UdsXLote=UdsXLote;		//	ET 3set16 Faltaba actualizar la matriz
						//error, "undefined"	arrProductos[thisPosArr].oferta.Marca=Marca;			//	ET 3set16 Faltaba actualizar la matriz
					
						ofertaProvInformada = 'S';
						ofertaVacia ='N';					//	ET	21ago17
						
						validarBotonPublicarOferta();
						MostrarFloatingBox_PROV();			//	ET 3set16 Faltaba actualizar la ventana de resumen
					}
				}else{
					//17set16	jQuery('tr#posArr_' + thisPosArr + ' .resultado').html('<img src="http://www.newco.dev.br/images/error.gif"/>');
					jQuery('#AVISOACCION_'+thisPosArr).html('<img src="http://www.newco.dev.br/images/error.gif"/>');
					
					if (data.OfertaActualizada.IDOferta==-2)	//	15nov17	Si se ha superado el plazo, recargamos la página para que se vea como ha quedado la oferta
						location.reload();
...
{
	//console.log('licCambiosEnOfertasSel:'+hayCambios);
...
function listadoExcelCompleto()
{
	...
	cadenaCSV=objLicitacion['Empresa']+': ('+objLicitacion['Codigo']+') '+objLicitacion['Titulo']+'\n\r\n\r'+strTitulosColumnasCSV;
	...
	for(var j=0; j<arrConsumoProvs.length; j++)
	{
		...
		{	//	COmprueba proveedor con ofertas /Mantenimiento/LICITACION/PRODUCTOSLICITACION/PROVEEDORESLICITACION/PROVEEDOR/TIENE_OFERTAS
			cadenaCSV+=arrConsumoProvs[j].NombreCorto+sepCSV;
		}
	...
	if(arrProductos.length > 0)
	{
		...
		for(var i=0; i<arrProductos.length; i++)
		{
			...
			if(arrProductos[ColumnaOrdenadaProds[i]].RefCliente != '')
			{
				thisRow = arrProductos[ColumnaOrdenadaProds[i]].RefCliente+sepCSV;	// Mostramos Ref.Cliente si existe
			}
			else
			{
				thisRow = arrProductos[ColumnaOrdenadaProds[i]].RefEstandar+sepCSV;	// Mostramos Ref.Estandar si no existe Ref.Cliente
			}

			// Col 2. Nombre
			thisRow += arrProductos[ColumnaOrdenadaProds[i]].Nombre+sepCSV;
			
			// Col 3. Numero de ofertas
			thisRow += arrProductos[ColumnaOrdenadaProds[i]].NumOfertas+sepCSV;

			// Col 4. Ud basica
			thisRow += arrProductos[ColumnaOrdenadaProds[i]].UdBasica+sepCSV;
			
			// Col 5. Precio hist
			if(mostrarPrecioIVA == 'S')
				thisRow += arrProductos[ColumnaOrdenadaProds[i]].PrecioHistIVA+sepCSV;
			else
				thisRow += arrProductos[ColumnaOrdenadaProds[i]].PrecioHist+sepCSV;
			
			// Col 6. Precio obj
			if(mostrarPrecioIVA == 'S')
			{
				if (arrProductos[ColumnaOrdenadaProds[i]].PrecioObjIVA != undefined)
					thisRow += arrProductos[ColumnaOrdenadaProds[i]].PrecioObjIVA;
			}
			else
			{
				if (arrProductos[ColumnaOrdenadaProds[i]].PrecioObj != undefined)
					thisRow += arrProductos[ColumnaOrdenadaProds[i]].PrecioObj;
			}
			thisRow += sepCSV;
			
			// Col 7. Ahorro
			thisRow += arrProductos[ColumnaOrdenadaProds[i]].AhorroMax+sepCSV;

			// Col 8. Cantidad
			thisRow += arrProductos[ColumnaOrdenadaProds[i]].Cantidad+sepCSV;

			// Col 8. Consumo
			if(mostrarPrecioIVA == 'S')
				thisRow += arrProductos[ColumnaOrdenadaProds[i]].ConsumoHistIVA+sepCSV;
			else
				thisRow += arrProductos[ColumnaOrdenadaProds[i]].ConsumoHist+sepCSV;

			// Celda/Columna TipoIVA
			thisCell = cellStartClass;
			if(IDPais != 55)
			{
				thisRow += arrProductos[ColumnaOrdenadaProds[i]].TipoIVA + '%'+sepCSV;
			}
			...
			//solodebug	if (i==0) 
			console.log(i+'/'+arrProductos[ColumnaOrdenadaProds[i]].Ofertas.length+' '+arrProductos[ColumnaOrdenadaProds[i]].IDProdLic);

			for(var j=0; j<arrProductos[ColumnaOrdenadaProds[i]].Ofertas.length; j++)
			{
			
				//solodebug
				console.log(j+' '+arrProductos[ColumnaOrdenadaProds[i]].IDProdLic+'	'+arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].IDProvLic
				...
				{	//	COmprueba proveedor con ofertas /Mantenimiento/LICITACION/PRODUCTOSLICITACION/PROVEEDORESLICITACION/PROVEEDOR/TIENE_OFERTAS
					...
					//solodebug	
					console.log(arrConsumoProvs[j].NombreCorto+' tiene ofertas:'+arrConsumoProvs[j].TieneOfertas
						+ ' NoInformada:'+arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].NoInformada
						+ ' NoOfertada:'+arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].NoOfertada
						);
					...
					{	// Si esta oferta esta informada
						thisRow += arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].Precio +sepCSV;
