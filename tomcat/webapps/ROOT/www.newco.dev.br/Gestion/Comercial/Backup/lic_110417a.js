...
function ValidarFormulario(oForm, nombre, flag){
	...
	if(nombre == 'productos'){
		...
		if((!errores) && (esNulo(listRefs))){
			...
			/*	Deactivamos, problemas si no hay cantidad o cantidad no separada correctamente
			...
			for (i=0;i<arrLista.length;++i)
			{
				if (arrLista[i]!='')
				{
					...
					if ((!jQuery.isNumeric(Cant))||(Restocadena!=''))
					{
						errores++;
...
function AnadirProductosAjax(oForm, Referencias, loop, loops, error)
{
	...
	jQuery.ajax({
		...
		success: function(objeto){
			...
			++loop;
			//console.log('AnadirProductosAjax: '+loop+'/'+loops+':'+cadEnvio);
