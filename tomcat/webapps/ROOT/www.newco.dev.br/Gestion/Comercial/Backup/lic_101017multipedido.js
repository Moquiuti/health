...
function NoIniciarLicitacion(IDEstado){

	//	6oct17	Forzamos revisión de productos informados, más seguro que hacee el control en cada modificación
	prodsInformados = 'S';
	for (j=0;((j<arrProductos.length) && (prodsInformados=='S'));++j)
	{
		//	solodebug	console.log('NoIniciarLicitacion. Comprobando producto:'+j+' Ud.Basica:'+arrProductos[j].UdBasica+' Cant:'+arrProductos[j].Cantidad);
		if ((arrProductos[j].UdBasica === undefined) || (arrProductos[j].UdBasica == '') || (arrProductos[j].Cantidad === undefined) || (arrProductos[j].Cantidad<=0))	prodsInformados='N';
	}
	...
	else
	{
		...
		if(prodsInformados == 'N')
			Mensaje+=alrt_NoIniciarLicProds
...
function AdjudicarOfertas(){
	...
	if(productosOlvidados == 'N'){
		...
		if(checkAdj){
			...
			//	CUIDADO REACTIVAR!!!!!!!!
			//
			if(confirmChk)	CambioEstadoLicitacion('ADJ');
