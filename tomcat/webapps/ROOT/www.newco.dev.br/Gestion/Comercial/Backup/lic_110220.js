...
//	Ultima revisión ET 13feb20 12:30 lic_110220.js
	...
	jQuery("#pes_lCentros").click(function(){
		...
		if(chk){
			...
			*/
			...
			//12nov19	prepararTablaProveedores();
		}
	});
...
function ValidarFormulario(oForm, nombre, flag){
	...
	if(nombre == 'datosGenerales')
	{
		...
		else
		{
			...
			//solodebug	debug('ValidarFormulario. datosGenerales. Comprobar fecha. Hoy:'+hoy+'...'+dd+'/'+mm+'/'+yyyy+' fecha dec.:'+oForm.elements['LIC_FECHADECISION'].value+' comparafechas:'+comparaFechas(oForm.elements['LIC_FECHADECISION'].value, dd+'/'+mm+'/'+yyyy));
	...
	//debug(comparaFechas(oForm.elements['LIC_FECHADECISION'].value, oForm.elements['LIC_FECHAADJUDICACION'].value));
		...
		debug('ValidarFormulario.datosGenerales. errores:'+errores+' msgError:'+msgError);
...
{
	debug('hasFilesDoc. form:'+form.name);
	for(var i=0; i<form.length; i++) {
	
		//if(form.elements[i].type == 'file' && (form.elements[i].name.substring(0,12) == 'inputFileDoc'))
		debug('hasFilesDoc. form:'+form.name+ ' element['+i+']:'+form.elements[i].name + ' value:'+form.elements[i].value);
		...
		jQuery.ajax({
			...
			success:function(data){
				...
				else if(valueID == 'SC')
				{
					...
					{
						//solodebug	debug("handleFileRequestDoc DOC_LICITACION. Actualizado OK. nombreDoc:"+nombreDoc);
