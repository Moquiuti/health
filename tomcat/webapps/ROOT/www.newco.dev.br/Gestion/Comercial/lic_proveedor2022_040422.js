...
//	Ultima revision ET 27may22 11:00 lic_proveedor2022_040422.js
...
function PreparaLineaProducto(numProd, contLinea)
{
	...
	//				thisCell += '<img src="" class="static"/>';
		...
		// Celda/Columna Nombre
	//27may22	thisCell = cellStartClassStyle.replace('#CLASS#','datosLeft');
	//27may22	thisCell = thisCell.replace('#STYLE#','padding:2px 0px 2px 4px;width:1000px;');
		thisCell = cellStartClass.replace('#CLASS#','datosLeft');
	//			thisCell += '<strong>' + arrProductos[ColumnaOrdenadaProds[numProd]].Nombre + '</strong>&nbsp;';
	//27may22	thisCell += '<strong>' + NombreProductoConEstilo(numProd) + ' ['+arrProductos[ColumnaOrdenadaProds[numProd]].Marcas+ ']</strong>&nbsp;';
		thisCell += NombreProductoConEstilo(numProd) + '&nbsp;<span class="fuentePeq">['+arrProductos[ColumnaOrdenadaProds[numProd]].Marcas+ ']</span>';
		
	//27may22	if ((arrProductos[ColumnaOrdenadaProds[numProd]].Marcas!='')||(arrProductos[ColumnaOrdenadaProds[numProd]].PrincActivo!=''))
	//27may22		thisCell += '<img src="http://www.newco.dev.br/images/2017/info.png" title="'+strPrincActivo+': '+arrProductos[ColumnaOrdenadaProds[numProd]].PrincActivo+'" class="static"/>'
		...
		*/
		...
		{
			//27may22 thisCell += '&nbsp;' + macroEnlaceIDStyle.replace('#ID#', 'btnAvanz_'+(Indice)).replace('#HREF#', functionJS).replace('#STYLE#', isNaN(valAux)?'display:none':'');//display:block
			//27may22 thisCell += macroImagen.replace('#SRC#', 'http://www.newco.dev.br/images/anadir.gif').replace('#TITLE#', str_AnyadirInfoAmpliada).replace('#ALT#', str_AnyadirInfoAmpliada);
...
function listadoExcelProv()
{
	...
	cadenaCSV=strTitulosColumnasCSVProv+saltoLineaCSV;
