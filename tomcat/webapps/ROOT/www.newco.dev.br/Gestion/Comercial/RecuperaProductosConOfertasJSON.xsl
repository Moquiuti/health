...
<xsl:template match="/Productos">
	...
	<xsl:text>"ListaProveedores":[</xsl:text>
      ...
      <xsl:text>",</xsl:text>
  		<xsl:text>"OfertasAdj":"</xsl:text>
  			<xsl:value-of select="LIC_PROV_OFERTASADJUDICADAS"/>
      <xsl:text>",</xsl:text>
    	<xsl:text>"OfertasAdjAux":"</xsl:text>
    		<xsl:value-of select="LIC_PROV_OFERTASADJUDICADAS"/>
			<xsl:text>"}</xsl:text>
	...
	<xsl:text>"ListaProductos":[</xsl:text>
		<xsl:for-each select="PRODUCTOSLICITACION/PRODUCTO">
			...
			<xsl:text>"Sospechoso":"</xsl:text>
				<xsl:choose>
				<xsl:when test="MUY_SOSPECHOSO">
					<xsl:text>2</xsl:text>
				</xsl:when>
				<xsl:when test="SOSPECHOSO">
					<xsl:text>1</xsl:text>
				</xsl:when>
				<xsl:otherwise>
					<xsl:text>0</xsl:text>
			...
			<xsl:text>",</xsl:text>
			<xsl:text>"AhorroMaximo":"</xsl:text>
				<xsl:value-of select="AHORRO_MAX"/>
			<xsl:text>",</xsl:text>
			<xsl:text>"SinAhorro":"</xsl:text>
				<xsl:choose><xsl:when test="AVISO_SIN_AHORRO">S</xsl:when><xsl:otherwise>N</xsl:otherwise></xsl:choose>
			<xsl:text>",</xsl:text>
			...
			<xsl:text>"Ofertas":[</xsl:text>
				<xsl:for-each select="OFERTA">
					...
					<xsl:text>"InfoAmpliadaOferta":"</xsl:text>
						<xsl:copy-of select="LIC_OFE_INFOAMPLIADA_JS/node()"/>
					<xsl:text>",</xsl:text>
