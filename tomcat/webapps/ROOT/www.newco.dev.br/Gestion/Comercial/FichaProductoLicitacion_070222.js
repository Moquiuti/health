...
//	Ultima revision ET 07feb22 16:40 FichaProductoLicitacion_070222.js
...
{
	//solodebug console.log('onloadEvents2. OfertasSeleccionadas:'+OfertasSeleccionadas()+' IDMOTIVOSELECCION:'+jQuery("#IDMOTIVOSELECCION").val());
...
function GuardarProductoSel(IDProdLic, Accion){
	...
	if ((MostrarMotivoSeleccion=='S')&&(gl_PrecioOfertaActual>gl_MejorPrecio)&&(IDMotivo=='')){
	...
	//solodebug	console.log('OfertasSeleccionadas. LicMultiOpcion:'+LicMultiOpcion+' multOfertas:'+multOfertas+ 'cantAdjud:'+cantAdjud+'CantidadTotal:'+CantidadTotal);
	...
	if(msgError == '')
	{
		...
		{
			if ((LicMultiOpcion!='S')&&(adjudOfertas!=''))
			{

				//solodebug	
				console.log('OfertasSeleccionadas. LicMultiOpcion:'+LicMultiOpcion+' adjudOfertas:'+adjudOfertas+ 'cantAdjud:'+cantAdjud+'CantidadTotal:'+CantidadTotal+' -> Adjudicación repartida');
					...
					success: function(objeto){
						...
						if(data.Resultado.Estado == 'OK'){
							...
							try
							{
								...
								*/
								window.opener.informaCantidades(IDProdLic, adjudOfertas, 'S');
							}
							catch(e)
							{
...
function chCantidadAdjudicada (ID)
{
	...
	//solodebug
	console.log('chCantidadAdjudicada. Comprobando ID:'+ID);
...
function ActualizarCentrosYProveedores()
{
	...
	var html='<tr class="subTituloTabla" id="cabTablaCentros">'+jQuery('#cabTablaCentros').html()+'</tr>';
	
	for (var i=0;i<arrCentros.length;++i)
	{
		html+='<tr class="gris">'
			+		'<td>'+arrCentros[i].linea+'&nbsp;</td>'
			+		'<td class="textLeft">'+arrCentros[i].Centro+'&nbsp;</td>'
			+		'<td class="textLeft">'+arrCentros[i].Marcas+'&nbsp;</td>'
			+		'<td class="textLeft">&nbsp;</td>'
			+		'<td style="align:left;"><input name="Cantidad_'+arrCentros[i].IDCentro+'" id="Cantidad_'+arrCentros[i].IDCentro+'" value="'+arrCentros[i].CantidadSF+'" class="cantidad" size="8" maxlength="8" type="text" onKeyUp="javascript:chCantidadCentro('+arrCentros[i].IDCentro+')"/></td><td class="acciones"><a id="btnCantidad_'+arrCentros[i].IDCentro+'" href="javascript:guardarDatosCompraCentro('+arrCentros[i].IDCentro+')" class="btnDestacado guardarCant" style="text-decoration:none;display:none;">'+str_Guardar+'</a></td><td id="Resultado_'+arrCentros[i].IDCentro+'" class="resultado">&nbsp;</td>'
			+		'<td>&nbsp;</td>'
			+	'</tr>';
		...
		for (var j=0;j<arrCentros[i].Proveedores.length;++j)
		{
			...
			html+='<tr class="gris">'
				...
				+		'<td style="align:left;"><input name="Cantidad_'+arrCentros[i].IDCentro+'_'+arrCentros[i].Proveedores[j].IDOfertaLic+'" id="Cantidad_'+arrCentros[i].IDCentro+'_'+arrCentros[i].Proveedores[j].IDOfertaLic+'" value="'+arrCentros[i].Proveedores[j].CantidadSF+'" class="cantidad" size="8" maxlength="8" type="text" onKeyUp="javascript:chCantidadCentroYProv('+arrCentros[i].IDCentro+','+arrCentros[i].Proveedores[j].IDOfertaLic+')"/></td><td class="acciones"><a id="btnCantidad_'+arrCentros[i].IDCentro+'_'+arrCentros[i].Proveedores[j].IDOfertaLic+'" href="javascript:guardarDatosCompraCentroYProv('+arrCentros[i].IDCentro+','+arrCentros[i].Proveedores[j].IDOfertaLic+','+arrCentros[i].Proveedores[j].IDProveedorLic+')" class="btnDestacado guardarOferta" style="text-decoration:none;display:none;">'+str_Guardar+'</a></td><td id="Resultado_'+arrCentros[i].IDCentro+'_'+arrCentros[i].Proveedores[j].IDOfertaLic+'" class="resultado">&nbsp;</td>'
				+		'<td>&nbsp;</td>'
				+	'</tr>';
		}
	}
