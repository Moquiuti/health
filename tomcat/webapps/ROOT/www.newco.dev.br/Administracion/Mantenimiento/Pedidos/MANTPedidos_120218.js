...
//	Ultima revision: ET 27feb18 11:00
...
//	23nov17	Cargar documento para adjuntarlo al pedido
function cargaDoc()
{
    var form=document.forms['frmMantenPedido'];
...
function periodicUpdateDoc(){
	...
	if(uploadFrameDoc && uploadFrameDoc.document && uploadFrameDoc.document.getElementsByTagName("p")[0]){
		var tipoDocHTML	= document.forms['frmMantenPedido'].elements['TIPO_DOC_HTML'].value;
...
//	var action = 'http://' + location.hostname + '/' + lang + 'confirmCargaDocumento.xsql';
	...
	var tipoDocDB	= document.forms['frmMantenPedido'].elements['TIPO_DOC_DB'].value;
	var tipoDocHTML	= document.forms['frmMantenPedido'].elements['TIPO_DOC_HTML'].value;
...
//		form.target	= target;
		...
		jQuery.ajax({
			...
			success:function(data){
				...
				var tipo = document.forms['frmMantenPedido'].elements['TIPO_DOC_HTML'].value;

				//vaciamos la carga documentos
				document.forms['frmMantenPedido'].elements['inputFileDoc'+tipo] = '';
...
//				var proveedor = document.forms['frmMantenPedido'].elements['IDPROVEEDOR'].value;
				...
				jQuery('#docBox').empty().append('&nbsp;&nbsp;&nbsp;&nbsp;<a href="http://' + location.hostname + '/Documentos/' + fileDoc + '" target="_blank">' + nombreDoc + '</a>').show();
...
function ActualizarComentario(){
    ...
    var jsForm=document.forms['MensajeJS'];
    var mo_id = form.elements['MO_ID'].value;
    var comentario = form.elements['MO_COMENTARIOS'].value;
    ...
    jQuery.ajax({
        ...
        type:	"GET",
        data:	"MO_ID="+mo_id+"&COMENTARIOS="+encodeURI(comentario),
        contentType: "application/xhtml+xml",
        success: function(objeto){
            var data = eval("(" + objeto + ")");
