...
<!--
	Cabecera de la ficha de empresaNuevo disenno 2022.
	Ultima revision: ET 9mar23 09:30 EMPDetalleCabecera2022_100223.js
-->
...
<head>
	...
	<title><xsl:value-of select="/CabeceraEmpresa/EMPRESA/EMPRESA" disable-output-escaping="yes"/></title>
	...
	<script type="text/javascript" src="http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/EMPDetalleCabecera2022_100223.js"></script>
	...
	<div class="ZonaTituloPagina">
		<p class="TituloPagina">
			<xsl:value-of select="document($doc)/translation/texts/item[@name='empresa']/node()"/>:&nbsp;
 			<xsl:choose>
				<xsl:when test="(EMPRESA/MVM or EMPRESA/ADMIN or EMPRESA/COMERCIAL or EMPRESA/CDC) and EMPRESA/EMPRESAS/field">
					...
					<xsl:with-param name="claSel">w600px fuenteGrande</xsl:with-param>
					<!--<xsl:with-param name="style">width:600px;font-size:20px;height:28px</xsl:with-param>-->
					</xsl:call-template>
					&nbsp;
					<span class="amarillo">EMP_ID:&nbsp;<xsl:value-of select="EMPRESA/IDEMPRESA"/></span>
				</xsl:when>
				<xsl:otherwise><xsl:value-of select="EMPRESA/EMPRESA" disable-output-escaping="yes"/></xsl:otherwise>
			</xsl:choose>
			<span class="CompletarTitulo">
				
				<xsl:if test="EMPRESA/IDEMPRESADELUSUARIO=19638"><!-- Para usuarios MVMC permitimos abrir la ficha completa-->
					<a href="javascript:FichaCompleta()" class="btnNormal"><xsl:value-of select="document($doc)/translation/texts/item[@name='Ficha_completa_proveedor']/node()"/></a>&nbsp;
				</xsl:if>
