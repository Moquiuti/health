...
<head>
	...
	<xsl:choose>
		<xsl:when test="/LogsEmpresa/LANG"><xsl:value-of select="/LogsEmpresa/LANG" /></xsl:when>
		<xsl:otherwise>spanish</xsl:otherwise>
	</xsl:choose>  
	</xsl:variable>
	<xsl:variable name="doc">http://www.newco.dev.br/General/texts_<xsl:value-of select="$lang"/>.xml</xsl:variable>        <!--idioma fin-->
        
	<title><xsl:value-of select="/LogsEmpresa/LOGS/EMPRESA"/>:&nbsp;<xsl:value-of select="document($doc)/translation/texts/item[@name='Ver_logs']/node()"/></title>
    <!--style-->
    ...
    <xsl:choose>
		...
		<div class="ZonaTituloPagina">
			<p class="TituloPagina">
				<xsl:value-of select="document($doc)/translation/texts/item[@name='Ver_logs']/node()"/>:&nbsp;<xsl:value-of select="/LogsEmpresa/LOGS/EMPRESA"/>
				&nbsp;&nbsp;
				<span class="CompletarTitulo">
					<a class="btnNormal" title="Ficha Empresa" style="text-decoration:none;">
						<xsl:attribute name="href">javascript:chMantenUsuario(<xsl:value-of select="/LogsEmpresa/LOGS/IDEMPRESA"/>);</xsl:attribute>
						<xsl:value-of select="document($doc)/translation/texts/item[@name='volver']/node()"/>
					</a>
				</span>
			</p>
		</div>
        <xsl:if test="/LogsEmpresa/LOGS/LOG">
			...
			<table cellspacing="6px" cellpadding="6px">
				...
				<tbody class="corpo_tabela">
	       		<xsl:for-each select="/LogsEmpresa/LOGS/LOG">
				<tr class="conhover">
