...
//	Ultima revision: ET 27feb23 12:15 EMPManten2022_270223.js

jQuery(document).ready(globalEvents);

function globalEvents(){

	if (BloquearMaestro=='S') jQuery('#IDCATALOGOMAESTRO').attr("disabled", true);
	...
	// Recupero bien los saltos de linea del area text comercial, que no esta funcionando correctamente
	if(jQuery('textarea[name="EMP_REFERENCIAS"]').length > 0)
	{
		...
		jQuery('textarea[name=EMP_REFERENCIAS]').val(jQuery('textarea[name=EMP_REFERENCIAS]').val().replace(/<br xmlns:xsl=\"http:\/\/www.w3.org\/1999\/XSL\/Transform\">/gi,'\n'));	//7jul22
		
		//console.log(jQuery('textarea[name=EMP_REFERENCIAS]').val());
	}
}//fin de globalEvents
...
/*function SeleccionaLogos(IDPROVE,ID,ACCION){
	...
	var ACTION="http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/LogosEmpresa2022.xsql";;
...
function SeleccionaLogos(IDProveedor,ID,Accion)
{
	...
	jQuery.ajax({
		url:"http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/LogosEmpresa.xsql",
...
function ModificarMinimoPorCliente(idCliente,idCentro, accion)
{
	...
	document.location="http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/PedidoMinimoPorCliente2022.xsql"+opciones;
}
...
//abrir la pagina de coste trasporte por cliente
function CosteTransportePorCliente(){
	...
	document.location="http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/CosteTransportePorCliente2022.xsql"+opciones;
}
...
function  Comentarios (Opciones)
{
    ...
    MostrarPag('http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/Comentarios2022.xsql'+Parametros,'Comentarios');
};
...
{
	form=document.forms['MantenEmpresa'];
	
	if(validarFormulario(form)){

		form.elements['EMP_PROVINCIA'].value = form.elements['PROVINCIA'].value;

		form.elements['ACCION'].value = 'MODIFICAR';				//19abr20

		form.action = "EMPManten2022.xsql";								//19abr20
		...
		if ((tipo=='GUARDAR')||((tipo=='ACTIVAR')&&(confirm(strConfirmarActivar))))
		{
	        SubmitForm(form);
		}
	}
...
function validarFormulario(form){
	...
	*/
	...
	if((document.forms[0].elements['EMP_POTENCIAL_COMPRAS'])&&(document.forms[0].elements['EMP_POTENCIAL_COMPRAS'].value!='') && (!checkRegEx(document.forms[0].elements['EMP_POTENCIAL_COMPRAS'].value, regex_potcompras)) ){
		errores=1;
	...
	if((document.forms[0].elements['EMP_POTENCIAL_CATALOGO'])&&(document.forms[0].elements['EMP_POTENCIAL_CATALOGO'].value!='') && (!checkRegEx(document.forms[0].elements['EMP_POTENCIAL_CATALOGO'].value, regex_potcat)) ){
		errores=1;
		alert(document.forms['MensajeJS'].elements['FORMATO_NUMERO'].value);
...
*/
	...
	//	19abr20 Nuevos campos para GPO
	if((!errores) && (document.forms[0].elements['EMP_ESCATALOGOMAESTRO_CHK'])){
		if(document.forms[0].elements['EMP_ESCATALOGOMAESTRO_CHK'].checked==true)
			document.forms[0].elements['EMP_ESCATALOGOMAESTRO'].value='on';
		else
			document.forms[0].elements['EMP_ESCATALOGOMAESTRO'].value='';
	}

	//	19abr20 Nuevos campos para GPO
	if((!errores) && (document.forms[0].elements['EMP_ADJUDICACIONESDECATPADRE_CHK'])){
		if(document.forms[0].elements['EMP_ADJUDICACIONESDECATPADRE_CHK'].checked==true)
			document.forms[0].elements['EMP_ADJUDICACIONESDECATPADRE'].value='on';
		else
			document.forms[0].elements['EMP_ADJUDICACIONESDECATPADRE'].value='';
	}
...
/*		7nov22 Ocultamos "bloquear muestras"
//09-4-13 si es cliente cojo valor de EMP_BLOQUEARMUESTRAS_CHK cliente
    ...
    /*	7nov22 Ya no es necesario este campo
    ...
    //3feb23 Nuevo campo:EMP_AUTORIZARTODOSCENTROS (se habia perdido este bloque de codigo, no se guardaba el campo)
	if ((!errores) && (document.forms[0].elements['EMP_AUTORIZARTODOSCENTROS_CHK'])){
		if(document.forms[0].elements['EMP_AUTORIZARTODOSCENTROS_CHK'].checked==true){
			document.forms[0].elements['EMP_AUTORIZARTODOSCENTROS'].value='on';
        }
		else
			document.forms[0].elements['EMP_AUTORIZARTODOSCENTROS'].value='';
	}
    ...
    // 4mar21 Nuevo campo:EMP_LIC_SEPARARCATEGORIA
	if ((!errores) && (document.forms[0].elements['EMP_LIC_SEPARARCATEGORIA_CHK'])){
		if(document.forms[0].elements['EMP_LIC_SEPARARCATEGORIA_CHK'].checked==true){
			document.forms[0].elements['EMP_LIC_SEPARARCATEGORIA'].value='on';
        }
		else
			document.forms[0].elements['EMP_LIC_SEPARARCATEGORIA'].value='';
	}

    // 31mar20 Nuevo campo:EMP_PED_DIFERENCIARDEPOSITO
	if ((!errores) && (document.forms[0].elements['EMP_PED_DIFERENCIARDEPOSITO_CHK'])){
		if(document.forms[0].elements['EMP_PED_DIFERENCIARDEPOSITO_CHK'].checked==true){
			document.forms[0].elements['EMP_PED_DIFERENCIARDEPOSITO'].value='on';
        }
		else
			document.forms[0].elements['EMP_PED_DIFERENCIARDEPOSITO'].value='';
	}
	...
	//	Proveedor no navegable por defecto
	...
	//	5may21
	if((!errores) && (document.forms[0].elements['EMP_SEGUIMIENTOPEDIDOSDESDEMVM_CHK'])){
		if(document.forms[0].elements['EMP_SEGUIMIENTOPEDIDOSDESDEMVM_CHK'].checked==true)
			document.forms[0].elements['EMP_SEGUIMIENTOPEDIDOSDESDEMVM'].value='on';
		else
			document.forms[0].elements['EMP_SEGUIMIENTOPEDIDOSDESDEMVM'].value='';
	}

	/*//	30set21
	if((!errores) && (document.forms[0].elements['EMP_AVISARATODOSSEGUNDERECHOS_CHK'])){
		if(document.forms[0].elements['EMP_AVISARATODOSSEGUNDERECHOS_CHK'].checked==true)
			document.forms[0].elements['EMP_AVISARATODOSSEGUNDERECHOS'].value='on';
		else
			document.forms[0].elements['EMP_AVISARATODOSSEGUNDERECHOS'].value='';
	}*/


	//	30set21
	if((!errores) && (document.forms[0].elements['EMP_PED_AVISARATODOS_CHK'])){
		if(document.forms[0].elements['EMP_PED_AVISARATODOS_CHK'].checked==true)
			document.forms[0].elements['EMP_PED_AVISARATODOS'].value='on';
		else
			document.forms[0].elements['EMP_PED_AVISARATODOS'].value='';
	}

	//	13dic21
	if((!errores) && (document.forms[0].elements['EMP_LIC_AVISARATODOS_CHK'])){
		if(document.forms[0].elements['EMP_LIC_AVISARATODOS_CHK'].checked==true)
			document.forms[0].elements['EMP_LIC_AVISARATODOS'].value='on';
		else
			document.forms[0].elements['EMP_LIC_AVISARATODOS'].value='';
	}

	//	13dic21
	if((!errores) && (document.forms[0].elements['EMP_LIC_COMPARTIR_CHK'])){
		if(document.forms[0].elements['EMP_LIC_COMPARTIR_CHK'].checked==true)
			document.forms[0].elements['EMP_LIC_COMPARTIR'].value='on';
		else
			document.forms[0].elements['EMP_LIC_COMPARTIR'].value='';
	}
    ...
    //var f=checkRegEx(inputText[i], regex_car_raros); alert ('valor f '+f);
        if(checkRegEx(inputText[i], regex_car_raros)){
			...
			console.log('Caracter incorrecto, entrada ['+i+']:'+inputText[i]);			//16mar21
			cadError=inputText[i];
            errores=1;
            raros=1;
        }
        else { }
...
function pedidoMinimo(nombre,form){
  if(nombre=="EMP_PEDMINIMOACTIVO_CHECK"){
    if(form.elements[nombre].checked==true){
      ...
      //7nov22	form.elements['EMP_PEDMINIMO_LOGISTICA'].disabled=false;
      ...
      //7nov22	form.elements['EMP_PEDMINIMO_LOGISTICA'].value='';
      //7nov22	form.elements['EMP_PEDMINIMO_LOGISTICA'].disabled=true;
...
//	25nov19 Copia el logo a todos los centros
function btnCopiarLogo()
{
	var d = new Date();
	
	var IDEmpresa=jQuery('#IDEMPRESA').val();
	
	console.log('btnCopiarLogo. IDEmpresa:'+IDEmpresa);

	jQuery.ajax({
		url:"CopiarLogoACentrosAJAX.xsql",
		data: "EMP_ID="+IDEmpresa+"&_="+d.getTime(),
		type: "GET",
		async: false,
		contentType: "application/xhtml+xml",
...
//	28dic22 Abre la pagina con los logs de la empresa
function verLogs(){
	var IDEmpresa=jQuery('#IDEMPRESA').val();
	document.location="http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/LogsEmpresa2022.xsql?EMP_ID="+IDEmpresa;
}
