	...
	<xsl:choose>
		...
		<thead>
			...
			<tr class="select">
				...
				<td colspan="2">
					<xsl:copy-of select="document($doc)/translation/texts/item[@name='lineas_por_pagina']/node()"/>Líneas por página:&nbsp;
					<xsl:call-template name="desplegable">
	...
	<xsl:when test="/BuscadorCentros/CENTROS/CENTRO">
		...
		<tfoot>
			...
			<tr>
				<td colspan="14">
					<div class="botonCenter">
						<a href="javascript:Continuar();" title="Buscar los usuarios correspondientes a las empresas seleccionadas"><xsl:value-of select="document($doc)/translation/texts/item[@name='buscar_usuarios']/node()"/></a>
					</div>
				</td>
			</tr>
		</tfoot>
