...
//	ultima revision: ET 25may22 19:07 EMPDocs2022_220222.js. Cuidado, en caso de cambios actualizar tambien EMPNuevaSimpleHTML
  ...
  if(msg != ''){
    ...
    jQuery.ajax({
      ...
      beforeSend:function(){
		...
		jQuery('#confirmBoxDocEmpresa').hide();
      ...
      success:function(data){
		...
		jQuery('#confirmBoxDocEmpresa').show();
...
function revisarDocumento(IDDocumento)
{
	...
	//solodebug 
	console.log('revisarDocumento:'+IDDocumento+'. estado: '+stat);
	...
	//solodebug 
	console.log('revisarDocumento:'+IDDocumento+'. nuevo estado: '+stat);
	...
	jQuery.ajax({
		...
		success:function(data){
			...
			//solodebug 
			console.log('revisarDocumento:'+IDDocumento+': '+stat+': '+color+'. Res AJAX: '+data);
		}
	});
...
function DibujaTablas()
{
	...
	{
		//solodebug debug('DibujaTablas ('+i+'):'+arrCategorias[i].Nombre+' docs:'+arrCategorias[i].NumDocs+'='+arrCategorias[i].Documentos.length);
...
{
	//solodebug	debug('DibujaTabla. Categoria:'+Categoria.Nombre+':'+Categoria.Documentos.length);
	...
	var strHiddenStatus='<input type="hidden" id="st_[ID]" value="[NOMBRECOLOR]"/>';
	...
	var strCabecera='<div class="divLeft textCenter space40px">'
					+'<span class="tituloTabla">[TITULO]</span>'
					...
					+'<th class="w1px">&nbsp;</th>'
					+'<th>'+strUsuario+'</th>'
					+'<th class="w200px">'+strFecha+'</th>'
					+'<th class="w200px">'+strVencimiento+'</th>'
					+'<th>'+strUsuarioRev+'</th>'
					+'<th class="w100px">'+strFechaRev+'</th>'
					+'<th class="w1px"></th>';
	
	if (v_BorrarOfertas=='S')
		strCabecera+='<th class="uno"></th>';//25may22 Quitamos la palabra "borrar" '+strBorrar+'
		
	strCabecera+='</tr></thead><tbody class="corpo_tabela">';

	var strLineaDoc='<tr class="conhover"><td id="doc_[ID]" style="background:[COLOR]">[CONTADOR]</td><td class="textLeft">&nbsp;<a href="http://www.newco.dev.br/Documentos/[URL]">[NOMBRETIPO]</a><br><span class="fuentePeq">[NOMBRE]</span></td>'
    	+'<td class="textLeft"><a href="javascript:DocsEmpresa([IDEMPRESA]);">[CENTROOEMPRESA]</a></td><td></td><td class="textLeft fuentePeq">[USUARIO]</td><td align="left"><input type="text"  class="campopesquisa w120px" name="fecha_[ID]" value="[FECHAALTA]" maxlength="10" id="fecha_[ID]">'
		+'<img src="http://www.newco.dev.br/images/2022/refresh-button.png" class="imgMiddle" alt="Actualizar Fecha" onclick="javascript:ActualizarFechaOferta(\'[ID]\',\'[IDEMPRESA]\',\'[TIPO]\',\'\');">'
    	+'</td><td align="left"><input type="text" class="campopesquisa w120px" name="fechaFinal_[ID]" value="[FECHACAD]" maxlength="10" id="fechaFinal_[ID]">'
		+'<img src="http://www.newco.dev.br/images/2022/refresh-button.png" class="imgMiddle" alt="Actualizar Fecha final" onclick="javascript:ActualizarFechaOferta(\'[ID]\',\'[IDEMPRESA]\',\'[TIPO]\',\'\');">'
		+'&nbsp;[CADUCADO]</td><td class="textLeft fuentePeq">[USUARIOREV]</td><td>[FECHAREV]</td><td><a id="btnEstado_[ID]" class="btnDestacado" href="javascript:revisarDocumento([ID]);">Rev.</a>'
    	+'<div id="waitBoxEst_[ID]" class="gris" style="display:none; margin-top:5px;">'+strCargando+'...&nbsp;</div>'
    	+'</td>';
		
	if (v_BorrarOfertas=='S')	
		strLineaDoc+='<td><a href="javascript:EliminarDocumento(\'[ID]\',\'[TIPO]\',\'\');"><img src="http://www.newco.dev.br/images/2022/icones/del.svg" alt="eliminar"></a><div id="waitBoxOferta_[ID]" class="gris" style="display:none; margin-top:5px;">'+strCargando+'...&nbsp;</div></td>';
	...
	var strPieYFinTabla='<tfoot class="rodape_tabela"><tr><td colspan="11">&nbsp;</td></tr></tfoot></table></div></div>';
	...
	for (var i=0;i<Categoria.Documentos.length;++i)
	{
		...
		linea=linea.replace(/\[NOMBRETIPO\]/g,Categoria.Documentos[i].NombreTipo).replace(/\[IDEMPRESA\]/g,Categoria.Documentos[i].IDEmpresa);
		linea=linea.replace(/\[CENTROOEMPRESA\]/g,Categoria.Documentos[i].CentroOEmpresa).replace(/\[USUARIO\]/g,Categoria.Documentos[i].Usuario);
		...
		linea=linea.replace(/\[CADUCADO\]/g,Caducado).replace(/\[NOMBRECOLOR\]/g,Categoria.Documentos[i].Color).replace(/\[COLOR\]/g,color);
