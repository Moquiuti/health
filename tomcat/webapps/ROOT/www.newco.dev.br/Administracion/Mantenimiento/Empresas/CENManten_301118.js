// JavaScript Document
//	Ultima revision: ET 30nov18 09:45
...
function ActualizarNuevoCentro(form){
	...
	if(validarFormulario(form))
	{
		...
		form.action = Path+"CENMantenSave.xsql";	//"CENNuevoSave.xsql";
...
function ActualizarDatos(form){
	if(validarFormulario(form))
	{
		...
		form.action = Path+"CENMantenSave.xsql";
		form.enctype = "application/x-www-form-urlencoded";
		form.method = "post";
		form.target = '';
...
function NuevoLugarEntrega(){
	...
	MostrarPag(Path+'LugaresEntrega.xsql?CEN_ID='+idCentro+
		'&LUGAR_ID=0'+
...
function NuevoCentroConsumo(){
	...
	MostrarPag(Path+'CentrosConsumo.xsql?CEN_ID='+idCentro+
		'&CENTROCONSUMO_ID=0'+
		'&ACCION=','centroconsumo');
}

function EditarLugarEntrega(cen_id, lugar_id){
	MostrarPag(Path+'LugaresEntrega.xsql?CEN_ID='+cen_id+
		'&LUGAR_ID='+lugar_id+
		'&ACCION=EDITAR', 'lugaresentrega');
}

function EditarCentroConsumo(cen_id, centroconsumo_id){
	MostrarPag(Path+'CentrosConsumo.xsql?CEN_ID='+cen_id+
