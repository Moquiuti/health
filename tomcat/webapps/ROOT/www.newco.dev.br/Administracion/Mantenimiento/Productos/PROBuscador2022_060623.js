//	JS Buscador cat.proveedores
// 	Ultima revision ET 6jun23 11:37 PROBuscador2022_060623.js
...
function globalEvents(){
	...
	jQuery("#elimOrdenacion").click(function(){
		...
		formu.action = 'PROBuscador2022.xsql';
		SubmitForm(formu);
	});
}//fin de globalEvents
...
function Busqueda(formu,accion){
	...
	//8mar22	window.parent.showLoading();
	SubmitForm(formu);
}
...
function soloUnProd(ref){
	...
	Busqueda(document.forms['formResultados'],'PROResultados2022.xsql');
}
		...
		if(isMVM == 'S'){
			if(document.forms['formBusca'].elements['PRODUCTO'].value == '' && document.forms['formBusca'].elements['FIDPROVEEDOR'].value == '0' && document.forms['formBusca'].elements['IDCLIENTE'].value == '-1'){
				//if(confirm(document.forms['MensajeJS'].elements['TEXT_BUSCA_CONFIRM'].value))
					Busqueda(document.forms[0],'./PROResultados2022.xsql');
			}else
				Busqueda(document.forms[0],'./PROResultados2022.xsql');
		}else
			Busqueda(document.forms[0],'./PROResultados2022.xsql');
...
/*	Movido a PROResultados2022_080322.js
...
function OrdenarPor(Orden){
	...
	if(form.elements['ORDEN'].value==Orden)
		if(form.elements['SENTIDO'].value=='ASC')	
			form.elements['SENTIDO'].value='DESC';
		else						
...
function AplicarFiltro(){
	...
	form.action='PROResultados2022.xsql';
	// Ejecutamos la funcion que muestra el loading...
	//	8mar22	window.parent.showLoading();
...
function NuevaAltaProveedor(IDProveedor){
    ...
    ventanaOrigen.location = 'http://www.newco.dev.br/Administracion/Mantenimiento/Productos/PROManten2022.xsql?PROVEEDOR='+IDProveedor;
}


//	10jul18 Listado Excel
function DescargarExcel()
{
	var	oForm=document.forms['formBusca'];
	
	var producto=oForm.elements['PRODUCTO'].value, idcliente='', idproveedor;

	if (oForm.elements['FIDPROVEEDOR'])	//
		idproveedor=oForm.elements['FIDPROVEEDOR'].value;
	else
		idproveedor=oForm.elements['IDPROVEEDOR'].value;
	
	

	var modifProv='',rechazados='',docOk='',docPendAprobar='',docPendiente='',esPack='',enPack='',sinPrivados='',sinEmplantillar='',sinPrecioMVM='',sinOfertaMVM='',sinOfertaFNCP='',sinOfertaRecol='';
	
	if (jQuery('#IDCLIENTE').length>0) idcliente=oForm.elements['IDCLIENTE'].value;
	
	if ((jQuery('#ESPACK').length>0)&&(oForm.elements['ESPACK'].checked)) esPack='S';
	if ((jQuery('#ENPACK').length>0)&&(oForm.elements['ENPACK'].checked)) enPack='S';
	if ((jQuery('#DOC_OK').length>0)&&(oForm.elements['DOC_OK'].checked)) docOk='S';
	if ((jQuery('#DOC_PEND_APROBAR').length>0)&&(oForm.elements['DOC_PEND_APROBAR'].checked)) docPendAprobar='S';
	if ((jQuery('#DOC_PENDIENTE').length>0)&&(oForm.elements['DOC_PENDIENTE'].checked)) docPendiente='S';
	if ((jQuery('#RECHAZADOS').length>0)&&(oForm.elements['RECHAZADOS'].checked)) rechazados='S';
	if ((jQuery('#MODIFICADOSPROV').length>0)&&(oForm.elements['MODIFICADOSPROV'].checked)) modifProv='S';
	
	if ((jQuery('#SINPRIVADOS').length>0)&&(oForm.elements['SINPRIVADOS'].checked)) sinPrivados='S';
	if ((jQuery('#SINEMPLANTILLAR').length>0)&&(oForm.elements['SINEMPLANTILLAR'].checked)) sinEmplantillar='S';
	if ((jQuery('#SINPRECIOMVM').length>0)&&(oForm.elements['SINPRECIOMVM'].checked)) sinPrecioMVM='S';
	if ((jQuery('#SINOFERTAMVM').length>0)&&(oForm.elements['SINOFERTAMVM'].checked)) sinOfertaMVM='S';
	if ((jQuery('#SINOFERTAFNCP').length>0)&&(oForm.elements['SINOFERTAFNCP'].checked)) sinOfertaFNCP='S';
	if ((jQuery('#SINOFERTARECOLETAS').length>0)&&(oForm.elements['SINOFERTARECOLETAS'].checked)) sinOfertaRecol='S';
	
	var param="IDCLIENTE="+idcliente+"&IDPROVEEDOR="+idproveedor+"&PRODUCTO="+producto+"&MODIFICADOSPROV="+modifProv+"&RECHAZADOS="+rechazados
						+"&DOC_OK="+docOk+"&DOC_PEND_APROBAR="+docPendAprobar+"&DOC_PENDIENTE="+docPendiente
						+"&ESPACK="+esPack+"&ENPACK="+enPack+"&SINPRIVADOS="+sinPrivados+"&SINEMPLANTILLAR="+sinEmplantillar+"&SINPRECIOMVM="+sinPrecioMVM+"&SINOFERTAMVM="+sinOfertaMVM
						+"&SINOFERTAFNCP="+sinOfertaFNCP+"&SINOFERTARECOLETAS="+sinOfertaRecol;
						
	//solodebug console.log('DescargarExcel:'+param);

	jQuery.ajax({
		url:	"http://www.newco.dev.br/Administracion/Mantenimiento/Productos/PROResultados_Excel.xsql",
		data:	param,
		type:	"GET",
...
//	23ago18 Deja activo un unico check de documentacion
function checkDocumentacion(activo)
{
	isChecked=jQuery('#'+activo).is(':checked');			//	1jul22 antes:.attr('checked')
	
	jQuery('.documentacion').prop('checked', false);
	jQuery('#'+activo).prop('checked', isChecked);
}
