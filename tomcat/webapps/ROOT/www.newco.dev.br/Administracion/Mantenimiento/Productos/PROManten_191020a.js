...
// Ultima revisión: ET 22ene20 PROManten_191020a.js
...
function EliminarDocumentoDeProducto(doc){
	...
	if(confirm(document.forms['mensajeJS'].elements['SEGURO_ELIMINAR_DOCUMENTO'].value)){
		...
		jQuery.ajax({
			url:"http://www.newco.dev.br/Administracion/Mantenimiento/Productos/confirmEliminaDocumentoProd.xsql",
			data: "IDUSUARIO="+usuario+"&IDDOCUMENTO="+docu+"&IDPRODUCTO="+producto,
			type: "GET",
			contentType: "application/xhtml+xml",
			beforeSend:function(){
...
function addFile(id){
	...
	cargaImagen(document.forms['form1'],'{$tipo}');	//	22ene20
...
function handleFileRequest(resp){
	...
	var UrlAccion = 'http://www.newco.dev.br/Administracion/Mantenimiento/Productos/PROMantenSave.xsql';
	...
	if(resp instanceof Array && resp.length > 0){
		...
		if(msg == ''){
			...
			//redundante con la línea anterior AsignarAccion(document.forms['form1'],UrlAccion);
