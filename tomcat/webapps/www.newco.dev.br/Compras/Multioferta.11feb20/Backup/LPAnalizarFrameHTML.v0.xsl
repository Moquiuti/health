...
<head>
	...
	<script type="text/javascript" src="http://www.newco.dev.br/Compras/Multioferta/LPAnalizar.260515.js"></script>
...
//		var msgTodasCantidadesMismoSigno='';
		...
		// Variables globales que me indican que tipo de plantilla utilizar
		var IDPlantilla;
		var IDPais;
		var IDIdioma;
		...
		accion			= '<xsl:value-of select="/Analizar/ANALISIS/ACCION"/>';
...
*/
		...
		var str_NoSePuedeEnviarOferta	= '<xsl:value-of select="document($doc)/translation/texts/item[@name='no_se_puede_enviar_oferta']/node()"/>';
		var str_FechaPagoMayor		= '<xsl:value-of select="document($doc)/translation/texts/item[@name='fecha_pago_mayor']/node()"/>';
		var str_FechaLimiteRespuesta	= '<xsl:value-of select="document($doc)/translation/texts/item[@name='fecha_limite_respuesta']/node()"/>';
		var str_FechaEntregaMayor	= '<xsl:value-of select="document($doc)/translation/texts/item[@name='fecha_entrega_mayor']/node()"/>';
		var str_FechaEntregaMayorFechaLimite	= '<xsl:value-of select="document($doc)/translation/texts/item[@name='fecha_entrega_mayor_fecha_limite']/node()"/>';
...
<xsl:for-each select="/Analizar/ANALISIS/LINEAS/LINEA_ROW">
		...
		lineas['PrecioProdDeterminado']	= '<xsl:value-of select="PRECIO_PRODUCTO_DETERMINADO"/>';
	...
	<thead>
		<tr class="titulosNoAlto">
			...
			<th align="left">
				...
				<span style="padding-left:30px;"><input type="text" name="strFiltro" id="strFiltro" size="30"/></span>
			</th>
		...
		<xsl:if test="$OcultarPrecioReferencia='N' or /Analizar/ANALISIS/IDPAIS = '55'">
			...
			<!--<th class="zerouno">precio ha bajado o subido&nbsp;</th>-->
		...
		<xsl:otherwise>
			...
			<!-- importe total linea-->
			<th class="uno">
				<!-- DC - 14may15 - Quitamos enlace y funcionalidad para marcar desmarcar todos los checkboxes
...
<!--
				...
				<input type="hidden" name="DESCRIPCION_PRODUCTO_{PLL_ID}">
					<xsl:choose>
					<xsl:when test="NOMBREPRIVADO!=''">
						<xsl:attribute name="value"><xsl:value-of select="NOMBREPRIVADO"/></xsl:attribute>
					</xsl:when>
					<xsl:otherwise>
...
-->
	...
	<xsl:if test="/Analizar/ANALISIS/IDPAIS = '55'">
		...
		<tr>
			<td colspan="7">
<!-- DC - 14may15 - Ya tenemos busquedas por filtro en la tabla
       ...
       <thead>
			<tr class="titulosNoAlto"> <!-- ref mvm + provee -->
            		<th class="ocho" align="left">
                    	...
                    	<xsl:choose>
                        <xsl:when test="LINEA_ROW/PRODUCTOS/PRODUCTOS_ROW[1]/PRO_CATEGORIA = 'F' and /Analizar/ANALISIS/UTILIZAR_REFERENCIA_PROVEEDOR">
                        	11&nbsp;<xsl:value-of select="document($doc)/translation/texts/item[@name='codigo_nacional']/node()"/>
                        </xsl:when>
                         <xsl:otherwise>
                         	<xsl:value-of select="document($doc)/translation/texts/item[@name='ref_cliente']/node()"/>.
                        </xsl:otherwise>
