...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="/">
	...
	{
		document.location.href='http://www.newco.dev.br/Compras/PedidosProgramados/PedidosProgramados.xsql?PEDP_ID='+IDPrograma+'&ACCION=CAMBIOESTADO&ESTADO='+Estado;
	}
        ...
        <xsl:variable name="doc">http://www.newco.dev.br/General/texts_<xsl:value-of select="$lang"/>.xml</xsl:variable>        <!--idioma fin-->
            ...
            <div class="divLeft"> <!--     style="border-top:1px solid grey;border-bottom:1px solid grey;"      -->
                ...
                <tr class="subTituloTabla">
		          	<td align="center" class="diez">
						<xsl:copy-of select="document($doc)/translation/texts/item[@name='proxima_entrega_2line']/node()"/>
					</td>
                  
                	<td class="quince textLeft">
                    	<xsl:value-of select="document($doc)/translation/texts/item[@name='centro']/node()"/>
					</td>
                	<!--quitado 17-12-2015   <td align="center" class="dies"><xsl:value-of select="document($doc)/translation/texts/item[@name='usuario']/node()"/></td>-->

                    <td class="cinco"><xsl:copy-of select="document($doc)/translation/texts/item[@name='numero']/node()"/></td>
                    <td class="dos">&nbsp;</td>
                	<td class="textLeft"><xsl:value-of select="document($doc)/translation/texts/item[@name='nombre']/node()"/></td>
                	<td class="seis"><xsl:copy-of select="document($doc)/translation/texts/item[@name='inicio']/node()"/></td>
                	<td class="seis"><xsl:copy-of select="document($doc)/translation/texts/item[@name='fin']/node()"/></td>
                	<xsl:choose><!--si es proveedor no enseño proveedor-->
                    	<xsl:when test="/PedidosProgramados/LISTAPEDIDOSPROGRAMADOS/ROL = 'VENDEDOR'">
                       		<td class="uno">&nbsp;</td>
                        </xsl:when>
                        <xsl:otherwise>
                        	 <td class="dies textLeft">&nbsp;<xsl:value-of select="document($doc)/translation/texts/item[@name='proveedor']/node()"/></td>
                        </xsl:otherwise>
                    </xsl:choose>
                    <xsl:choose><!--si es proveedor no enseño si esta visible-->
                        ...
                        <xsl:otherwise>
                        	 <td class="tres"><xsl:value-of select="document($doc)/translation/texts/item[@name='visible_proveedor']/node()"/></td>
                        </xsl:otherwise>
                    </xsl:choose>
                	...
                	<xsl:choose><!--si es proveedor no enseño confirmar-->
                        ...
                        <xsl:otherwise>
                        	<td class="tres"><xsl:value-of select="document($doc)/translation/texts/item[@name='confirmar']/node()"/></td>
                        </xsl:otherwise>
                    </xsl:choose>
				  ...
				  <xsl:choose>
					...
					<xsl:otherwise><!--es editable el programa-->
						...
						<xsl:choose>
							...
							<xsl:when test="INACTIVO='N'"><a href="javascript:CambiarEstadoPrograma({ID},'DESACTIVADO');"><img src="http://www.newco.dev.br/images/suspender.gif" border="0"/></a></xsl:when>
						</xsl:choose>
					  </xsl:otherwise>
					</xsl:choose>
				</td>
                <td class="textLeft">  
                <a href="javascript:MostrarPagPersonalizada('http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/CENDetalle.xsql?ID={IDCENTRO}&amp;VENTANA=NUEVA','DetalleEmpresa',100,80,0,0);">
                ...
                <xsl:otherwise>
                    &nbsp;
                	<a href="javascript:MostrarPagPersonalizada('http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/EMPDetalle.xsql?EMP_ID={IDPROVEEDOR}&amp;ESTADO=CABECERA&amp;VENTANA=NUEVA','DetalleEmpresa',100,80,0,0);" onmouseover="window.status='Ver información sobre el proveedor';return true;">
            	...
            	<td style="text-align:right;"><xsl:value-of select="SUBTOTAL"/>&nbsp;<xsl:value-of select="document($doc)/translation/texts/item[@name='eur']/node()"/></td>
 ...
 <!--idioma-->
		...
		<table class="grandeInicio">
				...
				<tr>
					...
					<td>&nbsp;<a href="javascript:MostrarPagPersonalizada('http://www.newco.dev.br/Administracion/Mantenimiento/Productos/PRODetalle.xsql?PRO_ID={IDPRODUCTO}','producto',100,80,0,0)">
