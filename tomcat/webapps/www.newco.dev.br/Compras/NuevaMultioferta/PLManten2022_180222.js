...
//	ultima revisión: ET 18feb22 09:30 PLManten2022_180222.js


//	19nov19 Cambio de la plantila de trabajo
function cbPlantillaChange()
{
 	var formu = document.forms[0];
	formu.elements['PL_ID'].value=formu.elements['IDPLANTILLA'].value;
	formu.elements['ACCION'].value='';
    SubmitForm(formu);
}
...
function EmpresasAutorizadas(){
  ...
  if(form.elements['IDPLANTILLA'].value==0){
    alert(msgSinPlantilla);
  }else{
    var opciones='?PL_ID='+ form.elements['IDPLANTILLA'].value;
...
function MostrarTodos(Tipo, ID)
{
	//solodebug
	console.log('MostrarTodos:'+Tipo+':'+ID);

 	var formu = document.forms[0];
	formu.elements['ACCION'].value='MOSTRARTODOS'+Tipo;
	formu.elements['PARAMETROS'].value=ID;
    SubmitForm(formu);
}

// 18nov19 Ocultar todos los productos al usuario
function OcultarTodos(Tipo, ID)
{
	//solodebug
	console.log('OcultarTodos:'+Tipo+':'+ID);

 	var formu = document.forms[0];
	formu.elements['ACCION'].value='OCULTARTODOS'+Tipo;
	formu.elements['PARAMETROS'].value=ID;
    SubmitForm(formu);
}

// 18nov19 Bloquear todos los productos al usuario
function BloquearTodos(Tipo, ID)
{
	//solodebug
	console.log('BloquearTodos:'+Tipo+':'+ID);

 	var formu = document.forms[0];
	formu.elements['ACCION'].value='BLOQUEARTODOS'+Tipo;
	formu.elements['PARAMETROS'].value=ID;
    SubmitForm(formu);
}

// 18nov19 Mostrar todos los productos al usuario
function MostrarEnCentro(IDCentro)
{
	//solodebug
	console.log('MostrarEnCentro:'+IDCentro);

 	var formu = document.forms[0];
	formu.elements['ACCION'].value='MOSTRARENCENTRO';
	formu.elements['PARAMETROS'].value=IDCentro;
    SubmitForm(formu);
}

// 18nov19 Ocultar todos los productos al usuario
function OcultarEnCentro(IDCentro)
{
	//solodebug
	console.log('OcultarEnCentro:'+IDCentro);

 	var formu = document.forms[0];
	formu.elements['ACCION'].value='OCULTARENCENTRO';
	formu.elements['PARAMETROS'].value=IDCentro;
    SubmitForm(formu);
}


//	20nov19 Cambia a la página de lista de productos de la plantilla
function ListaProductosPlantilla()
{
 	var formu = document.forms[0];
	var IDPlantilla=formu.elements['PL_ID'].value;
	window.location.href="http://www.newco.dev.br/Compras/Multioferta/LPLista2022.xsql?PL_ID="+IDPlantilla;
}
