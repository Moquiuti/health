...
<!--
	...
	Ultima revision:ET 9jun23 08:22 ZonaPlantilla2022_290523.js
-->
...
<head>
	...
	<script type="text/javascript" src="http://www.newco.dev.br/Compras/NuevaMultioferta/ZonaPlantilla2022_290523.js"></script>
...
<xsl:text disable-output-escaping="yes"><![CDATA[
			...
			if(puedeCrearPlantillas=='S'){
				if(carp_actual==-1){
					...
					objFrame.location.href='../Multioferta/PLManten2022.xsql?BOTON=NUEVA';
				}
			}else{
...
<body class="areaizq" onLoad="montarDesplegable(arrayPlantillas,document.forms[0].elements['IDPLANTILLA'],'{//ZonaPlantilla/AREAPLANTILLAS/PLANTILLAS/field/@current}');CargarListaProductos(document.forms[0].elements['IDCARPETA'].value, document.forms[0].elements['IDPLANTILLA'].value,'','');">
	...
	<xsl:otherwise>
		<form action="ZonaPlantilla2022.xsql" name="plantillas" method="POST">
		...
		<xsl:if test="/ZonaPlantilla/AREAPLANTILLAS/IDPORTAL = 'MVM' and /ZonaPlantilla/AREAPLANTILLAS/SEPARAR_FARMACIA">
			...
			<xsl:if test="ZonaPlantilla/AREAPLANTILLAS/PLANTILLAS/EDICION">
				<tr>
					...
					<!--	9jun23 Desactivamos el boton de borrar plantilla, muy peligroso
					<td>
			...
			<xsl:if test="ZonaPlantilla/AREAPLANTILLAS/VERCARPETAS">
				<tr>
					...
					<td colspan="3" height="25px;" align="center" valign="bottom">
						<a href="javascript:PrepararOfertaPedido('DIRECTO');" title="Preparar pedido" class="btnDestacado w150px"> <xsl:value-of select="document($doc)/translation/texts/item[@name='preparar_pedido']/node()"/></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					</td>
					<td>&nbsp;</td>
				</tr>
			</xsl:if>
			...
			<xsl:otherwise>
				<!--PREPARAR PEDIDO. El "a" se debee alinear desde el elemento superior-->
				<div class="textCenter"><a class="btnDestacado w150px" href="javascript:PrepararOfertaPedido('DIRECTO');" title="Preparar pedido"> <xsl:copy-of select="document($doc)/translation/texts/item[@name='preparar_pedido']/node()"/></a></div>
			</xsl:otherwise>
			</xsl:choose>
