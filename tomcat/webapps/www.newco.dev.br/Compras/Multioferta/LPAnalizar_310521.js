...
//	Última revisión: ET 31may21 18:00 LPAnalizar_310521.js
...
function globalEvents(){
	...
	console.log('globalEvents. LP_ID:'+LP_ID);
...
function PreparaPedido()
{
	...
	//	ET	26may17	
	//solodebug
	console.log("PreparaPedido:"+arrayLineasProductos.length+' tipoPlantilla:'+tipoPlantilla);
		...
		jQuery.ajax({
			...
			success: function(objeto)
			{
				...
				for (var i=0;i<data.Lineas.length;++i)
				{
					...
					/*
					lineas['LLP_ID']		= data.Lineas[i].LLP_ID;
					lineas['DescripcionLinea']	=  data.Lineas[i].DescripcionLinea;
					lineas['ReferenciaLinea']	=  data.Lineas[i].ReferenciaLinea;
					...
					for (var j=0;i<data.Lineas[i].Productos.length;++j)
					{
						var producto = data.Lineas[i].Productos[j];
...
function dibujaLineasPedido(){
	...
	//solodebug	console.log('dibujaLineasPedido. OcultarNoSeleccionados:'+v_OcultarNoSeleccionados);
...
//	jQuery.each(arrayLineasProductos, function(key, linea){
	jQuery.each(ColumnaOrdenadaProds, function(key, value){
		...
		//solodebug	console.log('dibujaLineasPedido ('+value+'):'+arrayLineasProductos[value].Mostrar);
		...
		jQuery.each(arrayLineasProductos[value].Productos, function(key2, producto){
			...
			if (producto.Bonificado=='S')
			{
				...
				console.log('Bonificado:'+htmlBonif);
			}
...
function Seleccionar(obj){
	...
	//solodebug	
	console.log('Seleccionar. IDDivisa:'+IDDivisa);
	...
	if (!jQuery("input.SELECCION[name='SELECCION_" + posArr + "']").attr('checked'))
	{
		...
		//solodebug	
		console.log('Seleccionar. IDDivisaProducto:'+arrayLineasProductos[ColumnaOrdenadaProds[posArr]].Productos[0].IDDivisa);
...
{
	//solodebug	console.log('revisaDivisaActiva');
	...
	{
		//solodebug	console.log('revisaDivisaActiva arrayLineasProductos('+i+') IDDivisa:'+arrayLineasProductos[i].Productos[0].IDDivisa+' Seleccionado:'+arrayLineasProductos[i].Productos[0].Seleccionado);
	...
	//solodebug	console.log('revisaDivisaActiva IDDivisa:'+IDDivisa);
}
...
{
	//solodebug	console.log('mostrarSegunDivisa IDDivisa:'+IDDivisa+' poslugarEntrega:'+poslugarEntrega+' PedMin:'+arrayLugaresEntrega[poslugarEntrega][9]);
...
{
	console.log('cambiaADivisaProducto pos:'+pos);
...
function enviarPedido(){
	...
	//	22ene21	Control del tipo de cambio en caso de divisa!=0
	if (IDDivisa!=0)
	{
...
function Actua(){
	...
	var	Especial='',LineaEspecial, IDControlDivisa='';	//	24ene18	Control no juntar productos regulados y no regulados. 11ene21 Una unica divisa
	...
	jQuery.each(arrayLineasProductos, function(key, linea){
		if(linea.Productos[0].Seleccionado == 'S'){
			...
			//solodebug	console.log('Actua DivProducto:'+linea.Productos[0].IDDivisa+' IDControlDivisa:'+IDControlDivisa);			
			//console.log('Actua .IDDivisa:'+IDDivisa);
	...
	//solodebug	console.log('Actua:['+selectCantidades+']');
...
function ExportarExcel()
{
	...
	jQuery.each(arrayLineasProductos, function(key, linea){
		if(linea.Productos[0].Seleccionado == 'S'){
			var Cant= linea.Productos[0].Cantidad_SinFormato;
			var Ref	= linea.Productos[0].RefCliente;
