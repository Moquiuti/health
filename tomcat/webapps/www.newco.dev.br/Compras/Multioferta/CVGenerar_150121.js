...
//	ultima revision: ET 15ene21 18:30 CVGenerar_150121.js
...
function globalEvents(){
	...
	//solodebug	
	console.log(ComentarioPorDefecto);
	...
	jQuery('#comentParametros').html('<strong>'+ComentarioDeParametros.replace(/\n/g,'<BR/>')+'</strong>');

	//solodebug	
	console.log(ComentarioDeParametros);
...
function EnviarPedidos(formPrincipal, linka, formPedido){
	...
	if (conMotivoPorOrdenNo1=='S')
	{
		...
		for(var i=0; i<formPrincipal.length; i++)
		{
			...
			console.log('EnviarPedidos nom:'+nom);
	...
	for(var i=0; i<formPrincipal.length; i++){
		...
		if(nom.indexOf("COMENTARIO_") != -1){
			...
			coments	+= "|" + mo_id + "#" + ComentarioDeParametros + '\n\n'+ formPrincipal.elements['COMENTARIO_'+mo_id].value + "#" + formPrincipal.elements['NUMERO_OFERT_PED_'+mo_id].value + "#";
