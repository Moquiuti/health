...
<!--
	Listado básico de pedidos, llamado desde el EIS mensual y alarmas
	ultima revision: ET 13abr20 18:12
-->
									...
									<tr>
										<xsl:choose>
											<xsl:when test="IDINCIDENCIA!=''">
												<xsl:attribute name="class">conhover claroAlerta</xsl:attribute>
											</xsl:when>
											<xsl:otherwise>
												<xsl:attribute name="class">conhover claro</xsl:attribute>
											</xsl:otherwise>
										</xsl:choose>
										<!--bloqueado-->
										<td>
