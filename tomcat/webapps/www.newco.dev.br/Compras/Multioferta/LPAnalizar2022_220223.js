...
//	Ultima revision: ET 23feb23 11:10 LPAnalizar2022_220223.js
...
function cargaProductosAjax(Pag)
{
	...
	else
	{
		...
		jQuery.ajax({
			cache:	false,
			url:	'http://www.mvmnucleo.com/Compras/Multioferta/LPProductosPlantillaAjax.xsql',
			type:	"GET",
...
var macroInputText2	= '<input type="text" size="#SIZE#" name="#NAME#" id="#ID#" class="#CLASS#" value="#VALUE#" style="#STYLE#" maxlength="#MAXLENGTH#" onBlur="#ONBLUR#" onInput="#ONINPUT#" #DISABLED#/>';
...
function dibujaLineasPedido(){
	...
	if (v_OcultarNoSeleccionados=='SI')
	{
		...
		thisRow += thisCell;
		thisCell = cellHAlignColClassStart.replace('#ALIGN#', 'left').replace('#CLASS#', 'encabezadoBloque').replace('#COLSPAN#', '10');
...
//	jQuery.each(arrayLineasProductos, function(key, linea){
	jQuery.each(ColumnaOrdenadaProds, function(key, value){
		...
		jQuery.each(arrayLineasProductos[value].Productos, function(key2, producto){
			...
			var avisoOrden= '';
			if (producto.Orden>1) avisoOrden=' <img src="http://www.mvmnucleo.com/images/bolaAmbar.gif" title="Orden:'+producto.Orden+'"/>';
            ...
            if (producto.Image != '')
			{
                thisCell += '<img src="http://www.mvmnucleo.com/images/fotoListadoPeq.gif" id="'+producto.IDImage+'" onmouseover="verFoto(\''+producto.IDImage+'\');" onmouseout="verFoto(\''+producto.IDImage+'\');" /> ';
                thisCell += '<div id="verFotoPro_'+producto.IDImage+'" clas="divFotoProBusca" style="display:none;" ><img src="http://www.mvmnucleo.com/Fotos/'+producto.Image+'" /> ';
			}
			thisCell += cellEnd;
			thisRow += thisCell;
			...
			{
				htmlBonif='<img src="http://www.mvmnucleo.com/images/info.gif" title="'+str_BonifCantCompra+':'+producto.CantBonif+'. '+str_BonifCantObs+':'+producto.CantGratis+'">';
				...
				*/
				...
				if (producto.TarAnt_Importe!='')
				{
					if (producto.TarAnt_Cambio=='SUBE')
						thisCell += '<img src="http://www.mvmnucleo.com/images/order_a_rojo.gif" title="'+producto.TarAnt_Fecha+':'+producto.TarAnt_Importe+'"/>';
					else if (producto.TarAnt_Cambio=='BAJA')
						thisCell += '<img src="http://www.mvmnucleo.com/images/order_d_verde.gif" title="'+producto.TarAnt_Fecha+':'+producto.TarAnt_Importe+'"/>';
				}
			...
			if(tipoPlantilla == 'viamedNuevo' || tipoPlantilla == 'nuevoModeloViejoPedido' || tipoPlantilla == 'nuevoModeloNuevoPedido'){
				...
				if(ocultarPrecioReferencia=='N'){
					if(producto.VariacionPrecio == 'Caro'){
						thisCell += '<img src="http://www.mvmnucleo.com/images/order_a_rojo.gif"/>';
					}else if(producto.VariacionPrecio == 'Barato'){
						thisCell += '<img src="http://www.mvmnucleo.com/images/order_d_verde.gif"/>';
					}
			...
			thisMacro = thisMacro.replace('#ID#', 'NuevaCantidad_' + key);
			...
			//22feb23 functionJS= 'javascript:UnidadesALotes(this);';
			//thisMacro = thisMacro.replace('#ONBLUR#', 'UnidadesALotes('+key+');');
			thisMacro = thisMacro.replace('#ONBLUR#', 'javascript:RevisarSeleccionados();');
			//22feb23 functionJS= 'javascript:Seleccionar(this);';
			thisMacro = thisMacro.replace('#ONINPUT#', 'javascript:Seleccionar('+key+');');
			...
			//22feb23 thisMacro = thisMacro.replace('#ONCLICK#', 'clickCheckbox(this);');
...
function RevisarSeleccionados()
{
	...
	{
		if (jQuery("input.SELECCION[name='SELECCION_" + i + "']").is(':checked'))
		{
...
function UnidadesALotes(posArr){
	...
	//solodebug	debug('UnidadesALotes. INICIO. posArr:'+posArr+' udsXLote:'+udsXLote+' txtUds:'+txtUds+' uds:'+uds);

	if(tarifa=="")	hayPrecio=0;

	if(isNaN(uds) || (uds=="") ||(!esEnteroConSigno(txtUds))){
		alert(strCantidadIncorrecta);
		uds=1;
		jQuery('#NuevaCantidad_'+posArr).val(1);
	}

	//solodebug	debug('UnidadesALotes. 1. udsXLote:'+udsXLote+' uds:'+uds);
	...
	//solodebug	debug('UnidadesALotes. FINAL. udsXLote:'+udsXLote+' uds:'+uds+' -> nuevasUds:'+nuevasUds);
...
//22feb23 function Seleccionar(obj){
function Seleccionar(posArr){
	//22feb23 var posArr	= obj.name.substr(14, obj.name.length);

	//solodebug	debug('Seleccionar. IDDivisa:'+IDDivisa);
	
	if (!jQuery("input.SELECCION[name='SELECCION_" + posArr + "']").is(':checked')) 	// 1jul22 .attr('checked')
	{
		jQuery("input.SELECCION[name='SELECCION_" + posArr + "']").prop('checked', true);
	...
	jQuery.each(ColumnaOrdenadaProds, function(key, value){
		if(jQuery("#NuevoImporte_" + key).length < 0){
			if(arrayLineasProductos[value].Productos[0].Seleccionado == 'S'){
				...
				jQuery("input.SELECCION[name='SELECCION_" + key + "']").prop('checked',false);		//1jul22 attr
			}
		}else{
...
//22feb23 function clickCheckbox(obj){
function clickCheckbox(posArr){
//22feb23 	var posArr	= obj.name.replace('SELECCION_', '');
//22feb23 	var isChecked	= jQuery(obj).is(':checked');
...
function verDetalleProducto(idProd){
    ...
    jQuery("#detalleProd tbody").empty();
     ...
     if(idProd != ''){
        jQuery.ajax({
		url: 'http://www.mvmnucleo.com/Administracion/Mantenimiento/Productos/PRODetalleAJAX.xsql',
		data: "PRO_ID="+idProd,
                type: "GET",
                contentType: "application/xhtml+xml",
                beforeSend: function(){
                ...
                success: function(objeto){
                    ...
                    if(data.Producto.imagen != ''){
                        tbodyProd +=  "<td rowspan='4'><img src='http://www.mvmnucleo.com/Fotos/"+data.Producto.imagen+"' /></td>";
                    }
                    tbodyProd +="</tr>";
		...
		if (Ref!='')
		{
			...
			for (j=0;((j<arrayLineasProductos.length) && (!Encont));++j)
			{
				...
				if ((arrayLineasProductos[ColumnaOrdenadaProds[j]].Productos[0].RefCliente==Ref)||(arrayLineasProductos[ColumnaOrdenadaProds[j]].Productos[0].RefPrivada==Ref))
				{
					...
					if (!jQuery("input.SELECCION[name='SELECCION_" + ColumnaOrdenadaProds[j] + "']").is(':checked')) 	// 1jul22 .attr('checked')
					{
						jQuery("input.SELECCION[name='SELECCION_" + ColumnaOrdenadaProds[j] + "']").prop('checked', true);
