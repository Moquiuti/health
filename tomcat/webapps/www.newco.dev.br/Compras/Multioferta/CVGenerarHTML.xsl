...
<!--	
	Segundo paso de la creación del pedido: condiciones particulares
	ultima revision: ET 2jul21 10:00  CVGenerar_290421.js
-->
...
<!--	<script type="text/javascript" src="http://www.newco.dev.br/General/jquery.js"></script>-->
	...
	<script type="text/javascript" src="http://www.newco.dev.br/Compras/Multioferta/CVGenerar_290421.js"></script>
	...
	<xsl:otherwise>
		<div class="divleft">
			...
			<div class="ZonaTituloPagina">
				<p class="Path"><xsl:value-of select="document($doc)/translation/texts/item[@name='Inicio']/node()"/>&nbsp;/&nbsp;<xsl:value-of select="document($doc)/translation/texts/item[@name='Pedidos']/node()"/>&nbsp;/&nbsp;<span class="FinPath"><xsl:value-of select="document($doc)/translation/texts/item[@name='enviar_pedido']/node()"/></span>
					<!--solodebug <span class="CompletarTitulo amarillo" style="width:200px;">
					LP_ID:<xsl:value-of select="/Generar/MULTIOFERTAS/LP_ID"/>
					</span>-->
				</p>
				<p class="TituloPagina">
					...
					<span class="CompletarTitulo" style="width:500px;">
						<!--	Incluir los botones	-->
						<!--	10feb20 Volver atrás da muchos problemas debido a las recargas
			...
			<table class="buscador">
				<tr class="sinLinea">
					...
					<td class="treinta">
						...
						<xsl:choose>
						<xsl:when test="MULTIOFERTAS/CENTRALCOMPRAS">
							<select name="IDCENTRO" style="width:400px;height:20px;font-size:15px;"  onChange="inicializarDesplegableCentros(this.value);"/>
						</xsl:when>
						<xsl:otherwise>
							<input type="hidden" name="IDCENTRO" value="{/Generar/MULTIOFERTAS/MULTIOFERTAS_ROW[1]/MO_IDCENTROCLIENTE}"/>
						</xsl:otherwise>
						</xsl:choose>
							<select name="IDLUGARENTREGA" style="height:20px;font-size:15px;" onChange="ActualizarTextoLugarEntrega(this.value);"/>
						</p>
					</td>
				...
				<tr class="sinLinea">
					...
					<td>&nbsp;
						<!--	7feb20 Sin desplegable de plazos
		...
		<div class="divleft">
			...
			<!--	Quitamos el pie de página con los datos de contacto de MVM
...
<xsl:for-each select="MULTIOFERTAS_ROW">
	...
	<table class="buscador" border="0" style="margin-top:40px;">
	...
	<xsl:if test="/Generar/MULTIOFERTAS/MULTIOFERTAS_ROW/MO_OBSERVACIONES != ''">
		...
		<tr class="sinLinea">
			...
			<xsl:otherwise>
				...
				<br/>
				<!--LP_ID:<xsl:value-of select="/Generar/MULTIOFERTAS/LP_ID"/>...<xsl:value-of select="PARAMETRO1"/><xsl:value-of select="PARAMETRO2"/><xsl:value-of select="PARAMETRO3"/>-->
				<textarea name="COMENTARIO_{MO_ID}" class="COMENTARIO" cols="70" rows="10">
					<!--	15ene21 No desactivamos el espacio de comentarios
					<xsl:if test="PARAMETRO1!='' or PARAMETRO2!='' or PARAMETRO3!=''">
						<xsl:attribute name="disabled">disabled</xsl:attribute>
					</xsl:if>
					-->
					<!-- Se actualiza desde JS
					<xsl:copy-of select="COMENTARIOPORDEFECTO/COMENTARIO"/>...<xsl:value-of select="PARAMETRO1"/><xsl:value-of select="PARAMETRO2"/><xsl:value-of select="PARAMETRO3"/>
					<xsl:if test="PARAMETRO1!=''">
						<xsl:text disable-output-escaping="yes"><![CDATA[	
			...
			<br/>
			<a href="http://www.newco.dev.br/Compras/Multioferta/CVGenerar_MO.xsql?LP_ID={/Generar/MULTIOFERTAS/LP_ID}">Recarga</a>
			</td>
			<td rowspan="4" class="dos">&nbsp;</td>
	...
	<tr>
		...
		<xsl:when test="LMO_CATEGORIA = 'F'">
			...
			<xsl:when test="REFERENCIACLIENTE != ''">
				...
				<input type="hidden" id="REF_{LMO_ID}" value="{REFERENCIACLIENTE}"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="PRO_REFERENCIA"/>
				<input type="hidden" id="REF_{LMO_ID}" value="{PRO_REFERENCIA}"/>
			</xsl:otherwise>
			</xsl:choose>
		</xsl:when>
		<xsl:when test="LMO_CATEGORIA = 'N'">
			...
			<xsl:when test="REFERENCIAPRIVADA!=''">
				...
				<input type="hidden" id="REF_{LMO_ID}" value="{REFERENCIAPRIVADA}"/>
			</xsl:when>
			</xsl:choose>
		</xsl:when>
		</xsl:choose>
