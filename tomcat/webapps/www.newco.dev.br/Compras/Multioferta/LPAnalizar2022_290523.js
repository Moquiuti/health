...
//	Ultima revision: ET 16may23 11:30 LPAnalizar2022_290523.js
...
function inicializarDesplegableCentros(IDCentro){
	...
	for(var n=0; n<arrayCentros.length; n++){
		...
		if(arrayCentros[n][0]==centroSeleccionado){
			//29may23 document.forms['formBot'].elements['IDCENTRO'].options[document.forms['formBot'].elements['IDCENTRO'].length]=new Option('['+arrayCentros[n][1]+']',arrayCentros[n][0]);
	...
	for(var n=0;n<arrayCentrosConsumo.length;n++){
		if(arrayCentrosConsumo[n][1]==centroSeleccionado){
			...
			if(arrayCentrosConsumo[n][0]==IDCentroCoste){
				//29may23 document.forms['formBot'].elements['IDCENTROCONSUMO'].options[document.forms['formBot'].elements['IDCENTROCONSUMO'].length]=new Option('['+arrayCentrosConsumo[n][3]+']',arrayCentrosConsumo[n][0]);	//	7mar08	ET	+' ('+arrayCentrosConsumo[n][2]
...
var macroInputText2	= '<input type="text" size="#SIZE#" name="#NAME#" id="#NAME#" class="#CLASS#" value="#VALUE#" style="#STYLE#" maxlength="#MAXLENGTH#" onBlur="#ONBLUR#" onInput="#ONINPUT#" #DISABLED#/>';
...
var macroCheckbox	= '<input type="checkbox" name="#NAME#" id="#NAME#" class="#CLASS#" value="#VALUE#" #DISABLED# #CHECKED# onClick="#ONCLICK#">';
...
function dibujaLineasPedido(){
	...
	if (v_OcultarNoSeleccionados=='SI')
	{
		...
		thisRow += thisCell;
		thisCell = cellHAlignColClassStart.replace('#ALIGN#', 'left').replace('#CLASS#', 'encabezadoBloque textLeft').replace('#COLSPAN#', '10');
	...
	jQuery.each(ColumnaOrdenadaProds, function(key, value){
		...
		jQuery.each(arrayLineasProductos[value].Productos, function(key2, producto){
			...
			if(v_OrdenarProductos == 'NO' && (lastGrupo!=producto.Grupo || lastSubfamilia!=producto.Subfamilia))
			{
				...
				thisCell = cellHAlignColClassStart.replace('#ALIGN#', '').replace('#CLASS#', 'textLeft encabezadoBloque');
            ...
            if (minimalista == 'S'){
                ...
                thisCell += anchorStyleHrefStrt.replace('#STYLE#', 'text-decoration:none;').replace('#HREF#', 'javascript:FichaProducto(' + producto.IDProducto+',\'producto\',100,80,0,0);');
			...
			thisMacro = thisMacro.replaceAll('#NAME#', 'NuevaCantidad_' + key);				//	16may23 Tambien para el ID
			...
			//16may23	functionJS= 'javascript:UnidadesALotes(this);';
			//16may23	thisMacro = thisMacro.replace('#ONBLUR#', functionJS);
			thisMacro = thisMacro.replace('#ONBLUR#', 'javascript:RevisarSeleccionados();');
			//16may23 functionJS= 'javascript:Seleccionar(this);';
			functionJS= 'javascript:Seleccionar('+ key+');';
			...
			thisMacro = macroCheckbox.replaceAll('#NAME#', 'SELECCION_' + key);
			...
			//16may23	thisMacro = thisMacro.replace('#ONCLICK#', 'clickCheckbox(this);');
			thisMacro = thisMacro.replace('#ONCLICK#', 'clickCheckbox('+ key+');');
			thisCell += thisMacro;
			thisCell += cellEnd;
...
function validarCantidades(){
	...
	jQuery.each(ColumnaOrdenadaProds, function(key, value){
		...
		if(uds%udsXLote == 0){
			...
			//16may23 jQuery("input.cantidad[name='NuevaCantidad_" + key + "']").val(nuevasUds);
			jQuery("#NuevaCantidad_" + key ).val(nuevasUds);
		}else{
			...
			//16may23 jQuery("input.cantidad[name='NuevaCantidad_" + key + "']").val(nuevasUds);
			jQuery("#NuevaCantidad_" + key).val(nuevasUds);
			//16may23 jQuery("input.cantidad[name='NuevaCantidad_" + key + "']").focus();
			jQuery("#NuevaCantidad_" + key).focus();
			//16may23 jQuery("input.cantidad[name='NuevaCantidad_" + key + "']").blur();	// Esto se hace para lanzar la funcion Cantidad2Importe()
			jQuery("#NuevaCantidad_" + key).blur();	// Esto se hace para lanzar la funcion Cantidad2Importe()
		}
...
//	22feb23 Se producen algunos casos extrannos en los que no se activa "onBlur". por si acaso revisamos toda las unidades
function RevisarSeleccionados()
{
	//solodebug	debug('RevisarSeleccionados. INICIO.');
	for (var i=0;i<arrayLineasProductos.length;++i)
	{
		//16may23 if (jQuery("input.SELECCION[name='SELECCION_" + i + "']").is(':checked'))
		if (jQuery("#SELECCION_" + i).is(':checked'))
		{
			//solodebug	debug('RevisarSeleccionados. LLamar a UnidadesALotes.'+i);			
			UnidadesALotes(i);
		}
	}
}
...
// Comprueba que las unidades son coherentes con los lotes
function UnidadesALotes(posArr){

	//22feb23	var uds		= parseInt(obj.value);
	//16may23 var posArr	= obj.name.substr(14, obj.name.length);
	...
	var nuevasUds, hayPrecio=1, lotes, uds;

	if(tarifa=="")	hayPrecio=0;

	var txtUds	= jQuery('#NuevaCantidad_'+posArr).val();

	//Comprueba posibles errores en la cantidad
	if(isNaN(txtUds) || txtUds==""){
		alert('('+posArr+').'+txtUds+':'+strCantidadIncorrecta);
		uds=1;
		//16may23	obj.value=1;
		jQuery("#NuevaCantidad_" + posArr).val(1);
	}else{
		if(!esEnteroConSigno(txtUds)){
			alert('('+posArr+').'+txtUds+':'+strCantidadIncorrecta);
			uds=1;
			//16may23	obj.value=1;
			jQuery("#NuevaCantidad_" + posArr).val(1);
	...
	if(uds%udsXLote==0){
		...
		nuevasUds=udsXLote*lotes;
		// Editamos el valor del input text para la cantidad correspondiente
		//16may23 jQuery("input.cantidad[name='NuevaCantidad_" + posArr + "']").val(nuevasUds);
		jQuery("#NuevaCantidad_" + posArr).val(nuevasUds);
	...
	//solodebug	
	debug('UnidadesALotes. udsXLote:'+udsXLote+' uds:'+uds+' -> nuevasUds:'+nuevasUds);
...
//	Al cambiar la cantidad, de paso seleccionamos la línea
function Seleccionar(posArr){
	//16may23	function Seleccionar(obj){
	//16may23 var posArr	= obj.name.substr(14, obj.name.length);

	//solodebug	
	debug('Seleccionar. IDDivisa:'+IDDivisa);
	
	//16may23	if (!jQuery("input.SELECCION[name='SELECCION_" + posArr + "']").is(':checked')) 	// 1jul22 .attr('checked')
	if (!jQuery("#SELECCION_"+posArr).is(':checked')) 	// 1jul22 .attr('checked')
	{
		//16may23	jQuery("input.SELECCION[name='SELECCION_" + posArr + "']").prop('checked', true);
		jQuery("#SELECCION_"+posArr).prop('checked', true);
	...
	jQuery.each(ColumnaOrdenadaProds, function(key, value){
		if(jQuery("#NuevoImporte_" + key).length < 0){
			if(arrayLineasProductos[value].Productos[0].Seleccionado == 'S'){
				...
				//16may23 jQuery("input.SELECCION[name='SELECCION_" + key + "']").prop('checked',false);		//1jul22 attr
				jQuery("#SELECCION_"+ key).prop('checked',false);		//1jul22 attr
			}
		}else{
...
function clickCheckbox(posArr){
//16may23	function clickCheckbox(obj){
//16may23 	var posArr	= obj.name.replace('SELECCION_', '');
//16may23 	var isChecked	= jQuery(obj).is(':checked');

	var isChecked	= jQuery('#SELECCION_'+posArr).is(':checked');
...
function Actua(){
	...
	//	22feb23 Comprobamos que no haya quedado alguna unidad descuadrada con el empaquetamiento
	RevisarSeleccionados();

	//	Recuperamos el centro seleccionado en el desplegable
		...
		if (Ref!='')
		{
			...
			for (j=0;((j<arrayLineasProductos.length) && (!Encont));++j)
			{
				...
				if ((arrayLineasProductos[ColumnaOrdenadaProds[j]].Productos[0].RefCliente==Ref)||(arrayLineasProductos[ColumnaOrdenadaProds[j]].Productos[0].RefPrivada==Ref))
				{
					...
					//16may23 jQuery("input.cantidad[name='NuevaCantidad_" + ColumnaOrdenadaProds[j] + "']").val(Cant);
					jQuery("#NuevaCantidad_" + ColumnaOrdenadaProds[j]).val(Cant);
					
					//16may23 if (!jQuery("input.SELECCION[name='SELECCION_" + ColumnaOrdenadaProds[j] + "']").is(':checked')) 	// 1jul22 .attr('checked')
					if (!jQuery("#SELECCION_" + ColumnaOrdenadaProds[j]).is(':checked')) 	// 1jul22 .attr('checked')
					{
						//16may23 jQuery("input.SELECCION[name='SELECCION_" + ColumnaOrdenadaProds[j] + "']").prop('checked', true);
						jQuery("#SELECCION_" + ColumnaOrdenadaProds[j]).prop('checked', true);
