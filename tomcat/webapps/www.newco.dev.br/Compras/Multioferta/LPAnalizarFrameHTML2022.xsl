...
<!-- 
	Primer paso del pedido manual. Nuevo disenno 2022.
	ultima revision ET 16may23 11:36 LPAnalizar2022_290523.js
-->
...
<head>
	...
	<title><xsl:value-of select="document($doc)/translation/texts/item[@name='pedido']/node()"/>:&nbsp;<xsl:value-of select="/Analizar/ANALISIS/LINEAS/LINEA_ROW[1]/PRODUCTOS/PRODUCTOS_ROW/CENTROPROVEEDOR"/></title>
	...
	<script type="text/javascript" src="http://www.newco.dev.br/Compras/Multioferta/LPAnalizar2022_290523.js"></script>
...
<xsl:when test="//ERROR_USUARIOVENDEDOR_NOINFORMADO">
	...
	<div class="ZonaTituloPagina">
		<p class="TituloPagina">
			<xsl:value-of select="document($doc)/translation/texts/item[@name='pedido']/node()"/>:&nbsp;<xsl:value-of select="/Analizar/ANALISIS/CABECERA/PROVEEDOR"/>
			<span class="CompletarTitulo300">				
				...
				<xsl:when test="(/Analizar/ANALISIS/PEDIDO_MINIMO/IMPORTE != '0' and /Analizar/ANALISIS/PEDIDO_MINIMO/IMPORTE != '')">
					<a class="btnDestacado" href="javascript:enviarPedido(document.forms['formBot'],document.forms,'LPAnalizarSave2022.xsql',{/Analizar/ANALISIS/PEDIDO_MINIMO/IMPORTE});" title="Continuar" id="botonContinuar"><xsl:value-of select="document($doc)/translation/texts/item[@name='continuar']/node()"/></a>
				</xsl:when>
				<xsl:otherwise>
					<a class="btnDestacado" href="javascript:Actua(document.forms['formBot'],document.forms,'LPAnalizarSave2022.xsql');" title="Continuar" id="botonContinuar"><xsl:value-of select="document($doc)/translation/texts/item[@name='continuar']/node()"/></a>
				</xsl:otherwise>
				</xsl:choose>
			</span>
		</p>
...
<xsl:template match="LISTA">
	...
	<div class="ZonaTituloPagina">
		<p class="TituloPagina">
   			<xsl:value-of select="document($doc)/translation/texts/item[@name='pedido']/node()"/>:&nbsp;<xsl:value-of select="/Analizar/ANALISIS/CABECERA/PROVEEDOR"/>
			<span id="spBotones" class="CompletarTitulo800">
			...
			<form name="formBot" method="POST">
				...
				<tr class="sinLinea">
					...
					<xsl:otherwise>
						...
						<select name="IDCENTRO" class="w400px" style="display:none;">
					...
					<xsl:for-each select="//TODOSLUGARESENTREGA/CENTRO/LUGARESENTREGA">
						<select name="IDLUGARENTREGA_{@idCentro}" class="lugarEntrega w300px" style="display:none;" onChange="ActualizarTextoLugarEntrega(this.value);">
				...
				<tr class="sinLinea">
					...
					<td class="datosLeft">
						<p class="w200px">
							...
							<input type="text" class="campopesquisa w100px" name="FECHA_ENTREGA" id="FECHA_ENTREGA"  size="10" maxlength="10"/>
						</p>
						<!--
				...
				<tr class="sinLinea">
					...
					<xsl:when test="/Analizar/ANALISIS/PEDIDO_MINIMO/ACTIVO!='N'">
						<td class="textCenter" colspan="5">
							<span class="fuentePeq">
								...
								<xsl:if test="/Analizar/ANALISIS/PEDIDO_MINIMO/DETALLE != ''"><xsl:value-of select="/Analizar/ANALISIS/PEDIDO_MINIMO/DETALLE"/>.&nbsp;</xsl:if>
								<xsl:if test="RETRASO">
									<xsl:choose>
					...
					<xsl:otherwise>
						<td class="textCenter" colspan="5">
							...
							<span class="fuentePeq">
							<xsl:value-of select="/Analizar/ANALISIS/PEDIDO_MINIMO/PROVEEDOR" /><xsl:value-of select="document($doc)/translation/texts/item[@name='no_requiere_pedido_minimo']/node()"/>
								<xsl:if test="RETRASO">
									<!--&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-->
									<xsl:choose>
										<xsl:when test="RETRASO[@mensaje]='15'">
...
<xsl:template match="LINEAS">
	...
	<div class="tabela tabela_redonda">
		<table id="lineas" class="w100" cellspacing="6px" cellpadding="6px">
		<thead class="cabecalho_tabela">
		<tr>
			<!-- ref mvm + provee -->
			<th class="w100px" align="left">
			<!--5dic22 Ya no se utiliza
			<xsl:choose>
			...
			<th class="textLeft w300px">
				...
				&nbsp;<input type="text" class="campopesquisa w150px" name="strFiltro" id="strFiltro"/>
				<xsl:choose>
			...
			<th>
				<xsl:attribute name="class">
					<xsl:choose>
					<xsl:when test="/Analizar/ANALISIS/SIN_MARCA">w1px</xsl:when>
	...
	<!--	Este es el caso que se utiliza en BRASIL-->
	<tfoot class="rodape_tabela"><!--forzamos el padding, o queda la tabla muy desmontada-->
		<tr>
			<td colspan="14">&nbsp;</td>
		</tr>
		...
		<form name="sumaTotal">
		<tr class="sinLinea">
			<td class="textLeft paddingPeq" rowspan="3">
				<xsl:attribute name="colspan">
				<xsl:choose>
				<xsl:when test="/Analizar/ANALISIS/IDPAIS = '55'">6</xsl:when>
				...
				<a id="EnviarProductosPorRef" class="btnNormal" href="javascript:IncluirProductosPorReferencia();">
			...
			<xsl:otherwise>
				...
				<td class="textRight paddingPeq" colspan="2">
					<input type="text" class="noinput textRight w150px" name="sumaTotal" onFocus="this.blur();" value="0,00"/>
				</td>

				<input type="hidden" class="noinput" name="sumaTotalBrasil"  onFocus="this.blur();"/>
				<input type="hidden" class="noinput" name="COSTE_LOGISTICA"  value="0"/>
			</xsl:otherwise>
			</xsl:choose><!--fin choose pais-->

			<td class="textRight paddingPeq">&nbsp;</td>
		</tr>

		<!--coste de transporte + suma total brasil-->
		<xsl:if test="/Analizar/ANALISIS/IDPAIS = '55'">
			<tr class="sinLinea paddingPeq">
				<td class="textRight paddingPeq" colspan="2">
					<strong><xsl:value-of select="document($doc)/translation/texts/item[@name='coste_trasporte']/node()"/>:</strong>
				</td>
				<td colspan="2" class="textRight paddingPeq">
					<input type="text" class="noinput textRight w150px" name="COSTE_LOGISTICA" value="0,00"/>
				</td>
				<td colspan="2" class="textRight paddingPeq">&nbsp;</td>
			</tr>
			...
			<tr class="sinLinea">
				...
				<td colspan="2" class="textRight paddingPeq">
					<input type="text" class="noinput textRight w150px" name="sumaTotalBrasil" onFocus="this.blur();" value="0,00"/>
				</td>
				<td colspan="2" class="textRight paddingPeq">&nbsp;</td>
			</tr>
		</xsl:if>
...
<xsl:variable name="OcultarPrecioReferencia"><xsl:value-of select="/Analizar/ANALISIS/OCULTAR_PRECIO_REFERENCIA/@Ocultar"/></xsl:variable>
	...
	<div class="linha_separacao_cotacao_y"></div>
	<div class="tabela tabela_redonda">
		<table id="lineas" cellspacing="10px" cellpadding="10px">
		<thead class="cabecalho_tabela">
		<tr>
			<!-- ref mvm + provee -->
		...
		<tr>
			...
			<xsl:when test="/Analizar/ANALISIS/PEDIDO_MINIMO/IMPORTE != '0' and /Analizar/ANALISIS/PEDIDO_MINIMO/IMPORTE != ''">
				<div class="boton" id="divContinuar">
					<a href="javascript:enviarPedido(document.forms['formBot'],document.forms,'LPAnalizarSave2022.xsql',{/Analizar/ANALISIS/PEDIDO_MINIMO/IMPORTE});" title="Continuar" id="botonContinuar"><xsl:value-of select="document($doc)/translation/texts/item[@name='continuar']/node()"/></a>
				</div>
			</xsl:when>
			<xsl:otherwise>
				<div class="boton" id="divContinuar">
					<a href="javascript:Actua(document.forms['formBot'],document.forms,'LPAnalizarSave2022.xsql');" title="Continuar" id="botonContinuar"><xsl:value-of select="document($doc)/translation/texts/item[@name='continuar']/node()"/></a>
				</div>
			</xsl:otherwise>
			</xsl:choose>
			</td>
