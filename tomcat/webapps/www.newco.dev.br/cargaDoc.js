...
function addDocFile(id){
	if(document.getElementById('MantenEmpresa')){ var remove = document.forms['MantenEmpresa'].elements['REMOVE'].value;}//manten empresa
        if(document.getElementById('frmManten')){ var remove = document.forms['frmManten'].elements['REMOVE'].value;}//manten centros
	if(document.getElementById('form1')){ var remove = document.forms['form1'].elements['REMOVE'].value;}
        if(document.getElementById('Form1')){ var remove = document.forms['Form1'].elements['REMOVE'].value;}//wfstatus
	...
	if(uploadElem.value != ''){
		...
		uploadFilesDoc[uploadFilesDoc.length] = uploadElem.value;
                ...
                if (document.getElementById("botonCargar")){
                    jQuery('#botonCargar').css({display: ""});                    
                }
...
function periodicUpdateDoc(form){
	...
	if(uploadFrameDoc && uploadFrameDoc.document && uploadFrameDoc.document.getElementsByTagName("p")[0]) {
		...
		if(document.getElementById('MantenEmpresa')){ var tipoDoc = document.forms['MantenEmpresa'].elements['TIPO_DOC'].value; }
                if(document.getElementById('frmManten')){ var tipoDoc = document.forms['frmManten'].elements['TIPO_DOC'].value; }
		if(document.getElementById('form1')){ var tipoDoc = document.forms['form1'].elements['TIPO_DOC'].value; }
                if(document.getElementById('Form1')){ alert('mimi'+document.forms['Form1'].elements['TIPO_DOC'].value); var tipoDoc = document.forms['Form1'].elements['TIPO_DOC'].value; }//wfstatus
                
                if(document.getElementById('waitBoxDoc'+tipoDoc)){
                    document.getElementById('waitBoxDoc'+tipoDoc).style.display = 'none';
                }
                else {
                     document.querySelectorAll(".waitBoxDoc"+tipoDoc).style.display = 'none';
                }
...
function handleFileRequestDoc(resp){
	...
	if(document.getElementById('MantenEmpresa')){
		...
		var borr_ante = document.forms['Form1'].elements['BORRAR_ANTERIORES'].value;
		var tipoDoc = document.forms['Form1'].elements['TIPO_DOC'].value;
		var prove = document.forms['Form1'].elements['IDPROVEEDOR'].value;
	}
			...
			success:function(data){
				...
				if(document.getElementById('frmManten')){
					...
					jQuery("#carga"+tipo).hide();
                                ...
                                //reinicializo los campos del wfstatus para subir ficheros de albaran
				if(document.getElementById('Form1')){
					document.forms['Form1'].elements['inputFileDoc'].value = '';
					var tipo = document.forms['Form1'].elements['TIPO_DOC'].value;
					var proveedor = document.forms['Form1'].elements['IDPROVEEDOR'].value;
				...
				if(tipo == 'FT'){
					...
					SeleccionaFichas(proveedor,'IDFICHA',currentDocID);
                                ...
                                else if(tipo == 'ALBARAN')	SeleccionaLogos(proveedor,'IDALBARAN');
