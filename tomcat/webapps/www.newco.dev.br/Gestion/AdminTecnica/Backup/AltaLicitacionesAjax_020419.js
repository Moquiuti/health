...
//	ultima revision ET 2abr19 16:46
...
function Inicio()
{
	...
	jQuery("#MODELOCARGA").html(Modelo);	
}
...
function ProcesarCadenaPedidoTXT(ListaProductos)
{
	...
	for (i=0;i<=numRefs;++i)
	{
		var Centro,Ref,Cantidad,LugarEntrega='',CentroCoste='',Descripcion='',Precio='',UdBasica='',CodCenEnt='',CodProv='',CodPed='';
		var Producto	= Piece(Referencias, '·',i);

		//solodebug	alert('['+Producto+']');
		if (posCodPed>=0) CodPed= Piece(Producto, ':', posCodPed);
		if (posCodCenFac>=0) Centro= Piece(Producto, ':', posCodCenFac);
		...
		if (posCodDescr>=0) Descripcion= Piece(Producto, ':', posCodDescr);
		if (posPrecio>=0) Precio= Piece(Producto, ':', posPrecio);

		//solodebug		
		console.log('ProcesarCadenaTXT. Oferta ('+i+'). Centro:'+Centro+' CodPed:'+CodPed+'. Ref:'+Ref+'. Cant:'+Cantidad+'. Entr:'+LugarEntrega+'. CenCoste:'+CentroCoste
