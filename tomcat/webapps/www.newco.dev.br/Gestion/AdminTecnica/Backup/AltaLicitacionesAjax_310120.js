...
//	ultima revision ET 23ene20 09:22 AltaLicitacionesAjax_310120.js
...
function PreparaModelo()
{
	...
	if (str_ModeloFicPedidos!='')
	{
		...
		for (var i=0;i<=PieceCount(Modelo,'|');++i)
		{
			...
			//solodebug	console.log('Inicio. Nueva columna('+i+'):'+Cadena);
...
function prepararEnvio(nombreFichero, ficRequisicao, ficTituloPdc, ficDataVencimento, ficHoraVencimento, ficMoeda, ficObservacao, ficUrgencia)
{
	...
	{
		//31ene20	debug('prepararEnvio ficObservacao:'+ficObservacao+' err:'+e);
		Observacao='';
	}
	
	
	var datosFichero='Titulo_Pdc:::'+(ficTituloPdc=='')?'':ScapeHTMLString(ficTituloPdc)+'|'		//27ene20 Otro problema si el título no está informado		
	...
	console.log('prepararEnvio. datosFichero:'+datosFichero);
...
function ProcesarCadenaPedidoTXT(ListaProductos)
{
	...
	for (i=0;i<=numRefs;++i)
	{
		...
		else
		{
			...
			if ((IDSolicitud!='NR_COT_COMPRA')&&((Ref!='')||((Cantidad!='')&&(Cantidad!='0'))))		//	15dic17	Eliminamos líneas sin ref ni cantidad, o que empiecen por NR_COT_COMPRA
			{
				var items		= [];
				items['CodPed']	= CodPed;
