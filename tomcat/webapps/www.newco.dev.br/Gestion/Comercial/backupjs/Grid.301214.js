...
(function(window, document, undefined) {
	...
	GridProto.convertData = function(data) {
		...
		if ((base = data.Head || data.Body || data.Foot || null)) {
			...
			cols = this.columns;
			if (data.Head) {
				this.convertDataItem(h, data.Head, "<DIV class='g_C g_HR g_R", cols, this.options.allowColumnResize, false);
			} else {
				this.css.rules[".g_Head"] = { display : "none" };
			}
			if (data.Body) {
				this.convertDataItem(b, data.Body, "<DIV class='g_C g_BR g_R", cols, false, true);
			} else {
				this.css.rules[".g_BodyFixed"] = { display : "none" };
			}
			if (data.Foot) {
				this.convertDataItem(f, data.Foot, "<DIV class='g_C g_FR g_R", cols, false, false);
			} else {
	...
	//////////////////////////////////////////////////////////////////////////////////
	GridProto.convertDataItem = function(arr, rows, rowClass, cols, allowColResize, editClass) {
		var rowIdx = rows.length, 
		    rowDiv, row, colIdx, colIdxCompara;

// Si estamos en la tabla compacta o en la tabla normal
if(this.options.tableType === 'compacta'){
	colIdxCompara = 3;	// Columna del precio historico
}else if(this.options.tableType === 'normal'){
	colIdxCompara = 3;	// Columna del precio historico
}
...
//			rowDiv = rowClass + (--rowIdx) + "'>";
			...
			while (colIdx) {
// DC TRICK - 22/09/14 - Comparo ofertas con los precio historico para dar colores
				rowDiv = rowClass + (rowIdx);
				if(editClass && colIdx > this.options.fixedCols && row[colIdx-1] !== undefined){
					if(parseFloat(row[colIdx-1].replace(',','.')) !== 'NaN'){
						if(parseFloat(row[colIdx-1].replace(',','.')) < parseFloat(row[colIdxCompara].replace(',','.'))){
							rowDiv += " greenLic ";
						}
						if(parseFloat(row[colIdx-1].replace(',','.')) == parseFloat(row[colIdxCompara].replace(',','.'))){
							rowDiv += " orangeLic ";
...
// FIN DC TRICK
	...
	GridProto.alignColumns = function(reAlign, fromInit) {
		...
		for (var i=0, len=this.columns; i<len; i++) {
			...
			// DC - 02/10/14 - Condicion para limitar el ancho de la columna 'nombre producto'
			if(i==2 && colWidth > 480)
				colWidth = 480;
	...
	GridProto.sortRawData = function(colIdx, colSortAs, sortAsc) {
		...
		this.convertDataItem(this.cellData.body, rawData, "<DIV class='g_C g_BR g_R", this.columns, false, true);
		this.generateGridBody();
	...
	GridProto.getSortResult = function(type, colIdx, ltVal, gtVal, a, b, keyA, keyB) {
		...
		// DC - 22/06/14 - Hago la transformacion para poder ordenar segun el formato espanyol de numeros
		if(type === "number"){
			a = a.replace('.','').replace(',','.');
			b = b.replace('.','').replace(',','.');
		}
