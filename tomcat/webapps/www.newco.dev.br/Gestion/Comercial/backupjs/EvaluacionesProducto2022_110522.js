//	JS Buscador/listado de Evaluaciones de producto. Nuevo disenno 2022.
...
function RecuperaUsuariosCentro(){
	...
	if (form.elements['IDCENTROCLIENTE'])	//12may22 Cuando se ha guardado la evaluacion, no hace existe IDCENTROCLIENTE
	{
...
function GuardarNuevaEvaluacion(form){
    form.action= 'http://www.newco.dev.br/Gestion/Comercial/NuevaEvaluacionProductoSave2022.xsql';
    form.submit();
}
...
function MuestrasEnviadasProveedor(form){
    ...
    form.action='http://www.newco.dev.br/Gestion/Comercial/EvaluacionProducto2022.xsql';
    form.submit();
}
...
//	Comprueba errores y avanza en el proceso
...
function DescargarExcel(){
	...
	var FDiagnostico	= jQuery('#FDIAGNOSTICO').val();
	var d			= new Date();

	jQuery.ajax({
		url: 'http://www.newco.dev.br/Gestion/Comercial/EvaluacionesExcel.xsql',
		data: "FIDEMPRESA="+FIDEmpresa+"&FIDCENTRO="+FIDCentro+"&FIDCOORD="+FIDResponsable+"&FIDPROVEEDOR="+FIDProveedor+"&FIDPRODUCTO="+FIDProducto+"&FTEXTO="+FTexto+"&FESTADO="+FEstado+"&FDIAGNOSTICO="+FDiagnostico+"&_="+d.getTime(),
        type: "GET",
        contentType: "application/xhtml+xml",
        beforeSend: function(){
                null;
...
//	11may22 Nueva evaluacion de producto (desde el buscador)
function NuevaEvaluacion()
{
	document.location="http://www.newco.dev.br/Gestion/Comercial/NuevaEvaluacionProducto2022.xsql";
}

//	11may22 Crea una copia de la evaluacion actual **DESACTIVADO**
function CopiaEvaluacion(IDEval)
{
	MostrarPagPersonalizada("http://www.newco.dev.br/Gestion/Comercial/EvaluacionProductoCopia2022.xsql?ID_EVAL="+IDEval,'Evaluacion',100,80,0,0);
}

//	12may22 Vuelve al buscador de evaluaciones
function Evaluaciones()
{
	document.location="http://www.newco.dev.br/Gestion/Comercial/EvaluacionesProducto2022.xsql";
}
