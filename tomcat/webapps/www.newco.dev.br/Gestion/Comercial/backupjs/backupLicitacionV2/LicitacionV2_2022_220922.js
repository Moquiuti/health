...
//	Ultima revisión ET 19set22 12:28 LicitacionV2_2022_220922.js
...
function AbrirProducto(Pos)
{
	...
	else 
	{
		...
		for (var i=0; i<arrProductos[Pos].Ofertas.length;++i)
		{
			...
			Linea=Linea.replaceAll('[SPANCONSUMO]','<span class="'+Color+'">'+arrProductos[Pos].Ofertas[i].CONSUMO+'</span>');		//19set22 CUidado, antes formateaDivisa(arrProductos[Pos].Ofertas[i].CONSUMO
			...
			if (botonPedido=='S')
			{
				...
				if (arrProductos[Pos].Ofertas[i].INCLUIDO_EN_PEDIDO=='S')
				{
					...
					//PENDIENTE if (isLicAgregada=='S')
					//PENDIENTE 	botPedido+='<a class="btnDestacadoPeq" id="btnPedido_[LIC_OFE_ID]" href="javascript:Pedido('+Pos+','+i+');">'+strPedido+'</a>'
				}
				else
				{
					if (isLicAgregada=='S')
...
//	15ago22 Prepara el desplegable de motivos
...
function SeleccionadaOferta(PosProd, ID)
{
	...
	if ((isLicMultiopcion=='S')||((MesesDuracion==0)&&(isLicAgregada=='N')))			//	16dic19 Faltaba incluir la opción de SPOT	// 27ago20 Pero no para agregadas
	{
		...
		var Consumo=(arrProductos[PosProd].PrecioOfertaActual*CantidadActual).toFixed(2);
...
function Pedido(PosProd, PosOfe)
{
	...
	var IDProveedorLic=arrProductos[Pos].Ofertas[i].IDPROVEEDORLIC,					//21set22 jQuery("#IDProveedorLic_"+IDOfertaLic).val(),
		Proveedor=arrProductos[Pos].Ofertas[i].PROVEEDOR,
		IDMultioferta=arrProductos[Pos].Ofertas[i].MO_ID,
		IDEstadoMO=arrProductos[Pos].Ofertas[i].MO_STATUS,
		PedMinimo=arrProductos[Pos].Ofertas[i].PROV_PEDIDOMINIMO,
		Consumo=desformateaDivisa(arrProductos[Pos].Ofertas[i].CONSUMO),
		CodPedido=arrProductos[Pos].Ofertas[i].CODPEDIDO,
