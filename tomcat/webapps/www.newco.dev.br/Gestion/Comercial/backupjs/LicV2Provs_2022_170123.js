...
//	Ultima revisión ET 17ene23 16:45 LicV2Provs_2022_170123.js
	...
	*/
				...
				if(esAutor == 'S'){
					if((EstadoLic == 'EST')||(EstadoLic == 'COMP')){
						...
						{
							//	19feb20 ocultamos para licitaciones continuas, pero desde Brasil nos comentan que les resulta útil para que aparezca en la página de INICIO del proveedor
							thisMacro = macroEnlace3.replace('#CLASS#', 'accRollBack');
							thisMacro = thisMacro.replace('#HREF#', 'javascript:modificaEstadoProveedor(' + arrProveedores[ColumnaOrdenadaProvs[i]].IDProvLic + ', \'CURS\');');
							thisMacro = thisMacro.replace('#STYLE#', 'text-decoration:none;');
							thisCell += thisMacro;
				...
				if(arrProveedores[ColumnaOrdenadaProvs[i]].IDEstadoProv == 'INF'){
					...
					thisMacro = thisMacro.replace('#STYLE#', 'text-decoration:none;');
					thisCell += thisMacro;
					thisMacro = macroImagen.replace('#SRC#', 'http://www.newco.dev.br/images/2017/print.png');
					thisMacro = thisMacro.replace('#ALT#', str_ImprimirOferta);
					thisMacro = thisMacro.replace('#TITLE#', str_ImprimirOferta);
					thisCell += thisMacro;
					thisCell += macroEnlaceEnd + '&nbsp;';
					...
					thisMacro = thisMacro.replace('#STYLE#', 'text-decoration:none;');
					thisCell += thisMacro;
					thisMacro = macroImagen.replace('#SRC#', 'http://www.newco.dev.br/images/2017/excel.png');
					thisMacro = thisMacro.replace('#ALT#', str_ListadoExcel);
					thisMacro = thisMacro.replace('#TITLE#', str_ListadoExcel);
					thisCell += thisMacro;
					thisCell += macroEnlaceEnd + '&nbsp;';
...
function ordenamientoBurbujaProvs(col, tipo){
	...
	//solodebug debug('ordenamientoBurbujaProvs ('+col+', '+tipo+')');
	...
	for(var i=0; i<totalProvs; i++){
		...
		//solodebug	debug('ordenamientoBurbujaProvs ('+col+', '+'tipo). Proveedor('+i+')'+arrProveedores[ColumnaOrdenadaProvs[i]].NombreCorto+':'+arrProveedores[ColumnaOrdenadaProvs[i]][col]+'->'+valAux);
...
//	20oct22 listado excel desde la matriz JS, sin peticion a la base de datos (recuperado desde lic_proveedor2022_040422.js)
