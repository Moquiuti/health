//	Carga documentos JS
//	copiado desde Administracion/Mantenimiento/Empresas/CargaDocumentos_260419.js para su uso en la gestión comercial
//	Ultima revision 4jun19 09:23
...
function addDocFile(){
	var remove = document.forms['frmSeg'].elements['DOCUMENTOS_BORRADOS'].value;
...
function periodicUpdateDoc(form){
	...
	if(uploadFrameDoc && uploadFrameDoc.document && uploadFrameDoc.document.getElementsByTagName("p")[0]) 
	{
		...
		tipoDoc = document.forms['frmSeg'].elements['TIPO_DOC'].value;
...
function handleFileRequestDoc(resp){
	...
	var usuario = document.forms['frmSeg'].elements['ID_USUARIO'].value;
	var borrados = document.forms['frmSeg'].elements['DOCUMENTOS_BORRADOS'].value;
	var borr_ante = document.forms['frmSeg'].elements['BORRAR_ANTERIORES'].value;
	var tipoDoc = document.forms['frmSeg'].elements['TIPO_DOC'].value;
	var prove = document.forms['frmSeg'].elements['IDEMPRESAUSUARIO'].value;
	...
	if(msg != ''){
		...
		jQuery.ajax({
			...
			success:function(data){
				...
				document.forms['frmSeg'].elements['inputFileDoc'].value = '';
				var tipo = document.forms['frmSeg'].elements['TIPO_DOC'].value;
				var IDEmpUsuario = document.forms['frmSeg'].elements['IDEMPRESAUSUARIO'].value;
				document.getElementById('confirmBox').style.display = 'none';
				jQuery("#carga").hide();
...
function borrarDoc(){
	...
	action = 'BorrarDocumentoSeguimientoAJAX.xsql';
	valueID	= jQuery('#ES_IDEntrada').val();

	//solodebug
	debug("borrarDoc valueID:"+valueID);	
