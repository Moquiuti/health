...
//	Ultima revisión ET 7jul23 11:15 lic2022_060723.js
//	Cambios de version deben trasladarse a MantLicitacionHTML2022.xsl,ProductosParaExcelHTML2022.xsl,ImprimirOfertaHTML2022.xsl
...
{	
	...
	//	10dic21 Aviso de licitacion atendida por otro vendedor
	if ((rol=='VENDEDOR')&&(ofertaBloqueada=='S'))
			alert(alrt_NoEsResponsable.replace('[[USUARIO]]',NombreVendedor));
	...
	jQuery("#pes_lDatosGenerales").click(function(){
		...
		debug("#pes_lDatosGenerales.click");
	...
	jQuery("#pes_lProductos").click(function(){
		...
		//solodebug	debug("#pes_lProductos.click");
	...
	jQuery("#pes_lProductosCompacta").click(function(){
		...
		debug("#pes_lProductosCompacta.click");
		var chk = true;
		
		//solodebug	debug("#pes_lProductosCompacta");
...
function ColocaFloatingBox()
{
	...
	//solodebug	debug('ColocaFloatingBox.mostrarResumenFlotante:'+mostrarResumenFlotante+' top: '+(jQuery(window).scrollTop()+90)+' left:'+(jQuery(window).width()-900));
}
...
function cargaDatosOfertasProveedoresAjax(PosProvActual)
{
	...
	{	
		...
		//solodebug debug('cargaDatosOfertasProveedoresAjax: TIENE OFERTA. Proveedor('+PosProvActual+ '): ['+arrProveedores[PosProvActual].IDProvLic+'/'+arrProveedores[PosProvActual].IDProveedor+'] '+arrProveedores[PosProvActual].NombreCorto);
			...
			success: function(objeto)
			{
				...
				if(data.ProductosLicitacion.ListaProductosOfertas.length > 0)
				{
					...
					jQuery.each(data.ProductosLicitacion.ListaProductosOfertas, function(key, producto)
					{
						...
						arrProductos[fila].Ofertas[PosProvActual].RegistroSanitario	= producto.RegistroSanitario;
						...
						if(producto.FichaTecnica.ID != ''){
							...
							//solodebug debug('Proveedor:'+arrProveedores[PosProvActual].NombreCorto+' fila:'+fila+' PosProvActual:'+PosProvActual+' FichaTecnica:'+arrProductos[fila].Ofertas[PosProvActual].FichaTecnica.ID);
                    	}


						arrProductos[fila].Ofertas[PosProvActual].RegSanitario	= [];
						if(producto.RegSanitario.ID != ''){
							arrProductos[fila].Ofertas[PosProvActual].RegSanitario.ID		= producto.RegSanitario.ID;
							arrProductos[fila].Ofertas[PosProvActual].RegSanitario.Nombre	= producto.RegSanitario.Nombre;
							arrProductos[fila].Ofertas[PosProvActual].RegSanitario.Descripcion	= producto.RegSanitario.Descripcion;
							arrProductos[fila].Ofertas[PosProvActual].RegSanitario.Url		= producto.RegSanitario.Fichero;
							arrProductos[fila].Ofertas[PosProvActual].RegSanitario.Fecha	= producto.RegSanitario.Fecha;
							//solodebug debug('Proveedor:'+arrProveedores[PosProvActual].NombreCorto+' fila:'+fila+' PosProvActual:'+PosProvActual+' RegSanitario:'+arrProductos[fila].Ofertas[PosProvActual].RegSanitario.ID);
                    	}


						arrProductos[fila].Ofertas[PosProvActual].CertExperiencia	= [];
						if(producto.CertExperiencia.ID != ''){
							arrProductos[fila].Ofertas[PosProvActual].CertExperiencia.ID		= producto.CertExperiencia.ID;
							arrProductos[fila].Ofertas[PosProvActual].CertExperiencia.Nombre	= producto.CertExperiencia.Nombre;
							arrProductos[fila].Ofertas[PosProvActual].CertExperiencia.Descripcion	= producto.CertExperiencia.Descripcion;
							arrProductos[fila].Ofertas[PosProvActual].CertExperiencia.Url		= producto.CertExperiencia.Fichero;
							arrProductos[fila].Ofertas[PosProvActual].CertExperiencia.Fecha		= producto.CertExperiencia.Fecha;
							//solodebug debug('Proveedor:'+arrProveedores[PosProvActual].NombreCorto+' fila:'+fila+' PosProvActual:'+PosProvActual+' CertExperiencia:'+arrProductos[fila].Ofertas[PosProvActual].CertExperiencia.ID);
                    	}
				...
				{
					//solodebug debug('data.ProductosLicitacion.ListaProductosOfertas.length=0');
	...
	//solodebug debug('cargaDatosOfertasProveedoresAjax: FIN');
...
function ValidarFormulario(oForm, nombre, flag){
	...
	//debug(comparaFechas(oForm.elements['LIC_FECHADECISION'].value, oForm.elements['LIC_FECHAADJUDICACION'].value));
		...
		if(!errores){
			...
			oForm.action="http://www.newco.dev.br/Gestion/Comercial/MantLicitacion2022.xsql";
	...
	*/
	...
	if(nombre == 'provPedidoMinimo'){
		...
		if((!errores) && (esNulo(oForm.elements['LIC_PROV_PEDIDOMINIMO'].value))){
			errores++;
			msgErrores+=val_faltaPedidoMinimo+'\n';
			...
			msgErrores+=val_malPedidoMinimo+'\n';
			oForm.elements['LIC_PROV_PEDIDOMINIMO'].focus();
		}
		...
		if((!errores) && (IDPais==55) && (esNulo(oForm.elements['LIC_PROV_FRETE'].value))){	//solo para Brasil controlaremos el campo FRETE
			errores++;
			msgErrores+=val_faltaFrete+'\n';
			oForm.elements['LIC_PROV_PLAZOENTREGA'].focus();
		}else if((!errores) && (esNulo(oForm.elements['LIC_PROV_PLAZOENTREGA'].value))){
			errores++;
			msgErrores+=val_faltaPlazoEntrega+'\n';
			oForm.elements['LIC_PROV_PLAZOENTREGA'].focus();
		}
		
		//if ((docObligatorio=='S') && (esNulo(oForm.elements['LIC_PROV_IDDOCUMENTO'].value)))	//	15nov21 Comprobar documento obligatorio
		if (docObligatorio=='S') 	//	15nov21 Comprobar documento obligatorio
		{
			errores++;
			msgErrores+=alrt_faltaDocumentoObligatorio+'\n';
		}
...
//21dic21 Inicializa proveedores insertados desde nueva seleccion. Funcion recurrente.
		...
		success: function(objeto){
			...
			if(data.ListaUsuarios.length > 0){
				...
				jQuery.each(data.ListaUsuarios, function(key, usuario){
					var tbodyUsuario = "<tr style='border-bottom:1px solid #7d7d7d;'><td class='textLeft'>";
		...
		success: function(objeto){
			...
			if(data.ListaCentros.length > 0){
				...
				jQuery.each(data.ListaCentros, function(key, centro){
					var tbodyCentro = "<tr style='border-bottom:1px solid #7d7d7d;'><td colspan='2'>&nbsp;</td><td class='textLeft'>";

					tbodyCentro +=  centro.NombreCorto+"</td>" +
							"<td class='textLeft'>" + centro.Nombre+"</td>" +
							"<td>";
		...
		jQuery.ajax({
			...
			var data = JSON.parse(objeto);
				...
				if(data.NuevoEstado.estado == 'OK'){
					location.href = 'http://www.newco.dev.br/Gestion/Comercial/MantLicitacion2022.xsql?LIC_ID=' + IDLicitacion;
				}else{
...
function SeleccionaFichas(IDProveedor,IDTipo,currentDocID,accion){
	...
	//solodebug	console.log('SeleccionaFichas. IDProveedor:'+IDProveedor+' IDTipo:'+IDTipo+'currentDocID:'+currentDocID+' accion:'+accion);
			...
			success: function(objeto){
				...
				if(data.Filtros != ''){
					...
					for(var i=0; i<data.Filtros.length; i++){
						...
						items['URL']	= data.Filtros[i].Fitro.file;
						arrFichasTecnicas.push(items);
						
						//solodebug	console.log('SeleccionaFichas. IDProveedor:'+IDProveedor+' incluyendo FT:'+data.Filtros[i].Fitro.id+' - '+data.Filtros[i].Fitro.nombre);
...
//cambio de registro sanitario segun el proveedor
function SeleccionaRegSan(IDProveedor,IDTipo,currentDocID,accion){
	var d = new Date();

	//solodebug	console.log('SeleccionaRegSan. IDProveedor:'+IDProveedor+' IDTipo:'+IDTipo+'currentDocID:'+currentDocID+' accion:'+accion);

	if(IDProveedor != -1 && IDProveedor != 0){
		jQuery.ajax({
			cache:	false,
			url:	'http://www.newco.dev.br/Administracion/Mantenimiento/Productos/RegistrosSanitariosProveedor.xsql',
			type:	"GET",
			data:	"IDPROVEEDOR="+IDProveedor+"&IDDOC_ACTUAL="+currentDocID+"&_="+d.getTime(),
			contentType: "application/xhtml+xml",
			success: function(objeto){
				...
				if(data.Filtros != ''){
					...
					for(var i=0; i<data.Filtros.length; i++){
						...
						//solodebug	console.log('SeleccionaRegSan. IDProveedor:'+IDProveedor+' incluyendo RS:'+data.Filtros[i].Fitro.id+' - '+data.Filtros[i].Fitro.nombre);
...
//cambio de certificado de experiencia segun el proveedor
function SeleccionaCertExp(IDProveedor,IDTipo,currentDocID,accion){
	var d = new Date();

	//solodebug	console.log('SeleccionaCertExp. IDProveedor:'+IDProveedor+' IDTipo:'+IDTipo+'currentDocID:'+currentDocID+' accion:'+accion);

	if(IDProveedor != -1 && IDProveedor != 0){
		jQuery.ajax({
			cache:	false,
			url:	'http://www.newco.dev.br/Administracion/Mantenimiento/Productos/CertificadosExperienciaProveedor.xsql',
			type:	"GET",
			data:	"IDPROVEEDOR="+IDProveedor+"&IDDOC_ACTUAL="+currentDocID+"&_="+d.getTime(),
			contentType: "application/xhtml+xml",
			success: function(objeto){
				...
				if(data.Filtros != ''){
					...
					for(var i=0; i<data.Filtros.length; i++){
						...
						//solodebug	console.log('SeleccionaCertExp. IDProveedor:'+IDProveedor+' incluyendo RS:'+data.Filtros[i].Fitro.id+' - '+data.Filtros[i].Fitro.nombre);
					...
					jQuery(".IDCertExp").each(function(){
						// Capturamos el valor actual seleccionado en el desplegable
						valor = jQuery(this).val();
						// Recargamos el desplegable
						jQuery(this).html(Resultados);
...
//cambio de certificado de experiencia segun el proveedor
function SeleccionaFichaSeg(IDProveedor,IDTipo,currentDocID,accion){
	var d = new Date();

	//solodebug	console.log('SeleccionaFichaSeg. IDProveedor:'+IDProveedor+' IDTipo:'+IDTipo+'currentDocID:'+currentDocID+' accion:'+accion);

	if(IDProveedor != -1 && IDProveedor != 0){
		jQuery.ajax({
			cache:	false,
			url:	'http://www.newco.dev.br/Administracion/Mantenimiento/Productos/FichaSeguridadProveedor.xsql',
			type:	"GET",
			data:	"IDPROVEEDOR="+IDProveedor+"&IDDOC_ACTUAL="+currentDocID+"&_="+d.getTime(),
			contentType: "application/xhtml+xml",
			success: function(objeto){
				...
				if(data.Filtros != ''){
					...
					for(var i=0; i<data.Filtros.length; i++){
						...
						//solodebug	console.log('SeleccionaFichaSeg. IDProveedor:'+IDProveedor+' incluyendo RS:'+data.Filtros[i].Fitro.id+' - '+data.Filtros[i].Fitro.nombre);
					...
					jQuery(".IDFichaSeg").each(function(){
						// Capturamos el valor actual seleccionado en el desplegable
						valor = jQuery(this).val();
						// Recargamos el desplegable
						jQuery(this).html(Resultados);
...
var macroSelectOnChangeDis	= '<select name="#NAME#" id="#ID#" class="#CLASS#" onChange="#ONCHANGE#" disabled="disabled">';		//6abr23, para desplegables de documentos
...
{
	//solodebug	debug('prepararTablaProductos. dibujaTabla:'+dibujaTabla);
...
//	3may22 Extrae en debug la lista completa de proveedores - SOLO PARA DEBUG
function DebugProveedores()
{
	
	debug('DebugOfertas. Long.arrays:'+arrConsumoProvs.length+' vs '+arrProveedores.length);
	for(var i=0;i<arrConsumoProvs.length;++i)
	{
		debug('DebugProveedores ('+i+','+arrConsumoProvs[i].columna + '): '+ arrConsumoProvs[i].IDProvLic+' '+ arrConsumoProvs[i].NombreCorto + ' vs '+arrProveedores[i].IDProvLic+' '+ arrProveedores[i].NombreCorto);
	}
}


//	3may22 Extrae en debug la lista completa de ofertas - SOLO PARA DEBUG
function DebugOfertas()
{
	
	debug('DebugOfertas. arrProductos.length:'+arrProductos.length);
	for(var i=0;i<arrProductos.length;++i)
		for(var j=0;j<arrProductos[i].Ofertas.length;++j)
		{
			//solodebug
			debug('DebugOfertas ('+i+','+j+')'+arrProductos[i].RefEstandar+' OFERTA:'+arrProductos[i].Ofertas[j].IDProvLic+' NoInformada:'+arrProductos[i].Ofertas[j].NoInformada+' NoOfertada:'+arrProductos[i].Ofertas[j].NoOfertada);
		}
}
...
function dibujaTablaProductosOFE()
{
	...
	if(totalProductos > 0){
		...
		for(var i=firstProduct; i<lastProduct; i++){
			...
			// Celda/Columna numeracion
			thisCell = cellStartClass.replace('#CLASS#', 'textLeft borderLeft');
			thisCell += '&nbsp;' + contLinea + '&nbsp;';
...
//				thisCell += '<img src="" class="static"/>';
			...
			thisCell = cellStartClass.replace('#CLASS#', 'borderLeft precioRef textRight');
			if(mostrarPrecioIVA == 'S'){
				if(isAutor == 'S' && estadoLicitacion != 'CONT'){
			...
			thisCell = cellStartClass.replace('#CLASS#', 'borderLeft textRight');
			if(mostrarPrecioIVA == 'S'){
				if(isAutor == 'S' && estadoLicitacion != 'CONT'){
			...
			for(var j=0; j<arrProductos[ColumnaOrdenadaProds[i]].Ofertas.length; j++){
				...
				//solodebug	debug('dibujaTablaProductosOFE ('+i+','+j+') OrdenCol:'+ColumnaOrdenadaProds[i]+' IDProdLic:'+arrProductos[ColumnaOrdenadaProds[i]].IDProdLic+' IDProvLic:'+arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].IDProvLic
				//solodebug		+' IDOferta:'+arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].IDOferta+' TieneOfertas:'+arrConsumoProvs[j].TieneOfertas);
				...
				//solodebug	debug('dibujaTablaProductosOFE ('+i+','+j+') col.prov:'+arrConsumoProvs[j].columna + ' '+ arrConsumoProvs[j].NombreCorto + +' Ofe:'+arrConsumoProvs[j].TieneOfertas);
				...
				if (arrConsumoProvs[j].TieneOfertas=='S'){	//	COmprueba proveedor con ofertas /Mantenimiento/LICITACION/PRODUCTOSLICITACION/PROVEEDORESLICITACION/PROVEEDOR/TIENE_OFERTAS
					...
					//solodebug					debug('dibujaTablaProductosOFE ('+i+','+j+') col.prov:'+arrConsumoProvs[j].columna + ' '+ arrConsumoProvs[j].NombreCorto + ' Ofe:'+arrConsumoProvs[j].TieneOfertas
					//solodebug	+' DATOS OFERTA. '+arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].IDProvLic+' NoInformada:'+arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].NoInformada+' NoOfertada:'+arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].NoOfertada);
					...
					{	// Si esta oferta esta informada
						...
						if (isLicMultiopcion=='N')
						{
							...
							//solodebug			debug('dibujaTablaProductosOFE ('+i+','+j+') col.prov:'+arrConsumoProvs[j].columna + ' isLicMultiopcion:'+ isLicMultiopcion);
							...
							//solodebug		debug('dibujaTablaProductosOFE ('+i+','+j+') col.prov:'+arrConsumoProvs[j].columna + ' isLicMultiopcion NO. thisCell:'+thisCell);
						}
						else
						{
							...
							//solodebug		debug('dibujaTablaProductosOFE ('+i+','+j+') col.prov:'+arrConsumoProvs[j].columna + ' isLicMultiopcion SI. thisCell:'+thisCell);
						}
						thisCell += cellEnd;
						thisRow += thisCell;
	...
	*/
						...
						// Celda para consumo
						thisCell = cellStartClassStyle.replace('#CLASS#', 'borderLeft textRight colConsumo');
						thisCell = thisCell.replace('#STYLE#', 'display:none;');
						if(mostrarPrecioIVA == 'S'){
						...
						// Celda para ahorro
						thisCell = cellStartClassStyle.replace('#CLASS#', 'borderLeft textRight colAhorro');
						thisCell = thisCell.replace('#STYLE#', 'display:none;');
...
function dibujaTablaProductosEST(){
	...
	if(totalProductos > 0){
		...
		for(var i=firstProduct; i<lastProduct; i++){
			...
			// Celda/Columna Nombre
//			thisCell = cellStartClass.replace('#CLASS#','textLeft') + arrProductos[ColumnaOrdenadaProds[i]].Nombre + cellEnd;
		...
		if (arrConsumoProvs[i].TieneOfertas=='S'){	//	COmprueba proveedor con ofertas /Mantenimiento/LICITACION/PRODUCTOSLICITACION/PROVEEDORESLICITACION/PROVEEDOR/TIENE_OFERTAS
			...
			if(mostrarPrecioIVA == 'S'){
				if(arrConsumoProvs[i].ConsumoIVA != '0,00' && arrConsumoProvs[i].ConsumoIVA != ''){
					...
					htmlTFOOT += cellStartClassStyle.replace('#CLASS#', 'borderLeft textRight colAhorro');
					htmlTFOOT = htmlTFOOT.replace('#STYLE#', 'display:none;');
					htmlTFOOT += '<strong>';
...
function FichaProductoLic(IDProdLic){
	MostrarPagPersonalizada('http://www.newco.dev.br/Gestion/Comercial/FichaProductoLicitacion2022.xsql?LIC_PROD_ID='+IDProdLic+'&LIC_ID='+IDLicitacion,'Ficha Producto Licitacion',100,100,0,0);
}

// Funcion para abrir la ficha de una empresa en un pop-up
function FichaEmpresa(IDEmpresa, Pestanna){
	MostrarPagPersonalizada('http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/EMPDetalleFrame2022.xsql?EMP_ID='+IDEmpresa+'&ESTADO=CABECERA&PESTANNA='+Pestanna,'Detalle Empresa',100,80,0,0);
}
...
function calcularFloatingBox(){
	...
	//29may19	jQuery('#FBConsPot').html(FormatoNumero(ConsumoPot.toFixed(2)));
	//29may19	jQuery('#FBConsPrev').html(FormatoNumero(ConsumoPrev.toFixed(2)));
	//29may19	jQuery('#FBAhorro').html(FormatoNumero(Ahorro.toFixed(2)) + '%');
...
function recalcularFloatingBox(obj, flagPedMin, ofertaGuardada)
{
	...
	if(newNumOfertas > 0){
		//29may19	jQuery('#FBConsPot').html(FormatoNumero(ConsumoPot.toFixed(2)));
		//29may19	jQuery('#FBConsPrev').html(FormatoNumero(ConsumoPrev.toFixed(2)));
		//29may19	jQuery('#FBAhorro').html(FormatoNumero(newAhorro.toFixed(2)) + '%');
...
function validarPedidosMinimosDOM(){
	...
	for(var i=0; i<arrConsumoProvs.length; i++){
		arrConsumos[i]	= 0;
		//29jul22 arrPedsMin[i]	= parseFloat(arrConsumoProvs[i].PedidoMin.replace("PUNTO","").replace(",","."));
		arrPedsMin[i]	= desformateaDivisa(arrConsumoProvs[i].PedidoMin);
	}
		...
		if(jQuery("tr#posArr_" + key).length){
			...
			jQuery(".RADIO_" + key).each(function(key2, thisRadio){
				if(jQuery(thisRadio).is(':checked')){ // Si esta checked, entonces sumamos el consumo en la posicion del array que toca
					...
					try
					{
						//29jul22 arrConsumos[thisPos] += parseFloat(producto.Ofertas[thisPos].Consumo.replace("PUNTO","").replace(",","."));
						arrConsumos[thisPos] += desformateaDivisa(producto.Ofertas[thisPos].Consumo);
					}
					catch(e)
					{
		...
		// Si el producto en cuestion NO se muestra por pantalla, recuperamos los datos del propio objeto JS
			...
			jQuery.each(producto.Ofertas, function(key2, oferta){
				if(producto.Ofertas[key2].OfertaAdjud == 'S'){
					numOfertas++;
					//29jul22 arrConsumos[key2] += parseFloat(producto.Ofertas[key2].Consumo.replace(".","").replace(",","."));
					arrConsumos[key2] += desformateaDivisa(producto.Ofertas[key2].Consumo);
					return false;
				}
			});
		}
...
function validarPedidoMinimoProv(objRadio){
	...
	//29jul22 var provPedMin	= parseFloat(arrConsumoProvs[posArrProv].PedidoMin.replace(".","").replace(",","."));
	var provPedMin	= desformateaDivisa(arrConsumoProvs[posArrProv].PedidoMin);
	...
	if(lastPosArrProv != null)
		//29jul22 provPedMinLast	= parseFloat(arrConsumoProvs[lastPosArrProv].PedidoMin.replace(".","").replace(",","."));
		provPedMinLast	= desformateaDivisa(arrConsumoProvs[lastPosArrProv].PedidoMin);
			...
			jQuery(".RADIO_" + key).each(function(key2, thisRadio){
				...
				if(thisPosArrProv == posArrProv){ // Este es el radio button que cumple con el parametro posArrProv
					if(jQuery(thisRadio).is(':checked')){ // Si esta checked, entonces sumamos el consumo
						isAnyChecked = true;
						//29jul22 Consumo += parseFloat(producto.Ofertas[posArrProv].Consumo.replace(".","").replace(",","."));
						Consumo += desformateaDivisa(producto.Ofertas[posArrProv].Consumo);
					}
				}
				...
				if(thisPosArrProv == lastPosArrProv && lastPosArrProv != null){ // Este es el radio button que cumple con el parametro posArrProv
					if(jQuery(thisRadio).is(':checked')){ // Si esta checked, entonces sumamos el consumo
						...
						//29jul22 ConsumoLast += parseFloat(producto.Ofertas[lastPosArrProv].Consumo.replace(".","").replace(",","."));
						ConsumoLast += desformateaDivisa(producto.Ofertas[lastPosArrProv].Consumo);
					}
				}
			});
		// Si el producto en cuestion NO se muestra por pantalla, recuperamos los datos del propio objeto JS
...
function compruebaUdsXLote(){
	...
	jQuery.each(arrProductos, function(key, producto){
		Cantidad	= producto.Cantidad;
		//29jul22 CantidadFormat	= parseFloat(Cantidad.replace(".","").replace(",","."));
		CantidadFormat	= desformateaDivisa(Cantidad);
		UdsXLote	= '0';
		UdsXLoteFormat	= 0;
		...
		jQuery.each(producto.Ofertas, function(key2, oferta){
			if(oferta.OfertaAdjud == 'S'){
				...
				//29jul22 UdsXLoteFormat	= parseFloat(UdsXLote.replace(".","").replace(",","."));
				UdsXLoteFormat	= desformateaDivisa(UdsXLote);
				return false;
			}
		});

		//	solodebug	if(UdsXLoteFormat) debug('compruebaUdsXLote.UdsXLoteFormat:'+UdsXLoteFormat+' CantidadFormat:'+CantidadFormat+' esEntero:'+esEntero(Round((CantidadFormat/UdsXLoteFormat)*1000)/1000));
		...
		if(UdsXLoteFormat && !esEntero(Round((CantidadFormat/UdsXLoteFormat)*1000)/1000)){
			toReturn = false;

			//	solodebug	debug('compruebaUdsXLote.UdsXLoteFormat:'+UdsXLoteFormat+' CantidadFormat:'+CantidadFormat+' esEntero:'+esEntero(Round((CantidadFormat/UdsXLoteFormat)*1000)/1000)+ ' return FALSE');

			return false;
		}
	});

	//	solodebug	debug('compruebaUdsXLote.UdsXLoteFormat:'+UdsXLoteFormat+' CantidadFormat:'+CantidadFormat+' esEntero:'+esEntero(Round((CantidadFormat/UdsXLoteFormat)*1000)/1000)+ ' return '+toReturn);
...
function calcularCantidadesAuto(){
	...
	jQuery.each(arrProductos, function(key, producto){
		...
		if((UdsXLoteFormat>0) && !esEntero(Round((CantidadFormat/UdsXLoteFormat)*1000)/1000)){
			cont++
			...
			thisAviso = thisAviso.replace('[[UNIDADES_NEW]]', newCantidadStr.replaceAll('.',','));
			
			//	14ene20 Pedimos confirmación antes de hacer el cambio
			if (confirm(thisAviso))
			{

				//	22/2/16	ET	ListaProductos += producto.IDProdLic + '|' + newCantidad.replace('PUNTO',',') + '#';
				ListaProductos += producto.IDProdLic + '|' + newCantidadStr.replaceAll('.',',') + '#';
...
function calcularCantidadesAutoAgr()
{
	...
	jQuery.each(arrProductosPorCentro, function(key, productoPorCentro)
	{
		...
		for (j=0;j<productoPorCentro.Centros.length;++j)
		{
			...
			IDCentro	= productoPorCentro.Centros[j].IDCentro;
			Cantidad	= productoPorCentro.Centros[j].Cantidad;
			//29jul22 CantidadFormat	= parseFloat(Cantidad.replace("PUNTO","").replace(",","."));
			CantidadFormat	= desformateaDivisa(Cantidad);
			UdsXLote	= '0';
			UdsXLoteFormat	= 0;
			thisAviso = alrt_avisoCambioUnidades;
			...
			*/
			...
			jQuery.each(productoConOfertas.Ofertas, function(key2, oferta){
				...
				if(oferta.OfertaAdjud == 'S'){
					...
					UdsXLote	= oferta.UdsXLote;
					//29jul22 UdsXLoteFormat	= parseFloat(UdsXLote.replace("PUNTO","").replace(",","."));
					UdsXLoteFormat	= desformateaDivisa(UdsXLote);
					return false;
				}
			});
			...
			if(UdsXLoteFormat && !esEntero(Round((CantidadFormat/UdsXLoteFormat)*1000)/1000)){
				...
				ListaProductos += IDCentro + '|' + IDProductoLic + '|' + newCantidadStr.replaceAll('.',',') + '#';
				...
				{
					CantidadTotal+=parseFloat(productoPorCentro.Centros[l].Cantidad.replaceAll(',','.'));	//REVISAR PARA desformateaDivisa
				...
				arrProductos[PosProducto].Cantidad=CantidadTotal.toString().replaceAll('.',',');
			}
		}
	});
...
function validarPedidosMinimos(option){
	...
	jQuery.each(arrProductos, function(key, producto){
		jQuery.each(producto.Ofertas, function(key2, oferta){
			if(oferta.OfertaAdjud == 'S'){
				Cantidad		= producto.Cantidad;
				//29jul22 CantidadFormato		= Cantidad.replace("PUNTO","").replace(",",".");
				CantidadFormato		= desformateaDivisa(Cantidad);
//				(mostrarPrecioIVA == 'S') ? PrecioOferta = oferta.PrecioIVA : PrecioOferta = oferta.Precio;
				PrecioOferta = oferta.Precio;
				//29jul22 PrecioOfertaFormato	= PrecioOferta.replace("PUNTO","").replace(",",".");
				PrecioOfertaFormato	= desformateaDivisa(PrecioOferta);
	...
	jQuery.each(arrProvTotal, function(key, value){
		//29jul22 if(value > 0 && value < arrPedMin[key].replace("PUNTO","").replace(",",".")){
		if(value > 0 && value < desformateaDivisa(arrPedMin[key])){
			error = true;
...
function dibujaSubirFT(ID){
	...
	thisCell = cellStartClass.replace('#CLASS#', 'textLeft trenta');
	...
	thisMacro = thisMacro.replace('#ID#', 'inputFileDoc_FT_' + ID);
	...
	thisCell = cellStart;
	thisCell += divStartIDAlign.replace('#ID#', 'waitBoxDoc_FT_' + ID).replace('#ALIGN#', 'center');
	thisCell += '&nbsp;';
	thisCell += divEnd;
	thisCell += divStartIDStyAlg.replace('#ID#', 'confirmBox_FT_' + ID).replace('#STYLE#', 'display:none;').replace('#ALIGN#', 'center');
...
//	24mar21 Recupera las observaciones del pedido correspondientes al centro seleccionado
function recuperaObservacionesPedido(IDCentroUsuario)
{
	var d = new Date();
	...
	jQuery.ajax({
		url:	"http://www.newco.dev.br/Gestion/Comercial/ObservacionesPedidoAJAX.xsql",
		...
		success: function(objeto){
			var data = JSON.parse(objeto);
			jQuery("#LIC_OBSPEDIDO").val(data.Observaciones);
		}
	});
}
...
function calcularFloatingBox_EST(){
	...
	jQuery('#FBConsHist').html(FormatoNumero(ConsumoHist.toFixed(2)));
	jQuery('#FBConsObj').html(FormatoNumero(ConsumoObj.toFixed(2)));
...
function FichaProductoOfe(IDProdLic,IDProdOfeLic){
	MostrarPagPersonalizada('http://www.newco.dev.br/Administracion/Mantenimiento/Productos/PRODetalle2022.xsql?LIC_PROD_ID='+IDProdLic+'&LIC_OFE_ID='+IDProdOfeLic,'',100,100,0,0);
}
		...
		success: function(objeto){
			...
			if(data.Resultado.Estado == 'OK')
			{
				...
				else if(tipo == 'DOC_PROV_LICITACION')
				{
					...
					jQuery("#divDatosDocumento").hide();
					jQuery("#LIC_PROV_IDDOCUMENTO").val('');
...
function BorrarTodosProductos()
{
	...
	{
		document.forms['form1'].action="http://www.newco.dev.br/Gestion/Comercial/MantLicitacionAcciones2022.xsql?ACCION=BORRARPRODUCTOS&LIC_ID="+IDLicitacion;
...
function BorrarTodosProveedores()
{
	...
	{
		document.forms['form1'].action="http://www.newco.dev.br/Gestion/Comercial/MantLicitacionAcciones2022.xsql?ACCION=BORRARPROVEEDORES&LIC_ID="+IDLicitacion;
		SubmitForm(document.forms['form1']);
	}
}
...
function inicializaFloatingBox()
{
	...
	for(var j=0;j<arrProductos.length;j++)
	{
		...
		var PrecioTxt =((mostrarPrecioIVA == 'S')&&(IDPais!=55)) ?  arrProductos[j].PrecioHistIVA : arrProductos[j].PrecioHist;			//6jul23 annadido parche por pais, proveedor mal definido
		var Precio= ((PrecioTxt=='')?0:parseFloat(PrecioTxt.replace(",",".")));
...
function Recarga()
{
	window.open('http://www.newco.dev.br/Gestion/Comercial/MantLicitacion2022.xsql?LIC_ID='+IDRegistro,'_self');
...
//	15jun21 Muestra el boton para guardar cambio de usuario Gestor o Autor
...
// Cambia usuario Gestor o Autor de la licitacion por otro (debe ser del mismo centro)
...
function PermitirDescarga()
{
	...
	{
		document.forms['infoPedido'].action="http://www.newco.dev.br/Gestion/Comercial/MantLicitacionAcciones2022.xsql?ACCION=PERMITIRDESCARGA&LIC_ID="+IDLicitacion;
		SubmitForm(document.forms['infoPedido']);
	}
	else
