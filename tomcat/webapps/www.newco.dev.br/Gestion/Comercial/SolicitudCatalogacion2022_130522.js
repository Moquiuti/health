//	JS para solicitudes de catalogación (buscador y ficha)
//	Ultima revisión: ET 16may22 10:36 SolicitudCatalogacion2022_130522.js
...
function globalEvents(){
	// Click en 'Alta Producto'
	jQuery("#pes_lAltaProducto").click(function(){
		
		MenuActivo('pes_lAltaProducto');
    	jQuery("#lAltaProducto").show();
    	jQuery("#lAltaFichero").hide();
	});

	// Click en 'Alta Fichero'
	jQuery("#pes_lAltaFichero").click(function(){
		MenuActivo('pes_lAltaFichero');
    	jQuery("#lAltaProducto").hide();
    	jQuery("#lAltaFichero").show();
	});
}

//	13may22 Destaca el menu activo
function MenuActivo(Nombre)
{
	jQuery("a.MenuLic").attr('class', 'MenuLic MenuInactivo');
	jQuery("#"+Nombre).attr('class', 'MenuLic MenuActivo');
}

function abrirCatalogoPrivado(SolProdID, desc){
    MostrarPagPersonalizada('http://www.newco.dev.br/Administracion/Mantenimiento/Productos/CatalogoPrivadoProductoEmpresa2022.xsql?IDEMPRESA=1&ORIGEN=SOLICITUD&INPUT_SOL=PROD_ESTAN_' + SolProdID + '&DESCRIPCION=' + desc,'Catalogo privado producto',100,70,0,40);
}

function abrirCatalogoPoveedores(SolProdID, desc){
    MostrarPagPersonalizada('http://www.newco.dev.br/Gestion/Comercial/BuscarProveedoresEval2022.xsql?SOL_PROD_ID='+ SolProdID +'&DESCRIPCION='+ desc+'&ORIGEN=SOLICITUD', +'Catalogo proveedores',100,70,0,40);
}

function abrirFichaCatalogacion(SolProdID, cliente){
    var idProd = document.forms['SolicitudCatalogacion'].elements['IDPROD_'+SolProdID].value
    MostrarPagPersonalizada('http://www.newco.dev.br/ProcesosCdC/MantenimientoReducido/MantenimientoReducido2022.xsql?PRO_ID=' + idProd +"&EMP_ID=" + cliente + "&SOL_PROD_ID="+ SolProdID +"&ORIGEN=SOLICITUD", +'Ficha de catalogación',100,70,0,40);
}
...
function recuperarIDProdEstandard(idEmpresa,solProdID){
    ...
    var ref = document.forms['SolicitudCatalogacion'].elements['PROD_ESTAN_'+solProdID].value;
     ...
     jQuery.ajax({
		...
		success: function(objeto){
			...
			if(data.Resultado.Estado > 1){
                            ...
                            document.forms['SolicitudCatalogacion'].elements['IDPRODUCTO_'+solProdID].value = data.Resultado.Estado;
...
function anadirProductoConID(idProd, solProdID){
    var form = document.forms['SolicitudCatalogacion'];
...
function abrirSolicitud(IDSol){
    window.open('SolicitudCatalogacion2022.xsql?SC_ID='+IDSol,'_self',false);
}
...
function anadirProducto(){
    ...
    var cantidad = form.elements['CANTIDAD'].value;
		...
		if(precio !== ''){
			//29jul22 var precioFmt = precio.replace(',', '.');
			var precioFmt = desformateaDivisa(precio);
			if(isNaN(precioFmt)){
			 ...
			 return;
				...
				//	utilizan el "." como separador decimal, corregimos manualmente
...
function NuevaSolicitudCatalogacion(){
	window.open('NuevaSolicitudCatalogacion2022.xsql','_self',false);
}
...
function ValidarFormulario(oForm,IDEstado){
    ...
    if(oForm.elements['SC_PRODUCTO'] && oForm.elements['SC_PRODUCTO'].value === ''){
			oForm.elements['SC_PRODUCTO'].focus();
			alert(producto_obligatorio);
			jQuery('#BotonSubmit').show();
			return;
		}

		if(oForm.elements['SC_PRECIO'] && oForm.elements['SC_PRECIO'].value !== ''){
      //29jul22 var precio = oForm.elements['SC_PRECIO'].value.replace(',', '.');
      var precio = desformateaDivisa(oForm.elements['SC_PRECIO'].value);
      if(isNaN(precio)){
			 oForm.elements['SC_PRECIO'].focus();
			 alert(precio_mal_formato);
			 jQuery('#BotonSubmit').show();
			 return;
      }else{
      	//29jul22	oForm.elements['SC_PRECIO'].value = precio.replace('PUNTO', ',');
        oForm.elements['SC_PRECIO'].value = formateaDivisa(precio);
      }
		}

    if(oForm.elements['SC_CANTIDAD'] && oForm.elements['SC_CANTIDAD'].value !== ''){
      if(isNaN(oForm.elements['SC_CANTIDAD'].value) || (oForm.elements['SC_CANTIDAD'].value % 1 !== 0)){
        oForm.elements['SC_CANTIDAD'].focus();
  			alert(cantidad_mal_formato);
  			jQuery('#BotonSubmit').show();
  			return;
      }
...
*/
		...
		oForm.target	= '_self';
		AsignarAccion(oForm,'http://www.newco.dev.br/Gestion/Comercial/NuevaSolicitudCatalogacionSave2022.xsql')
		SubmitForm(oForm);
		return;
		...
		oForm.encoding	= 'application/x-www-form-urlencoded';
		oForm.target	= '_self';
		AsignarAccion(oForm,'http://www.newco.dev.br/Gestion/Comercial/SolicitudCatalogacionSave2022.xsql')
		SubmitForm(oForm);
		return;
...
function anadirFilasProductos(oForm){
	...
	jQuery.each(arrRows, function(index, row){
		...
		//29jul22 precio      = ("4" in arrProd) ? arrProd[4].replace('PUNTO', ',') : '';
		precio      = ("4" in arrProd) ? formateaDivisa(arrProd[4]) : '';
...
function cambiarEstado(IDEstado){
	...
	jQuery.ajax({
		cache:	false,
		url:	'http://www.newco.dev.br/Gestion/Comercial/CambiarEstadoSolicitudAJAX.xsql',
		type:	"GET",
...
//	13may22 Abre el formulario para nueva solicitud
function NuevaSolicitud(){
	document.location='http://www.newco.dev.br/Gestion/Comercial/NuevaSolicitudCatalogacion2022.xsql';
}


//	13may22 Abre la ficha de solicitud de catalogacion
function Solicitud(IDSolicitud){
	document.location="http://www.newco.dev.br/Gestion/Comercial/SolicitudCatalogacion2022.xsql?SC_ID="+IDSolicitud;
}

//	13may22 Vuelve a la pagina del buscador de solicitudes de catalogacion
function Solicitudes(){
	document.location="http://www.newco.dev.br/Gestion/Comercial/SolicitudesCatalogacion2022.xsql";
}


//	13may22 Cambia el estado de la solicitud
function GuardaYCambiaEstado(Estado)
{
	ValidarFormulario(document.forms['SolicitudCatalogacion'],Estado);
}
