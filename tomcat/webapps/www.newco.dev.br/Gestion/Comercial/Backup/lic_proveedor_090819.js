...
//	Ultima revisión ET 24jul19 10:03
...
function publicarPedidoMinimo(oForm){
	...
	jQuery.ajax({
		...
		type:	"GET",
		data:	"LIC_PROV_ID="+IDProvLic+"&LIC_PROV_PEDIDOMINIMO="+encodeURIComponent(PedidoMinimoProv)+"&LIC_PROV_COMENTARIOSPROV="+ComentariosProv+"&LIC_PROV_FRETE="+Frete+"&LIC_PROV_PLAZOENTREGA="+PlazoEntrega+"&_="+d.getTime(),
...
function AnadirOfertas(oForm){
	...
	jQuery('table#lProductos_PROVE tbody tr.infoProds').each(function(index, element){
		...
		if (IDPais==55)
		{
			RefProv		= arrProductos[thisPosArr].RefEstandar.toUpperCase()+'-'+jQuery('#UdsLote_' + thisPosArr).val()+'-'+jQuery('#Marca_' + thisPosArr).val().toUpperCase();	//	Misma formula que se aplica en la base de datos
		}
		else
		{
...
function dibujaTablaProductosPROVE(){
	...
	if(totalProductos > 0){
		...
		console.log('dibujaTablaProductosPROVE. numOfertas:'+numOfertas+' ofertaVacia:'+ofertaVacia);
...
function BorrarOferta(thisPosArr)
{
	...
	guardarOfertaFila(thisPosArr, 'BORRAR');
...
function guardarOfertaFila(thisPosArr, tipo)
{
	...
	//solodebug	
	console.log("guardarOfertaFila. thisPosArr:"+thisPosArr+' RecuperaPosicion:'+RecuperaPosicion(thisPosArr)+ ' Indice:'+Indice+" tipo:"+tipo);
...
*/
	...
	if(enviarOferta){
		...
		if (IDPais==55)
		{
			RefProv		= arrProductos[Indice].RefEstandar.toUpperCase()+'-'+jQuery('#UdsLote_' + thisPosArr).val()+'-'+jQuery('#Marca_' + thisPosArr).val().toUpperCase();	//	Misma formula que se aplica en la base de datos
		}
		else
		{
			RefProv = (jQuery('#RefProv_' + thisPosArr).length) ? jQuery('#RefProv_' + thisPosArr).val() : '' ;
...
function ActualizarOfertas(Referencias)
{
	...
	for (var i=0;i<=numRefs;++i)
	{
		...
		if ((Ref=='')||(Ref=='REFCLIENTE'))
		{
			...
			//solodebug		
			console.log('>>>> ActualizarOfertas [INICIO] Oferta ('+i+'). Ref:'+Ref+' Saltando línea vacía');
		...
		else
		{
			...
			for (var j=0;((j<arrProductos.length) && (!Encont));++j)
			{
				...
				if ((arrProductos[ColumnaOrdenadaProds[j]].RefCliente==Ref)||(arrProductos[ColumnaOrdenadaProds[j]].RefEstandar==Ref))
				{
					...
					{
						//solodebug	console.log("ActualizarOfertas. Comprobando:"+j+" RefProv:"+arrProductos[ColumnaOrdenadaProds[j]].Oferta.RefProv+" NumAlt:"+arrProductos[ColumnaOrdenadaProds[j]].NumAlternativas+ 'Completar:'+Completar);
							...
							||((!Completar)&&(arrProductos[ColumnaOrdenadaProds[j]].Oferta.RefProv==RefProv)))		//	Misma ref.proveedor
						{
							...
							guardarOfertaFila(arrProductos[ColumnaOrdenadaProds[j]].linea-1, '')
						...
						else
						{
							...
							if (!ExisteOtra)
							{
								...
								//solodebug	console.log("ActualizarOfertas. Crear nuevo:"+RefProv);
								...
								*/
								...
								guardarOfertaFila(totalProductos-1, '');
								ofertaVacia='N';
