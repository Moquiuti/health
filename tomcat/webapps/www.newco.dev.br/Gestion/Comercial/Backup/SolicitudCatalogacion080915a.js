...
function globalEvents(){
    ...
    jQuery(".catalogo").change(function() {
        document.reload();
    });
...
function abrirCatalogoPoveedores(SolProdID, desc){
    MostrarPagPersonalizada('http://www.newco.dev.br/Gestion/Comercial/BuscarProveedoresEval.xsql?SOL_PROD_ID='+ SolProdID +'&DESCRIPCION='+ desc, +'Catalogo proveedores',100,70,0,40);
}

function abrirFichaCatalogacion(SolProdID){
    var idProd = document.forms['SolicitudCatalogacion'].elements['IDPROD_'+SolProdID].value
    MostrarPagPersonalizada('http://www.newco.dev.br/ProcesosCdC/MantenimientoReducido/MantenimientoReducido.xsql?PRO_ID=' + idProd + "&SOL_PROD_ID="+ SolProdID +"&ORIGEN=SOLICITUD", +'Ficha de catalogación',100,70,0,40);
}

//una vez que recuperamos del catalogo privado la ref estandar, buscamos el id prod estandar que guardamos en un campo hidden y desp retrasmitimos de nuevo el producto con anadirProducto aï¿½adiendole el id prod estandar
...
//aï¿½adimos el id prod estandar despues de haberlo recuperado de la ref estandar con recuperarIDProdEstandard
...
// para los textos que los enseï¿½e bien
...
//cuando guardamos una nueva solicitud abrimos la pï¿½gina de solicitud despues para aï¿½adir mas productos
function abrirSolicitud(IDSol){
    window.open('SolicitudCatalogacion.xsql?SC_ID='+IDSol,'_self',false);
}

//aï¿½adimos uno a uno los productos de la solicitud despues de haberla creada con un producto por defecto
function anadirProducto(form){
    ...
    jQuery.ajax({
		...
		url:	'http://www.newco.dev.br/Gestion/Comercial/AnadirProductosSolicitudAJAX.xsql',
		type:	"GET",
		data:	"IDSOLICITUD="+IDSol+"&REFCLIENTE="+refCli+"&REFPROVEEDOR="+refProv+"&PRODUCTO="+prod+"&PROVEEDOR="+prov+"&PRECIO="+precio+"&CONSUMO="+consumo+"&IDPRODESTANDAR=",
		contentType: "application/xhtml+xml",
		success: function(objeto){
			var data = eval("(" + objeto + ")");
...
// Abre la pï¿½gina para crear una nueva solicitud de catalogaciï¿½n
function NuevaSolicitudCatalogacion(){
	window.open('NuevaSolicitudCatalogacion.xsql','_self',false);
}

// Funciï¿½n que valida los campos de los formularios antes de hacer el submit
function ValidarFormulario(oForm,IDEstado){
	if(IDEstado == 'NUEVA'){
		// Escondemos el botï¿½n de enviar
...
//Enseï¿½ar el input file para subir documentos
...
//	var action = 'http://' + location.hostname + '/' + lang + 'confirmCargaDocumento.xsql';
	...
	var tipoDocHTML	= document.forms['SolicitudCatalogacion'].elements['TIPO_DOC_HTML'].value;
	// En este caso usamos el IDEmpresa del cliente como parï¿½metro de entrada para IDPROVEEDOR
	var IDEmpresa	= '';
