//	Ultima revision ET 7dic18 11:16
...
function GuardarProductoSel(IDProdLic){
	...
	if ((IDOfertaLic=='')&&(!confirm(alrt_NoHaSeleccionadoOferta)))		//	30nov18	Solicitamos confirmación antes de guardar producto sin oferta seleccionada
		return;
	
	//solodebug	
	console.log("IDLIC="+IDLicitacion+"&IDPRODUCTOLIC="+IDProdLic+"&IDOFERTA="+IDOfertaLic+"&IDMOTIVO="+IDMotivo+"&MOTIVO="+encodeURIComponent(Motivo));
			...
			jQuery.ajax({
				...
				success: function(objeto){
					...
					if(data.Resultado.Estado == 'OK'){
						...
						try
						{
							console.log("actualizarRadio. INICIO");
							//window.opener.actualizarRadio(IDProdLic, IDOfertaLic);
							console.log("actualizarRadio. FIN");
						}
						catch(e)
						{
						}
