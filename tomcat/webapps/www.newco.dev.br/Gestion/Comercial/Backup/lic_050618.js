...
//	ultima revision: ET 5jun16 10:31
...
function ValidarFormulario(oForm, nombre, flag){
	...
	//console.log(comparaFechas(oForm.elements['LIC_FECHADECISION'].value, oForm.elements['LIC_FECHAADJUDICACION'].value));
		...
		if ((jQuery('#CHK_LIC_URGENTE').length) && (oForm.elements['CHK_LIC_URGENTE'].checked)){	//24may18	Primero comprobamos que existe el checkbox
			oForm.elements['LIC_URGENTE'].value='S';
		}
		else{
			if (jQuery('#CHK_LIC_URGENTE').length){			//	Si existe el checkbox, asignamos su valor, sino mantenemos el que hubiera
				oForm.elements['LIC_URGENTE'].value='N';
			}
		}
		
		if ((jQuery('#CHK_LIC_SOLICDATOSPROV').length) && (oForm.elements['CHK_LIC_SOLICDATOSPROV'].checked)){	//24may18	Primero comprobamos que existe el checkbox
			oForm.elements['LIC_SOLICDATOSPROV'].value='S';
		}
		else{
			if (jQuery('#CHK_LIC_SOLICDATOSPROV').length){			//	Si existe el checkbox, asignamos su valor, sino mantenemos el que hubiera
...
function guardarOfertaFila(thisPosArr){
    ...
    var ProdNombre1    = arrProductos[thisPosArr].Nombre;    //7abr18
	var RefCliente	= (arrProductos[thisPosArr].RefCliente != '') ? arrProductos[thisPosArr].RefCliente : arrProductos[thisPosArr].RefEstandar;
	var precioObj	= arrProductos[thisPosArr].PrecioObj;
	...
	if(!errores && esNulo(precioFormat)){
		...
		if	((arrProductos[thisPosArr].PrecioMin!='' && precioFormat < parseFloat(arrProductos[thisPosArr].PrecioMin.replace(/\./g,"").replace(',', '.')))||(arrProductos[thisPosArr].PrecioMax!='' && precioFormat > parseFloat(arrProductos[thisPosArr].PrecioMax.replace(/\./g,"").replace(',', '.'))))
		{
			controlPrecio +=val_PrecioFueraRango.replace("[[REF]]",RefCliente).replace("[[PRECIO]]",precio).replace("[[UDBASICA]]",arrProductos[thisPosArr].UdBasica).replace("[[MIN]]",arrProductos[thisPosArr].PrecioMin).replace("[[MAX]]",arrProductos[thisPosArr].PrecioMax)
		}
...
*/
	...
	if(enviarOferta){
		...
		var Descripcion = encodeURIComponent(ScapeHTMLString(jQuery('#Desc_' + thisPosArr).val()));
		var Marca = encodeURIComponent(ScapeHTMLString(jQuery('#Marca_' + thisPosArr).val()));
...
function StringtoCSV(Cadena)
{
	...
	//solodebug
	if (CadCSV!=Cadena) console.log('StringtoCSV. ['+Cadena+'] -> ['+CadCSV+']');
