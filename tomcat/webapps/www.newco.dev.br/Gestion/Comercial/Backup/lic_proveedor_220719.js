...
//	Ultima revisión ET 22jul19 10:03
...
function PreparaLineaProducto(numProd, contLinea)
{
	...
	//				thisCell += '<img src="" class="static"/>';
		...
		if((arrProductos[ColumnaOrdenadaProds[numProd]].Oferta.Precio == '')&&(arrProductos[ColumnaOrdenadaProds[numProd]].OfertaAnterior === 'S'))
		{
			...
			thisMacro	= macroEnlaceAccIDStyle.replace('#CLASS#', 'guardarOferta btnGris');
	...
	if(enviarOferta){
		...
		jQuery.ajax({
			...
			beforeSend: function(){
				...
				jQuery('#btnGuardar_'+thisPosArr).attr('class','guardarOferta btnDestacado gris');
				jQuery('#btnAlter_'+thisPosArr).attr('class','guardarOferta btnDestacado gris');
			},
			error: function(objeto, quepaso, otroobj){
