...
//	17/9/16 ET Al editar línea a línea (cantidades por centro o oferta proveedor) solo mostrar el botón de "guardar" si hay cambios en la línea
//	17/9/16 ET Recargamos toda la página si hay cambios importantes en los productos
//	20/9/16 ET la recarga anteriro solo la hacemos si son muchos los proveedores que han informado la licitacion
//	21/9/16 ET Solucionamos varios problemas debidos al cambio del día 20
//	22/9/16 ET Solucionamos varios problemas debidos al cambio del día 20
//	28/9/16 ET Bucle al corregir las cantidades en función de las unidades por lote
//	30/9/16 ET No se actualiza correctamente el ceontador de número de ofertas por producto
//	6/10/16	ET Problema de redondeo al corregir las unidades por lote
//	16nov16	ET Ajustes para el cambio de diseño
...
//	ultima revision: 30/12/16 12:42
...
function cargaDatosOfertasPorProveedores()
{
	...
	for(var i=0; i < arrProveedores.length; i++){
		...
		//solodebug	lProveedoresConOfertas+=arrProveedores[i].NombreCorto+'('+arrProveedores[i].IDProvLic+') ';
		//solodebug	if (i<5) alert('cargaDatosOfertasProveedor '+arrProveedores[i].IDProvLic+' tieneOfertas:'+arrProductos[0].TieneOfertas);
		
		//solodebug	if (arrProveedores[i].IDProveedor==16296) alert(arrProveedores[i].IDProveedor+'/'+arrProveedores[i].IDProvLic+' '+arrProveedores[i].NombreCorto+' TieneOfertas:'+arrProveedores[i].TieneOfertas);

		if (arrProveedores[i].TieneOfertas=='S'){
		
			//solodebug	lProveedoresConOfertas+=arrProveedores[i].NombreCorto+'('+arrProveedores[i].IDProvLic+') ';
			//solodebug	if (i<3) alert('cargaDatosOfertasProveedor '+arrProveedores[i].IDProvLic+':'+arrProductos[0].Ofertas[i].IDProvLic);

			
			//solodebug	jQuery('#idEstadoLic').offsetHeight;//show(); el show no parece ser suficiente en desarrollo, según funcone en internet lo mantendremos o no
		...
		//solodebug	else{
		//solodebug		lProveedoresSinOfertas+=arrProveedores[i].NombreCorto+' ';
		//solodebug	}
	}


	//	24set16	Recorremos el array de productos para actualizar el contador de productos seleccioandos
	//solodebug	numProdsSeleccion =	0;		//24set16	Inicializamos el contador de productos seleccionados
	//solodebug	for(var j=0; j <arrProductos.length; j++){
	//solodebug		if (arrProductos[j].TieneSeleccion == 'S'){
	//solodebug	
	//solodebug	alert('fila '+j+' tiene seleccion');
	//solodebug	
	//solodebug		 	numProdsSeleccion++;
	//solodebug		 }
	//solodebug	}
	
	//solodebug	alert('numProdsSeleccion:'+numProdsSeleccion);
...
//28set16 Para mejorar rendimiento, en caso de actualización en pantalla, recuperamosdatos desde pantalla y no vía ajax
...
function cargaDatosOfertasProveedor(LicProvID){
	...
	//pendiente	jQuery('#idEstadoLic').html(arrProveedores[columna].NombreCorto);
