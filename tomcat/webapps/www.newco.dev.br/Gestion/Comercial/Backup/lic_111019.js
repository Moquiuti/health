...
//	Ultima revisión ET 11oct19 11:00
...
var		totalProvs,			// numero total de proveedores en la licitacion
		ColumnaOrdenacionProvs	= 'NombreNorm',		//	19mar19	Por defecto ordenamos por nombre del producto 'linea',
		OrdenProvs		= '',
...
{	
	...
	if ((rol=='COMPRADOR')||(numOfertas==0))
	{
		...
		//solodebug	console.log("onloadEvents. Activar pes_lDatosGenerales");
	...
	else
	{
		...
		//solodebug	console.log("onloadEvents. Activar pes_lProductos");
	...
	jQuery("#pes_lProductosCompacta").click(function(){
		...
		//solodebug
		console.log("#pes_lProductosCompacta");
...
function pes_lProductos_Click()
{
	...
	if (PedidoMinimoPend=='S')
	{
		...
		//solodebug	console.log('pes_lProductos_Click. PedidoMinimoPend:S');
...
function cargaDatosOfertasProveedoresAjax(PosProvActual)
{
	...
	{
		debug('cargaDatosOfertasProveedoresAjax: PosProvActual:'+PosProvActual+ ' total:'+arrProveedores.length+ ' => TODOS CARGADOS');
		jQuery('#idEstadoLic').html(mensajeEstadoActual);
		ctrlCargaProv=false;
	...
	{	
		...
		jQuery.ajax({
			...
			success: function(objeto)
			{
				...
				//solodebug	debug('cargaDatosOfertasProveedoresAjax. objeto:'+objeto);

				if(data.ProductosLicitacion.ListaProductosOfertas.length > 0)
				{
				
					//solodebug	debug('cargaDatosOfertasProveedoresAjax. EACH');
					...
					jQuery.each(data.ProductosLicitacion.ListaProductosOfertas, function(key, producto)
					{
						...
						arrProductos[fila].Ofertas[PosProvActual].CantAdjud	= producto.CantidadAdjudicada;	//	Informar oferta adjudicada
...
function ValidarFormulario(oForm, nombre, flag){
	...
	if(nombre == 'datosGenerales')
	{
		...
		//solodebug	console.log('ValidarFormulario. datosGenerales. fecha dec.:'+oForm.elements['LIC_FECHADECISION'].value);
		...
		else
		{
			...
			//solodebug	console.log('ValidarFormulario. datosGenerales. Comprobar fecha. Hoy:'+hoy+'...'+dd+'/'+mm+'/'+yyyy+' fecha dec.:'+oForm.elements['LIC_FECHADECISION'].value+' comparafechas:'+comparaFechas(oForm.elements['LIC_FECHADECISION'].value, dd+'/'+mm+'/'+yyyy));
			
			if(!errores && comparaFechas(oForm.elements['LIC_FECHADECISION'].value, dd+'/'+mm+'/'+yyyy) == '<')
			{
				errores++;
		...
		//solodebug	console.log('ValidarFormulario.datosGenerales. msgError:'+msgError);
		...
		{
			//solodebug	console.log('ValidarFormulario.datosGenerales. '+oForm.elements['LIC_FECHADECISION'].value
			...
			if(!errores && (oForm.elements['LIC_MESES'].value==0) && ((!comparaFechas(oForm.elements['LIC_FECHADECISION'].value, oForm.elements['LIC_FECHAENTREGAPEDIDO'].value) == '>') ||
					(comparaFechas(oForm.elements['LIC_FECHADECISION'].value, oForm.elements['LIC_FECHAENTREGAPEDIDO'].value) == '=')))
			{
				errores++;
				//alert(val_FechaDecisionAntigua);
	...
	//debug(comparaFechas(oForm.elements['LIC_FECHADECISION'].value, oForm.elements['LIC_FECHAADJUDICACION'].value));
		...
		console.log('ValidarFormulario.datosGenerales. errores:'+errores+' msgError:'+msgError);
	...
	if(nombre == 'provPedidoMinimo'){
		...
		//solodebug debug("provPedidoMinimo. isLicAgregada:"+isLicAgregada);
		
		/*if (isLicAgregada=='S') Desactivamos por ahora el pedido minimo por centro
		{
			//	Recorre los textbox de pedido mínimo
			var PedMinPorCentro='';
			jQuery(".chkpedmin").each(function()
			{
				var IDCentro	= this.id.replace("pedminimocen_","");
				var PedMinimo	= this.value;

				//solodebug	debug("COmprobar ped.min. IDCentro:"+IDCentro+" PedMinimo:" + PedMinimo);
				
				if((!errores) && (esNulo(PedMinimo)))
				{
					errores++;
					alert(val_faltaPedidoMinimo);
					this.focus();
				}
				else if(!errores && isNaN(PedMinimo.replace(",",".")))
				{
					errores++;
				...
				{
					PedMinPorCentro+=(PedMinPorCentro==''?'':'#')+IDCentro+'|'+PedMinimo;
				}
				
			});

			if (!errores)
				oForm.elements['LIC_PROV_PEDIDOMINIMO'].value=PedMinPorCentro;

			//solodebug	debug("COmprobar ped.min. PedMinPorCentro:"+ PedMinPorCentro+' LIC_PROV_PEDIDOMINIMO:'+oForm.elements['LIC_PROV_PEDIDOMINIMO'].value);
		}
		else
		{*/
...
function AnadirProductos(oForm){
	...
	//solodebug 
	debug('AnadirProductos:'+Referencias+' sendProductos:'+sendProductos+' loops:'+loops);
...
function AnadirProductosAjax(oForm, Referencias, loop, loops, error)
{
	...
	//solodebug	debug('AnadirProductosAjax: '+Referencias+ 'limit:'+limit+' sendProductos:'+sendProductos);
	...
	//solodebug	debug('AnadirProductosAjax: '+cadEnvio+' final');
...
function addDocFile(uploadForm, ID){
	...
	debug('addDocFile. form:'+uploadForm.name+' ID:'+ID);
	...
	if(uploadElem.value != ''){
		...
		debug('addDocFile. Doc:'+uploadElem.value);
		
		
	}else{
		uploadFilesDoc.splice(ID, 1);
		
		debug('addDocFile. Doc:'+uploadFilesDoc.value);
...
function cargaDoc(uploadForm, tipo, valueID){
	...
	if(msg != ''){
		...
		if(hasFilesDoc(uploadForm)){
			debug('cargaDoc. form:'+uploadForm.name+ ' tipo:'+tipo+ ' ID:'+valueID+ ' hasfiles ok!');
...
function periodicUpdateDoc(uploadForm, valueID, tipo){
	...
	debug('periodicUpdateDoc. tipo:'+tipo+ ' ID:'+valueID+' uploadForm:'+uploadForm.name);
	...
	debug('periodicUpdateDoc. tipo:'+tipo+ ' ID:'+valueID+' form:'+uploadForm.name+'::'+logFormElements(uploadForm));
...
// DC - 06oct14 - Se hace esta funcion dinamica para que se puedan subir fichas tecnicas y contratos en dos forms diferentes...
					...
					{
						debug("handleFileRequestDoc. Actualizado OK. nombreDoc:"+nombreDoc);
...
function dibujaTablaProductosOFE(){
	...
	//solodebug	MuestraMatrizOfertas();
	//solodebug	
	console.log("dibujaTablaProductosOFE INICIO. totalProductos:"+totalProductos);
...
//				thisCell += '<img src="" class="static"/>';
	...
	*/
	...
	//solodebug	
	console.log("dibujaTablaProductosOFE FINAL. totalProductos:"+totalProductos);
...
function dibujaTablaProveedoresADJ(){
	...
	if(totalProvs > 0){
		for(var i=0; i<totalProvs; i++)
		{
			if ((arrProveedores[ColumnaOrdenadaProvs[i]].IDEstadoProv=='INF')||(arrProveedores[ColumnaOrdenadaProvs[i]].IDEstadoProv=='ADJ')||(arrProveedores[ColumnaOrdenadaProvs[i]].IDEstadoProv=='FIRM')||(SoloProvInformados=='N'))
			{
...
function recalcularFloatingBox(obj, flagPedMin, ofertaGuardada){
	...
	//solodebug	13abr17	
	debug('RecalcularFloatingBox. INICIO: firstProduct'+firstProduct+ ' oldOfertaID:'+oldOfertaID+' thisOfertaID:'+thisOfertaID+ ' oldConsumoPot:'+oldConsumoPot+' oldConsumoPrev:'+oldConsumoPrev);
	...
	for(var i=0; i<ColumnaOrdenadaProds.length; i++){
		if(ColumnaOrdenadaProds[i] == thisPosArr){
			...
			if(wasChecked){			
				...
				//solodebug	13abr17	
				debug('RecalcularFloatingBox. waschecked: firstProduct'+firstProduct+ 'i:'+i+ ' oldOfertaID:'+oldOfertaID+' arrRadios[(i-firstProduct)].OfertaID:'+arrRadios[(i-firstProduct)].OfertaID);
	...
	for(var j=0; j<arrProductos[thisPosArr].Ofertas.length; j++)
	{
		...
		{	
			...
			//solodebug	
			debug('RecalcularFloatingBox.IDOferta:'+ thisOfertaID+ ' fila:'+thisPosArr+' Col:'+j+'. Estaba adjudicada:'+arrProductos[thisPosArr].Ofertas[j].OfertaAdjud+' Ahora:'+(isChecked?'S':'N'));
	...
	//solodebug	
	debug('RecalcularFloatingBox. FINAL: wasChecked:'+wasChecked+' isChecked:'+isChecked+' oldNumOfertas:'+oldNumOfertas+' newNumOfertas:'+newNumOfertas+' ConsumoPot:'+ConsumoPot+' ConsumoPrev:'+ConsumoPrev);
...
*/
		...
		//solodebug	
		debug('RecalcularFloatingBox: Mostrar. newNumOfertas:'+newNumOfertas);
	}
	else
	{
		//solodebug	
		debug('RecalcularFloatingBox: NO Mostrar. newNumOfertas:'+newNumOfertas);
	}
...
function calcularCantidadesAuto(){
	...
	jQuery.each(arrProductos, function(key, producto){
		...
		if((UdsXLoteFormat>0) && !esEntero(Math.round((CantidadFormat/UdsXLoteFormat)*1000)/1000)){
			...
			//debug(cont+' '+producto.Nombre);
			//debug(CantidadFormat+' vs '+UdsXLoteFormat);
			//debug((CantidadFormat/UdsXLoteFormat));
			//debug(Math.ceil(CantidadFormat/UdsXLoteFormat));
			//debug(newCantidad);
...
//				(mostrarPrecioIVA == 'S') ? PrecioOferta = oferta.PrecioIVA : PrecioOferta = oferta.Precio;
	...
	console.log('validarPedidosMinimos error:'+error);
...
function AgregarLicitacion()
{
	var IDLicitacionSec=jQuery("#LICITACIONES_EST").val(),
		LicitacionSec=jQuery("#LICITACIONES_EST").text();
...
{
	//solodebug	debug("inicializaFloatingBox. INICIO");

	ConsumoPot=0;ConsumoPrev=0;NumOfertas=0;numProdConOfe=0;numProdsAdjud=0; 
	...
	for(var j=0;j<arrProductos.length;j++)
	{
		...
		for(var i=0;i<arrProductos[j].Ofertas.length;i++)
		{
			...
			//solodebug
			//debug("informaCantidades("+i+") IDProdLic:"+IDProdLic+" multOfertas:"+multOfertas + ' CantAdjud:'+arrProductos[j].Ofertas[i].CantAdjud+ " ConsumoPot:"+ConsumoPot+' ConsumoPrev:'+ConsumoPrev+' Ahorro:'+Ahorro);
			...
			if (arrProductos[j].Ofertas[i].OfertaAdjud==='S')
			{
				...
				//solodebug	debug("inicializaFloatingBox("+i+") ADJUDICADA. IDProdLic:"+arrProductos[j].IDProdLic+ " IDOfertaLic:" + arrProductos[j].Ofertas[i].ID
				//solodebug		+' CantAdjud:'+CantAdjud+' PrecioOferta:'+PrecioOferta+ " ConsumoPot:"+ConsumoPot+' ConsumoPrev:'+ConsumoPrev);
	...
	debug("inicializaFloatingBox. FINAL. ConsumoPot:"+ConsumoPot+' ConsumoPrev:'+ConsumoPrev+' Ahorro:'+Ahorro+' numProdConOfe:'+numProdConOfe+' numProdsAdjud:'+numProdsAdjud+' numProvsAdj:'+numProvsAdj);
