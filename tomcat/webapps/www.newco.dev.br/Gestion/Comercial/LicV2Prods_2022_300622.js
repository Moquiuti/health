...
//	Ultima revisión ET 8jul22 12:25 LicV2Prods_2022_300622.js
...
//				thisCell += '<img src="" class="static"/>';
			...
			for(var j=0; j<arrProductos[ColumnaOrdenadaProds[i]].Ofertas.length; j++){
				...
				//solodebug
						+' IDOferta:'+arrProductos[ColumnaOrdenadaProds[i]].Ofertas[j].ID+' ColProveedor:'+ColProveedor+' TieneOfertas:'+arrConsumoProvs[ColProveedor].TieneOfertas);
				...
				//solodebug	
				debug('dibujaTablaProductosOFE ('+i+','+j+') col.prov ('+arrConsumoProvs[j].Orden + '): '+ arrConsumoProvs[j].NombreCorto +' Ofe:'+arrConsumoProvs[j].TieneOfertas);
			
				//	Inserta celdas vacias hasta llegar hasta la columna correcta
				debug('dibujaTablaProductosOFE ('+i+','+j+'). Incluir columnas vacias. INI. NumCols:'+NumColsMatriz+' ColActual:'+Columna+' ColMatrizProv:'+arrConsumoProvs[ColProveedor].ColMatriz);



				for (var k=Columna;((k<NumColsMatriz)&&(arrConsumoProvs[ColProveedor].ColMatriz<Columna));++k)
				{
					debug('dibujaTablaProductosOFE ('+i+','+j+') Col. vacia:'+k+' hasta:'+arrConsumoProvs[ColProveedor].ColMatriz);
				
					thisRow += cellStart+cellEnd;
					++Columna;
				}
				debug('dibujaTablaProductosOFE ('+i+','+j+'). Incluir columnas vacias. FIN. NumCols:'+NumColsMatriz+' ColActual:'+Columna+' ColMatrizProv:'+arrConsumoProvs[ColProveedor].ColMatriz);

				if (arrConsumoProvs[ColProveedor].TieneOfertas=='S'){	//	COmprueba proveedor con ofertas /Mantenimiento/LICITACION/PRODUCTOSLICITACION/PROVEEDORESLICITACION/PROVEEDOR/TIENE_OFERTAS
			*/
			...
			for(var k=0; k<NumColsMatriz; k++)
			{
				...
				var ColProv=-1;
				for (var l=0;((ColProv==-1)&&(l<arrConsumoProvs.length));++l)
				{
					if (arrConsumoProvs[l].ColMatriz==k) ColProv=l;
				}
				...
				else
				{
					...
					//solodebug		debug('dibujaTablaProductosOFE ('+i+','+j+') col.prov:'+arrConsumoProvs[j].Orden + ' '+ arrConsumoProvs[j].NombreCorto + ' Ofe:'+arrConsumoProvs[j].TieneOfertas
					...
					{	// Si esta oferta esta informada
						...
						if (isLicMultiopcion=='N')
						{
							...
							//solodebug			debug('dibujaTablaProductosOFE ('+i+','+j+') col.prov:'+arrConsumoProvs[j].Orden + ' isLicMultiopcion:'+ isLicMultiopcion);
							...
							//solodebug		debug('dibujaTablaProductosOFE ('+i+','+j+') col.prov:'+arrConsumoProvs[j].Orden + ' isLicMultiopcion NO. thisCell:'+thisCell);
						}
						else
						{
							...
							//solodebug		debug('dibujaTablaProductosOFE ('+i+','+j+') col.prov:'+arrConsumoProvs[j].Orden + ' isLicMultiopcion SI. thisCell:'+thisCell);
						}
						thisCell += cellEnd;
						thisRow += thisCell;
...
function ColocaFloatingBox()
{
	...
	//solodebug	
	debug('ColocaFloatingBox.mostrarResumenFlotante:'+mostrarResumenFlotante+' top: '+(jQuery(window).scrollTop()+90)+' left:'+(jQuery(window).width()-900));
}
...
{
	debug('recalcularFloatingBox: desactivado.');
...
/*	var thisPosArr, thisOfertaID, PrecioHist, Cantidad, 
	...
	//solodebug	
	debug('RecalcularFloatingBox: oldOfertaID:'+oldOfertaID+' thisOfertaID:'+thisOfertaID);
	...
	if(wasChecked){
		...
		//solodebug	
		debug('recalcularFloatingBox wasChecked:'+wasChecked+' thisOfertaID:'+thisOfertaID+' oldOfertaID:'+oldOfertaID);
		...
		if(thisPosArrProv !== oldPosArrProv){
			...
			//solodebug	
			debug('recalcularFloatingBox thisPosArrProv:'+thisPosArrProv+' oldPosArrProv:'+oldPosArrProv);
				...
				success: function(objeto){
					...
					if(data.ProductosActualizados.estado == 'OK'){
						...
						// Recuperamos los datos de los productos con sus ofertas via ajax
							...
							//version lic_*: recuperaDatosProductosDesdeLista(arrayCambiosProductos);
							CargaOfertas(0);
...
{
	//solodebug();	
	debug('clickRadio. codRadio:'+codRadio+' IDOfertaLic:'+IDOfertaLic);
	
	//solodebug();	debug('clickRadio. 1. Array productos:\n'+VuelcaArrayProductos(-1));
	
	pos=BuscaFilaProducto(codRadio);
	//solodebug();	
	debug('clickRadio. Producto (col.ord.:'+pos+'):'+arrProductos[pos].RefCliente+':'+arrProductos[pos].Nombre);
...
function BuscaFilaProducto(codRadio)
{
	...
	//solodebug 
	debug('BuscaFilaProducto. codRadio:'+codRadio+' Res:'+Res);
...
function SeleccionadaOfertaEnMatriz(PosProd, ID)
{
	...
	//solodebug
	debug('SeleccionadaOfertaEnMatriz (1). ID:'+ID+' Producto ('+PosProd+'):'+arrProductos[PosProd].RefCliente+':'+arrProductos[PosProd].Nombre);
	...
	//solodebug 
	debug('SeleccionadaOfertaEnMatriz (2). PosProd:'+PosProd+' ID:'+ID+' PosOfe:'+PosOfe);	
	...
	if ((isLicMultiopcion=='S')||((MesesDuracion==0)&&(isLicAgregada=='N')))			//	16dic19 Faltaba incluir la opción de SPOT	// 27ago20 Pero no para agregadas
	{
		...
		//solodebug	
		console.log('SeleccionadaOfertaEnMatriz: ID:'+ID+' isLicMultiopcion:'+isLicMultiopcion+' MesesDuracion:'+MesesDuracion);
		...
		if (arrProductos[PosProd].Ofertas[PosOfe].OfertaAdjud=='N')
		{
			...
			//solodebug	
			console.log('SeleccionadaOfertaEnMatriz: ID:'+ID+' PrecioOfertaActual:'+arrProductos[PosProd].PrecioOfertaActual+' MejorPrecio:'+arrProductos[PosProd].MejorPrecio+' MARCANDO ADJUDICADA');
		...
		else
		{
			...
			//solodebug	
			console.log('SeleccionadaOfertaEnMatriz: ID'+ID+' PrecioOfertaActual:'+arrProductos[PosProd].PrecioOfertaActual+' MejorPrecio:'+arrProductos[PosProd].MejorPrecio+'  MARCANDO NO ADJUDICADA');
	...
	else
	{
		...
		{
			//solodebug	
			console.log('SeleccionadaOfertaEnMatriz: ID. '+ID+' NO multiopcion, MARCANDO ADJUDICADA, desmarcando otras');
		...
		{
			//solodebug	
			console.log('SeleccionadaOfertaEnMatriz: ID. '+ID+' NO multiopcion, MARCANDO NO ADJUDICADA');
	...
	//	solodebug
	debug('SeleccionadaOfertaEnMatriz. Array productos:\n'+VuelcaArrayProductos(PosProd));
...
function IncluirProductosPorRef()
{
	...
	if(!errores && (IDPais!=55) && (oForm.elements['LIC_TIPOIVA'].value < 0 || oForm.elements['LIC_TIPOIVA'].value == '')){
		errores++;
	...
	if (loop==loops)
	{
		if (!error)
		{
			...
			//11dic20	{
				...
				recuperaDatosProductos();			//20jul22
