...
<!--
	...
	Ultima revision: 27jul23 11:00 LicitacionV2_2022_270723.js LicV2Prods_2022_270723.js LicV2Provs_2022_170123.js LicV2Centros_2022_100722.js
-->
...
<head>
	...
	var cargaOfertasStatus='Inicio';			//	11ene23 Informa del estado de carga de ofertas, para evitar acciones durante la carga
	...
	var isAdmin		= '<xsl:choose><xsl:when test="/Mantenimiento/LICITACION/ADMIN">S</xsl:when><xsl:otherwise>N</xsl:otherwise></xsl:choose>';		//	14abr23
	...
	var abrirTodosProds ='S';			//	10oct22 Mostrar todos los productos abiertos por defecto
	...
	//	3mar23 Nombres niveles
	var txtNivel1='<xsl:value-of select="/Mantenimiento/LICITACION/NOMBRESNIVELES/NIVEL1"/>';
	var txtNivel2='<xsl:value-of select="/Mantenimiento/LICITACION/NOMBRESNIVELES/NIVEL2"/>';
	var txtNivel3='<xsl:value-of select="/Mantenimiento/LICITACION/NOMBRESNIVELES/NIVEL3"/>';

	//	Textos para construir tablas de datos
	var str_Abrir		= '<xsl:value-of select="document($doc)/translation/texts/item[@name='Abrir']/node()"/>';
	var str_Cerrar		= '<xsl:value-of select="document($doc)/translation/texts/item[@name='Cerrar']/node()"/>';
	...
	var strObj='<xsl:value-of select="document($doc)/translation/texts/item[@name='Obj']/node()"/>';
	var strNumPedidos='<xsl:value-of select="document($doc)/translation/texts/item[@name='eis_numero_pedidos']/node()"/>';
	var strCantidad='<xsl:value-of select="document($doc)/translation/texts/item[@name='cant']/node()"/>';
	...
	var strUdBasica='<xsl:value-of select="document($doc)/translation/texts/item[@name='Ud']/node()"/>';
	...
	var	strAvisoCantidad='<xsl:value-of select="document($doc)/translation/texts/item[@name='At_cant_oferta']/node()"/>';
	var	strAvisoPrecio='<xsl:value-of select="document($doc)/translation/texts/item[@name='At_valor_oferta']/node()"/>';
	var	strAvisoPedMin='<xsl:value-of select="document($doc)/translation/texts/item[@name='Pedido_bajo_minimo']/node()"/>';
	...
	<!-- Textos y avisos incluyendo productos -->
	...
	<!-- 22set22 Avisos enviar pedidos	-->
	var str_RevisarPedMinimo	= '<xsl:value-of select="document($doc)/translation/texts/item[@name='revisar_cantidad_producto_por_pedminimo_con_datos']/node()"/>';
	var str_RevisarCantProd		= '<xsl:value-of select="document($doc)/translation/texts/item[@name='revisar_cantidad_producto_por_unidadeslote_con_datos']/node()"/>';
	...
	<!-- 30set22 avisos para descartar ofertas	-->
	...
	var alrt_NoBorrarOfertaAdjudicada	= '<xsl:value-of select="document($doc)/translation/texts/item[@name='No_borrar_oferta_adjudicada']/node()"/>';
	...
	<!-- 25abr23 Programacion de entregas	-->
	...
	<!--23set22	<xsl:if test="count(/Mantenimiento/LICITACION/PROVEEDORESLICITACION/PROVEEDOR) &gt; 0">-->
	<xsl:for-each select="/Mantenimiento/LICITACION/PROVEEDORESLICITACION/PROVEEDOR">
		...
		items['ConsumoAdj_Float'] = desformateaDivisa('<xsl:value-of select="LIC_PROV_CONSUMOADJUDICADO"/>');			// 28oct22 Para actualizar el calculo del consumo de proveedores
		...
		items['TieneOfertas']	= '<xsl:choose><xsl:when test="TIENE_OFERTAS">S</xsl:when><xsl:otherwise>N</xsl:otherwise></xsl:choose>';	//	22set22 antes en arrConsumoProvs
	...
	-->
<!--23set22	var arrConsumoProvs	= new Array();	-->
...
-->
	...
	<xsl:for-each select="/Mantenimiento/LICITACION/PRODUCTOSLICITACION/PRODUCTO">
		...
		items['PrecioMax']	= '<xsl:value-of select="LIC_PROD_PRECIOMAXIMO"/>';
		items['Entregas']	= '<xsl:value-of select="LIC_PROD_ENTREGAS"/>';		//21abr23 Nueva funcionalidad de programacion de entregas
	...
	<script type="text/javascript" src="http://www.newco.dev.br/Gestion/Comercial/LicitacionV2_2022_270723.js"></script>
	<script type="text/javascript" src="http://www.newco.dev.br/Gestion/Comercial/LicV2Provs_2022_170123.js"></script>
	<script type="text/javascript" src="http://www.newco.dev.br/Gestion/Comercial/LicV2Prods_2022_270723.js"></script>
	...
	<div class="cabecalho_cotacao">
			...
			<xsl:if test="/Mantenimiento/LICITACION/AUTOR and /Mantenimiento/LICITACION/LIC_IDESTADO = 'EST'">
				...
				<xsl:if test="/Mantenimiento/LICITACION/AGREGAR_LICITACION">
					<xsl:call-template name="desplegable">
						...
						<xsl:with-param name="style">position:absolute;top:8px;left:500px</xsl:with-param>
					</xsl:call-template>
					&nbsp;
					<a class="btnDestacado" style="position:absolute;top:7px;left:710px" id="botonAgregarLici" href="javascript:AgregarLicitacion();">
						<xsl:value-of select="document($doc)/translation/texts/item[@name='agregar_licitacion']/node()"/>
					</a>
					&nbsp;
				</xsl:if>
			</xsl:if>			
			<span class="CompletarTitulo640">
				...
				<xsl:if test="/Mantenimiento/LICITACION/AUTOR and /Mantenimiento/LICITACION/LIC_IDESTADO = 'EST' and /Mantenimiento/LICITACION/INFORMAR_COMPRAS">
					<a class="btnDestacado" style="position:relative;top:7px;" id="botonInformarCompras" href="javascript:NoInformacionCompras();">
				...
				<xsl:if test="/Mantenimiento/LICITACION/AUTOR and /Mantenimiento/LICITACION/LIC_IDESTADO = 'COMP' and /Mantenimiento/LICITACION/COMPLETAR_COMPRA_CENTRO">					
					...
					<a class="btnDestacado" style="position:relative;top:7px;" id="botonPublicarCompra" href="javascript:PublicarDatosCompra();">
						<xsl:value-of select="document($doc)/translation/texts/item[@name='publicar_datos_compra']/node()"/>
					</a>
					&nbsp;
				</xsl:if>
				<!--19dic22 boton para SOLICITAR FIRMAS-->
				<xsl:if test="((/Mantenimiento/LICITACION/LIC_MESESDURACION>0) or (/Mantenimiento/LICITACION/LIC_PEDIDOSPENDIENTES>0)) and (/Mantenimiento/LICITACION/LIC_REQUIEREFIRMAS='S' and /Mantenimiento/LICITACION/LIC_IDESTADO!='FIRM' and not(/Mantenimiento/LICITACION/LICITACION_FIRMADA))">
					<a class="btnDestacado" style="position:relative;top:7px;" id="botonSolicitarFirmas" href="javascript:SolicitarFirmas();"><xsl:value-of select="document($doc)/translation/texts/item[@name='Sol_firmas']/node()"/></a>&nbsp;
				</xsl:if>
				<!--21dic22 boton para FIRMAR-->
				<xsl:if test="/Mantenimiento/LICITACION/LIC_IDESTADO='FIRM' and /Mantenimiento/LICITACION/BOTON_FIRMA">
					<a class="btnDestacado" style="position:relative;top:7px;" id="botonFirmar" href="javascript:FirmaORechazo('FIRMA');"><xsl:value-of select="document($doc)/translation/texts/item[@name='firma']/node()"/></a>&nbsp;
					<a class="btnDestacado" style="position:relative;top:7px;" id="botonRechazar" href="javascript:PrepararRechazo();"><xsl:value-of select="document($doc)/translation/texts/item[@name='rechazar']/node()"/></a>&nbsp;
					<span  id="txtMotivoRechazo" style="position:relative;top:7px;display:none"><label><xsl:value-of select="document($doc)/translation/texts/item[@name='motivo']/node()"/>:&nbsp;</label></span>
					<input type="text" class="campopesquisa w300px" id="motivoRechazo" style="position:relative;top:7px;display:none"/>&nbsp;
					<a class="btnDestacado" style="position:relative;top:7px;display:none" id="botonEnviarRechazo" href="javascript:FirmaORechazo('RECHAZO');"><xsl:value-of select="document($doc)/translation/texts/item[@name='enviar']/node()"/></a>&nbsp;
				</xsl:if>
				<!-- 20oct22 Botones para exportar a CSV	
				...
				&nbsp;-->
				<!-- 20oct22 Botones para exportar a CSV	-->
				<a class="btnNormal" style="position:relative;top:7px;">
					<xsl:choose>
					<xsl:when test="/Mantenimiento/LICITACION/ROL='VENDEDOR'">
						<xsl:attribute name="href">javascript:listadoExcelProv(<xsl:value-of select="/Mantenimiento/LICITACION/PRODUCTOSLICITACION/LIC_PROV_ID"/>);</xsl:attribute>
					</xsl:when>
				...
				<xsl:if test="/Mantenimiento/LICITACION/AUTOR and (/Mantenimiento/LICITACION/LIC_IDESTADO = 'EST' or /Mantenimiento/LICITACION/LIC_IDESTADO = 'COMP')">
					...
					<xsl:if test="/Mantenimiento/LICITACION/INICIAR_LICITACION">
						<a id="botonIniciarLici" style="position:relative;top:7px;" href="javascript:IniciarLicitacion('CURS');">
					...
					<xsl:if test="/Mantenimiento/LICITACION/ENVIAR_GESTOR">
						<a id="botonIniciarLici" style="position:relative;top:7px;" href="javascript:EnviarGestor();">
						<xsl:attribute name="class">
						<xsl:choose>
						<xsl:when test="/Mantenimiento/LICITACION/LIC_NUMEROLINEAS='' or /Mantenimiento/LICITACION/LIC_NUMEROLINEAS=0">btnGris</xsl:when>
						<xsl:otherwise>btnDestacado</xsl:otherwise>
				...
				<xsl:if test="/Mantenimiento/LICITACION/AUTOR and /Mantenimiento/LICITACION/LIC_PORPRODUCTO = 'S' and (/Mantenimiento/LICITACION/LIC_IDESTADO = 'INF' or /Mantenimiento/LICITACION/LIC_IDESTADO = 'CURS')"><!-- and not BLOQUEARADJUDICACION-->
					...
					<xsl:when test="/Mantenimiento/LICITACION/LIC_MULTIOPCION='S'">
						<a class="btnDestacado" id="botonAdjudicarSelec" style="position:relative;top:7px;" href="javascript:AdjudicarOfertasMultiples();">
						<xsl:value-of select="document($doc)/translation/texts/item[@name='adjudicar']/node()"/>
						</a>
						<span id="idAvanceAdjudicar" style="display:none;position:relative;top:7px;" class="clear">0/0</span>
					</xsl:when>
					<xsl:otherwise>
						<xsl:choose>
						<xsl:when test="/Mantenimiento/LICITACION/PRODUCTOSLICITACION/PRODUCTOS_OLVIDADOS">
							<a class="btnDeshabilitado" id="botonAdjudicarSelec" style="position:relative;top:7px;" href="javascript:AdjudicarOfertas();">
							<xsl:value-of select="document($doc)/translation/texts/item[@name='adjudicar']/node()"/></a>
						</xsl:when>
						<xsl:when test="/Mantenimiento/LICITACION/LIC_MESESDURACION>0 or /Mantenimiento/LICITACION/LIC_PEDIDOSPENDIENTES>0"><!--10ene23 Faltaba condicion sobre LIC_PEDIDOSPENDIENTES-->
					...
					&nbsp;
						<a class="btnDestacado" id="botonEstContrato" style="position:relative;top:7px;" href="javascript:PasarAEstadoContrato();">
				...
				<xsl:when test="((/Mantenimiento/LICITACION/AUTOR)or(/Mantenimiento/LICITACION/LICITACION_FIRMADA)) and /Mantenimiento/LICITACION/BOTON_CATALOGO_PROVEEDOR">
					<a class="btnDestacado" id="botonInsCatProv"  style="position:relative;top:7px;" href="javascript:ComprobarCatalogoProveedor();">
					...
					<span id="waitBotonInsEnPlant" style="display:none;position:relative;top:7px;" class="clear"><img src="http://www.newco.dev.br/images/loading.gif"/></span>
					&nbsp;
				</xsl:when>
				<!-- cuando lici esta contratada, y es AUTOR  boton para el usuario autor para incluir en plantillas -->
				<xsl:when test="((/Mantenimiento/LICITACION/AUTOR)or(/Mantenimiento/LICITACION/LICITACION_FIRMADA)) and /Mantenimiento/LICITACION/BOTON_CATALOGO_PRIVADO">
					<a class="btnDestacado" id="botonInsEnPlant" style="position:relative;top:7px;" href="javascript:ComprobarYAdjudicarCatalogoCliente({/Mantenimiento/LICITACION/LIC_ID});">
						<xsl:value-of select="document($doc)/translation/texts/item[@name='boton_insertar_en_plantillas']/node()"/>
					</a>
					<span id="waitBotonInsEnPlant" style="display:none;position:relative;top:7px;" class="clear"><img src="http://www.newco.dev.br/images/loading.gif"/></span>
					&nbsp;
				...
				<xsl:when test="((/Mantenimiento/LICITACION/AUTOR)or(/Mantenimiento/LICITACION/LICITACION_FIRMADA)) and (/Mantenimiento/LICITACION/LIC_IDESTADO = 'CAD' or /Mantenimiento/LICITACION/LIC_IDESTADO = 'CONT')">
					<a class="btnDestacado" id="botonRenovarLic" style="position:relative;top:7px;" href="javascript:RenovarLicitacion();">
				...
				<xsl:if test="((/Mantenimiento/LICITACION/AUTOR)or(/Mantenimiento/LICITACION/LICITACION_FIRMADA)) and /Mantenimiento/LICITACION/BOTON_PEDIDO">
					<a class="btnDestacado" id="botonGenerarPedido" style="position:relative;top:7px;" href="javascript:GenerarPedido();">
						<xsl:value-of select="document($doc)/translation/texts/item[@name='boton_generar_pedido']/node()"/>
                    </a>
					<span id="waitBotonGenPedido" style="display:none;position:relative;top:7px;" class="clear"><img src="http://www.newco.dev.br/images/loading.gif"/></span>
					&nbsp;
				</xsl:if>
				<xsl:if test="((/Mantenimiento/LICITACION/AUTOR)or(/Mantenimiento/LICITACION/LICITACION_FIRMADA)) and /Mantenimiento/LICITACION/BOTON_LICITACIONPRODUCTOSSINOFERTA">

					<a class="btnDestacado" id="botonLicitacionHija" style="position:relative;top:7px;" href="javascript:CrearLicitacionHija();">
						<xsl:value-of select="document($doc)/translation/texts/item[@name='boton_licitar_productos_no_adjudicados']/node()"/>
                    </a>
					<span id="waitBotonLicHija" style="display:none;position:relative;top:7px;" class="clear"><img src="http://www.newco.dev.br/images/loading.gif"/></span>
					&nbsp;
				</xsl:if>

				<xsl:if test="((/Mantenimiento/LICITACION/AUTOR)or(/Mantenimiento/LICITACION/LICITACION_FIRMADA)) and /Mantenimiento/LICITACION/BOTON_CONTINUARLICITACION">

					<a class="btnDestacado" id="botonContinuarLicitacion" style="position:relative;top:7px;" href="javascript:ContinuarLicitacion();">
						<xsl:value-of select="document($doc)/translation/texts/item[@name='boton_continuar_licitacion']/node()"/>
                    </a>
					<span id="waitBotonContLic" style="display:none;position:relative;top:7px;" class="clear"><img src="http://www.newco.dev.br/images/loading.gif"/></span>
					&nbsp;
				</xsl:if>

				<!-- Botones para licitaciones ACTIVAS-->
				<xsl:if test="((/Mantenimiento/LICITACION/AUTOR)or(/Mantenimiento/LICITACION/LICITACION_FIRMADA)) and (/Mantenimiento/LICITACION/LIC_IDESTADO = 'INF' or /Mantenimiento/LICITACION/LIC_IDESTADO = 'CURS')">
					<a href="javascript:MejoresPrecios();" style="vertical-align:top;"><img src="http://www.newco.dev.br/images/2022/icones/medalha.svg"/></a>
				</xsl:if>
				<!-- 23set22 Vencedores tambien para licitaciones SUS, solicitado por UNIMED	-->
				<xsl:if test="(/Mantenimiento/LICITACION/LIC_IDESTADO = 'SUS' or /Mantenimiento/LICITACION/LIC_IDESTADO = 'INF' or /Mantenimiento/LICITACION/LIC_IDESTADO = 'CURS' or /Mantenimiento/LICITACION/LIC_IDESTADO = 'CONT' or /Mantenimiento/LICITACION/LIC_IDESTADO = 'CAD')"><!-- carrito aunque no sea autor-->
					<span id="icoVencGris" style="margin-left:20px; vertical-align:top;"><img src="http://www.newco.dev.br/images/2022/icones/car2gris.png"/></span>
					<a id="icoVenc" href="javascript:Vencedores();" style="margin-left:20px; vertical-align:top;display:none;"><img src="http://www.newco.dev.br/images/2022/icones/car2.svg"/></a>
		...
		<div class="cabecalho_cotacao_linha_3">
			...
			<p><label><xsl:value-of select="document($doc)/translation/texts/item[@name='fecha_decision']/node()"/>:&nbsp;</label>
				<input class="campopesquisa" type="date" id="FECHADECISION" name="FECHADECISION" onChange="javascript:ActivaBotonGuardarVencimiento();"/>
				<input class="campopesquisa" type="time" id="HORAMINUTODECISION" name="HORAMINUTODECISION" value="{/Mantenimiento/LICITACION/LIC_HORADECISION}:{/Mantenimiento/LICITACION/LIC_MINUTODECISION}"  onChange="javascript:ActivaBotonGuardarVencimiento();"/>
				<a id="btnGuardarFechaDecision" class="btnDestacado marginLeft50" href="javascript:GuardarVencimiento()" style="display:none"><xsl:value-of select="document($doc)/translation/texts/item[@name='guardar']/node()"/></a>			
			</p>
			...
			<!--disabled="disabled"-->
				<select id="selVista" class="w200px" onChange="javascript:CambiaVista();" >
					...
					<xsl:if test="((/Mantenimiento/LICITACION/AUTOR)or(/Mantenimiento/LICITACION/VER_USUARIOS_CENTROS))">
					...
					<option id="svMatriz" value="pes_lMatriz"><xsl:value-of select="document($doc)/translation/texts/item[@name='MATRIZ']/node()"/></option>
...
<!--	Datos generales	-->
<div class="marginLeft50 divLeft Vista" id="lDatosGenerales" style="display:none">
	<xsl:choose>
	<xsl:when test="/Mantenimiento/NUEVALICITACION">
	<input type="hidden" name="DatosGenerales" value="Tabla_Datos_Generales_Nuevo"/>
	...
	<input type="hidden" name="DatosGenerales" value="Tabla_Datos_Generales_Autor"/>
	...
	<xsl:otherwise>
	<input type="hidden" name="DatosGenerales" value="Tabla_Datos_Generales"/>
		...
		<div id="lpCatPrivProductos">
			...
			<xsl:otherwise>
				<tr>
					...
					<td class="datosLeft">
						<select name="LIC_TIPOIVA_PORCAT" id="LIC_TIPOIVA_PORCAT">
...
<div id="lProductos" class="divLeft Vista">
	...
	<div id="divBuscador" class="divLeft" style="display:none">
		...
		<tr>
			...
			<th class="textRight">
				<br/>
				<a id="btnAbrirCerrar" class="btnNormal" href="javascript:AbrirOCerrarTodo();">
	...
	<div class="divLeft" id="TablaProductos">
		...
		<!--	Dejamos mucho espacio en la parte inferior, para que el scroll llegue hasta el final de la tabla	-->
		<div class="divLeft" style="height:6000px;" id="divFinPedidos">
			<br/>&nbsp;
		</div>
	</div>
			...
			<xsl:when test="/Mantenimiento/LICITACION/USUARIOSLICITACION/USUARIO">
				...
				<tr class="conhover">
					...
					<td>
						<xsl:if test="LIC_USU_FIRMA='S'">
							<xsl:choose>
							<xsl:when test="LIC_USU_FECHAFIRMA!=''">
							   <xsl:value-of select="LIC_USU_FECHAFIRMA"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="ESTADOLICITACION"/>
							</xsl:otherwise>
					...
					<td class="textLeft"><xsl:value-of select="LIC_USU_COMENTARIOS"/></td>
					<td>
	...
	<div class="overlay-container" id="progEntregas">
		<div class="window-container zoomout w1000px">
			...
			<div class="divLeft h50px fondoGris textLeft">
				<br/>
					<!--&nbsp;<span id="progEntrTitulo" class="tituloTabla"></span>-->
			...
			<div class="divLeft w1000px">
				<!--<p><xsl:value-of select="document($doc)/translation/texts/item[@name='Programacion_de_entregas']/node()"/></p>-->
				<br/>
