...
//	Ultima revisión ET 27jul23 11:10 LicitacionV2_2022_270723.js


//	Variables globales
var gl_ProductoAbierto = '-1';				//	Producto abierto
var tamPaqueteProductos = 30;				//	Tamanno del paquete de carga de productos


//	Para carga de ficheros
...
var macroEnlaceRef	= '<a href="javascript:chFichaProductoLicitacion(\'#LIC_ID#\',\'#LIC_PROD_ID#\',\'LICV2\');">';
var macroEnlaceRef2	= '<a href="javascript:chFichaProductoLicitacion(\'#LIC_ID#\',\'#LIC_PROD_ID#\',\'LICV2\');" style="text-decoration:none;">';
...
function ListaProductos()
{
	...
	for (var i=0;i<arrProductos.length;++i)
	{
		...
		if (arrProductos[i].CumpleFiltro=='S')
		{
			...
			//21abr23 linea=linea.replace('[REFCLIENTE]',((arrProductos[i].RefCliente!='')?arrProductos[i].RefCliente:arrProductos[i].RefEstandar));
			if (arrProductos[i].RefCentro!='')
				linea=linea.replace('[REFCLIENTE]',arrProductos[i].RefCentro);
			if (arrProductos[i].RefCliente!='')
				linea=linea.replace('[REFCLIENTE]',arrProductos[i].RefCliente);
			else
				linea=linea.replace('[REFCLIENTE]',arrProductos[i].RefEstandar);
...
//	20jun23 Activa el boton de vencedores
function ActivarVencedores()
{
	jQuery("#icoVencGris").hide();
	jQuery("#icoVenc").show();
}
...
function CargaOfertas(Pos)
{
	...
	//solodebug debug('CargaOfertas. Pos:'+Pos);
	...
	if (Pos==arrProductos.length)
	{
		...
		//	11oct22 Al acabar de cargar las ofertas activa el desplegable de pestannas, opcion "Matriz"
		//13feb23 jQuery('#selVista').prop('disabled', false);
		jQuery('#svMatriz').show();
		...
		var h=jQuery("#lProductos").height();
		//solodebug	
		debug("divFinPedidos height:"+h+' -> '+(h/20));
		jQuery("#divFinPedidos").height(h/20);
		
		//	20jun23 Activamos el boton de vencedores
		ActivarVencedores();
	...
	{
		//13feb23 jQuery('#selVista').prop('disabled', 'disabled');		//11oct22 desactivamos el desplegable mientras se cargan ofertas, 13feb23 solo descativamos la matriz
		jQuery('#svMatriz').hide();
	}
	...
	//solodebug	debug('CargaOfertas. listaProductos:'+listaProductos);
	...
	jQuery.ajax({
		...
		success: function(objeto){
			...
			*/
			...
			{
				//solodebug	debug('JSON OK. CargaOfertas. Recorriendo array:'+j);
				...
				if(data.OfertasProductosLic[j].posicion && data.OfertasProductosLic[j].posicion!= ''){
					...
					for (var i=0; i<data.OfertasProductosLic[j].ofertas.length; ++i)
					{
						...
						if (arrProductos[Pos].IDProdLic==6293984)
						{
							debug('CargaOfertas:'+Pos+'. Oferta:'+data.OfertasProductosLic[j].ofertas[i].contador+' Prov:'+data.OfertasProductosLic[j].ofertas[i].PROVEEDOR+' Ref:'+data.OfertasProductosLic[j].ofertas[i].LIC_OFE_REFERENCIA
								+'. COnsumo en array:'+Oferta["CONSUMO"]+'. Consumo en JSON:'+data.OfertasProductosLic[j].ofertas[i].LIC_OFE_CONSUMO);							
						}
						...
						*/
						...
						//solodebug	debug('CargaOfertas:'+Pos+'. Oferta:'+data.OfertasProductosLic[j].ofertas[i].contador+' Prov:'+data.OfertasProductosLic[j].ofertas[i].PROVEEDOR+' Ref:'+data.OfertasProductosLic[j].ofertas[i].LIC_OFE_REFERENCIA+' Precio:'+data.OfertasProductosLic[j].ofertas[i].LIC_OFE_PRECIO);
						...
						{
							debug('CargaOfertas:'+Pos+'. Oferta:'+data.OfertasProductosLic[j].ofertas[i].contador+' Prov:'+data.OfertasProductosLic[j].ofertas[i].PROVEEDOR+' Ref:'+data.OfertasProductosLic[j].ofertas[i].LIC_OFE_REFERENCIA
								+' Precio:'+Precio+'.AhorroOferta:'+AhorroOferta+' Adjudicada:'+data.OfertasProductosLic[j].ofertas[i].LIC_OFE_ADJUDICADA);							
						}*/
						...
						if (data.OfertasProductosLic[j].ofertas[i].LIC_OFE_ADJUDICADA=='S')
						{
							...
							//	27jul23 Faltaba asignar el prcio de la oferta actual
							if((arrProductos[Pos].PrecioOfertaActual==null)||(arrProductos[Pos].PrecioOfertaActual==''))
								arrProductos[Pos].PrecioOfertaActual=Precio;
							
							
						}
						else if (((Ahorro=='')||((AhorroSel=='N')&&(AhorroOferta>Ahorro)))&&(arrProductos[Pos].NoAdjudicado=='S'))		//22jun23 Faltaba comprobar que no hubiera otro adjudicado
						{
							Ahorro=AhorroOferta;
						}
						...
						if (arrProductos[Pos].IDProdLic==6293984)
						{
							debug('CargaOfertas:'+Pos+'.Ahorro:'+Ahorro+' MejorPrecio:'+MejorPrecio);							
						}*/
					...
					if (arrProductos[Pos].IDProdLic==6293984)
					{
						debug('CargaOfertas:'+Pos+'.Ahorro:'+Ahorro);							
					}
...
function AbrirProducto(Pos, Foco)
{
	...
	var lineaDatosProducto=//16jun22'<div class="linha_separacao"></div>'
					...
					+'<div class="div_coluna_1_cotacao_detalhes">'
						...
						+'[PROG_ENTREGAS]'					//24abr23 Programacion de entregas
						+'[GUARDAR_PRODUCTO]'
						+'</div>'
					+'</div>'
					+'</div>'
					...
					+'<div class="div_coluna_2_cotacao">'
						+'<div class="titulo_cotacao_detalhes">'
							+'[STRULTPEDIDO]'				//8mar23 '<p><a href="javascript:FichaPedido([MO_ID]);">'+strUltPedido+':</a></p>'
						+'</div>[ULTIMOPEDIDO]'
					+'</div>'
	...
	var linTablaOfertas='<tr id="OFE_[LIC_OFE_ID]" class="filaOferta conhover">'
					...
					+'<td class="textLeft"><a href="javascript:FichaEmpresa([IDPROVEEDOR],\'DOCUMENTOS\');">[NOTAPROV][PROVEEDOR]</a>'
					...
					+'<td id="CONSUMOADJ_[LIC_OFE_ID]">[CUMPLE_PED_MIN][PROV_CONSUMOADJUDICADO]</td>'
	...
	//	30set22 Motivo para seleccionar opcion no1 o para descartar producto, 27jul23 cambiamos el nombre a BOR para evitar ambiguedad con el de seleccion
	var linMotSeleccionBor='<tr id="lMotivoBor_[LIC_OFE_ID]" name="lMotivoBor_[LIC_OFE_ID]" style="display:none;">'
				+'<td class="color_status">&nbsp;</td><td>&nbsp;</td><td colspan="10">'+strMotivo+':&nbsp;[DEPMOTIVOSELECCIONBOR]&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+strExplicacion+':&nbsp;'
				+'<input type="inputext" class="campopesquisa w500px" name="MOTIVOSELECCIONBOR_[LIC_OFE_ID]" id="MOTIVOSELECCIONBOR_[LIC_OFE_ID]" value="[MOTIVOSELECCIONBOR]"/>'
				+'<td colspan="8" class="textLeft">'
				+'	<a class="btnDestacado" href="javascript:EjecDescartarOferta([POS], [LIC_OFE_ID]);">'+strGuardar+'</a>'
				+'	<a class="btnNormal" id="btnDescartarBor_[LIC_OFE_ID]" href="javascript:CancDescartarOferta([LIC_OFE_ID]);">'+strCancelar+'</a>'
				+'</td>'
				+'</tr>';
	...
	var linMotCambioPrecio='<tr id="lMotivoPrecio_[LIC_OFE_ID]" name="lMotivoPrecio_[LIC_OFE_ID]" style="display:none;">'
				...
				+'<input type="inputext" class="campopesquisa w500px" name="MOTIVOCAMBIOPRECIO_[LIC_OFE_ID]" id="MOTIVOCAMBIOPRECIO_[LIC_OFE_ID]" value="[MOTIVOCAMBIOPRECIO]"/>'
				+'<td colspan="8" class="textLeft">'
	...
	var linMotSeleccion='<tr id="lMotivoSel_[LIC_OFE_ID]" name="lMotivoSel_[LIC_OFE_ID]" style="display:none;">'
				+'<td class="color_status">&nbsp;</td><td>&nbsp;</td><td colspan="18">'+strMotivo+':&nbsp;[DEPMOTIVOSELECCION]&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+strExplicacion+':&nbsp;'
				+'<input type="inputext" class="campopesquisa w500px" name="MOTIVOSELECCION_[LIC_OFE_ID]" id="MOTIVOSELECCION_[LIC_OFE_ID]" value="[MOTIVOSELECCION]" onblur="javascript:GuardarProductoSel([POS],\'\');"/>'
				+'</tr>';

	//	Pie de la tabla de ofertas
	/*	27jul23 QUitamos el motivo seleccion de aqui
	var footTablaOfertas='<tr id="lMotivoSel_[POS]" style="display:none"><td class="color_status">&nbsp;</td><td colspan="22">'+strMotivo+':&nbsp;[IDMOTIVO_SEL]&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
						+strExpli+':&nbsp;'
						+'<input type="text" class="campopesquisa w500px" name="MOTIVOSELECCION_[POS]" id="MOTIVOSELECCION_[POS]" value="[MOTIVO_SEL]" onblur="javascript:GuardarProductoSel([POS],\'\');"/>'
						+'</td></tr></tbody><tfoot class="rodape_tabela"><tr><td colspan="23">&nbsp;</td></tr></tfoot></table></div>';
	*/
	var footTablaOfertas='</tbody><tfoot class="rodape_tabela"><tr><td colspan="23">&nbsp;</td></tr></tfoot></table></div>';
	...
	else 
	{
		...
		{
			var disabled='',fondo='';
			if (arrProductos[Pos].Entregas!='')
			{
				disabled='disabled="disabled"';
				fondo='fondoGrisClaro';
			}
		
			linDatosProd=linDatosProd.replace('[CANT_PRODUCTO]','<input '+disabled+' class="campopesquisa w80px '+fondo+'" type="text" size="5" name="Cantidad_[POS]" id="Cantidad_[POS]" value="'+arrProductos[Pos].Cantidad+'"/>');	//	27oct22 Incluimos _Pos en el nombre e ID del control	
		}
		else
		{
		...
		//	24abr23 Programacion de entregas
		if (arrProductos[Pos].Entregas!='')
		{
			//	16jun23 Construimos la lista, poniendo la parte entera de las cantidades
			var v_ListaEntregas='';
			for (var i=0;i<=PieceCount(arrProductos[Pos].Entregas,'#');++i)		//	Cuidado, devuelve 0 aunque haya una programacion, por que no acaba con #
			{
				var Entrega=Piece(arrProductos[Pos].Entregas,'#',i);
				if (Entrega!='')
					v_ListaEntregas+=Piece(Entrega,'|',0)+':'+Piece(Piece(Entrega,'|',1),'.',0)+' ';	//	6jun23 Cortamos punto decimal
			}			
		
			linDatosProd=linDatosProd.replace('[PROG_ENTREGAS]','<p><a href="javascript:EntregasProducto('+Pos+');"><img src="http://www.newco.dev.br/images/2022/icones/calendar-clock.png" class="valignM" title="'+str_ProgEntregas+': '+v_ListaEntregas+'"/></a>&nbsp;</p>');
		}
		else
			linDatosProd=linDatosProd.replace('[PROG_ENTREGAS]','');
		...
		if (arrProductos[Pos].UltCompra.MO_ID!='')
		{
			...
			linDatosProd=linDatosProd.replace('[STRULTPEDIDO]','<p><a href="javascript:FichaPedido([MO_ID]);">'+strUltPedido+':</a></p>');
					
		}
		else
		{
			UltCompra='<div class="historico_complementar_cotacao">'+strSinPedidosTri+'</div>';					
			linDatosProd=linDatosProd.replace('[STRULTPEDIDO]','<p>'+strUltPedido+'</p>');
		}
		...
		linDatosProd=linDatosProd.replaceAll('[MO_ID]',arrProductos[Pos].UltCompra.MO_ID);		//	8mar23	
		...
		for (var i=0; i<arrProductos[Pos].Ofertas.length;++i)
		{
			...
			if (arrProductos[Pos].Ofertas[i].NO_CUMPLE_PEDIDO_MINIMO=='N')
				...
				Linea=Linea.replace('[CUMPLE_PED_MIN]','<img src="http://www.newco.dev.br/images/urgente.gif" title="'+strAvisoPedMin+'"/>');
			...
			//	19abr23 Incluimos la valoracin en estrellas del proveedor
			var ImgEstrellas = '<img src="http://www.newco.dev.br/images/StarSmall'+arrProveedores[PosProveedor].Estrellas+'.png" height="18px" width="18px" class="static"/>&nbsp;';
			Linea=Linea.replaceAll('[NOTAPROV]',ImgEstrellas);
			...
			if (arrProductos[Pos].Ofertas[i].EMPAQUETAMIENTO_SOSPECHOSO=='S')
				//21jun23 Linea=Linea.replace('[EMPAQUETAMIENTO_SOSPECHOSO]','<span class="rojo2">&nbsp;?</span>');
				Linea=Linea.replace('[EMPAQUETAMIENTO_SOSPECHOSO]','<img src="http://www.newco.dev.br/images/2022/icones/Exclamacion.png" title="'+strAvisoCantidad+'"/>');
			else
			...
			//	Precio sospechoso
			/*	21jun23 Sustituimos por icono con title de ayuda
			...
			if ((arrProductos[Pos].Ofertas[i].MUY_SOSPECHOSO=='S')||(arrProductos[Pos].Ofertas[i].SOSPECHOSO=='S'))
				Linea=Linea.replace('[PRECIO_SOSPECHOSO]','<img src="http://www.newco.dev.br/images/2022/icones/Exclamacion.png" title="'+strAvisoPrecio+'"/>');
			else	
				Linea=Linea.replace('[PRECIO_SOSPECHOSO]','');
			...
			var Consumo;
			if (arrProductos[Pos].Ofertas[i].OfertaAdjud=='S')
				Consumo=FormatoNumero((desformateaDivisa(arrProductos[Pos].Ofertas[i].PRECIO) * arrProductos[Pos].Ofertas[i].CANTIDADADJUDICADA).toFixed(2));
			else
				Consumo=arrProductos[Pos].Ofertas[i].CONSUMO;
								
			Linea=Linea.replaceAll('[SPANCONSUMO]','<span class="'+Color+'">'+Consumo+'</span>');
			...
			if (botonPedido=='N')
			{
				var linMotivo=linMotSeleccionBor;
				linMotivo=linMotivo.replace('[DEPMOTIVOSELECCIONBOR]', DesplegableMotivosSeleccion('BOR',arrProductos[Pos].Ofertas[i].ID, ''));
				linMotivo=linMotivo.replace('[MOTIVOSELECCIONBOR]', '');
				Linea+=linMotivo;
			...
			linMotivo=linMotivo.replace('[DEPMOTIVOSELECCION]', DesplegableMotivosSeleccion('', arrProductos[Pos].Ofertas[i].ID, 
															arrProductos[Pos].IDMotivoSel, "javascript:GuardarProductoSel([POS],\'\');"));
															
			linMotivo=linMotivo.replace('[MOTIVOSELECCION]', arrProductos[Pos].MotivoSel);
			Linea+=linMotivo;
		...
		foot=foot.replace('[MOTIVO_SEL]',arrProductos[Pos].MotivoSel);
		var foot=foot.replaceAll('[POS]',Pos);
		*/

		var foot=footTablaOfertas;
		...
		//	27jul23 jQuery("#lMotivoSel_"+Pos).show();	
			jQuery("#lMotivoSel_"+arrProductos[Pos].Ofertas[UltSel].ID).show();
...
function DespOpciones(valor)
{
	...
	strDespOpciones+=cad
	for (i=1;i<=32;++i)			//	ampliamos hasta 32 solicitado por OHSJD
	{
...
/*	27jul23 Utilizamos una unica funciona para desplegable de motivos para borrar o para seleccion
//	27abr22 Prepara el desplegable de motivos
function DesplegableMotivos(Pos, IDMotivo)
{
	var ListaDesplegable='<select id="IDMOTIVOSELECCION_'+Pos+'" class="w400px" onChange="javascript:GuardarProductoSel('+Pos+',\'\');">';
...
//	30set22 Prepara el desplegable de motivos, 27jul23 nuevos parametros
function DesplegableMotivosSeleccion(Tipo, IDOferta, IDMotivo, onChangeF)
{
	var ListaDesplegable='<select id="IDMOTIVOSELECCION'+Tipo+'_'+IDOferta+'" class="w400px" onChange="'+onChangeF+'";>';
	for (var i=0;i<arrMotivos.length;++i)
	{
		var isSel='';
		if (IDMotivo==arrMotivos[i].ID)
	...
	//solodebug debug("DesplegableMotivosSeleccion ("+Pos+","+IDMotivo+"):"+ListaDesplegable);
...
function SeleccionadaOferta(PosProd, ID)
{
	...
	//	Comprueba que no haya mas de 1 oferta seleccionada si es un producto programado
	if ((arrProductos[PosProd].Seleccionadas>=1)&&(PieceCount(arrProductos[PosProd].Entregas,'#')>1)&&(jQuery("#ADJUD_"+ID).prop("checked")))
	{
		alert(alrt_ProdProgSolo1Sel);
	...
	if ((jQuery("#ADJUD_"+ID).prop("checked"))&&(MostrarMotivoSeleccion=='S')&&(arrProductos[PosProd].PrecioOfertaActual>arrProductos[PosProd].MejorPrecio))
		//27jul23 jQuery("#lMotivoSel_"+PosProd).show();		
		jQuery("#lMotivoSel_"+ID).show();
	else
	{
		//27jul23 jQuery("#IDMOTIVOSELECCION_"+PosProd).val("");			//	7nov19	Reinicializamos el desplegable
		//27jul23 jQuery("#MOTIVOSELECCION_"+PosProd).val("");			//	29jun22	Reinicializamos el desplegable
		//27jul23 jQuery("#lMotivoSel_"+PosProd).hide();


		jQuery("#IDMOTIVOSELECCION_"+ID).val("");
		jQuery("#MOTIVOSELECCION_"+ID).val("");
		jQuery("#lMotivoSel_"+ID).hide();
	}
...
/*
//27jul23 COmprueba si el motivo de seleccion esta informado para una oferta que no sea la mas barata
function CompruebaMotivoSeleccionInformado(PosProd, IDOferta)
{
	if ((MostrarMotivoSeleccion=='S')&&(arrProductos[PosProd].Seleccionadas>0)&&(arrProductos[PosProd].Ofertas[i].PRECIO>arrProductos[PosProd].MejorPrecio))
	{
		IDMotivo=jQuery("#IDMOTIVOSELECCION_"+IDOferta).val();
		Motivo	=jQuery("#MOTIVOSELECCION_"+IDOferta).val();	
...
function GuardarProductoSel(PosProd, Accion){
	...
	debug('GuardarProductoSel: '+PosProd+' Accion:'+Accion);
	...
	if ((MostrarMotivoSeleccion=='S')&&(arrProductos[PosProd].Seleccionadas>0)&&(arrProductos[PosProd].PrecioOfertaActual>arrProductos[PosProd].MejorPrecio))
	{
		...
		*/
		...
		var ID=IDOfertaSeleccionada(PosProd);

		//	27jul23 Cambiamos nomenclatura para poner los motivos debajo de cada oferta	
		IDMotivo=jQuery("#IDMOTIVOSELECCION_"+ID).val();
		Motivo	=jQuery("#MOTIVOSELECCION_"+ID).val();	

		//solodebug	
		console.log('GuardarProductoSel. Oferta ID seleccionada:'+ID+' IDMotivo:'+IDMotivo+' Motivo:'+Motivo);
		...
		for (var i=0;i<arrProductos[PosProd].Ofertas.length;++i)
		{
			...
			if (Orden=='1')
				if (opcion1=='S')
				{
					...
					jQuery('#ORDEN_'+PosProd+'_'+IDOferta).val('');		//10mar23
				}
				else
					opcion1='S';
		}
	...
	else if (arrProductos[PosProd].Seleccionadas>1)
	{
		...
		/*	15feb22 Mas eficiente recorrer array javascript
		...
		for (var i=0;i<arrProductos[PosProd].Ofertas.length;++i)
		{
			var ID=arrProductos[PosProd].Ofertas[i].ID;
			if (jQuery("#ADJUD_"+ID).prop("checked"))
			{
				var Cantidad=desformateaDivisa(jQuery("#CANTADJUD_"+ID).val());
				cantAdjud+=Cantidad;
				multOfertas+=ID+'#'+Cantidad+'|';
			...
			jQuery.ajax({
				...
				success: function(objeto){
					...
					if(data.Resultado.Estado == 'OK'){
						...
						//solodebug	
						console.log('OfertasSeleccionadas. ERROR en SeleccionarProductoAJAX. Pos:'+PosProd+' IDProdLic:'+IDProdLic+' adjudOfertas:'+adjudOfertas+' -> Adjudicación repartida. Res:'+data);
					}
				}
			});
		}
				...
				jQuery.ajax({
					...
					success: function(objeto){
						...
						if(data.Resultado.Estado == 'OK'){
							...
							alert(alrt_GuardarSelKo);
							//solodebug	
							console.log('OfertasSeleccionadas. ERROR en SeleccionarProductoRepartidoAJAX. Pos:'+PosProd+' IDProdLic:'+IDProdLic+' adjudOfertas:'+adjudOfertas+' -> Adjudicación repartida. Res:'+data);
						}
					}
				});
			}
				...
				jQuery.ajax({
					...
					success: function(objeto){
						...
						if(data.Resultado.Estado == 'OK'){
							...
							alert(alrt_GuardarSelKo);
							//solodebug	
							console.log('OfertasSeleccionadas. ERROR en SeleccionarProductoMultipleAJAX. Pos:'+PosProd+' IDProdLic:'+IDProdLic+' adjudOfertas:'+adjudOfertas+' -> Adjudicación multiple. Res:'+data);
						}
					}
				});
		...
		if (arrProductos[i].NoAdjudicado!='S')
		{
			...
			for (var j=0;j<arrProductos[i].Ofertas.length;++j)
			{
				...
				if (arrProductos[i].Ofertas[j].CANTIDADADJUDICADA>0)
				{
					...
					//solodebug debug('ActualizaTotales [1]. Prov:'+arrProveedores[PosProv].NombreCorto+' ofertas:'+arrProveedores[PosProv].OfertasAdj+' cons.Ant.:'+arrProveedores[PosProv].ConsumoAdj+'->'+arrProveedores[PosProv].ConsumoAdj_Float);
...
{		
	...
	if (Pos==19)
		debug('ActualizaOfertas. Pos:'+Pos+' Of.Adjud:'+arrProductos[Pos].Seleccionadas+'. Of.Tot:'+arrProductos[Pos].Ofertas.length+ '. Ahorro:'+arrProductos[Pos].Ahorro);
...
function MejoresPreciosTodo(Pos)
{
	...
	//solodebug	
	debug('MejoresPrecios:'+Pos+'---> SeleccionaMejorOferta');
	
	if (isLicMultiopcion=='S')
		SeleccionaOfertas(Pos);
	else
...
//	13mar23 Selecciona la mejor oferta para el producto
function SeleccionaOfertas(Pos)
{
	//solodebug 
	debug('SeleccionaOfertas:'+Pos);
	...
	for (var i=0;i<arrProductos[Pos].Ofertas.length;++i)
	{
		...
		if (i<16)
		{
			...
			//solodebug 
			debug('SeleccionaOfertas. ['+Pos+'] Oferta('+i+') Seleccionar:'+arrProductos[Pos].Ofertas[i].ID+' Orden:'+(i+1));
			
			arrProductos[Pos].Ofertas[i].ORDEN=i+1;
			jQuery("#ORDEN_"+Pos+'_'+arrProductos[Pos].Ofertas[i].ID).val(i+1);
		}
		else
		{
		
			//solodebug 
			debug('SeleccionaOfertas. ['+Pos+'] Oferta('+i+') Desactivar:'+arrProductos[Pos].Ofertas[i].ID);
			
			arrProductos[Pos].Ofertas[i].ORDEN='';
			jQuery("#ORDEN_"+Pos+'_'+arrProductos[Pos].Ofertas[i].ID).val('');
		}
		
	}
		
	GuardarProductoSel(Pos, '');

	//solodebug 
	debug('SeleccionaOfertas. ['+Pos+'] Activar:'+arrProductos[Pos].Ofertas[0].ID);
}
...
{
	//solodebug 
	debug('SeleccionaMejorOferta:'+Pos);

	//	Sale directamente si no tiene ofertas
	if (arrProductos[Pos].Ofertas.length==0) return;

	//solodebug 
	debug('SeleccionaMejorOferta. ['+Pos+'] Activar:'+arrProductos[Pos].Ofertas[0].ID);
	...
	{
		//solodebug 
		debug('SeleccionaMejorOferta. ['+Pos+'] Desactivar:'+arrProductos[Pos].Ofertas[i].ID);
...
function GuardarDatosGenerales()
{
	...
	if(jQuery("#LIC_FECHAADJUDICACION").length){
		if(!esNulo(oForm.elements['LIC_FECHAADJUDICACION'].value)){
			if(CheckDate(oForm.elements['LIC_FECHAADJUDICACION'].value)){
				...
				//solodebug	
				var FechaDecTxt=jQuery('#FECHADECISION').val();
				console.log('ValidarFormulario. Compara fechas. FECHADEC:'+FechaDecTxt+' vs FECHAADJ:'+oForm.elements['LIC_FECHAADJUDICACION'].value);

				var fechaDec=FechaDeControlDate(jQuery('#FECHADECISION').val());		//18oct22
	...
	var Continua=oForm.elements['LIC_CONTINUA'].value,
		...
		PagoAplzObli='',PreciObjEstr='';
		
	//	30mar23 COmprobamos que existe para evitar error por "undefined"	
	if (jQuery('#LIC_PAGOAPLAZODOOBLIGATORIO').length>0)
		PagoAplzObli=jQuery('#LIC_PAGOAPLAZODOOBLIGATORIO').val();	
		
		
	if (jQuery('#LIC_PRECIOOBJETIVOESTRICTO').length>0)
		PreciObjEstr=jQuery('#LIC_PRECIOOBJETIVOESTRICTO').val();
...
var BorrarUsuario='<a class="accBorrar" href="javascript:modificaUsuario([LIC_USU_ID],\'B\');"><img src="http://www.newco.dev.br/images/2022/icones/del.svg"></a>';	//02mar23 Cambiamos nombre, si no se confunde con el de centro
			...
			if(data.ListaUsuarios.length > 0){
				...
				jQuery.each(data.ListaUsuarios, function(key, usuario){
					...
					if(IDUsuario == usuario.ID){
						...
						fila=fila.replace('[BORRAR]',BorrarUsuario.replace('[LIC_USU_ID]',usuario.IDUSUARIO_LIC));
					}else{
						fila=fila.replace('[BORRAR]','');
					}
			...
			success: function(objeto){
				...
				if(data.OfertaActualizada.IDOferta > 0)
				{
					...
					if(data.OfertaActualizada.TodosInformados == 'Si')
					{
						...
						arrProductos[Pos].Ofertas[PosOferta].UNIDADESPORLOTE=UdsXLote;			//16jun23 Ponia i en lugar de PosOferta
						
						//	Asignamos valor PRECIO en el array, recalculamos ahorro
						arrProductos[Pos].Ofertas[PosOferta].PRECIO=PrecioTxt;
						jQuery('#Precio_'+IDOferta).val(PrecioTxt);								//16jun23 Antes: jQuery('#PrecioOriginal_'+IDOferta).val(PrecioTxt);
						jQuery("#lMotivoPrecio_"+IDOferta).hide();
						...
						var Color=CompruebaColorOferta(Pos, PosOferta);
...
function DatosProducto(Pos){
	...
	if(!errores && !esNulo(valAux) && isNaN(valAux)){
		errores++;
		//8mar23 alert(val_malPrecioRef);
		msgErrores+=val_malPrecioRef+'\n';
	...
	if(!errores && !esNulo(valAux) && isNaN(valAux)){
		errores++;
		//8mar23 alert(val_malPrecioObj);
		msgErrores+=val_malPrecioObj+'\n';
	...
	if(!errores && esNulo(valAux)){
		errores++;
		//8mar23 alert(val_faltaUdBasica);
		msgErrores+=val_faltaUdBasica+'\n';
	...
	if(!errores && MesesDuracion == '0' && (esNulo(valAux))){
		errores++;
		//8mar23 alert(val_faltaCantidad);
		jQuery('#Cantidad_'+Pos).focus();
		msgErrores+=val_faltaCantidad+'\n';
		return;
	}else if(!errores && parseFloat(valAux) === 0){
		errores++;
		//8mar23 alert(val_ceroCantidad);
		jQuery('#Cantidad_'+Pos).focus();
		msgErrores+=val_ceroCantidad+'\n';
		return;
	}else if(!errores && isNaN(valAux)){
		errores++;
		//8mar23 alert(val_malCantidad);
		jQuery('#Cantidad_'+Pos).focus();
		msgErrores+=val_malCantidad+'\n';
		return;
	}
...
function ActualizarProducto(Pos, AvisarCambioCantidad){
	var listaProductos;
	var d = new Date();
	var nuevaCant=parseInt(jQuery('#Cantidad_'+Pos).val().replace('.',''));			//	27oct22 Incluimos _Pos en el nombre e ID del control, 8mar23 quitamos punto que se convierte en "1"
	var IDProdLic=arrProductos[Pos].IDProdLic;

	//solodebug	console.log('ActualizarProducto '+Pos+' Cant:'+arrProductos[Pos].Cantidad+' nuevaCant:'+nuevaCant);
	...
	//solodebug	console.log('ActualizarProducto '+Pos+' listaProductos:'+listaProductos);
	
	//	Si hay cambios en la cantidad, actualiza también las ofertas adjudicadas
	//	8mar23 el problema se produce al convertir el numero 1.000=1
	if (arrProductos[Pos].Cantidad!=nuevaCant)
	{

		//solodebug	console.log('ActualizarProducto '+Pos+' hay cambio en cantidad');
		...
		if (AvisarCambioCantidad=='S')
		{
			...
			//solodebug	console.log('ActualizarProducto '+Pos+' QuitarSeleccionesProducto');
	...
	//solodebug	console.log('ActualizarProducto '+Pos+'.AJAX:ActualizarProductos.xsql listaProductos:'+listaProductos);
	...
	jQuery.ajax({
		...
		success: function(objeto){
			...
			if(data.ProductosActualizados.estado == 'OK'){
				...
				//solodebug	console.log('ActualizarProducto '+Pos+'. AJAX OK, AbrirProducto');
				...
				//solodebug	console.log('ActualizarProducto '+Pos+'. AJAX KO, alert + return');
...
function QuitarSeleccionesProducto(Pos)
{
	...
	arrProductos[Pos].Seleccionadas=0;

	for (var i=0;i<arrProductos[Pos].Ofertas.length;++i)
	{
		var ID	= arrProductos[Pos].Ofertas[i].ID;

		//solodebug	console.log('QuitarSeleccionesProducto '+Pos+'. Comprobando oferta '+i+' ID:'+ID+ ' Adjud:'+arrProductos[Pos].Ofertas[i].OfertaAdjud);
...
function listadoExcelCompleto()
{
	...
	for(var j=0; j<arrProveedores.length; j++)
	{
		...
		{	//	COmprueba proveedor con ofertas /Mantenimiento/LICITACION/PRODUCTOSLICITACION/PROVEEDORESLICITACION/PROVEEDOR/TIENE_OFERTAS
			...
			var Columna=[];
			Columna["ID"]=arrProveedores[j].IDProveedor;
			Columna["Proveedor"]=arrProveedores[j].NombreCorto;
			
			//solodebug 	console.log('listadoExcelCompleto. arrTitulos. Incluir: ('+arrProveedores[j].IDProveedor+')'+arrProveedores[j].NombreCorto);
			
			arrTitulos.push(Columna);
		}
	...
	if(arrProductos.length > 0)
	{
		...
		{
			//	solodebug
			//	solodebugif (arrProductos[i].RefEstandar=='T0000006')
			//	solodebug	debug('listadoExcelCompleto. Producto('+ i +') IDProdLic:'+ arrProductos[i].IDProdLic+' RefEst:'+arrProductos[i].RefEstandar);
			...
			//solodebug	if (i==0) 	debug('listadoExcelCompleto. Producto '+ i+' Ofertas:'+arrProductos[i].Ofertas.length+' IDProdLic:'+arrProductos[i].IDProdLic);

			//	25may23 crea las celdas vacias
			arrValores=new Array();
			for (var k=0;k<arrTitulos.length;++k)
			{
				var Columna=[];
				if (IDPais==55) Columna["Txt"] = StringtoCSV('')+StringtoCSV('');
				Columna["Txt"] = StringtoCSV('');

				arrValores.push(Columna);				
			}
			...
			for(var j=0; j<arrProductos[i].Ofertas.length; j++)
			{
				...
				//solodebug	debug('listadoExcelCompleto. Producto('+ i +') IDProdLic:'+ arrProductos[i].IDProdLic+' Oferta('+j+'):'+arrProductos[i].Ofertas[j].IDProvLic
				//solodebug					+' '+arrProductos[i].Ofertas[j].IDOferta+' of.prov.:'+arrProveedores[PosProveedor].TieneOfertas);

				//25may23 IMPORTANTE: Ya no se utilizan NoInformada, NoOfertada. Recorremos las columnas hasta llegar a la que toca
				var Encontrada='N';
				for (var k=0;((k<arrTitulos.length)&&(Encontrada=='N'));++k)
				{
					
					//	solodebugif ((arrProductos[i].RefEstandar=='T0000006') || (i<4))
					//	solodebug	debug('listadoExcelCompleto. Producto('+ i +') RefEstandar:'+ arrProductos[i].RefEstandar+' Oferta('+j+'), comprobando proveedor('+k+'):'+arrProductos[i].Ofertas[j].IDPROVEEDOR
					//	solodebug		+' ? '+arrTitulos[k].ID);
						
					if (arrProductos[i].Ofertas[j].IDPROVEEDOR==arrTitulos[k].ID)
					{
					
						if (IDPais==55) arrValores[k].Txt = StringtoCSV(arrProductos[i].Ofertas[j].MARCA)+NumbertoCSV(arrProductos[i].Ofertas[j].PRECIO );
						arrValores[k].Txt= NumbertoCSV(arrProductos[i].Ofertas[j].PRECIO );
...
//	14abr23 Incluimos las funciones de gestion de documentos
...
function cargaDoc(uploadForm, tipo, valueID){
	...
	//solodebug	
	debug('cargaDoc. form:'+uploadForm.name+ ' tipo:'+tipo+ ' ID:'+valueID);
...
function hasFilesDoc(uploadForm)
{
	//solodebug debug('hasFilesDoc. form:'+uploadForm.name+' FormElements:'+logFormElements(uploadForm));
	...
	for(var i=0; i<uploadForm.length; i++) {
		...
		if(uploadForm.elements[i].type == 'file' && (uploadForm.elements[i].name.substring(0,12) == 'inputFileDoc') && uploadForm.elements[i].value != '' ){
			return true;
		}
	}
	return false;
...
function periodicUpdateDoc(uploadForm, valueID, tipo){
	...
	if(uploadFrameDoc && uploadFrameDoc.document && uploadFrameDoc.document.getElementsByTagName("p")[0]) 
	{
		...
		else if(tipoDoc == 'FT' || tipoDoc == 'RS' || tipoDoc == 'CE'|| tipoDoc == 'FS'){	//	15nov21
	...
	if(msg != ''){
		...
		jQuery.ajax({
			...
			beforeSend:function(){
				...
				else if(tipoDoc == 'FT' || tipoDoc == 'RS' || tipoDoc == 'CE' || tipoDoc == 'FS'){	//	15nov21, FS 21feb23
					document.getElementById('confirmBox_'+tipo+'_'+valueID).style.display = 'none';
					document.getElementById('waitBoxDoc_'+tipo+'_'+valueID).src = 'http://www.newco.dev.br/images/loading.gif';
				}else{
			...
			success:function(data){
				...
				else if(valueID == 'SC')
				{
					...
					//15nov21	document.getElementById('confirmBox_'+tipo+'_'+valueID).style.display = 'block';
					if(tipoDoc == 'FT'){
						document.getElementById('confirmBox_'+tipo+'_'+valueID).style.display = 'block';
						//recargamos las fichas
						SeleccionaFichas(prove,'IDFICHA_' + valueID, currentDocID);
						jQuery('#verDocFT_'+valueID).show();
						jQuery("#inputFileDoc_FT_"+valueID).hide();
					}
					else if(tipoDoc == 'RS'){
						document.getElementById('confirmBox_'+tipo+'_'+valueID).style.display = 'block';
						//recargamos registro sanitario
						SeleccionaRegSan(prove,'IDREGSAN_' + valueID, currentDocID);
						jQuery('#verDocRS_'+valueID).show();
						jQuery("#inputFileDoc_RS_"+valueID).hide();
					}
					else if(tipoDoc == 'CE'){
						document.getElementById('confirmBox_'+tipo+'_'+valueID).style.display = 'block';
						//recargamos certificados de experiencia
						SeleccionaCertExp(prove,'IDCERTEXP_' + valueID, currentDocID);
						jQuery('#verDocCE_'+valueID).show();
						jQuery("#inputFileDoc_CE_"+valueID).hide();
					}
					else if(tipoDoc == 'FS'){
						document.getElementById('confirmBox_'+tipo+'_'+valueID).style.display = 'block';
						//recargamos ficha de seguridad
						SeleccionaFichaSeg(prove,'IDFICHASEG_' + valueID, currentDocID);
						jQuery('#verDocFS_'+valueID).show();
						jQuery("#inputFileDoc_FS_"+valueID).hide();
					}
...
*/
				...
				else if(tipoDoc == 'FT' || tipoDoc == 'RS' || tipoDoc == 'CE'|| tipoDoc == 'FS'){	//	15nov21, 20nov23
					var uploadElem = document.getElementById("inputFileDoc_" +tipo+'_'+ valueID);
				}
				else
				{
...
//	24abr23 Presenta el pop-up con la programacion de entregas
function EntregasProducto(Pos)
{
	var cadProv='';
	// si hay mas de una oferta seleccionadas, muestra aviso y sale
	if (arrProductos[Pos].Seleccionadas>1)
	{
		alert(alrt_VariasSelNoPermiteProg);
		return;		
	}

	//	Si hay una unica oferta, incluimos la informacin del proveedor y marca seleccionados
	if (arrProductos[Pos].Seleccionadas==1)
	{
		var fin=false;
		for(var i=0;(i<arrProductos[Pos].Ofertas.length)&&(!fin);++i)
		{
			if (arrProductos[Pos].Ofertas[i].OfertaAdjud=='S')
			{
				cadProv='<label>'+strProveedor+':&nbsp;</label>'+arrProductos[Pos].Ofertas[i].PROVEEDOR+'&nbsp;&nbsp;&nbsp;<label>'+strMarca+':&nbsp;</label>'+arrProductos[Pos].Ofertas[i].MARCA;
				fin=true;
			}
		}
	}
	...
	var strProducto=cadProv+'&nbsp;&nbsp;&nbsp;<label>'+strCantidad+':&nbsp;</label>'+arrProductos[Pos].Cantidad+'&nbsp;'+arrProductos[Pos].UdBasica+'&nbsp;&nbsp;&nbsp;<label>'+str_CantProg+':&nbsp;</label>&nbsp;<span id="totProgr"/>';

	//solodebug	debug("EntregasProducto:"+ strProducto);
	
	jQuery("#progEntrPosProd").val(Pos);
	jQuery("#progEntrTitulo").html('<label>'+str_ProgEntregas+'</label>:&nbsp('+strRef+')&nbsp'+arrProductos[Pos].Nombre.substring(0,80));
	jQuery("#progEntrCant").html(strProducto);

	var Entregas=arrProductos[Pos].Entregas;

	//solodebug	debug("EntregasProducto:"+ Entregas+' Num:'+PieceCount(Entregas,'#'));
	...
	//	16jun23 Vaciamos el final de la tabla
	for (;i<=9;++i)		
	{
		jQuery("#dataEntr"+(i+1)).val('');
...
//	26abr23 Comprueba la cantidad cambiada en un programa
...
//	26abr23 Muestra el total del programa
function MuestraTotalPrograma()
{
	...
	jQuery("#totProgr").html(total+'&nbsp;'+arrProductos[Pos].UdBasica);
}


//	25abr23 Guarda la programacion correspondiente a un producto
function GuardarProgramacion()
{
	...
	//solodebug
	debug("GuardarProgramacion. Pos:"+ Pos);
	...
	for (i=1;i<=10;++i)
	{
		...
		//solodebug
		debug("GuardarProgramacion. Pos:"+ Pos+ 'Prog('+i+'):'+fechaOrig);
	...
	//	Si ha cambiado la cantidad, pedir confirmacin antes de guardar
	if (total!=arrProductos[Pos].Cantidad)
	{
		var aviso;
		//	Si hay ofertas seleccionadas, avisar que tambien se perdera la seleccion
		if (arrProductos[Pos].Seleccionadas>0)
	...
	jQuery.ajax({
		...
		data:	"LIC_ID="+IDLicitacion+"&LIC_PROD_ID="+arrProductos[Pos].ID+"&ENTREGAS="+encodeURIComponent(cadPrograma),
		type:	"GET",
		contentType: "application/xhtml+xml",
		beforeSend: function(){
			null;
		...
		success: function(objeto){
			...
			if(data.Licitacion.Estado == 'OK'){
				...
				arrProductos[Pos].Entregas=cadPrograma;
				...
				if (cambioCant)
				{
					jQuery('#Cantidad_'+Pos).val(total);
					QuitarSeleccionesProducto(Pos);
					ActualizarProducto(Pos,'N');
				}
...
//	25may23	Crear nueva licitacion de productos no pedidos, incluye proveedores y ofertas, traido desde lic2022_060423.js
	...
	jQuery.ajax({
		...
		success: function(objeto){
			...
			if(data.Resultado.Estado == 'OK'){
				...
				alert(alrt_LicitacionHijaOK);
				location.href = 'http://www.newco.dev.br/Gestion/Comercial/MantLicitacion2022.xsql?LIC_ID=' + data.Resultado.IDNuevaLic;
			}else{
				jQuery("#waitBotonContLic").hide();
