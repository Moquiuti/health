...
//	Ultima revisión ET 10ene23 17:30 LicV2Prods_2022_100123.js
...
{
	debug('recalcularFloatingBox: redirigido a calcularFloatingBox');
...
function AdjudicarOfertas(){
	...
	//10ene23 if (ctrlCargaProv) return;	//	Desactivamos durante la carga de proveedores

	jQuery('.botonAccion').hide();
	
	//21set16	Hacemos aqui las comprobaciones básicas que pueden dar lugar a errores antes de adjudicar
	//03oct16	Solo mostramos el aviso si no hay productos selecionados	if (((arrProductos.length>numProductos) && (numProdsSeleccion<numProductos))||(numProdsSeleccion==0)) {
	//10ene23	if (numProdsSeleccion==0) {
