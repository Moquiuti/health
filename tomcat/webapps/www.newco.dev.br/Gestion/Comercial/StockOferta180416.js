/*
	StockOferta180416.js
	Librería de funciones para el mantenimiento de anuncios de Stock
	David/Mirta/ET
	Ultima modificacion: 12/5/16 09:05
*/
...
function RecuperaBienText(){
    ...
    if(jQuery("#STOCK_DESCRIPCION").length > 0)
		jQuery("#STOCK_DESCRIPCION").html(jQuery("#STOCK_DESCRIPCION").text().replace(/<br>/gi,'\n'));
}

function FinalizarOferta(oForm,estado,proy){
...
function ValidarFormulario(oForm, IDEstado, proyecto){
    
	//alert('Proyecto:'+proyecto+ ' IDEstado:'+IDEstado);
    
	var regex_numero	= new RegExp("^[0-9]+$","g"); // Expresion regular para controlar el campo ud lote que solo puede incluir números, puntos o comas
        
	if(IDEstado == 'NUEVA' && proyecto == 'MVM'){
  ...
  // validar club vip tiendas
   else if(IDEstado == 'NUEVA' && proyecto == 'CVT'){
		// Escondemos el botón de enviar
		jQuery('#BotonSubmit').hide();
        if(oForm.elements.SO_IDFAMILIA && (oForm.elements.SO_IDFAMILIA.value == '' || oForm.elements.SO_IDFAMILIA.value == '-1' ) ){
			oForm.elements.SO_IDFAMILIA.focus();
			alert(familia_obligatoria);
			jQuery('#BotonSubmit').show();
			return;
		}
        if(oForm.elements.SO_IDSUBFAMILIA && (oForm.elements.SO_IDSUBFAMILIA.value == '' || oForm.elements.SO_IDSUBFAMILIA.value == '-1' ) ){
			oForm.elements.SO_IDSUBFAMILIA.focus();
			alert(subfamilia_obligatoria);
			jQuery('#BotonSubmit').show();
			return;
		}
         if(oForm.elements.SO_IDMARCA && (oForm.elements.SO_IDMARCA.value == '' || oForm.elements.SO_IDMARCA.value == '-1' ) ){
			oForm.elements.SO_IDMARCA.focus();
			alert(marca_obligatoria);
			jQuery('#BotonSubmit').show();
			return;
		}
		if(oForm.elements.SO_TITULO && oForm.elements.SO_TITULO.value == ''){
       ...
       if(oForm.elements.SO_REFCLIENTE && oForm.elements.SO_REFCLIENTE.value == ''){
        ...
        //precio lote
		if(oForm.elements.SO_PRECIOLOTE && oForm.elements.SO_PRECIOLOTE.value == ''){
			oForm.elements.SO_PRECIOLOTE.focus();
			alert(precio_lote_obligatorio);
			jQuery('#BotonSubmit').show();
			return;
		}else if(oForm.elements.SO_PRECIOLOTE){
			var precio = oForm.elements.SO_PRECIOLOTE.value.replace(',','.');
			...
			if(!esNulo(precio) && isNaN(precio)){
				...
				oForm.elements.SO_PRECIOLOTE.value = precio.replace('.',',');
			}
		}
                //precio retail lote
        if(oForm.elements.SO_PRECIOPUBLICOLOTE && oForm.elements.SO_PRECIOPUBLICOLOTE.value == ''){
			oForm.elements.SO_PRECIOPUBLICOLOTE.focus();
			alert(precio_retail_lote_obligatorio);
			jQuery('#BotonSubmit').show();
			return;
		}else if(oForm.elements.SO_PRECIOPUBLICOLOTE){
			var precio = oForm.elements.SO_PRECIOPUBLICOLOTE.value.replace(',','.');
			...
			if(!esNulo(precio) && isNaN(precio)){
				...
				oForm.elements.SO_PRECIOPUBLICOLOTE.value = precio.replace('.',',');
			}
		}
        if(oForm.elements.SO_PLAZOENVIO && oForm.elements.SO_PLAZOENVIO.value == ''){
			oForm.elements.SO_PLAZOENVIO.focus();
			alert(plazo_envio_obligatorio);
			jQuery('#BotonSubmit').show();
			return;
		}
        //coste de transporte
		/*	18abr16	Ya no se informa
        if(oForm.elements.SO_COSTEENVIO && oForm.elements.SO_COSTEENVIO.value == ''){
			oForm.elements.SO_COSTEENVIO.focus();
			alert(coste_envio_obligatorio);
			jQuery('#BotonSubmit').show();
			return;
		}else if(oForm.elements.SO_COSTEENVIO){
			var precio = oForm.elements.SO_COSTEENVIO.value.replace(',','.');
			...
			if(!esNulo(precio) && isNaN(precio)){
				...
				oForm.elements.SO_COSTEENVIO.value = precio.replace('.',',');
			}
		}
		*/
...
// Funcion que valida los campos de los formularios antes de hacer el submit, en etse caso para usuario MVM
function ValidarFormularioCdC(oForm, IDEstado, proyecto){
	...
	//	OFERTA para MVM
	if(IDEstado == 'OFERTA' &&  proyecto == 'MVM'){
		// Escondemos el boton de enviar
	...
	//	OFERTA para CLUBVIPTIENDAS
	else if(IDEstado == 'OFERTA' &&  proyecto == 'CVT'){
            // Escondemos el botÃ³n de enviar
		jQuery('#BotonSubmit').hide();
                oForm.elements.STOCK_ESTADO.value = IDEstado;
                
       if(oForm.elements.STOCK_IDFAMILIA && (oForm.elements.STOCK_IDFAMILIA.value == '' || oForm.elements.STOCK_IDFAMILIA.value == '-1' ) ){
			oForm.elements.STOCK_IDFAMILIA.focus();
			alert(familia_obligatoria);
			jQuery('#BotonSubmit').show();
			return;
		}
        if(oForm.elements.STOCK_IDSUBFAMILIA && (oForm.elements.STOCK_IDSUBFAMILIA.value == '' || oForm.elements.STOCK_IDSUBFAMILIA.value == '-1' ) ){
			oForm.elements.STOCK_IDSUBFAMILIA.focus();
			alert(subfamilia_obligatoria);
			jQuery('#BotonSubmit').show();
			return;
		}
        if(oForm.elements.STOCK_IDMARCA && (oForm.elements.STOCK_IDMARCA.value == '' || oForm.elements.STOCK_IDMARCA.value == '-1' ) ){
			oForm.elements.STOCK_IDMARCA.focus();
			alert(marca_obligatoria);
			jQuery('#BotonSubmit').show();
			return;
		}
		if(oForm.elements.STOCK_TITULO && oForm.elements.STOCK_TITULO.value == ''){
			oForm.elements.STOCK_TITULO.focus();
			alert(titulo_obligatorio);
			jQuery('#BotonSubmit').show();
			return;
		}
        ...
        if(oForm.elements.STOCK_DESCRIPCION && oForm.elements.STOCK_DESCRIPCION.value == ''){
			oForm.elements.STOCK_DESCRIPCION.focus();
			alert(descripcion_obligatoria);
			jQuery('#BotonSubmit').show();
			return;
		}
                ...
                //precio lote
		if(oForm.elements.STOCK_PRECIOLOTE && oForm.elements.STOCK_PRECIOLOTE.value == ''){
			oForm.elements.STOCK_PRECIOLOTE.focus();
			alert(precio_lote_obligatorio);
			jQuery('#BotonSubmit').show();
			return;
		}else if(oForm.elements.STOCK_PRECIOLOTE){
			var precio = oForm.elements.STOCK_PRECIOLOTE.value.replace(',','.');
			...
			if(!esNulo(precio) && isNaN(precio)){
				...
				oForm.elements.STOCK_PRECIOLOTE.value = precio.replace('.',',');
			}
		}
                //precio retail lote
        if(oForm.elements.STOCK_PRECIOPUBLICOLOTE && oForm.elements.STOCK_PRECIOPUBLICOLOTE.value == ''){
			oForm.elements.STOCK_PRECIOPUBLICOLOTE.focus();
			alert(precio_retail_lote_obligatorio);
			jQuery('#BotonSubmit').show();
			return;
		}else if(oForm.elements.STOCK_PRECIOPUBLICOLOTE){
			var precio = oForm.elements.STOCK_PRECIOPUBLICOLOTE.value.replace(',','.');
			...
			if(!esNulo(precio) && isNaN(precio)){
				...
				oForm.elements.STOCK_PRECIOPUBLICOLOTE.value = precio.replace('.',',');
			}
		}
                //coste de transporte
                if(oForm.elements.STOCK_COSTEENVIO && oForm.elements.STOCK_COSTEENVIO.value == ''){
			oForm.elements.STOCK_COSTEENVIO.focus();
			alert(coste_envio_obligatorio);
			jQuery('#BotonSubmit').show();
			return;
		}else if(oForm.elements.STOCK_COSTEENVIO){
			var precio = oForm.elements.STOCK_COSTEENVIO.value.replace(',','.');
			...
			if(!esNulo(precio) && isNaN(precio)){
				...
				oForm.elements.STOCK_COSTEENVIO.value = precio.replace('.',',');
			}
		}
            if(oForm.elements.STOCK_PLAZOENVIO && oForm.elements.STOCK_PLAZOENVIO.value == ''){
			oForm.elements.STOCK_PLAZOENVIO.focus();
			alert(plazo_envio_obligatorio);
			jQuery('#BotonSubmit').show();
			return;
		}
            if(oForm.elements.STOCK_IVA && oForm.elements.STOCK_IVA.value == ''){
			oForm.elements.STOCK_IVA.focus();
			alert(iva_obli);
			jQuery('#BotonSubmit').show();
			return;
		}
...
function BorrarOfertaStock(IDStockO){
	var d	= new Date();
        ...
        if(confirm('¿Está seguro que desea eliminar la oferta?')){
            jQuery.ajax({
		...
		jQuery.ajax({
			...
			success:function(data){
				...
				var fileDoc	= doc[0].file;
                                if (jQuery('#contenidoCVT')) jQuery('.contenidoCVT').hide();
...
function periodicUpdate(){
	...
	periodicTimer++;
        //añadido un alert pk si no no coje el texto del fichero subido, imagen, solo para mvm.
        /*if (document.forms['frmStockOfertaID']){
...
function SeleccionaFamilia(IDCategoria){
    ...
    var IDCategoria	= document.forms['frmStockOferta'].elements['IDCATEGORIA'].value;
	var d = new Date();
	var IDEmpresa;
...
function SeleccionaSubFamilia(IDFamilia){
	...
	if(IDFamilia == -1){
		...
		jQuery("#SO_IDSUBFAMILIA").val('-1');
                jQuery("#STOCK_IDSUBFAMILIA").val('-1');
		//if(mostrarCategoria == 'S')
	...
	jQuery.ajax({
		...
		success: function(objeto){
			...
			if (jQuery("#SO_IDSUBFAMILIA")) jQuery("#SO_IDSUBFAMILIA").html(Resultados).val('-1').show();
                        if (jQuery("#STOCK_IDSUBFAMILIA")) jQuery("#STOCK_IDSUBFAMILIA").html(Resultados).val('-1').show();
...
//	20abr16	CVT Enviar pedido
function EnviarPedido(oform)
{
	...
	var accion = 'http://www.newco.dev.br/Gestion/Comercial/StockPedidoSave.xsql';
	AsignarAccion(oform,accion);
	//alert('nombre form:'+nform+' '+oform.action);

	SubmitForm(oform);
}

// 11-05-2016 - Control texto explicativo otra forma de pago
jQuery(document).ready(function(){
    var parte1 = 'ES8321';
    var parte2 = '000900';
    var parte3 = '950211';
    var parte4 = '816440';
    var numC = parte1.toString().substr(0, 4)+ ' ' + parte1.toString().substr(4, 2 )+ parte2.toString().substr(0, 2) + ' ' + parte2.toString().substr(2, 4) +
        ' ' + parte3.toString().substr(0, 4) + ' ' + parte3.toString().substr(4, 2) + parte4.toString().substr(0, 2) + ' ' + parte4.toString().substr(2, 4)
    jQuery("#botonMasInfPag").mouseover(function(event){
      jQuery(".textoMasInfo").css("display", "block");
      jQuery("#cuentaIBAN").text("Cuenta IBAN: " + numC);
   });
   //jQuery("#botonMasInfPag").mouseout(function(event){
   //   jQuery(".textoMasInfo").css("display", "none");
   //});
});
