...
//	Ultima revision: ET 27jul22 15:02 EISDatos2022_140322.js
...
function globalEvents()
{
	...
	jQuery("#ANNO").change(function(){
		if(jQuery('#ANNO').val() < 2012 && (jQuery('#AGRUPARPOR').val() == 'CAT' || jQuery('#AGRUPARPOR').val() == 'FAM' || jQuery('#AGRUPARPOR').val() == 'SF' || jQuery('#AGRUPARPOR').val() == 'GRU')){
			...
			jQuery("#waitBox").show();
			document.forms['formEIS'].action = 'EISCambioAnyo2022.xsql';
	...
	/*	No mostramos opcion de borrar seleccion
...
function DatosCuadroDeMando(IDCuadroDeMando){
	...
	//solodebug	debug('DatosCuadroDeMando. IDCuadroDeMando:'+IDCuadroDeMando+'. Control IDEmpresa:'+IDEmpresa);
...
function ListaNivel(IDPadre,tipo){
	...
	//solodebug	debug('ListaNivel. IDPadre:'+IDPadre+'. tipo:'+tipo+'. Control IDEmpresa:'+IDEmpresa);
...
function NivelesEmpresa(IDPadre,nivel){
	...
	//solodebug debug('NivelesEmpresa. IDPadre:'+IDPadre+'. nivel:'+nivel+'. Control IDEmpresa:'+IDEmpresa);
	...
	// Si existe #IDEMPRESA => usuario MVM o MVMB
	if(jQuery("#IDEMPRESA").length > 0){
		var idEmpresaCons = jQuery("#IDEMPRESA").val();
	// Si NO existe #IDEMPRESA => Cliente
        }else
		var idEmpresaCons = IDEmpresaDelUsuario;

	jQuery.ajax({
		cache:	false,
		url:	'EISNivelesEmpresa.xsql',
		type:	"GET",
		data:	"IDEmpresa="+idEmpresaCons+"&_="+d.getTime(),
		contentType: "application/xhtml+xml",
		error: function(objeto, quepaso, otroobj){
			alert('error'+quepaso+' '+otroobj+''+objeto);
		},
		success: function(objeto){
			...
			var IDEmpresaNivel	= data.NivelesEmpresa.IDEmpresa;
			...
			if(nivel == 'CAT'){
				if(mostrarCategoria == 'S'){
					ListaNivel(IDEmpresaNivel,'CAT');
				}else{
					ListaNivel(IDEmpresaNivel,'FAMEMP');
				}
			}else if(nivel == 'GRU'){
				if(mostrarGrupo == 'S'){
...
function PrepararTablaDatos(){
	...
	jQuery.each(ResultadosTabla, function(key, value){
		...
		if((key.substring(0,4)=='ROW|')&&((Piece(key,'|',1)!=IDIndicadorActual)||(Piece(key,'|',2)!=IDGrupoActual)||(Piece(key,'|',3)!=LineaActual))){
			...
			//	Hay dos indicadores para los que se muestra la fila de totales
			if(mostrarFilaTotal=='S' || (mostrarFilaTotal=='N' && IDGrupoActual!='99999Total')){
...
function MostrarListado(Indicador, Grupo, Anno, Mes)
{
	...
	Enlace='http://www.newco.dev.br/Gestion/EIS/EISListado2022.xsql?'
			+'US_ID='+IDUsuario
...
function MostrarEISInicio(){
	MostrarPag('http://www.newco.dev.br/Gestion/EIS/EISInicio2022.xsql', 'Consulta');
}
...
function MostrarFichaAgrupacion(ID){
	...
	switch (AgruparPor)
	{
		case 'EMP':	Enlace='http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/EMPDetalleFrame2022.xsql?VENTANA=NUEVA&EMP_ID=';
					break;
		case 'EMP2':	Enlace='http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/EMPDetalleFrame2022.xsql?VENTANA=NUEVA&EMP_ID=';
					break;
		case 'CEN':	Enlace='http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/CENDetalle2022.xsql?ID=';
					break;
		case 'CEN2':	Enlace='http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/CENDetalle2022.xsql?ID=';
					break;
/*
		case 'PRO':		if(jQuery("#IDEMPRESA").length){
					...
					if(IDProdEstandar > 0){
						Enlace='http://www.newco.dev.br/ProcesosCdC/MantenimientoReducido/MantenimientoReducido2022.xsql?PRO_ID=' + IDProdEstandar + '&EMP_ID=' + IDEmpresaCliente;
...
/* 15mar22 No parece que se utilice
...
//	15mar22 Incluimos head y body, th en los titulos, etc
var headStart = '<thead class="cabecalho_tabela">';
var headEnd = '</thead>';
var bodyStart = '<tbody class="corpo_tabela">';
var bodyEnd = '</tbody>';
var footStart = '<tfoot class="rodape_tabela">>';
var footEnd = '</tfoot>';
var cellTitleStart = '<th class="textCenter">';
var cellTitleStartClass = '<th class="textCenter selected">';
var cellTitleStartMod = '';
var cellTitleStartIndic = '<th class="textCenter">';
var cellTitleEnd = '</th>';
var rowIndStart = '<tr class="subTituloTabla">';
var cellIndStart = '<th class="textLeft" colspan="14">&nbsp;&nbsp;';
var rowStart = '<tr class="conhover">';
var rowEnd = '</tr>';
var cellGroupStart = '<td class="textLeft tdEIS">';
var cellGroup = '&nbsp;<input type="checkbox" name="CHECK_GRUPO|#IDINDICADOR#|#IDGRUPO#"/>&nbsp;&nbsp;';
var cellTotalStart = '<td class="textLeft tdEIS">&nbsp;&nbsp;&nbsp;&nbsp;';
var cellStart = '<td class="textRight tdEIS">';
var cellEnd = '</td>';
...
function generarTabla(){
	...
	var cadenaHtml=headStart+'<tr>';

	//cadenaHtml=cadenaHtml+ cellTitleStartIndic + '&nbsp;&nbsp;Indicadores' +cellEnd;
	cadenaHtml+=cellTitleStartIndic;
	cadenaHtml+='<a href="javascript:OrdenarPorColumna(-1);">' + txtConcepto + '</a>';
	//Dejamos estos botones solo en la parte inferior de la tabla
	//cadenaHtml+=BotonArribaHTML(txtTodos, 'Seleccionar', 'javascript:Seleccionar();', 50)+BotonArribaHTML(txtResultados, 'Dejar todos los seleccionados', 'javascript:Activar(true);', 50)
	//						+BotonArribaHTML(txtVolver, 'Ver todas las lineas de datos', 'javascript:TodasLineasActivas();', 50)
	cadenaHtml+=cellTitleEnd;
	...
	for(i=1;i<=13;i++)
	{
		...
		cadenaHtml=cadenaHtml+ cellTitleStartMod +'<a href="javascript:OrdenarPorColumna('+(i-1)+');">'+ NombresMeses[i]+'</a>'+cellTitleEnd;
	}
	cadenaHtml+= headEnd + bodyStart;
    ...
    if(ResultadosTabla['MAX_LINEAS_'+IDIndicador] > 0 && ResultadosTabla['MAX_LINEAS_'+IDIndicador] != '')
	{
		...
		for(i=0;i<=NumLineas;i++)
		{
			...
			if(LineaActiva[ColumnaOrdenada[i]]=='S'){
				...
				cellStartThis=Replace(cellGroup,'#IDINDICADOR#',IDIndicadorActual);
				...
				cadenaHtml=cadenaHtml+rowStart+cellGroupStart+pos+cellStartThis+Replace(macroEnlaceGrupo,'#IDGRUPO#',IDGrupos[ColumnaOrdenada[i]])+NombresGrupos[ColumnaOrdenada[i]]+'</a>'+cellEnd;
				...
				for(k=1;k<=13;k++){
					...
					if(k<13){
						...
						if(Colores[ColumnaOrdenada[i]*13+k-1]=='R')	Clase='celdaconrojo';
						if(Colores[ColumnaOrdenada[i]*13+k-1]=='V')	Clase='celdaconverde';
					}else
						Enlace=macroEnlaceNormal;
					...
					if(Valores[ColumnaOrdenada[i]*13+k-1] < 1 && Valores[ColumnaOrdenada[i]*13+k-1] > 0){
						if(ConvertirPorcentaje == 'N'){
							cadenaHtml=cadenaHtml+cellStart+Enlace+AdjustNumber(Valores[ColumnaOrdenada[i]*13+k-1])+'</a>&nbsp;'+cellEnd;
                    	}else{
							cadenaHtml=cadenaHtml+cellStart+Enlace+AdjustNumber(Valores[ColumnaOrdenada[i]*13+k-1]*100)+'%</a>&nbsp;'+cellEnd;
						}
					}else{
						if(ConvertirPorcentaje == 'N'){
							cadenaHtml=cadenaHtml+cellStart+Enlace+AdjustNumber(Valores[ColumnaOrdenada[i]*13+k-1])+'</a>&nbsp;'+cellEnd;
	...
	cadenaHtml+=bodyEnd+footStart+'<tr><td colspan="14" align="left">'
							+BotonArribaHTML(txtTodos, 'Seleccionar', 'javascript:Seleccionar();', 50)
							+BotonArribaHTML(txtResultados, 'Dejar todos los seleccionados', 'javascript:Activar(true);', 50)
							+BotonArribaHTML(txtVolver, 'Ver todas las lineas de datos', 'javascript:TodasLineasActivas();', 50)
							+'</td></tr>'+footEnd;
...
function BotonArribaHTML(texto, explicacion, funcion, ancho){
	var cadenaHtml='<a class="btnDiscreto" href="#" onclick="'+funcion+'" >'+texto+'</a>';
	return cadenaHtml;
}
...
function LineaTotalesHTML(IDIndicador, Totales, linea, HayOcultas){
	...
	for(k=1;k<=13;k++){
		...
		cadenaHtml=cadenaHtml+cellStart+Enlace+AdjustNumber( Totales[k])+'&nbsp;</a>'+cellEnd;
	}
	cadenaHtml=cadenaHtml+rowEnd
...
function Enviar(){
	...
	if((formEis.elements['REFERENCIA'].value=='')||(ComprobarCadenasLargas(formEis.elements['REFERENCIA'].value,2)=='S')){
		...
		alert('Las cadenas para la búsqueda deben tener 2 caracteres o más. Por favor, revise "'+formEis.elements['REFERENCIA'].value+'"');
	}
}
...
function TablaEISAjax(){
	...
	console.log("TablaEISAjax.IDCUADROMANDO="+IDCuadro+"&ANNO="+Anno+"&IDEMPRESA="+IDEmpresa+"&IDCENTRO="+IDCentro+"&IDUSUARIO="+IDUsuarioSel+"&IDEMPRESA2="+IDEmpresa2+"&IDCENTRO2="+IDCentro2+"&IDPRODUCTO="+IDProducto+"&IDGRUPO="+IDGrupo+"&IDSUBFAMILIA="+IDSubfamilia+"&IDFAMILIA="+IDFamilia+"&IDCATEGORIA="+IDCategoria+"&IDESTADO="+IDEstado+"&REFERENCIA="+Referencia+"&CODIGO="+Codigo+"&AGRUPARPOR="+AgruparPor+"&IDRESULTADOS="+IDResultados+"&IDRATIO="+IDRatio);
	...
	jQuery.ajax({
		...
		success: function(objeto){
			...
			// Colocamos el nombre del cuadro en la cabecera de la pagina
			//jQuery('span#NombreIndicador').html(Indicador);

			var NombreCuadro		= data.DatosEIS[0].NombreCuadro;
			console.log('Nuevo cuadro:'+NombreCuadro);
			jQuery('span#NombreIndicador').html(NombreCuadro);
...
/*	20dic21 No permitimos borrar seleccion
...
function listadoExcel()
{
	...
	cadenaCSV=StringtoCSV(Cuadro)+saltoLineaCSV+saltoLineaCSV;

	// etiquetas de meses
	cadenaCSV+=StringtoCSV('');
	for(i=0;i<12;i++)
	{
		cadenaCSV+=StringtoCSV(NombresMeses[i+1]);
	}
	...
	for(var l=0;l<=NumLineas;l++)
	{
		if(LineaActiva[l]=='S')
		{
			...
			for(i=0;i<12;i++)
			{
				cadenaCSV+=NumbertoCSV(Valores[l*13+i]);
			}
			cadenaCSV+=saltoLineaCSV;
		}
	...
	DescargaMIME(StringToISO(cadenaCSV), 'DatosEIS.csv', 'text/csv');
