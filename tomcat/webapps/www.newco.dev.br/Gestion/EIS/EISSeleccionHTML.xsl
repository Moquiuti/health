...
<!--
	...
	Ultima revisión: ET 5set22 15:15 EISSeleccion_050922.js
-->
...
<xsl:text disable-output-escaping='yes'>&lt;!doctype html></xsl:text>
    ...
    <script type="text/javascript" src="http://www.newco.dev.br/Gestion/EIS/EISSeleccion_050922.js"></script>
    ...
    <script type="text/javascript">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
	    <!-- Titulo -->
		<form class="formEstandar" action="http://www.newco.dev.br/Gestion/EIS/EISSeleccion.xsql" name="Seleccion" method="POST">
            ...
            <!--<input type="hidden" name="AREAGEOGRAFICA" id="AREAGEOGRAFICA"/>-->
            <!--<input type="hidden" name="IDCLASIFICACION" id="IDCLASIFICACION"/>-->
			
			<!--
			<xsl:choose>
			<xsl:when test="/EISSeleccion/SELECCION/ADMINMVM">
            	<input type="hidden" name="ADMINMVM" id="ADMINMVM" value="S"/>
			</xsl:when>
			<xsl:otherwise>
            	<input type="hidden" name="ADMINMVM" id="ADMINMVM" value="N"/>
			</xsl:otherwise>
			</xsl:choose>
			-->
			...
			<div style="float: left; width:500px;">
				...
				<li>
                	...
                	<input class="muygrande" type="text" name="SEL_NOMBRE" id="SEL_NOMBRE" value="{/EISSeleccion/SELECCION/EIS_SEL_NOMBRE}"/>
				</li>
				<li>
					<label><xsl:value-of select="document($doc)/translation/texts/item[@name='nombre_corto']/node()"/>:&nbsp;</label>
                	<input class="grande" type="text" name="SEL_NOMBRECORTO" id="SEL_NOMBRECORTO" value="{/EISSeleccion/SELECCION/EIS_SEL_NOMBRECORTO}"/>
				</li>
				<li>
				...
				<xsl:when test="/EISSeleccion/SELECCION/ADMINMVM">
					<li>
                        <label><xsl:value-of select="document($doc)/translation/texts/item[@name='todas_las_empresas']/node()"/>:&nbsp;</label>
					...
					<li>
                        <label><xsl:value-of select="document($doc)/translation/texts/item[@name='todos_los_admin']/node()"/>:&nbsp;</label>
					...
					<li>
                        <label><xsl:value-of select="document($doc)/translation/texts/item[@name='AUTORIZADOS']/node()"/>:&nbsp;</label>
					...
					<li>
                        <label><xsl:value-of select="document($doc)/translation/texts/item[@name='EXCLUIDOS']/node()"/>:&nbsp;</label>
			...
			<div style="float:left;width:1200px;">
				...
				<br/>
				<br/>
            	<table class="buscador" id="nuevaSeleccion" style="display:;">
                	<tr class="sinLinea">
                    	<td>&nbsp;</td>
                    	<td class="labelRight">
							<input type="text" class="muypeq" name="FILTRO1" id="FILTRO1" onkeyUp="javascript:Filtro1();" style="width:600px;height:30px;font-size:20px;"/>
                        	<!--empresas-->
                        	<select name="EMPRESAS[]" id="EMPRESAS" multiple="multiple" size="15" style="width:600px;height:300px;display:none;">
                        	  <xsl:for-each select="/EISSeleccion/SELECCION/EMPRESAS/field/dropDownList/listElem">
                            	  <xsl:if test="ID != ''">
                        	...
                        	<!--centros-->
                        	<select name="CENTROS[]" id="CENTROS" multiple="multiple" size="15" style="width:600px;height:300px;display:none;">
                        	  <xsl:for-each select="/EISSeleccion/SELECCION/CENTROS/field/dropDownList/listElem">
                            	  <xsl:if test="ID != ''">
                            	   <option value="{ID}"><xsl:value-of select="listItem" /></option>
                            	  </xsl:if>
                        	...
                        	<!--proveedores-->
                        	<select name="PROVEEDORES[]" id="PROVEEDORES" multiple="multiple" size="15" style="width:600px;height:300px;display:;">
                        	  <xsl:for-each select="/EISSeleccion/SELECCION/PROVEEDORES/field/dropDownList/listElem">
                            	  <xsl:if test="ID != ''">
                            	   <option value="{ID}"><xsl:value-of select="listItem" /></option>
                            	  </xsl:if>
                        	...
                        	<!--tipos documentos-->
                        	<select name="TIPOSDOCUMENTOS[]" id="TIPOSDOCUMENTOS" multiple="multiple" size="15" style="width:600px;height:300px;display:;">
                        	  <xsl:for-each select="/EISSeleccion/SELECCION/TIPOSDOCUMENTOS/field/dropDownList/listElem">
                            	  <xsl:if test="ID != ''">
                            	   <option value="{ID}"><xsl:value-of select="listItem" /></option>
                            	  </xsl:if>
							...
							<textarea rows="10" cols="30" name="CODIGOS" id="CODIGOS"  style="width:600px;height:300px;"/>
                    	</td>
                    	<td class="uno">
                        	<a href="javascript:NuevoElemento();"><img src="http://www.newco.dev.br/images/flechaGrisDx.gif" alt="añadir"/></a>
							<BR/>
							<BR/>
							<BR/>
							<BR/>
							...
							<BR/>
							<BR/>
                        	<a href="javascript:AnadirEmpresaPorCod();"><img src="http://www.newco.dev.br/images/flechaGrisDx.gif" alt="añadir"/></a>
                    	</td>
                    	<td class="uno">
                        	<a href="javascript:QuitarElemento();"><img src="http://www.newco.dev.br/images/flechaGrisSx.gif" alt="quitar"/></a>
                    	</td>
                    	<td class="datosLeft">
							<input type="text" class="muypeq" name="FILTRO2" id="FILTRO2" onkeyUp="javascript:Filtro2();" style="width:600px;height:30px;font-size:20px;"/>
                        	<select name="REGISTRO_SEL[]" id="REGISTRO_SEL" multiple="multiple" style="width:600px;height:600px;">
                        		<xsl:for-each select="/EISSeleccion/SELECCION/REGISTROS/REGISTRO">
