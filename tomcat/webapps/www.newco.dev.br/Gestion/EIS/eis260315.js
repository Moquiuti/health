...
var		NumLineas,						//	Numero de lineas de la tabla (la ultima contiene el total)
		...
		LineaActiva=new Array(1000),	//	Indica si la linea de la tabla est� activa (y se mostrar�) u oculta
...
function globalEvents(){
	...
	// Si se cambia la seleccion del desplegable de Anno => Recargamos la p�gina haciendo peticion a EISCambioAnyo.xsql (sin pasar valor de los desplegbales)
	...
	if(jQuery('#ANNO').val() < 2012){
		...
		if(jQuery('#AGRUPARPOR').val() == 'FAM' || jQuery('#AGRUPARPOR').val() == 'SF' || jQuery('#AGRUPARPOR').val() == 'CAT' || jQuery('#AGRUPARPOR').val() == 'GRU')
			alert('No es posible agrupar por \'Familia\' o \'Subfamilia\' para a�os anteriores a 2012.\nPor favor, cambia uno de esos valores!');
	}
...
/*
	...
	jQuery("#ANNO").change(function(){
		if(jQuery("#ANNO").val() < 2012){
			...
			if(jQuery("#IDCATEGORIA").length > 0)		jQuery('#IDCATEGORIA').removeAttr('disabled');
			else if(jQuery("#IDFAMILIA").length > 0)	jQuery('#IDFAMILIA').removeAttr('disabled');
                }
	});
*/
...
function DatosCuadroDeMando(IDCuadroDeMando){
	...
	if(jQuery('#IDEMPRESA').length > 0)	IDEmpresaAux = jQuery('#IDEMPRESA').val();
        else					IDEmpresaAux = IDEmpresaDelUsuario;
...
function PreparaNumero(Cadena){
	...
	{
		Errores=Errores+'PreparaNumero: par�metro vacio:'+err+'\n';
	}
	//alert(Res);
	return Res;
}
...
//	Prepara los datos para ser enviados a la pagina con el gr�fico Google correspondiente
...
function MostrarXML(){
	...
	var Enlace='http://www.newco.dev.br/Gestion/EIS/EISDatosXML.xsql?'
			+'US_ID='+IDUsuario
			+'&'+'IDCUADROMANDO='+IDCuadro
			+'&'+'ANNO='+Anno
			+'&'+'IDEMPRESA='+IDEmpresa
...
//	PENDIENTE: separar ordenaci�n segun indicador
function ordenamientoBurbuja(col, tipo){
	...
	//	En caso de ordenaci�n descendente intercambiamos de forma sim�trica el vector de ordenacion
	if(tipo=='DESC'){
...
//	Genera de forma din�mica la tabla de datos del EIS
...
function Enviar(){
	...
	if((formEis.elements['REFERENCIA'].value=='')||(ComprobarCadenasLargas(formEis.elements['REFERENCIA'].value,2)=='S')){
		...
		alert('Las cadenas para la b�squeda deben tener 2 caracteres o m�s. Por favor, revise "'+formEis.elements['REFERENCIA'].value+'"');
	}
}
...
/* DC - 26mar15 - Ya no guardamos la seleccion segun las lineas que se muestran por pantalla
 * Ahora se tendra en cuenta las filas seleccionadas con los checkboxes
	// Recorremos el array LineaActiva para guardar los ID's de los que estan visibles en el orden que aparecen en pantalla
	for(var i=0; i<=NumLineas; i++){
		if(LineaActiva[ColumnaOrdenada[i]] == 'S')
			if(String_Seleccion == '')
				String_Seleccion = IDGrupos[ColumnaOrdenada[i]];
			else
				String_Seleccion += '|'+IDGrupos[ColumnaOrdenada[i]];
	}
*/
