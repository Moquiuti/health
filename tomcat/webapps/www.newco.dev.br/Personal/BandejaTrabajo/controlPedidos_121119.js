...
// Ultima revision 12nov19 08:50	controlPedidos_121119.js
...
/*  
	4mar19: quitamos botón avanzado, dejamos opciones UNIMED
...
function Enviar()
{
	var form=document.forms[0];
        form.action = "ControlPedidos.xsql";
        form.enctype = "application/x-www-form-urlencoded";
        form.method = "post";
        form.target = '';
	
	//solodebug
	alert('Enviar. Accion:'+jQuery('#ACCION').val());
	
	SubmitForm(form);
}

//	Revisa todos los campos y envía el formulario
function EnviarTodo()
{
...
function ActFechaEntrega(){
	...
	jQuery.ajax({
		...
		success: function(objeto){
			var data = eval("(" + objeto + ")");
                                //alert(data.estado);
			if(data.estado == 'OK'){ window.location.reload();}
