...
function cargaDoc(){
    ...
    if (typeDoc != null){  type = typeDoc.value; }
...
function addDocFile(id){
	//alert('id add doc file '+id + ' type '+type+'mo id'+moID);
...
function handleFileRequestDoc(resp){
	...
	if(msg != ''){
		...
		jQuery.ajax({
			...
			beforeSend:function(){
				document.getElementById('confirmBox'+type).style.display = 'none';
				document.getElementById('waitBoxDoc'+type).src = 'http://www.newco.dev.br/images/loading.gif';
			},
			...
			success:function(data){
				...
				//recargamos las ofertas y las fichas
                                if (document.forms['Form1']){
                                    ...
                                    document.getElementById('docLine_'+type).style.display = 'none';
                                }
                                if(document.forms['Cabecera']){
                                    document.forms['Cabecera'].elements['IDALBARAN'].value = doc[0].id_doc;
                                    document.getElementById('download_'+type).href = 'http://www.newco.dev.br/Documentos/'+doc[0].file;
                                    document.getElementById('download_'+type).title = doc[0].nombre;
                                    //visulizo link descarga
                                    document.getElementById('download_'+type).style.display = '';
                                    //oculto subida documento
                                    document.getElementById('docLine_'+type).style.display = 'none';
                                }
				...
				var clearedInput;
				var uploadElem = document.getElementById("inputFileDoc_" + type);
