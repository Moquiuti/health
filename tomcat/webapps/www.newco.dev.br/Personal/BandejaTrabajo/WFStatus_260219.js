// JS Página de Inicio
// Ultima revisión ET: 26feb19 09:16
...
{
	if (!Number.isInteger(IDOferta)) IDOferta=Piece(IDOferta,'_',1);			//	20feb19 desde el control de albarán pasara una cadena IDALBARAN_IDOFERTA
...
function DescargarExcel(){
  var form=document.forms[0];
	...
	var pagina='', lineasPorPagina='', Urgente='';
  ...
  if(form.elements.BLOQUEADO) bloqueado = (form.elements.BLOQUEADO_CHECK.checked?'S':'N');
  ...
  if(form.elements.SINSTOCKS) sinStocks =  (form.elements.SINSTOCKS_CHECK.checked?'S':'N');

  if(form.elements.PRODUCTO && form.elements.PRODUCTO.value !== '') producto = form.elements.PRODUCTO.value;

  if(form.elements.TODOS12MESES) todos12meses = (form.elements.TODOS12MESES_CHECK.checked?'S':'N');

  if(form.elements.INCFUERAPLAZO) incFueraPlazo = (form.elements.INCFUERAPLAZO_CHECK.checked?'S':'N');	

  if(form.elements.PED_ENTREGADOOK) pedEntregadoOk = (form.elements.PED_ENTREGADOOK_CHECK.checked?'S':'N');

  if(form.elements.PED_PEDIDONOCOINCIDE) pedPedidoNoCoincide = (form.elements.PED_PEDIDONOCOINCIDE_CHECK.checked?'S':'N');

  if(form.elements.PED_RETRASADO) pedRetrasado = (form.elements.PED_RETRASADO_CHECK.checked?'S':'N');

  if(form.elements.PED_ENTREGADOPARCIAL) pedEntregadoParcial = (form.elements.PED_ENTREGADOPARCIAL_CHECK.checked?'S':'N');

  if(form.elements.PED_RETRASODOCTECNICA) pedRetrasoDocTecnica = (form.elements.PED_RETRASODOCTECNICA_CHECK.checked?'S':'N');

  if(form.elements.PED_NOINFORMADOPLATAFORMA) pedNoInformadoPlataforma = (form.elements.PED_NOINFORMADOPLATAFORMA_CHECK.checked?'S':'N');

  if(form.elements.PED_PRODUCTOSANYADIDOS) pedProductosAnadidos = (form.elements.PED_PRODUCTOSANYADIDOS_CHECK.checked?'S':'N');
  ...
  if(form.elements.PED_MALAATENCIONPROV) pedMalaAtencionProv = (form.elements.PED_MALAATENCIONPROV_CHECK.checked?'S':'N');
  ...
  if(form.elements.PED_URGENTE && form.elements.PED_URGENTE.value !== '') Urgente = form.elements.PED_URGENTE.value;
	...
	jQuery.ajax({
		...
		data: "IDEMPRESA="+IDEmpresa+"&IDCENTRO="+IDCentro+"&IDPROVEEDOR="+IDProveedor+"&IDFILTROMOTIVO="+IDFiltroMotivo+"&IDFILTRORESPONSABLE="+IDFiltroResponsable+"&IDFILTROESTADO="+IDFiltroEstado+"&IDFILTROSEMAFORO="+IDFiltroSemaforo+"&IDFILTROTIPOPEDIDO="+IDFiltroTipoPedido+"&FARMACIA="+farmacia+"&MATERIAL="+material+"&CODIGOPEDIDO="+codigoPedido+"&SINSTOCKS="+sinStocks+"&PRODUCTO="+producto+"&TODOS12MESES="+todos12meses+"&INCFUERAPLAZO="+incFueraPlazo+"&PED_ENTREGADOOK="+pedEntregadoOk+"&PED_PEDIDONOCOINCIDE="+pedPedidoNoCoincide+"&PED_RETRASADO="+pedRetrasado+"&PED_ENTREGADOPARCIAL="+pedEntregadoParcial+"&PED_RETRASODOCTECNICA="+pedRetrasoDocTecnica+"&PED_NOINFORMADOPLATAFORMA="+pedNoInformadoPlataforma+"&PED_PRODUCTOSANYADIDOS="+pedProductosAnadidos+"&FECHA_INICIO="+fechaInicio+"&FECHA_FINAL="+fechaFinal+"&PED_MALAATENCIONPROV="+pedMalaAtencionProv+"&PED_URGENTE="+Urgente+"&PAGINA="+pagina+"&LINEA_POR_PAGINA="+lineasPorPagina+"&_="+d.getTime(),
    type: "GET",
    contentType: "application/xhtml+xml",
    error: function(objeto, quepaso, otroobj){
      alert('error'+quepaso+' '+otroobj+''+objeto);
