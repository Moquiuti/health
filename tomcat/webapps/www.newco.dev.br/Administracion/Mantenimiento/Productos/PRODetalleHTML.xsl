...
<!--
	...
	Ultima revision: ET 10jun22 15:10 PRODetalle_071020.js
-->
...
<head>
  ...
  <!--fin codigo etiquetas-->
	...
	<script type="text/javascript" src="http://www.newco.dev.br/Administracion/Mantenimiento/Productos/PRODetalle_071020.js"></script>
</head>
...
<xsl:template match="PRODUCTO">
	...
	<div><!--	contiene todo lo que hay bajo las pestañas	-->
		...
		<div class="ZonaTituloPagina">
			<p class="Path"><xsl:value-of select="document($doc)/translation/texts/item[@name='Inicio']/node()"/>&nbsp;/&nbsp;<span class="FinPath"><xsl:value-of select="document($doc)/translation/texts/item[@name='producto']/node()"/>&nbsp;</span>
			...
			<p class="TituloPagina">
				...
				<xsl:when test="PRO_ID">
					...
					<xsl:when test="string-length(PRO_NOMBRE) > 60">
						<xsl:value-of select="substring(PRO_NOMBRE, 1, 60)"/>...
					</xsl:when>
					<xsl:otherwise>
				...
				<span class="CompletarTitulo" style="width:760px;">
					<!--
					<xsl:if test="USUARIO/NOMBRE != ''">
						<xsl:value-of select="document($doc)/translation/texts/item[@name='responsable']/node()"/>:&nbsp;<xsl:value-of select="USUARIO/NOMBRE"/>
						&nbsp;
					...
					<xsl:value-of select="document($doc)/translation/texts/item[@name='fecha']/node()"/>: <xsl:value-of select="ALTA" />&nbsp;
					-->
					<xsl:if test="$usuario = 'CDC' and TIENE_DOCUMENTOS">
						<a class="btnDestacado" href="javascript:MostrarPagPersonalizada('http://www.newco.dev.br/Administracion/Mantenimiento/Productos/PRODocs.xsql?PRO_ID={/Productos/PRODUCTO/PRODUCTO/PRO_ID}','Documentación producto',100,80,0,-10);"  style="text-decoration:none;">
							<xsl:value-of select="document($doc)/translation/texts/item[@name='documentos']/node()"/>
						</a>
						&nbsp;
					</xsl:if>
					<xsl:if test="$usuario = 'CDC' and /Productos/PRODUCTO/PRODUCTO/PRO_ESPACK='S'">
						<a class="btnDestacado" href="javascript:MostrarPagPersonalizada('http://www.newco.dev.br/Administracion/Mantenimiento/Productos/PROPack.xsql?PRO_ID={/Productos/PRODUCTO/PRODUCTO/PRO_ID}','Pack del producto',100,80,0,-10);"  style="text-decoration:none;">
							<xsl:value-of select="document($doc)/translation/texts/item[@name='Pack']/node()"/>
						</a>
						&nbsp;
					</xsl:if>
					<xsl:if test="$usuario = 'CDC'">
						<a class="btnDestacado" href="javascript:MostrarPagPersonalizada('http://www.newco.dev.br/Gestion/Comercial/NuevaEvaluacionProducto.xsql?PRO_ID={/Productos/PRODUCTO/PRODUCTO/PRO_ID}&amp;EMP_ID={/Productos/PRODUCTO/PRODUCTO/IDPROVEEDOR}','Evaluación producto',100,80,0,-10);"  style="text-decoration:none;">
							<xsl:value-of select="document($doc)/translation/texts/item[@name='evaluacion']/node()"/>
						</a>
						&nbsp;
					</xsl:if>
					<xsl:if test="$usuario != 'OBSERVADOR'">
						<a class="btnDestacado" href="javascript:MostrarPagPersonalizada('http://www.newco.dev.br/Administracion/Mantenimiento/Productos/NuevaIncidencia.xsql?PRO_ID={/Productos/PRODUCTO/PRODUCTO/PRO_ID}&amp;USER={$usuario}','Evaluación producto',100,80,0,-10);"  style="text-decoration:none;">
                    ...
                    <xsl:if test="(MVM or ADMIN or CDC) and PRO_ID != ''">
                        <a class="btnNormal" href="javascript:MostrarPagPersonalizada('http://www.newco.dev.br/Administracion/Mantenimiento/Productos/PROManten.xsql?PRO_ID={PRO_ID}&amp;TIPO=M','Mantenimiento Producto',100,80,0,0);" title="Modificar producto" style="text-decoration:none;">
                            <xsl:value-of select="document($doc)/translation/texts/item[@name='Editar']/node()"/>Editar
                        </a>
						&nbsp;
                        <a class="btnNormal" href="javascript:abrirFichaCatalogacion('{PRO_ID}','{IDEMPRESADELUSUARIO}');" title="Ficha de catalogación" style="text-decoration:none;">
    ...
    <div class="divLeft70" style="height:30px;margin-top:10px;font-weight:bold;">
...
<xsl:if test="NOMBRE_PRIVADO or PRO_ID">
    ...
    <div class="divLeft">
	    ...
	    <!--<div class="divLeft30nopa" style="text-align:right;">-->
			...
			<xsl:if test="REFERENCIA_CLIENTE !=''">
     		...
     		<xsl:if test="REFERENCIA_PRIVADA !=''">
				<tr class="sinLinea">
					...
					<td class="datosLeft">
		        <a href="javascript:MostrarPagPersonalizada('http://www.newco.dev.br/ProcesosCdC/MantenimientoReducido/MantenimientoReducido.xsql?PRO_ID={PRO_ID}','Ficha Catalogación',100,80,0,-20);">
