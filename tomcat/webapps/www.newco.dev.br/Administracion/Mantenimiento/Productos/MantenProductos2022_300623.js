...
//	ultima revision ET 30jun23 09:40 MantenProductos2022_300623.js
...
function MostrarEstructuraDatos()
{
	...
	else if (opcion=='ADJUD')
	{
		...
		titulo+='&nbsp;o&nbsp;'+ncRefEstandar+'|[LIBERAR]|[LIBERAR]';
	...
	{
		var cadPrecioFab='';
		if (conPrecioFab=='S')		//30jun23
			cadPrecioFab='['+sepCampos+ncPrecioFab+sepCampos+ncDescuento+']';
		
		titulo=ncNifProveedor+sepCampos+ncRefProveedor+sepCampos+ncPrecio+'['+sepCampos+ncNombre+'['+sepCampos+ncFechaInicioTarifa+sepCampos+ncFechaFinalTarifa
							+'['+sepCampos+ncCantBonif+sepCampos+ncCantGratis+cadPrecioFab+']]]';
	}
...
function ProcesarActualizarTarifasTXT(CadenaCambios)
{
	...
	for (i=0;i<=numRefs;++i)
	{
		...
		CantGratis	= Piece(Producto, sepCampos, col);
		++col;
		PrecioFabrica= Piece(Producto, sepCampos, col);
		++col;
		Descuento	= Piece(Producto, sepCampos, col);

		//solodebug		
		console.log('MantenProductosTXT. Linea ('+i+'). CodProveedor:'+CodProveedor+'. Ref:'+Referencia+'. Prod:'+Nombre+'. Precio:'+Precio+'. FechaFinal:'+FechaFinal+'. CantBonif:'+CantBonif+'. CantGratis:'+CantGratis+' PrecioFabrica:'+PrecioFabrica+' Descuento:'+Descuento);
		...
		if ((Referencia!='RefProveedor')&&(Referencia!=''))		//	Eliminamos líneas vacias y linea de titulo. Cuidado, ponia &&(Nombre!='') pero este campo es opcional
		{
			...
			items['PrecioFabrica'] = PrecioFabrica;				//	30jun23 Nuevo campo, precio de fabrica
			items['Descuento'] = Descuento;						//	30jun23 Nuevo campo, descuento
			items['IDProducto'] = '';							//	inicializaremos vía ajax
			...
			if ((items['Precio']!='')&&(isNaN(parseFloat(items['Precio']))))
			{
				...
				licAvisosCarga += '('+(i+1)+') '+Referencia+': '+items['Precio']+': '+strPrecioNoNumerico +'<br/>';
			}
			...
			//	Precio
			if ((items['PrecioFabrica']!='')&&(isNaN(parseFloat(items['PrecioFabrica']))))
			{
				//solodebug licAvisosCarga += 'conterr6';
				licAvisosCarga += '('+(i+1)+') '+Referencia+': '+items['PrecioFabrica']+': '+strPrecioNoNumerico +'<br/>';
			}

			//	Precio
			if ((items['Descuento']!='')&&(isNaN(parseFloat(items['Descuento']))))
			{
				//solodebug licAvisosCarga += 'conterr6';
				licAvisosCarga += '('+(i+1)+') '+Referencia+': '+strDescuentoNoNumerico +'<br/>';
			}
...
function EnviarTodosTarifas(licNumRegistrosEnviados, ForzarNombre)
{
	...
	jQuery.ajax({
		...
		data:	"INTF_ID="+ficID
					...
					+"&NOMBRE="+encodeURIComponent(ScapeHTMLString(ficProductos[licNumRegistrosEnviados].Nombre))
					+"&PRECIO="+ficProductos[licNumRegistrosEnviados].Precio
					+"&OTROS=ACTUALIZAR_TARIFA|"+ficProductos[licNumRegistrosEnviados].FechaFinal+'|'+ficProductos[licNumRegistrosEnviados].CantBonif+'|'
								+ficProductos[licNumRegistrosEnviados].CantGratis+'|'+ficProductos[licNumRegistrosEnviados].PrecioFabrica+'|'+ficProductos[licNumRegistrosEnviados].Descuento
					+"&_="+d.getTime(),
		contentType: "application/xhtml+xml",
		error: function(objeto, quepaso, otroobj){
			ficError='ERROR_ENVIANDO';
...
//	3mar23 Descarga un CSV de ejemplo
function DescargarModelo()
{

	var NombreFichero=$( "#FIDOPCION" ).val()+'.csv';
	var cadenaCSV=jQuery("#formatoCarga").html();
	
	var Pos=cadenaCSV.indexOf("&nbsp;o&nbsp;");
	if (Pos>0)
		cadenaCSV=cadenaCSV.substring(0, Pos);
	
	cadenaCSV=cadenaCSV.replaceAll('[','').replaceAll(']','');
	
	//solodebug;
	console.log('DescargarModelo. NombreFichero:'+NombreFichero+' cadenaCSV:'+cadenaCSV);
	
	DescargaMIME(StringToISO(cadenaCSV), NombreFichero, 'text/csv');
