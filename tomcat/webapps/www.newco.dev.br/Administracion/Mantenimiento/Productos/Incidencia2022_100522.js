// JavaScript Document
//DETALLE PRODUCTO INCIDENCIAS MC 18-06-2013 Incidencia2022_100522.js
...
function GuardarNuevaIncidencia(form){
	...
	var user = form.elements['USER'].value;
		...
		for(var j = 0; j < radioSeguirUtilizando.length; j++){
                    if(radioSeguirUtilizando[j].checked){
			if(radioSeguirUtilizando[j].checked == true){
				form.elements['SEGUIR_UTILIZANDO'].value = radioSeguirUtilizando[j].value;
                                seguirUtilizando = radioSeguirUtilizando[j].value;
			}
		}
            }
               
        if (form.elements['SEGUIR_UTILIZANDO'].value == 'N') form.elements['SEGUIR_UTILIZANDO'].value = '';
        
        //alert(form.elements['SEGUIR_UTILIZANDO'].value);
			
	if (form.elements['INCIDENCIA'].value == ''){ msg += textoIncidenciaObli+'\n';}
	
        if (seguirUtilizando == ''){
                msg = msg + document.forms['mensajeJS'].elements['SEGUIR_UTILIZANDO_CDC_OBLI'].value;            
            }
            
	if (msg == ''){
		//si estoy en licitacion paso el id de licitacion
		if (idLic != ''){
			//10may22	form.action = "NuevaIncidenciaSave.xsql?USER="+user+"&LIC_OFE_ID="+idLic+"&LIC_PROD_ID="+licProdId;
			form.action = "Incidencia2022.xsql?USER="+user+"&LIC_OFE_ID="+idLic+"&LIC_PROD_ID="+licProdId;
			}
		else
		{ 
			//form.action = "NuevaIncidenciaSave.xsql";
			form.action = "Incidencia2022.xsql";
		}
...
function GuardarIncidencia(form,IDEstado){
	...
	var licProdId = form.elements['LIC_PROD_ID'].value;
        ...
        var seguirUtilizandoCdC = '';
        
        var radioSeguirUtilizando = document.getElementsByName('SEGUIR_UTILIZANDO_VALUES');
		for(var j = 0; j < radioSeguirUtilizando.length; j++){
                    if(radioSeguirUtilizando[j].checked){
                        seguirUtilizando = 'check';
			if(radioSeguirUtilizando[j].checked == true){
				form.elements['SEGUIR_UTILIZANDO'].value = radioSeguirUtilizando[j].value;
			}
                    }
        ...
        if (IDEstado == 'P'){
            var radioSeguirUtilizandoCdC = document.getElementsByName('SEGUIR_UTILIZANDO_CDC_VALUES');
		for(var j = 0; j < radioSeguirUtilizandoCdC.length; j++){
			if(radioSeguirUtilizandoCdC[j].checked == true){
				form.elements['SEGUIR_UTILIZANDO_CDC'].value = radioSeguirUtilizandoCdC[j].value;
                                seguirUtilizandoCdC = radioSeguirUtilizandoCdC[j].value;
			}
		}   
            if (form.elements['SEGUIR_UTILIZANDO_CDC'].value == 'N') form.elements['SEGUIR_UTILIZANDO_CDC'].value = '';
                //alert('seguir usando '+seguirUtilizandoCdC);
            if (seguirUtilizandoCdC == ''){
                msg = msg + document.forms['mensajeJS'].elements['SEGUIR_UTILIZANDO_CDC_OBLI'].value;            
            }
	...
	if(msg == ''){
		if(idLic != ''){
			...
			form.action = "Incidencia2022.xsql?LIC_OFE_ID="+idLic+"&LIC_PROD_ID="+licProdId+"&IDESTADO="+IDEstado;
		}else{
			//form.action = "IncidenciaSave.xsql?IDESTADO="+IDEstado;
			form.action = "Incidencia2022.xsql?IDESTADO="+IDEstado;
		}
	...
	if(msg != ''){
		...
		jQuery.ajax({
			...
			success:function(data){
				...
				//Informamos del IDDOC de la incidencia en el input hidden que toca y avisamos al usuario
				...
				jQuery('#borraDoc'+tipo).append('<a href="javascript:borrarDoc(' + IDDoc + ',\'' + tipo + '\')"><img src="http://www.newco.dev.br/images/2022/icones/del.svg"/></a>').show();
