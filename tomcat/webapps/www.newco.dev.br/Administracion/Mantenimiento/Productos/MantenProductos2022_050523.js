...
//	ultima revision ET 05mar23 11:40 MantenProductos2022_050523.js
...
function MostrarEstructuraDatos()
{
	...
	{
		titulo=ncNifProveedor+sepCampos+ncRefProveedor+sepCampos+ncPrecio+'['+sepCampos+ncNombre+'['+sepCampos+ncFechaInicioTarifa+sepCampos+ncFechaFinalTarifa+'['+sepCampos+ncCantBonif+sepCampos+ncCantGratis+']]]';
	}
...
function ProcesarActualizarTarifasTXT(CadenaCambios)
{
	...
	for (i=0;i<=numRefs;++i)
	{
		var CodProveedor,Referencia,Precio,Nombre,FechaInicio,FechaFinal;
		var Producto= Piece(CadenaCambios, '·',i);
		...
		Nombre		= Piece(Producto, sepCampos, col);
		++col;
		FechaInicio	= Piece(Producto, sepCampos, col);
		++col;
		...
		if ((Referencia!='RefProveedor')&&(Referencia!=''))		//	Eliminamos líneas vacias y linea de titulo. Cuidado, ponia &&(Nombre!='') pero este campo es opcional
		{
			...
			items['Nombre'] = Nombre;
			items['Precio'] = Precio.replace(/\./g,"");			//	Eliminamos los puntos (separador de miles)
			items['FechaInicio'] = FechaInicio;					//	05may23 Nuevo campo, fecha inicio de tarifa
...
function EnviarTodosTarifas(licNumRegistrosEnviados, ForzarNombre)
{
	...
	jQuery.ajax({
		...
		data:	"INTF_ID="+ficID
					...
					+"&NOMBRE="+encodeURIComponent(ScapeHTMLString(ficProductos[licNumRegistrosEnviados].Nombre))
					+"&PRECIO="+ficProductos[licNumRegistrosEnviados].Precio
					+"&OTROS=ACTUALIZAR_TARIFA|"+ficProductos[licNumRegistrosEnviados].FechaInicio+'|'+ficProductos[licNumRegistrosEnviados].FechaFinal+'|'+ficProductos[licNumRegistrosEnviados].CantBonif+'|'+ficProductos[licNumRegistrosEnviados].CantGratis
					+"&_="+d.getTime(),
		contentType: "application/xhtml+xml",
		error: function(objeto, quepaso, otroobj){
			ficError='ERROR_ENVIANDO';
