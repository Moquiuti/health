...
function proveedorManual(valor){
	if (valor == 'PROV_MANUAL'){
		...
		jQuery("#IDPROVEEDOR").val(valor);
                recuperaDocsProveedor(valor);
        }
}

function recuperaDocsProveedor(IDProv){
	var d = new Date();
	...
	jQuery.ajax({
		...
		type:	"GET",
		data:	"IDPROVEEDOR="+IDProv+"&_="+d.getTime(),
...
function NuevoProductoEquivalente(oForm){
	...
	var Proveedor		= (oForm.elements['PROV_EQUI_MANUAL'].value != '' ? codificacionAjax(oForm.elements['PROV_EQUI_MANUAL'].value) : '');
	var Referencia		= codificacionAjax(oForm.elements['REF_EQUI'].value);
	var Nombre		= codificacionAjax(oForm.elements['NOMBRE_EQUI'].value);
	var Marca		= codificacionAjax(oForm.elements['MARCA_EQUI'].value);
	var Comentarios		= codificacionAjax(oForm.elements['COMENTARIO_EQUI'].value);
	...
	jQuery.ajax({
		cache:	false,
		url:	'http://www.newco.dev.br/Administracion/Mantenimiento/Productos/ProductosEquivalentes.xsql',
		type:	"GET",
...
function borrarProdEquiv(IDProdEquiv){
	var NuevoEstado = 'B';
	var Comentarios = codificacionAjax('Borrado desde sistema');
	var d = new Date();

	jQuery.ajax({
		cache:	false,
		url:	'http://www.newco.dev.br/Administracion/Mantenimiento/Productos/ProdEquivBorrar.xsql',
		type:	"GET",
...
function SeleccionaFichas(IDPROVE,ID,ACCION){
