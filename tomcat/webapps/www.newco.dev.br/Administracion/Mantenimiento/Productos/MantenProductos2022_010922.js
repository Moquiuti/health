...
//	ultima revision ET 1set22 11:05 MantenProductos2022_010922.js
...
function ProcesarActualizarTarifasTXT(CadenaCambios)
{
	...
	for (i=0;i<=numRefs;++i)
	{
		...
		if ((Referencia!='RefProveedor')||(Referencia!='')||(Nombre!=''))		//	Eliminamos líneas vacias y linea de titulo
		{
			var items		= [];
			items['IDCliente']	= IDCliente;
			items['CodProveedor']	= CodProveedor;
...
function EnviarTodosTarifas(licNumProductosEnviados, ForzarNombre)
{
	...
	jQuery.ajax({
		...
		data:	"INTF_ID="+ficID
					...
					+"&NOMBRE="+encodeURIComponent(ScapeHTMLString(ficProductos[licNumProductosEnviados].Nombre))
					+"&PRECIO="+ficProductos[licNumProductosEnviados].Precio
					+"&OTROS=ACTUALIZAR_TARIFA|"+ficProductos[licNumProductosEnviados].FechaFinal+'||||||'+ficProductos[licNumProductosEnviados].CantBonif+ficProductos[licNumProductosEnviados].CantGratis
					+"&_="+d.getTime(),
		contentType: "application/xhtml+xml",
		error: function(objeto, quepaso, otroobj){
			ficError='ERROR_ENVIANDO';
