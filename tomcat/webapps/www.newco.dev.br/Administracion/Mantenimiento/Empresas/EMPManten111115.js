...
function selectProvincia(pais){
 jQuery('.provincias').hide();
 jQuery('#provincia_'+pais).show();
...
function ActualizarDatos(form){
	if(validarFormulario(form)){
		//multi pais => doy a provincia el valor de prov espanya o brasil
		//espanya
		if(document.getElementById('provincia_34').style.display != 'none' && form.elements['EMP_IDPAIS'].value == '34' && form.elements['PROVINCIA_34'].value != ''){
			form.elements['EMP_PROVINCIA'].value = form.elements['PROVINCIA_34'].value;
		}
		//brasil
		if(document.getElementById('provincia_55').style.display != 'none' && form.elements['EMP_IDPAIS'].value == '55' && form.elements['PROVINCIA_55'].value != ''){
			form.elements['EMP_PROVINCIA'].value = form.elements['PROVINCIA_55'].value;
		}
...
function validarFormulario(form){
	...
	var regex_nombre_pub	= new RegExp("^[0-9a-zA-Z]+$","g"); // Expresion regular para controlar el campo EMP_FAX que solo puede incluir números y letras (requisito MVM)
...
// Hacemos la peticion ajax en esta misma funcion pq sino no espera a recibir respuesta del servidor -- javascript funciona de foma asincrona
	...
	if(document.forms[0].elements['EMP_POTENCIAL_COMPRAS'].length && (!checkRegEx(document.forms[0].elements['EMP_POTENCIAL_COMPRAS'].value, regex_potcompras)) ){
		errores=1;
	...
	if(document.forms[0].elements['EMP_POTENCIAL_COMPRAS'].length && (!checkRegEx(document.forms[0].elements['EMP_POTENCIAL_CATALOGO'].value, regex_potcat)) ){
		errores=1;
		alert(document.forms['MensajeJS'].elements['FORMATO_NUMERO'].value);
