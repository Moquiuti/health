	...
	<![CDATA[
		<script type="text/javascript">
			...
			function verPestana(valor){
				...
				var idEmpresa	 = encodeURIComponent(oForm.elements['IDEMPRESA'].value);
			...
			function CondProveedorSend(oForm){
				...
				var idFormaPago	= encodeURIComponent(oForm.elements['IDFORMAPAGO'].value);
                                var idPlazoPago	= encodeURIComponent(oForm.elements['IDPLAZOPAGO'].value);
			...
			}//fin AsociarOferta a Todos

            //asociar una oferta a todos los productos de una empresa
			function AsociarDocComeTodos(oferta, tipo,nombre){
				//alert('of '+oferta+' tipo  '+tipo+' tipo doc'+nombre);
				var form = document.forms['OfertaForm'];
				var idempresa = form.elements['ID_EMPRESA'].value;
				//alert('mi emp '+idempresa);
				var idoferta = oferta;
				...
				if (confirm(document.forms['MensajeJS'].elements['SEGURO_ELIMINAR_OFERTA'].value)){
					...
					jQuery.ajax({
						...
						success:function(data){
                            ...
                            var url = document.URL+'&ZONA=DOCS'; //añado la zona así seguiré estando en la parte de documentos.
...
//							document.location.reload();
            ...
            function CambiarEmpresa(id){
                ...
                //http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/EMPDetalleFrame.xsql?EMP_ID=1
            }
      ...
      //carga de un documento
            ...
            function waitDoc(text) {
             ...
             function periodicUpdateDoc() {
                ...
                if (uploadFrameDoc && uploadFrameDoc.document && uploadFrameDoc.document.getElementsByTagName("p")[0]) {
                    ...
                    //alert('elem subido '+uploadFrameDoc.document.getElementsByTagName("p")[0]);
            ...
            function handleFileRequestDoc(resp) {
                ...
                //alert('resp doc '+resp.documentos);
                if (resp.documentos) {
                        ...
                        for (var i = 0; i < resp.documentos.length; i++) {
                            if (resp.documentos[i].error && resp.documentos[i].error != '') {
                                ...
                                if (resp.documentos[i].size) {
                                    //alert('resp '+resp.documentos[i].nombre);
        ...
        function addDocFile(id) {
            ...
            if (uploadElem.value != '') {
                ...
                if (!document.getElementById("inputDocLink")) {
                    ...
                    //alert(document.getElementById("docLine"));
        ...
        function asociarDocumentoPadre(id){
            ...
            for(i=0;i<form.length;i++){
               if(form.elements[i].name.substring(0,14) == 'OFERTAS_ANEXO_'){
                ...
                var doc_anexo = id;
                //alert('id doc anexo '+doc_anexo+' id padre '+doc_padre);
			...
			<xsl:if test="/Empresas/EMPRESA/MVM or /Empresas/EMPRESA/MVMB">
				<xsl:if test="/Empresas/EMPRESA/ADMIN">
					...
					<a style="text-decoration:none;">
						...
						<!--<xsl:attribute name="href">http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/EMPManten.xsql?ID=<xsl:value-of select="EMP_ID"/>&amp;ADMINISTRADORMVM=ADMINISTRADORMVM</xsl:attribute>-->
...
-->
		...
		<h1 class="titlePage cond_comercial cond_comercial_prov" style="display:none;float:left;width:60%;padding-left:20%;">
                    ...
                    <xsl:if test="/Empresas/EMPRESA/MVM or /Empresas/EMPRESA/MVMB">
                        ...
                        <!--enseño resumen arriba solo para proveedores, si es cliente enseño tabla directamente-->
...
<xsl:template name="condComercialProveedor">
	...
	<!-- FIN Pop-up para mostrar tabla resumen empresa -->
        ...
        <!--pestañas condic comerciales prove, licitaciones, incidencias, rotura stock proveedor, cliente y mvm visualiza-->
...
<!--PEDIDO MINIMO PARA PROVEE-->
	...
	<!--idioma fin-->
        ...
        <xsl:choose><!--si no hay pedidos minimos por centros enseño en 4 lineas el pedido minimo general, si no enseño todo como tabla-->
