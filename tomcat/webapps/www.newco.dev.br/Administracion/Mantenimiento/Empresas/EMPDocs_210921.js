...
function DibujaTabla(Categoria)
{
	...
	for (var i=0;i<Categoria.Documentos.length;++i)
	{
		...
		linea=linea.replace(/\[ID\]/g,Categoria.Documentos[i].ID).replace(/\[CONTADOR\]/g,Categoria.Documentos[i].Cont).replace(/\[URL\]/g,Categoria.Documentos[i].Url).replace(/\[NOMBRE\]/g,Categoria.Documentos[i].Tipo+':'+Categoria.Documentos[i].Nombre);
