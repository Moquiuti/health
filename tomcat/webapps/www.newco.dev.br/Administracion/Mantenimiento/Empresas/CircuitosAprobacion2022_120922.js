...
function GuardarCircuito()
{
	...
	if (document.forms['frmCirc'].elements['LISTAAPROBADORES'].value=='')
		Errores+=errAprobadorObligatorio+'\n';	
...
//	12set22 Cambia el tipo de circuito PEDIDO/LICITACION
function CambiaTipo()
{
	var Tipo=jQuery("#TIPO").val();

	//solodebug
	console.log('CambiaTipo:'+Tipo);
	
	jQuery("#LISTACONDICIONES").val('');
	...
	if (Tipo==='PED')
	{
		jQuery("#TDCONDICIONES").show();
		jQuery("#TDCONVENIO").show();
		jQuery("#TDIMPORTE").show();
		jQuery("#TDNIVEL").show();
		jQuery("#TDORDEN").show();
	}
	else if (Tipo==='LIC')
	{
		jQuery("#TDCONDICIONES").hide();
		jQuery("#TDIMPORTE").hide();
		jQuery("#TDCATEGORIAS").hide();	
		jQuery("#TDPROVEEDORES").hide();	
		jQuery("#TDCONVENIO").hide();	
		jQuery("#TDNIVEL").hide();	
		jQuery("#TDORDEN").hide();	
	}
}
...
function CambiaCondicion()
{
	...
	{
		jQuery("#TDCATEGORIAS").show();	
		jQuery("#TDPROVEEDORES").hide();	
	}
	else if (Condicion==='PROV')
	{
		jQuery("#TDCATEGORIAS").hide();	
		jQuery("#TDPROVEEDORES").show();	
	}
	...
	jQuery.ajax({
		...
		success: function(objeto){
			...
			if(data.CentrosConsumo.length > 0){
				...
				jQuery("#TDCENTROCOSTE").show();	
				jQuery("#IDCENTROCOSTE").show();	
			}
		}
	});
}
