...
<!--	
	Mostrar los historicos (logs) del centro
	Ultima revision: ET 9mar23 10:40
-->
...
<head>
	...
	<xsl:choose>
		<xsl:when test="/LogsCentro/LANG"><xsl:value-of select="/LogsCentro/LANG" /></xsl:when>
		<xsl:otherwise>spanish</xsl:otherwise>
	</xsl:choose>  
	</xsl:variable>
	<xsl:variable name="doc">http://www.newco.dev.br/General/texts_<xsl:value-of select="$lang"/>.xml</xsl:variable>        <!--idioma fin-->
        
	<title><xsl:value-of select="/LogsCentro/LOGS/CENTRO"/>:&nbsp;<xsl:value-of select="document($doc)/translation/texts/item[@name='Ver_logs']/node()"/></title>
    <!--style-->
    ...
    <xsl:choose>
		...
		<div class="ZonaTituloPagina">
			<p class="TituloPagina">
				<xsl:value-of select="document($doc)/translation/texts/item[@name='Ver_logs']/node()"/>:&nbsp;<xsl:value-of select="/LogsCentro/LOGS/CENTRO"/>
				&nbsp;&nbsp;
				<span class="CompletarTitulo">
					<a class="btnNormal">
						<xsl:attribute name="href">javascript:chMantenCentro(<xsl:value-of select="/LogsCentro/LOGS/IDCENTRO"/>);</xsl:attribute>
						<xsl:value-of select="document($doc)/translation/texts/item[@name='volver']/node()"/>
					</a>
				</span>
			</p>
		</div>
        <xsl:if test="/LogsCentro/LOGS/LOG">
			...
			<table cellspacing="6px" cellpadding="6px">
				...
				<tbody class="corpo_tabela">
	       		<xsl:for-each select="/LogsCentro/LOGS/LOG">
				<tr class="conhover">
