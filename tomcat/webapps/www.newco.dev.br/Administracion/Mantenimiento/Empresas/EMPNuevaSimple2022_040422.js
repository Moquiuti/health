...
//	Ultima revision: ET 04abr22 11:15 EMPNuevaSimple2022_040422.js
...
function validarFormulario(form)
{
	...
	//	8nov21 
	var fase=parseInt(jQuery("#FASE").val());
	if (fase=='2') 	//	Comprueba que haya alguna area geo seleccionada
	{
		Seleccionados=0;
		formSel=document.forms["frmAREA"];
		
		for (var i=0;i<formSel.elements.length;++i)
		{
			if (Piece(formSel.elements[i].name,'_',0)=="hid")
			{
				if (formSel.elements[i].value=='S') ++Seleccionados;
				//solodebug console.log(formSel.elements[i].name+': '+formSel.elements[i].value+'...Seleccionados:'+Seleccionados);
			}
		}
		if (Seleccionados==0)
		{
			alert(str_OBLI_SELECCION_AREAGEO);
			errores=1;
		}
	}	
	

	if (fase=='3')  //	Comprueba que haya alguna categoria seleccionada
	{
		Seleccionados=0;
		formSel=document.forms["frmCAT"];
		...
		if (Seleccionados==0)
		{
			alert(str_OBLI_SELECCION_CATEGORIA);
			errores=1;
		}
	}	
...
//	Cambia la seleccion
function cambiarSeleccion(IDSel, campo){
	var d = new Date();

	var Estado=jQuery("#hid_" + campo).val();
	//console.log('cambiarSeleccion. IDSel:'+IDSel+' campo:'+campo+' flag:'+flag+' Estado:'+Estado);
	...
	jQuery.ajax({
		...
		success:function(objeto){
			...
			if(data.Seleccion.estado == 'OK'){
				if(Estado=='N'){
					jQuery("#img_" + IDSel).attr("src","http://www.newco.dev.br/images/checkCenter.gif");
					jQuery("#hid_" + campo).val("S");
				}else{
					jQuery("#img_" + IDSel).attr("src","http://www.newco.dev.br/images/nocheck.gif");
					jQuery("#hid_" + campo).val("N");
				}
			}else{
...
// 4abr22 Creacion de una cookie con nombre sName y valor sValue (traido desde login_050122.js)
