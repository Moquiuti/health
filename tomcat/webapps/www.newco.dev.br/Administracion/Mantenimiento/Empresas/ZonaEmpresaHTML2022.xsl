...
<!--
	...
	ultima revision: ET 31mar23 12:38 ZonaEmpresa2022_310323.js
-->
  ...
  <xsl:template match="/">
    ...
    <html>
      <head>
		...
		<script type="text/javascript" src="http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/ZonaEmpresa2022_210322.js"></script>
		...
		var centroActual='<xsl:value-of select="/ZonaEmpresa/AREAEMPRESA/CENTROS/field/@current"/>';		//	para llamar a plantillas
	...
	<body class="areaizq">
		...
		<xsl:otherwise>   
			...
			<form action="ZonaEmpresa2022.xsql" name="Centros" method="POST">
		 	...
		 	<xsl:choose>
	   		<xsl:when test="ZonaEmpresa/AREAEMPRESA/USUARIOS/EDICION">	
						...
						<tr class="lineBorderBottom">
							<td colspan="4">
							  	<xsl:attribute name="class">
							  		<xsl:choose>
                 						<xsl:when test="DERECHOS='Bloq'">textLeft noventa fondoRojo</xsl:when>
										<xsl:otherwise>textLeft noventa</xsl:otherwise>
									</xsl:choose>
								</xsl:attribute>
								<!--	Link a la ficha del usuario	-->
                            	<xsl:apply-templates select="NOMBRE"/>
							</td>
							<td class="w10px" valign="top">
				...
				<xsl:otherwise>
                    ...
                    <tr>
                		<td class="h40px textCenter" colspan="5">		
                            <a href="javascript:NuevoUsuario();">
                                <xsl:value-of select="document($doc)/translation/texts/item[@name='nuevo']/node()"/>&nbsp;<xsl:value-of select="document($doc)/translation/texts/item[@name='usuario']/node()"/>
                            </a> 
                        </td>
                    </tr>
                    <tr class="h40px"><td colspan="5">&nbsp;</td></tr>
					<tr><td colspan="5" class="textCenter">
					<!--	Menus avanzados		-->
					<select id="ABRIR_MANT" class="w200px" onChange="javascript:abrirManten();">
						<option value=""><xsl:value-of select="document($doc)/translation/texts/item[@name='seleccionar']/node()"/></option>
						<option value="Lugares_entrega"><xsl:value-of select="document($doc)/translation/texts/item[@name='Lugares_entrega']/node()"/></option>
						<option value="Logotipos"><xsl:value-of select="document($doc)/translation/texts/item[@name='Logotipos']/node()"/></option>
						<xsl:if test="/ZonaEmpresa/AREAEMPRESA/ADMIN_MVM">
							<option value="plantillas"><xsl:value-of select="document($doc)/translation/texts/item[@name='plantillas']/node()"/></option>
							<option value="perfiles"><xsl:value-of select="document($doc)/translation/texts/item[@name='perfiles']/node()"/></option>
							<option value="Circuitos"><xsl:value-of select="document($doc)/translation/texts/item[@name='Circuitos']/node()"/></option>
						</xsl:if>
						<option value="vendedores"><xsl:value-of select="document($doc)/translation/texts/item[@name='vendedores']/node()"/></option>
						<option value="comentarios"><xsl:value-of select="document($doc)/translation/texts/item[@name='comentarios']/node()"/></option>
						<xsl:if test="/ZonaEmpresa/AREAEMPRESA/ADMIN_MVM">
							<option value="integracion"><xsl:value-of select="document($doc)/translation/texts/item[@name='integracion']/node()"/></option>
							<xsl:if test="not(/ZonaEmpresa/AREAEMPRESA/OTRO_PAIS)">
								<option value="selecciones"><xsl:value-of select="document($doc)/translation/texts/item[@name='selecciones']/node()"/></option>
							</xsl:if>
						</xsl:if>
					</select>
					</td></tr>
...
<xsl:template match="NOMBRE">
	...
	&nbsp;<a>  
	<xsl:attribute name="href">javascript:EditarUsuario('<xsl:value-of select="../ID"/>');</xsl:attribute>
		<xsl:value-of select="." disable-output-escaping="yes"/>
        <xsl:if test="../DERECHOS!='Bloq' and ../DERECHOS!='NOR'">&nbsp;<span class="fondoGris"><xsl:value-of select="../DERECHOS"/></span></xsl:if>
	</a>
</xsl:template>
