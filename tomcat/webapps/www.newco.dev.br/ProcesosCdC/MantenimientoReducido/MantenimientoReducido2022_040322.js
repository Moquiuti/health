//	Adjudicacion de productos. Nuevo disenno.
//	Ultima revision: ET 25may22 11:17. MantenimientoReducido2022_040322.js
...
// JavaScript Document
function onLoad(){
	//24mar22 calcularTodosAhorros(document.forms['form1']);

	//solodebug	console.log("Inicio. Precio cliente:"+jQuery("#PRECIO_CLIENTE").val());
	//jQuery("#PRECIO").val(jQuery("#PRECIO_CLIENTE").val());

	jQuery("#pedidos").attr('class', 'MenuActivo');
...
function CambioCliente(IDCliente){
  ...
  location.href="http://www.newco.dev.br/ProcesosCdC/MantenimientoReducido/MantenimientoReducido2022.xsql?PRO_ID=" + IDProducto + "&EMP_ID=" + IDCliente;
}
...
function QuitarDeLaPlantilla(empresa, producto){
	...
	form.method = 'post';
	form.action='http://www.newco.dev.br/ProcesosCdC/MantenimientoReducido/MantenimientoReducido2022.xsql?PRO_ID='+producto+'&ACTION=BORRAR&EMP_ID='+empresa;
    SubmitForm(form);
}
...
function MostrarCatalogoPrivadoProductoEmpresa(form){
	...
	MostrarPagPersonalizada('http://www.newco.dev.br/Administracion/Mantenimiento/Productos/CatalogoPrivadoProductoEmpresa2022.xsql?IDEMPRESA='+idEmpresa+'&REFERENCIA='+ref_prod,'producto',60,50,-30,800);
...
function GuardarCambiosCatalogacion(form){
	form.action = 'MantenimientoReducidoSave2022.xsql';
	form.method = "post";
...
function DerechosPlantillas(form){
		...
		//	11abr22 Faltaba incluir IDEmpresa
        MostrarPagPersonalizada('http://www.newco.dev.br/ProcesosCdC/MantenimientoReducido/MantenimientoReducidoDerechos2022.xsql?IDPRODUCTO='+idProducto+'&IDEMPRESA='+idempresa+'&LISTA_PLANTILLAS='+lista,'DerechoALasPlantillas',80,50,0,-50);
    }
}
...
function ExpandirNOExpandir(form,obj,origen)
{
	...
	if(obj.checked==false){
		...
		//24mar22	form.elements['PRECIO'].onfocus='';
		//24mar22	if(origen=='MVM'){
		//24mar22		form.elements['PRECIO'].value=form.elements['BACKUP_PRECIO'].value;
		//24mar22	}
		//24mar22	else{
		//24mar22		form.elements['PRECIO'].value=form.elements['PRECIO_HISTORICO'].value;
		//24mar22	}
	}
	else{
	...
	form.elements['PRECIO'].onfocus=blurPrecio;
		//24mar22	if(origen=='MVM'){
		//24mar22		form.elements['PRECIO'].value=form.elements['PRECIO_MVM'].value;
		//24mar22	}
		//24mar22	else{
		//24mar22		form.elements['PRECIO'].value=form.elements['BACKUP_PRECIO'].value;
		//24mar22	}
...
/*
//Abre la ficha de producto de proveedor
function FichaProducto(IDProducto)
{
	MostrarPagPersonalizada("http://www.newco.dev.br/Administracion/Mantenimiento/Productos/PRODetalle.xsql?PRO_ID="+IDProducto,'producto',100,80,0,0);
}

//Abre la ficha de empresa
function FichaEmpresa(IDEmpresa)
{
	MostrarPagPersonalizada("http://www.newco.dev.br/Administracion/Mantenimiento/Empresas/EMPDetalleFrame.xsql?EMP_ID="+IDEmpresa+"&amp;VENTANA=NUEVA",'empresa',100,80,0,0);
}*/

//	25may22 Analisis de pedidos
function AnalisisPedidos(IDProducto)
{
	MostrarPagPersonalizada('http://www.newco.dev.br/Personal/BandejaTrabajo/AnalisisPedidos2022.xsql?IDPRODUCTO='+IDProducto,'Analisis Lineas',100,80,0,-50)
}


//	25may22 Mantenimiento de producto estandar
function MantProductoEstandar(IDEmpresa, IDProdEstandar)
{
	MostrarPagPersonalizada("http://www.newco.dev.br/ProcesosCdC/CatalogoPrivado/MantProductosEstandar2022.xsql?CATPRIV_IDPRODUCTOESTANDAR="+IDProdEstandar+"&CATPRIV_IDEMPRESA="+IDEmpresa+"&ACCION=CONSULTARPRODUCTOESTANDAR&TIPO=CONSULTA&VENTANA=NUEVA",'Mant.prod.est.',100,80,0,0);
}
