//	Informar procedimiento en convocatoria Elementos Especiales
//	Ultima revisión: ET 4dic18 10:20
...
{
	if (confirm(strConfirmBorrarOferta))
	{
		document.forms["Procedimiento"].elements["ACCION"].value='BORRAR';
		document.forms["Procedimiento"].elements["PARAMETROS"].value='';
		Enviar();
	}
}
...
{
	//console.log('InputToFloat:'+control);
...
//	10oct18 Incluye una referencia en el primer textbox de referencia disponible (se le llamara desde el buscador de productos de Conv.Esp.
function IncluirReferencia(Ref)
{

	console.log('IncluirReferencia:'+Ref);

	var Pendiente=true;
	//	Recuperamos los datos de los productos
	for (i=0;(i<document.forms["Procedimiento"].elements.length)&&(Pendiente);++i)
	{
		if ((Piece(document.forms["Procedimiento"].elements[i].name,'_',0)=="REFPROV")&&(document.forms["Procedimiento"].elements[i].value==''))
		{
			document.forms["Procedimiento"].elements[i].value=Ref;
			Pendiente=false;
		}
	}
...
//	19oct18	Abre la ficha correspondiente a las ofertas para este procedimiento
function VerOfertas()
{
	var Url='http://www.newco.dev.br/ProcesosCdC/ConvocatoriasEspeciales/ProcedimientoYOfertas.xsql?'
