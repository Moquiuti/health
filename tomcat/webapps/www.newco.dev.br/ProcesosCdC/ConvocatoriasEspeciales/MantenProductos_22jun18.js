...
//	ultima revision ET 22jun18 10:00
...
function ProcesarFichero(fileName, fileContent)
{
	...
	if (opcion=='CLAS')
		...
		PreciosReferenciaTXT(fileContent);
}
...
function ProcesarProdEstandarTXT(CadenaCambios)
{
	...
	for (i=0;i<=numRefs;++i)
	{
		...
		if ((RefEstandar!='')||(Nombre!=''))		//	Eliminamos líneas vacias
		{
			...
			items['PrecioRef'] = PrecioRef.replace(".","");		//	Eliminamos los puntos (separador de miles)
...
function ProcesarCatProveedoresTXT(CadenaCambios)
{
	...
	for (i=0;i<=numRefs;++i)
	{
		...
		if ((Referencia!='')||(Nombre!=''))		//	Eliminamos líneas vacias
		{
			...
			items['Precio'] = Precio.replace(".","");		//	Eliminamos los puntos (separador de miles)
			items['TipoIVA'] = TipoIVA;
