...
<!--
	...
	Última revisión: ET 12dic22 11:50
-->
...
<head>
	...
	<script type="text/javascript">
		...
		{
			//solodebug	
			console.log('setCookies lang:'+lang+' idses:'+idsesionpar)
		
			if (idsesionpar!='')
			{
				//solodebug	alert('setCookies lang:'+lang+' idses:'+idsesionpar);
...
<xsl:otherwise>
	<body onLoad="setCookies();">
		...
		<xsl:choose>
		<xsl:when test="Main/USUARIO/ACCION='NUEVOPEDIDO'">http://www.newco.dev.br/Compras/NuevaMultioferta/PedidoDesdeCatalogo2022.xsql?LANG=<xsl:value-of select="Main/USUARIO/LANG"/>&amp;STYLE=basicMVM2022.css<!--<xsl:value-of select="Main/USUARIO/STYLE"/>-->&amp;HOJADEGASTOS=<xsl:value-of select="Main/HOJADEGASTOS"/>&amp;NUMCEDULA=<xsl:value-of select="Main/NUMCEDULA"/>&amp;NOMBREPACIENTE=<xsl:value-of select="Main/NOMBREPACIENTE"/>&amp;HABITACION=<xsl:value-of select="Main/HABITACION"/></xsl:when>
		<xsl:when test="Main/USUARIO/ACCION='EIS'">http://www.newco.dev.br/Gestion/EIS/EISConsultas2022.xsql?LANG=<xsl:value-of select="Main/USUARIO/LANG"/>&amp;STYLE=basicMVM2022.css<!--<xsl:value-of select="Main/USUARIO/STYLE"/>--></xsl:when>
		<xsl:otherwise>
		http://www.newco.dev.br/Personal/BandejaTrabajo/WFStatus2022.xsql?SES_ID=<xsl:value-of select="/Main/PARSES"/>&amp;LANG=<xsl:value-of select="Main/USUARIO/LANG"/>&amp;STYLE=basicMVM2022.css
